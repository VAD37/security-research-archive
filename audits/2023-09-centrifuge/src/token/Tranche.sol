...
contract TrancheToken is ERC20, ERC1404Like {
    ...
    constructor(uint8 decimals_) ERC20(decimals_) {}//@note Tranche token auth owner is InvestmentManager and Pool manager and Root. Which is special by having 2 owners interact with token
    ...
    function mint(address to, uint256 value) public override restricted(_msgSender(), to, value) {
        return super.mint(to, value);//@audit anyone can mint free token. as restricted only check to address
    }
    ...
    function checkTransferRestriction(address from, address to, uint256 value) public view returns (bool) {
        return restrictionManager.detectTransferRestriction(from, to, value) == SUCCESS_CODE(); //@note all transferRestriction check that all address must exist in the memberlist and timestamp still valid
    }
    ...
    function _msgSender() internal view virtual override returns (address sender) {
        if (isTrustedForwarder(msg.sender) && msg.data.length >= 20) {
            ...
            assembly {
                sender := shr(96, calldataload(sub(calldatasize(), 20)))//get last 20 hex bytes = 256 bit address and shift 96 = 256-160                
            }
