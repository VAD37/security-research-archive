...
contract LiquidityPool is Auth, IERC4626 {
    ...
    uint128 public latestPrice;//@note liquidityPool token price used in investmentManager. each pool have unique price updated slowly by gateway
    ...
    constructor(uint64 poolId_, bytes16 trancheId_, address asset_, address share_, address investmentManager_) {
        poolId = poolId_;//@ setup Gateway. multiple liquidityPools can share same poolID
        trancheId = trancheId_;//@ setup Gateway. unique ID per pool
        asset = asset_;//pool currency USDC or USDT. can be multiple
        share = TrancheTokenLike(share_);
        investmentManager = InvestmentManagerLike(investmentManager_);

        wards[msg.sender] = 1;//@note liquidity Pool disable factory auth after creation and give auth to Root and InvestmentManager
        emit Rely(msg.sender);
    }//@note liquidity pool have auth access to tranche
    ...
    function requestDepositWithPermit(uint256 assets, address owner, uint256 deadline, uint8 v, bytes32 r, bytes32 s)
        public
    {
        ERC20PermitLike(asset).permit(owner, address(investmentManager), assets, deadline, v, r, s);//@audit-ok H phantom permit issue do we have ERC20 token not revert on fallback? https://media.dedaub.com/phantom-functions-and-the-billion-dollar-no-op-c56f062ae49f
        investmentManager.requestDeposit(assets, owner);//@audit-ok L permit does not working with DAI. the main token of Centrifuge
        emit DepositRequested(owner, assets);
    }
    ...
    function collectDeposit(address receiver) public {
        investmentManager.collectDeposit(receiver);//@audit-ok all receiver must be manually whitelisted member
    ...
    function transferFrom(address, address, uint256) public returns (bool) {
        console.log("transferFrom %s",msg.sender);
        (bool success, bytes memory data) = address(share).call(bytes.concat(msg.data, bytes20(msg.sender)));
        _successCheck(success);//@audit-ok H external call using msg.data manipulated by user. What are some possible angle of attack
        return abi.decode(data, (bool));//@ LiquidityPool have auth access to tranche. so share call here basically full auth call
    }//@note liquidityPool transferFrom require user to approve liquidity pool first. Then anyone can transfer other user money?
    //@audit-ok H liquidityPool transferFrom use allowance from address LiquidityPool
    function transfer(address, uint256) public returns (bool) {//@ abuse external to increase allowance and bypass mint() check. so infinite mint
        console.log("transfer %s",msg.sender);
    ...
    function mint(address, uint256) public auth {//@note when receive investment aka token/money from other chain. mint share of pool. the real money is hold elsewhere
