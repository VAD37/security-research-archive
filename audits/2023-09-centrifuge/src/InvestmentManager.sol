...
contract InvestmentManager is Auth {
    ...
    mapping(address => mapping(address => LPValues)) public orderbook;// user => poolERC4626 address => LPValues
    ...
    function requestDeposit(uint256 currencyAmount, address user) public auth {
        address liquidityPool = msg.sender;//@txt.origin == user
        LiquidityPoolLike lPool = LiquidityPoolLike(liquidityPool);
        address currency = lPool.asset();//@USDC
        uint128 _currencyAmount = _toUint128(currencyAmount);

        // Check if liquidity pool currency is supported by the Centrifuge pool
        poolManager.isAllowedAsPoolCurrency(lPool.poolId(), currency);//@note check if pool have correct currency is quite interesting
        // Check if user is allowed to hold the restricted tranche tokens
        _isAllowedToInvest(lPool.poolId(), lPool.trancheId(), currency, user);//@restrictionManager.detectTransferRestriction(user) == success
        if (_currencyAmount == 0) {//@note ROOT must call updateMembers manually so user can interact with pool for limited amount of time
            // Case: outstanding investment orders only needed to be cancelled
            gateway.cancelInvestOrder(
                lPool.poolId(), lPool.trancheId(), user, poolManager.currencyAddressToId(lPool.asset())//@gas cache currency already exist
            );
            return;
        }

        // Transfer the currency amount from user to escrow. (lock currency in escrow).
        SafeTransferLib.safeTransferFrom(currency, user, address(escrow), _currencyAmount);//@take token ONLY from msg.sender
    ...
    function decreaseRedeemRequest(uint256 _trancheTokenAmount, address user) public auth {
        ...
        gateway.decreaseRedeemOrder(
            ...
            poolManager.currencyAddressToId(liquidityPool.asset()),
            trancheTokenAmount
        );//@audit-ok Lose root auth basically same as changing entire contract .Root cannot remove or increase user or investor fund. But can it abuse it through some creative mechanics.
    }

    function collectDeposit(address user) public auth {
        LiquidityPoolLike liquidityPool = LiquidityPoolLike(msg.sender);
        require(liquidityPool.checkTransferRestriction(address(0), user, 0), "InvestmentManager/not-a-member");//@audit-ok M InvestmentManager check restriction does not upgradable resistent like Tranche check.
    ...
    function handleExecutedCollectInvest(//@collect deposit?
        uint64 poolId,
        bytes16 trancheId,
        address recipient,
        uint128 currency,
    ...
    /// @dev Check if user exist in membership list. Decided by Root auth. List unique for each Tranche
