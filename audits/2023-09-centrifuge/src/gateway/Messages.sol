...
library Messages {
    ...
    function formatUpdateTrancheTokenMetadata(
        ...
        return abi.encodePacked(
            ...
            poolId,
            trancheId,
            _stringToBytes128(tokenName),//@audit-ok no abi.decode is safe ._stringToBytes128 return dynamic bytes memory. This can cause issue with encodepacked. and when decoding. //https://github.com/sherlock-audit/2023-06-tokensoft-judging/blob/main//130-M/156.md
            _stringToBytes32(tokenSymbol)
        );
    }
    ...
    function formatDomain(Domain domain, uint64 chainId) public pure returns (bytes9) {
        return bytes9(BytesLib.slice(abi.encodePacked(uint8(domain), chainId), 0, 9));//@audit formatDomain with chainID look funny. it was cut short of uint64 +uint8
    }
