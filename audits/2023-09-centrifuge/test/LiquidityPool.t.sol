...
contract LiquidityPoolTest is TestSetup {
    ...
    function testFileStealAuth() public {
        uint64 poolId=1;
        bytes16 trancheId= hex"abcd";
        uint128 currencyId = 1;
        vm.assume(currencyId > 0);
        address lPool_ = deployLiquidityPool(poolId, erc20.decimals(), "","", trancheId, currencyId);
        LiquidityPool lPool = LiquidityPool(lPool_);
        
        root.relyContract(lPool_, self);
        lPool.file("investmentManager", self);
        Investor investor = new Investor();
        homePools.updateMember(poolId, trancheId, address(investor), uint64(block.timestamp + 1000));
        address attacker = address(1231231);        
        vm.prank(address(self));
        lPool.share().mint(address(investor), 100000);
