...
An inverted boolean check in the `EpochDistributor.claimLogic()` function allows invalid merkle proofs to pass, enabling unauthorized reward claims.
...
function claimLogic(
        ...
        );//@audit H why this boolean check is false? verify should return true right? this require throw error if merkle root is correct. and skip if merkle root is wrong
...
The second `require` statement contains an inverted boolean check, meaning that an incorrect merkle proof is accepted while a valid one is rejected. This flaw allows anyone to submit arbitrary proof values and claim rewards, draining tokens from the `DefiAppHomeCenter.sol` contract.

## Impact Explanation

High: An attacker can drain rewards from the first epoch.

## Likelihood Explanation

Medium: With comprehensive merkle unit tests, this issue would likely have been discovered before deployment.

## Proof of Concept

Below is a test demonstrating how any random merkle proof value can bypass the merkle root check:
