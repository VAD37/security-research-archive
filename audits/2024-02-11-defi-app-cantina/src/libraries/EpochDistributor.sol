...
library EpochDistributor {
    ...
    function settleEpochLogic(
        ...
        uint256 epoch,//finalized epoch
        bytes32 balanceRoot, //admin merkle root
        ...
        );//@this thing just check final leaf belong to admin
    ...
    function claimLogic(
        ...
        require(
            ...
            EpochDistributor_invalidDistroProof()
        );//@audit-ok H why this boolean check is false? verify should return true right? this require throw error if merkle root is correct. and skip if merkle root is wrong
        address receiver = $e.userConfigs[distro.userId].receiver;
        if (!withStaking) Home($.homeToken).safeTransfer(receiver, distro.tokens);//@audit-ok Not an issue, this just helper part converting reward into LP.M This can cause imbalance of trading HOME/WETH into LP if everyone just do not stake more.stake tiny amount in wei and still receive same amount of tokens. this is set by admin.
