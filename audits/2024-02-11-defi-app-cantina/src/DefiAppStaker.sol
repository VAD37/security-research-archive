...
contract DefiAppStaker is MFDBase {
    ...
    function setGauge(IGauge _gauge) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (prevRewardToken == address(0)) {
            ...
            _recoverERC20(prevRewardToken, untracked);//@note if Gauge change, previous user should still be able withdraw their old rewards that is already tracked to flywheel.
            _removeReward($m, prevRewardToken);
            _addReward($m, newRewardToken);
        }

        $.gauge = newGauge;//@audit Gauge migration issue. Forgot to move assets over
        emit SetGauge(address(_gauge));
    }

    /// Hooks

    function _beforeStakeHook(uint256 _amount, address _onBehalf) internal override {//@allow called from helper/router
        DefiAppStakerStorage storage $ = _getDefiAppStakerStorage();
        ...
        if (_amount > 0 && _onBehalf != address(0) && getUserLocks(_onBehalf).length == 0) { //@audit-ok nope R is it possible to create Lock before stake()? to bypass this check, prevent user from getting rewards
            $.homeCenter.callHookRegisterStaker(_onBehalf);//@init default config for user "_onBehalf". 
        }
    }
