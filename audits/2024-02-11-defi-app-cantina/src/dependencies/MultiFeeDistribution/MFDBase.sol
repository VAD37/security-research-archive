    ...
    UUPSUpgradeable
{
    ...
    function _initialize_MFDBase(MFDBaseInitializerParams calldata initParams) internal {
        _checkNoZeroAddress(initParams.emissionToken);//HomeToken
        _checkNoZeroAddress(initParams.stakeToken);//uniswap pool, HOME/WETH
        _checkNoZeroAddress(initParams.oracleRouter);// HOME/USD
        _checkZeroAmount(initParams.rewardStreamTime);// 7 days
        _checkZeroAmount(initParams.rewardsLookback);// 1 days
        _checkZeroAmount(initParams.initLockTypes.length);// 1 - x1,3 -x3,6-x6,12-x12 months

        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        __Pausable_init();//@L missing __AccessControl_init doing nothing
        ...
        _setRewardStreamParams(initParams.rewardStreamTime, initParams.rewardsLookback);// lookback <= streamTime
        //@copy from _addReward
        ...
        $.rewardDistributors[msg.sender] = true;//whois?
    ...
    function stake(uint256 _amount, address _onBehalf, uint256 _typeIndex) external whenNotPaused {
        _beforeStakeHook(_amount, _onBehalf);//@ check address already init, set default receiver address
        MFDLogic.stakeLogic(_getMFDBaseStorage(), _amount, _onBehalf, _typeIndex, false);//new Lock to sorted array, increase totalLock of user and global,
        _afterStakeHook(_amount);//@take pool token from msg.sender
    }//@ call Gauge, get rewards. transfer all pool token to gauge under this contract name. 
//@gauge prevent transfer zero amount.
    /**
    ...
    function withdrawExpiredLocks() external whenNotPaused returns (uint256) {
        uint256 unlocked = getUserBalances(msg.sender).unlocked;
        if (unlocked == 0) revert AmountZero();
        _beforeWithdrawExpiredLocks(unlocked);//@get gauge reward, unstake same amount of poolToken from gauge
        MultiFeeDistributionStorage storage $ = _getMFDBaseStorage();
    ...
    function claimBounty(address _user, bool _execute) public whenNotPaused returns (bool issueBaseBounty) {
        ...
        _beforeWithdrawExpiredLocks(unlocked);//@audit withdraw user cash. But not do anything with it
    ...
    function claimAndCompound(address _onBehalf) external whenNotPaused {
        ...
        MFDLogic.updateReward($, _onBehalf);//@audit M bot compounder do not have explicit permission from user to compound user rewards. It can compound user rewards anyway. Look like compound or not toggle is from offchain
        uint256 length = $.rewardTokens.length;
        for (uint256 i; i < length;) {
    ...
    function removeReward(address _rewardToken) external {
        ...
        rd.rewardPerTokenStored = 0;//@audit-ok M reset rewards index does not reset user reward index. add them again will allow user to withdraw all available token
    ...
    function _setLockTypes(LockType[] memory _lockTypes) internal {
        MultiFeeDistributionStorage storage $ = _getMFDBaseStorage();
        delete $.lockTypes;//@audit-ok delete/push still replace old version. cannot access out of range length withouyt assembly code M this only delete length not replacing old variable.
