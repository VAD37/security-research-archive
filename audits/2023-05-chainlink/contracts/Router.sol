...
contract Router is IRouter, IRouterClient, ITypeAndVersion, OwnerIsCreator {
  ...
  function getFee(
    ...
    if (message.feeToken == address(0)) {
      ...
      message.feeToken = address(s_wrappedNative);//@note feeToken can still be zero meaning not support wrappedNative
    }
  ...
  function ccipSend(
    uint64 destinationChainSelector,
    Client.EVM2AnyMessage memory message
  ) external payable returns (bytes32) {//@audit-ok L accept overpayment with no refund in dev comment.
    address onRamp = s_onRamps[destinationChainSelector];
    if (onRamp == address(0)) revert UnsupportedDestinationChain(destinationChainSelector);
    ...
    if (message.feeToken == address(0)) {
      ...
      message.feeToken = s_wrappedNative;//@audit-ok feeToken can be set as nativeAddress directly. What happen then?
      feeTokenAmount = IEVM2AnyOnRamp(onRamp).getFee(message);//@note fee = gasFee + 0.1% transfer fee
      ...
      IERC20(message.feeToken).safeTransferFrom(address(this), onRamp, feeTokenAmount); //@audit send whole overpayment to onRamp. 
    } else {
      if (msg.value > 0) revert InvalidMsgValue();
      feeTokenAmount = IEVM2AnyOnRamp(onRamp).getFee(message);
      IERC20(message.feeToken).safeTransferFrom(msg.sender, onRamp, feeTokenAmount);//@note feeToken send directly to ramp. While crosschain token send to rampPool.
    }
  ...
  function _callWithExactGas(
    ...
    assembly {
      ...
      let toCopy := returndatasize()//@audit router EVM can be used to create new contract. How can this be exploited
  ...
  function recoverTokens(address tokenAddress, address to, uint256 amount) external onlyOwner {
    ...
    IERC20(tokenAddress).transfer(to, amount);//@audit not use safeTransfer
  }
