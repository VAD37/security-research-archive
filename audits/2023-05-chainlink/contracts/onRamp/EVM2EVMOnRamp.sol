...
contract EVM2EVMOnRamp is IEVM2AnyOnRamp, ILinkAvailable, AggregateRateLimiter, ITypeAndVersion {
  ...
  struct StaticConfig {
    ...
    uint64 defaultTxGasLimit; // -┘ Default gas limit for a tx //200_000
    address prevOnRamp; //          Address of previous-version OnRamp //zero
  }

  /// @dev Struct to contains the dynamic configuration
  struct DynamicConfig {
    address router; //            Router address // fixed
    address priceRegistry; // --┐ Price registry address // fixed
    uint32 maxDataSize; //      | Maximum payload data size // 500
    uint64 maxGasLimit; // -----┘ Maximum gas limit for messages targeting EVMs // 4_000_000
    uint16 maxTokensLength; // -┐ Maximum number of distinct ERC20 tokens that can be sent per message //5
    address ARM; // ------------┘ ARM address
  }

  /// @dev Struct to hold the execution fee configuration for a fee token
  struct FeeTokenConfig {
    uint96 networkFeeAmountUSD; // -┐ Flat network fee in USD //1e10 sLink
    uint64 multiplier; //           | Price multiplier for gas costs// 1e18 sLink
    uint32 destGasOverhead; // -----┘ Extra gas charged on top of the gasLimit // 100k sLink
  }
  ...
  struct FeeTokenConfigArgs {
    address token; // --------------┐ Token address // sourceFeeToken sLink , WETH
    uint64 multiplier; // ----------┘ Price multiplier for gas costs //1e18 ,2e18 for WETH
    uint96 networkFeeAmountUSD; // -┐ Flat network fee in USD //1e10 USD for chainlink , 5e8 USD for WETH
    uint32 destGasOverhead; // -----┘ Extra gas charged on top of the gasLimit //100_000 , 200_000 for WETH
  }
  ...
  struct TokenAndPool {
    address token;
    address pool; // LockReleaseTokenPool sLINK or BurnMintTokenPool WETH
  }
  ...
  bytes32 internal immutable i_metadataHash;// not entirely unique
  /// @dev Default gas limit for a transactions that did not specify
  /// a gas limit in the extraArgs.
  uint64 internal immutable i_defaultTxGasLimit; // 200_000
  /// @dev The link token address - known to pay nops for their work
  address internal immutable i_linkToken; // sLink
  /// @dev The chain ID of the source chain that this contract is deployed to
  uint64 internal immutable i_chainSelector;// ID 1
  /// @dev The chain ID of the destination chain
  uint64 internal immutable i_destChainSelector;// ID 2
  /// @dev The address of previous-version OnRamp for this lane
  address internal immutable i_prevOnRamp; // zero
  ...
  bool private s_allowlistEnabled; //false by default
  ...
  constructor(
    ...
    i_metadataHash = keccak256(//@note check what metadataHash do. This is unique for each OnRamp. unless this was deployed through CREATE2
      abi.encode(
    ...
    _setFeeTokenConfig(feeTokenConfigs);//@note there can be more tokenTransfer config than feeTokenConfig. Have feeToken does not mean we have normal config
    ...
    if (allowlist.length > 0) {
      s_allowlistEnabled = true;//test file disable this
      _applyAllowListUpdates(allowlist, new address[](0));// @audit-ok M onRamp constructor allowList inversed data. Suppose to add but instead remove old data
    }
  }
  ...
  function getSenderNonce(address sender) external view returns (uint64) {
    ...
    }//@note senderNonce can go through multiple old contracts. If old contracts can still be send nonce. This open up new attack
    return uint64(senderNonce);
  }

  /// @inheritdoc IEVM2AnyOnRamp
  function forwardFromRouter(//OnRamp take token from router. Consume rateLimit. To offRamp-rate limit controlled by tokenPool not offRamp.
    Client.EVM2AnyMessage calldata message,//@audit R token can be in limbo offchain. Then onRamp already lock token. offRamp cannot receive token dueto pooltoken pool limit
    uint256 feeTokenAmount,
    address originalSender
  ) external whenNotPaused whenHealthy returns (bytes32) {//@audit lack check if msg.sender is from router? Is this intentional to allow user to manually call execution
    Client.EVMExtraArgsV1 memory extraArgs = _fromBytes(message.extraArgs);//@note extraArgs can include extra tibbit without being decoded.
    ...
    if (message.feeToken == i_linkToken) {
      ...
      s_nopFeesJuels += uint96(feeTokenAmount);//@audit-ok need to check how feeToken work accross multiple contract
    } else {
      // the cast from uint256 to uint96 is considered safe, uint96 can store more than max supply of link token
      s_nopFeesJuels += uint96(//@audit M. if oracle was bricked. Price go to infinity. The fee token might be so big that there is not enough LINK to cover.
    ...
    Internal.EVM2EVMMessage memory newMessage = Internal.EVM2EVMMessage({
      ...
      receiver: address(uint160(decodedReceiver)),//@ok messageId do nothing. This more like for offchain interaction check which part of Message is missing
      data: message.data,
  ...
  function _validateMessage(
    uint256 dataLength, // message.data
    uint256 gasLimit,//from extraArgs
    Client.EVMTokenAmount[] calldata tokenAmounts, //message
    address originalSender//router Input msg.sender||user
  ) internal view {
  ...
  function getFee(Client.EVM2AnyMessage calldata message) external view returns (uint256) {
    ...
    FeeTokenConfig memory feeTokenConfig = s_feeTokenConfig[message.feeToken];//feeToken zero?
  ...
  function _getTokenTransferFee(
    ...
    for (uint256 i = 0; i < numerOfTokens; ++i) {
      ...
      if (transferFeeConfig.ratio > 0) {
        ...
        uint256 tokenValue = tokenPrice._calcUSDValueFromTokenAmount(tokenAmount.amount); // tokenPrice * tokenAmount / 1e18
        //tokenValue in USD $ 1e18
        // ratio represents multiples of 0.1bps, or 10e-5
        bpsValue = (tokenValue * transferFeeConfig.ratio) / 1e5; // bpsValue in 1e18
      }
  ...
  function getNops() public returns (NopAndWeight[] memory nopsAndWeights, uint256 weightsTotal) {
  ...
  function _setNops(NopAndWeight[] memory nopsAndWeights) internal {//@audit in test file. setNops override actually remove old data. https://github.com/code-423n4/2023-05-chainlink/blob/f5795088a8390094ffb362e30391c88923ad5033/vendor/openzeppelin-solidity/v4.8.0/utils/structs/EnumerableSet.sol#L67
  ...
  function payNops() public onlyOwnerOrAdminOrNop {
    ...
    for (uint256 i = 0; i < numberOfNops; ++i) {
      ...
      uint96 amount = uint96((totalFeesToPay * weight) / weightsTotal);//@audit L serious rounding issue
      fundsLeft -= amount;
  ...
  function withdrawNonLinkFees(address feeToken, address to) external onlyOwner {
    ...
    // of non-link fees.
    if (_linkLeftAfterNopFees() < 0) revert LinkBalanceNotSettled();//@audit I this can be frontrun.
  ...
  modifier onlyOwnerOrAdminOrNop() {
    if (msg.sender != owner() && msg.sender != s_admin && !s_nops.contains(msg.sender))
      revert OnlyCallableByOwnerOrFeeAdminOrNop();//@audit s_nops use as admin. Old s_nops still exist allow after set new s_nops
    _;
  }
