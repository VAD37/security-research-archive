...
contract PriceRegistry is IPriceRegistry, OwnerIsCreator {
  ...
  uint32 private immutable i_stalenessThreshold;// 12h = 12 * 60 * 60 = 43200

  constructor(
    Internal.PriceUpdates memory priceUpdates,//@audit-ok there is no check to confirm price is in correct Chain ID and selector.
  ...
  function convertTokenAmount(
    address fromToken,//fail when covert 0 token
    ...
    return (fromTokenAmount * _getValidatedTokenPrice(fromToken)) / _getValidatedTokenPrice(toToken);//@audit-ok L rounding issue like USDC->WBTC? 10e8 * 0.0001e18 / 20_000e18
  }
