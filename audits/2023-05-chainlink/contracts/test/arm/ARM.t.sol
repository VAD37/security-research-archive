...
contract ConfigCompare is Test {
    function assertConfigEq(
        ARM.Config memory actualConfig,
        ARM.Config memory expectedConfig
    ) public {
        ...
        assertEq(
            actualConfig.blessWeightThreshold,
            expectedConfig.blessWeightThreshold
        );
        assertEq(
            actualConfig.curseWeightThreshold,
            expectedConfig.curseWeightThreshold
        );
    }
}
...
contract ARM_constructor is ConfigCompare, ARMSetup {
    function testConstructorSuccess() public {
        ...
        (uint32 actualVersion, , ARM.Config memory actualConfig) = s_ARM
            .getConfigDetails();
...
contract ARM_voteToBlessRoots is ARMSetup {
    event VotedToBless(
        ...
        IARM.TaggedRoot taggedRoot,
        uint8 weight
    );

    // Success

    function _getFirstBlessVoterAndWeight()
        internal
        pure
    ...
    function test3RootSuccess_gas() public {
        ...
        for (uint256 i = 1; i <= 3; ++i) {
            ...
            assertEq(
                voterWeight,
                getWeightOfVotesToBlessRoot(makeTaggedRoot(i))
            );
    ...
    function testIsAlreadyBlessedIgnoredSuccess() public {
        ...
        uint256 votesToBlessBefore = getWeightOfVotesToBlessRoot(
            makeTaggedRoot(1)
        );
        ...
        assertEq(
            votesToBlessBefore,
            getWeightOfVotesToBlessRoot(makeTaggedRoot(1))
        );
    }
    ...
    function testInvalidVoterReverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(ARM.InvalidVoter.selector, STRANGER)
        );
...
contract ARM_unvoteToCurse is ARMSetup {
    ...
    function setUp() public override {
        ...
        bytes32 expectedCursesHash = keccak256(
            abi.encode(bytes32(0), makeCurseId(1))
        );
    ...
    function testOwnerSucceeds() public {
        ARM.Config memory cfg = armConstructorArgs();
        changePrank(OWNER);
        ARM.UnvoteToCurseRecord[]
            memory records = new ARM.UnvoteToCurseRecord[](1);
    ...
    event SkippedUnvoteToCurse(
        address indexed voter,
        bytes32 expectedCursesHash,
        bytes32 actualCursesHash
    );
...
contract ARM_voteToCurse is ARMSetup {
    ...
    function _getFirstCurseVoterAndWeight()
        internal
        pure
        returns (address, uint8)
    {
    ...
    function testVoteToCurseSuccess_gas() public {
        ...
        (address[] memory voters, , , uint16 votes, bool cursed) = s_ARM
            .getCurseProgress();
    ...
    function testOwnerCanCurseAndUncurse() public {
        ...
        {
            (address[] memory voters, , , uint24 accWeight, bool cursed) = s_ARM
                .getCurseProgress();
        ...
        ARM.UnvoteToCurseRecord[]
            memory unvoteRecords = new ARM.UnvoteToCurseRecord[](0);
        vm.expectEmit();
    ...
    function testAlreadyVotedReverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ARM.AlreadyVotedToCurse.selector,
                voter,
                makeCurseId(1)
            )
        );
...
contract ARM_ownerUnvoteToCurse is ARMSetup {
    ...
    function getCursersAndCurseCounts()
        internal
        pure
        returns (address[] memory cursers, uint32[] memory curseCounts)
    {
    ...
    function setUp() public virtual override {
        ...
        (
            address[] memory cursers,
            uint32[] memory curseCounts
        ) = getCursersAndCurseCounts();
        for (uint256 i = 0; i < cursers.length; ++i) {
    ...
    function makeUnvoteToCurseRecords()
        internal
        pure
        returns (ARM.UnvoteToCurseRecord[] memory)
    {
        (address[] memory cursers, ) = getCursersAndCurseCounts();
        ARM.UnvoteToCurseRecord[]
            memory records = new ARM.UnvoteToCurseRecord[](cursers.length);
        for (uint256 i = 0; i < cursers.length; ++i) {
...
contract ARM_setConfig is ConfigCompare, ARMSetup {
    ...
    function testVoteToBlessByEjectedVoterReverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(ARM.InvalidVoter.selector, BLESS_VOTER_4)
        );
        s_ARM.voteToBless(makeTaggedRootSingleton(2));
    }

    function testSetConfigMultipleTime() public {
        // Previous config included BLESS_VOTER_4. Change to new config that doesn't.
        ARM.Config memory cfg = getDifferentConfigArgs();
        changePrank(OWNER);
        (uint32 version1, , ) = s_ARM.getConfigDetails();
        logAllMemoryPosition();
        s_ARM.setConfig(cfg);
        (uint32 version2, , ) = s_ARM.getConfigDetails();
        assertEq(version1 + 1, version2);
        logAllMemoryPosition();
        s_ARM.setConfig(cfg);

        (uint32 version3, , ) = s_ARM.getConfigDetails();
        assertEq(version2 + 1, version3);
        logAllMemoryPosition();
        s_ARM.setConfig(cfg);
        (uint32 version4, , ) = s_ARM.getConfigDetails();
        assertEq(version3 + 1, version4);
        logAllMemoryPosition();
        s_ARM.setConfig(cfg);
        (uint32 version5, , ) = s_ARM.getConfigDetails();
        assertEq(version4 + 1, version5);
        logAllMemoryPosition();
    }

    function logAllMemoryPosition() private {
        uint count = 16;
        console.log("logging storage position");
        for (uint i = 0; i < count; i++) {
            console.logBytes32(s_ARM.debugStorageView(i));
        }
        (, , , uint256 curseWeight, bool cursed) = s_ARM.getCurseProgress();
        console.log("cursed weight", curseWeight);
    ...
    function testSetConfigSuccess_gas() public {
        ...
        (uint32 configVersionAfter, , ARM.Config memory configAfter) = s_ARM
            .getConfigDetails();
        ...
        assertEq(1, cursesHashes.length);
        assertEq(
            keccak256(abi.encode(bytes32(0), makeCurseId(1))),
            cursesHashes[0]
        );
        assertFalse(cursed);

        // Assert that good votes have been cleared
        uint256 votesToBlessRoot = getWeightOfVotesToBlessRoot(
            makeTaggedRoot(1)
        );
        assertEq(ZERO, votesToBlessRoot);
        assertFalse(
            hasVotedToBlessRoot(cfg.voters[0].blessVoteAddr, makeTaggedRoot(1))
        );
        assertFalse(
            hasVotedToBlessRoot(cfg.voters[1].blessVoteAddr, makeTaggedRoot(1))
        );
        vm.resumeGasMetering();
    }
