...
abstract contract OCR2Base is OwnerIsCreator, OCR2Abstract {
  ...
  function setOCR2Config(
    address[] memory signers,//4 signer
    address[] memory transmitters,// different from signer address
    uint8 f,//1
    bytes memory onchainConfig,//zero
    uint64 offchainConfigVersion,// 3
    bytes memory offchainConfig//zero
    ...
    for (uint256 i = 0; i < newSignersLength; ++i) {
      ...
      if (s_oracles[signer].role != Role.Unset) revert InvalidConfig("repeated signer address");//@audit OCR2 have repeated address check. Does other place have this check?
    ...
    s_configInfo.latestConfigDigest = _configDigestFromConfigData(
      block.chainid,//@audit R -ok transmit require data use hashDigest same as config. Is this true for all case?. 
      address(this),
      ++s_configCount,
      signers,
      transmitters,
  ...
  function transmit(
    ...
    bytes calldata report,//Internal.ExecutionReport struct || CommitReport
    bytes32[] calldata rs,
    bytes32[] calldata ss,
    ...
    if (rs.length != (configInfo.n + configInfo.f) / 2 + 1) revert WrongNumberOfSignatures();//@audit does test file work with maxium range of signer address?
    if (rs.length != ss.length) revert SignaturesOutOfRegistration();//@audit-ok does not check if all signature came from same person/transmitter
    ...
    bytes32 h = keccak256(abi.encodePacked(keccak256(report), reportContext));//@audit the only way to bypass transmit data is report bytes data or reportContext does not contain roundId.
    bool[MAX_NUM_ORACLES] memory signed;//@audit after signed here do what?

    uint256 numberOfSignatures = rs.length;
    for (uint256 i = 0; i < numberOfSignatures; ++i) {
      address signer = ecrecover(h, uint8(rawVs[i]) + 27, rs[i], ss[i]);//@audit why anyof the transmitter can reuse rs and ss signature?
      Oracle memory oracle = s_oracles[signer];
      if (oracle.role != Role.Signer) revert UnauthorizedSigner();
      if (signed[oracle.index]) revert NonUniqueSignatures();//this actually check duplicate signer. But can i reuse old signature? Nope use hash of report bytes data too.
      signed[oracle.index] = true;//@audit OCR2 sign when call transmit message. When we check if message have enough signature? The signed here share between multiple message?
    }
  }
