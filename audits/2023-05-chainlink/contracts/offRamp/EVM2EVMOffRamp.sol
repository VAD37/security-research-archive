...
contract EVM2EVMOffRamp is IAny2EVMOffRamp, AggregateRateLimiter, ITypeAndVersion, OCR2BaseNoChecks {
  ...
  constructor(
    StaticConfig memory staticConfig,//@ offramp config. different from onramp
    ...
    if (ICommitStore(staticConfig.commitStore).getExpectedNextSequenceNumber() != 1) revert CommitStoreAlreadyInUse();//@audit what happen with old offRamp? can this be upgraded version like onramp?
    //@audit-ok how can offRamp upgrade if commitStore is already in use. Does CommitStore unique?
    ...
    i_metadataHash = _metadataHash(Internal.EVM_2_EVM_MESSAGE_HASH);//@audit-ok L metadataHash lack version hashing.

    // Set new tokens and pools
    for (uint256 i = 0; i < sourceTokens.length; ++i) {//@audit L does not verify constructor length input.
  ...
  /// @dev we use the literal number 128 because using a constant increased gas usage.
  function _setExecutionState(uint64 sequenceNumber, Internal.MessageExecutionState newState) internal {//@audit-ok setExecution to bitmap zero/state Untouched will break neighbor sequence number. Add warning not to reset state to zero
    ...
    s_executionStates[sequenceNumber / 128] = bitmap;//@note @OK, No dirty bits. bitmap always have some dirty bit by another state. What to do with this?
  }
  ...
  function _execute(Internal.ExecutionReport memory report, bool manualExecution) internal whenHealthy {
    uint256 numMsgs = report.encodedMessages.length;//@note. anyone can execute manually for other people message
    ...
    for (uint256 i = 0; i < numMsgs; ++i) {
      ...
      hashedLeaves[i] = Internal._hash(decodedMessage, i_metadataHash);//@audit-ok hash is unique for each lane but not unique for offRampUpgrade.
      ...
      if (hashedLeaves[i] != decodedMessage.messageId) revert InvalidMessageId();//@audit-ok decodedMessage.messageID here is from user report input. So is decodedMessage. So hashedLeaves here just same as onRamp hashing
    ...
    for (uint256 i = 0; i < numMsgs; ++i) {
      ...
      ) revert AlreadyExecuted(message.sequenceNumber);//revert if IN_PROGRESS OR SUCCESS
      ...
      if (manualExecution) {
        ...
        if (!(isOldCommitReport || originalState == Internal.MessageExecutionState.FAILURE))//@audit-ok cannot manually execute? should be != failure?
          revert ManualExecutionNotYetEnabled();//@note reentrancy can execute another message with different ID
      } else {
      ...
      uint64 prevNonce = s_senderNonce[message.sender];//@audit if upgrade. It seem like nonce prevent user from manually execute anything. if non-strict is used?
      if (prevNonce == 0 && i_prevOffRamp != address(0)) {
        ...
        s_senderNonce[message.sender] = prevNonce;//s_senderNonce can be always 0, same as prevNonce
      }

      // UNTOUCHED messages MUST be executed in order always
      if (originalState == Internal.MessageExecutionState.UNTOUCHED) {//@audit this can be abused to make sure state always untouched and not failure.
        if (prevNonce + 1 != message.nonce) {
      ...
      Internal.MessageExecutionState newState = _trialExecute(message, offchainTokenData, manualExecution);//@audit manual execution also call try-catch. Can this reset state to failure to call again?
      ...
      if (message.strict) {
        ...
        s_senderNonce[message.sender]++;//@audit what is the consequence of no nonceBump for no strict msg. manual execution have no nonce bump
      }
  ...
  function _trialExecute(
    ...
    try this.executeSingleMessage(message, offchainTokenData, manualExecution) {} catch (bytes memory err) {//@audit auto execute can fail due to ratelimit. How can this be exploited?
      if (ReceiverError.selector == bytes4(err)) {//@note the only way to get FAILURE error is from self call executeSingleMessage outOfGas
        return Internal.MessageExecutionState.FAILURE;//@audit user can throw their own error to get FAILURE state
      } else {
  ...
  function executeSingleMessage(
    ...
    if (
      !message.receiver.isContract() || !message.receiver.supportsInterface(type(IAny2EVMMessageReceiver).interfaceId)//@note no arbitrary call from offRamp. message require receiver is AOE or support interface
    ) return;
    ...
    if (manualExecution) {
      ...
      gasLimit = ((gasleft() - 2 * (16 * message.data.length + GAS_FOR_CALL_EXACT_CHECK)) * 62) / 64;//@audit this *62 when it suppose to be *63
    }
    if (
  ...
  function _metadataHash(bytes32 prefix) internal view returns (bytes32) {
    return keccak256(abi.encode(prefix, i_sourceChainSelector, i_chainSelector, i_onRamp));//@note this is same as OnRamp hash.
  }//prefix = EVM_2_EVM_MESSAGE_HASH
