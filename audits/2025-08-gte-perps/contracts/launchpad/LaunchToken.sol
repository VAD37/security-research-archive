...
contract LaunchToken is ERC20 {
    ...
    function unlock() external onlyLaunchpad {
        unlocked = true;//@only unlock after 800M token sold. a uniswap pair is created
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        }//@both from,to not belong to launchpad/router, disable transfer
        ...
        }//@ during bonding, launchpad transfer cash to buyer will accrue fee.
        //@audit-ok unclear ramification. LaunchToken during launch can send share to LaunchPad account and router account. Router only accept launchpad.buy and .sell
    ...
    function _decreaseFeeShares(address account, uint256 amount) internal {
        ...
        unchecked {//@audit-ok M05 user buy more token and then sell it also sell their original share before graduation.
            totalFeeShare -= amount;
            bondingShare[account] -= amount;
        }//@audit L user sell share does not receive their own swap fee

        if (totalFeeShare == 0 && !unlocked) _endRewards();//@audit-ok M there is an issue where ending rewards during bonding phase? when there is no pair token launched yet.
        //@above check should be && unlocked?? only endRewards when pair exist and in launched phase?
