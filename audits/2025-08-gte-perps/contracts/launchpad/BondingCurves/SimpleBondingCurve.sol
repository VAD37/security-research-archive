...
contract SimpleBondingCurve is IBondingCurveMinimal {
    ...
    uint256 public VIRTUAL_BASE; // can be customized to change curve // 200M e18 token
    uint256 public VIRTUAL_QUOTE; // can be customized to change curve // 10e18 eth
    ...
    function init(bytes memory data) external onlyLaunchpad {
        //from launchpad initializer input by admin
        (uint256 virtualBase, uint256 virtualQuote) = abi.decode(data, (uint256, uint256)); //(200_000_000 ether, 10 ether).
    ...
    function initializeCurve(address token, uint256 totalSupply_, uint256 bondingSupply_) external onlyLaunchpad {
        _setReserves(token, VIRTUAL_QUOTE, bondingSupply_ + VIRTUAL_BASE); //reserve = 10 virtual quote, 200M virtual base. so 10 ether reserve for quote with 1B ether reserve for base
        _setSupply(token, totalSupply_, bondingSupply_); //1B supply, 800M bonding
        //@audit L what happen when virtual reserve does not match with totalSupply? BOndingSupply > TotalSupply
    ...
    function buy(address token, uint256 baseAmount) external onlyLaunchpad returns (uint256 quoteAmount) {
        ...
        console.log("buy base %e to quoteAmount: %e", baseAmount, quoteAmount);
        r.quoteReserve += quoteAmount;
        r.baseReserve -= baseAmount;
        console.log("quoteReserve before: %e", r.quoteReserve);
        console.log("baseReserve before: %e", r.baseReserve);
    }
    ...
    function sell(address token, uint256 baseAmount) external onlyLaunchpad returns (uint256 quoteAmount) {
        ...
        console.log("sell base %e to quoteAmount: %e", baseAmount, quoteAmount);
        r.quoteReserve -= quoteAmount;//@audit-ok M there is a case where r.quoteReserve < VIRTUAL_QUOTE. leads to underflow when emit events
        r.baseReserve += baseAmount;
        console.log("quoteReserve after: %e", r.quoteReserve);
        console.log("baseReserve after: %e", r.baseReserve);
    } //@why never use getBaseAmount though? from quote to base
    ...
    function baseSoldFromCurve(address token) external view returns (uint256) {
        console.log("bondingSupply: %e", supply[token].bondingSupply);
        console.log("baseReserve: %e", reserves[token].baseReserve);
    ...
    function quoteBoughtByCurve(address token) external view returns (uint256) {
        console.log("quoteReserve: %e", reserves[token].quoteReserve);
