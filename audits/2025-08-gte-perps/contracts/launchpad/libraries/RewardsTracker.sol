...
library RewardsTrackerLib {
        ...
        returns (uint256 baseAmount, uint256 quoteAmount)
    {
        ...
        self.totalShares += newShares;//@audit there is case where new shares is so small, it actually draining rewards away. increase totalShares but does not reduce rewardPerShare
        ...
        returns (uint256 accBaseRewardsPerShare, uint256 accQuoteRewardsPerShare)
    {
        ...
        if (self.pendingBaseRewards > 0) {
            accBaseRewardsPerShare += ((self.pendingBaseRewards * PRECISION_FACTOR) / uint128(totalShares));//@user have access to totalShares after gradutation. free earning quote token if base token is throw away
        }

        if (self.pendingQuoteRewards > 0) {
            accQuoteRewardsPerShare += ((self.pendingQuoteRewards * PRECISION_FACTOR) / uint128(totalShares));//@max 1e30 with 1 share?
        }//@audit-ok  no free increase stake back after reduce totalshares to 1 or 0.ERC4626 early inflation attack on rewards. mint small share, give pending rewards then inflate rewards value really high.
    }//@share always 800M though
}
