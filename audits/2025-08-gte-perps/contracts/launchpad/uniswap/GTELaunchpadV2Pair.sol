...
contract GTELaunchpadV2Pair is IUniswapV2Pair, IGTELaunchpadV2Pair, UniswapV2ERC20 {
    ...
    function _update(
        uint256 balance0,//balanceOf()
        uint256 balance1,//balanceOf()
        uint112 _reserve0,//prev reserve0
        uint112 _reserve1,//prev reserve1
        uint112 newLaunchpadFee0,// 0.1% amountIn0
        uint112 newLaunchpadFee1 // 0.1% amountIn1
    ) private {
        ...
        if (timeElapsed > 0 && _reserve0 != 0 && _reserve1 != 0) {
            ...
            //@audit L, oracle should use updated version of _reserve after fee decrease.
            if (launchpadFeeDistributor > address(0)) {//@distribute fee on everyswap is new
            ...
            accruedLaunchpadFee0 = totalLaunchpadFee0;//@audit-ok H why fee still removed from reserve if old fee is not removed or distributed away
        ...
        reserve0 = _reserve0 = uint112(balance0) - totalLaunchpadFee0;//@new balance still become part of liquidity. @reserve can reduced to zero with multiple 1000x swap in same block. this affect price
        reserve1 = _reserve1 = uint112(balance1) - totalLaunchpadFee1;//@audit-ok reserve price is reduced if it have fee accrued from swap on same block
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
...

        {
            ...
            uint256 balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(3));//@swap still take 0.3% fee V2. and then 0.1% fee is taken away later too
            uint256 balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(3));
            // console.log("amount0In: %e", amount0In);
            // console.log("amount1In: %e", amount1In);
            // console.log("balance0Adjusted: %e", balance0Adjusted);
            // console.log("balance1Adjusted: %e", balance1Adjusted);
        ...
        returns (uint112 fee0, uint112 fee1)
    {
        ...
        uint256 totalLpBal = this.totalSupply();//1.78885438199983175711733e23 liquidity from 800M and 40ETH
        uint256 launchpadLpBal = this.balanceOf(launchpadLp) + MINIMUM_LIQUIDITY;//min 1000 liquidity locked

        if (amount0In > 0) fee0 = uint112(amount0In.mul(REWARDS_FEE_SHARE).mul(launchpadLpBal) / (totalLpBal * 1000));//@fee reduced to zero if input < 1000, or simply too much other liquidity
        if (amount1In > 0) fee1 = uint112(amount1In.mul(REWARDS_FEE_SHARE).mul(launchpadLpBal) / (totalLpBal * 1000));//@fee is taken from reserve directly instead holding as liquidity of 0.4% fee
