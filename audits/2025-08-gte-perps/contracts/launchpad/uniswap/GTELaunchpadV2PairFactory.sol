...
contract GTELaunchpadV2PairFactory is IUniswapV2Factory {
    ...
    constructor(address _feeToSetter, address _launchpad, address _launchpadLp, address _launchpadFeeDistributor) {
        ...
        launchpadFeeDistributor = _launchpadFeeDistributor;//distributor
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        IUniswapV2Pair(pair).initialize(token0, token1, _launchpadLp, _launchpadFeeDistributor);//@audit-ok  User cannot create same pair as launchpad.factory have open permission to create new pair with empty launchPad address. so official pair and then there is user pair
