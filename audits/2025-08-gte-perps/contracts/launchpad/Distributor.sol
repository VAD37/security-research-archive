...
contract Distributor is OwnableRoles, IDistributor {
    ...
    function createRewardsPair(address launchAsset, address quoteAsset) external onlyLaunchpad {
        RewardPoolData storage rs = RewardsTrackerStorage.getRewardPool(launchAsset);
        RewardPoolData storage rsq = RewardsTrackerStorage.getRewardPool(quoteAsset);//@rsq must be empty
        ...
        rs.initializePair(launchAsset, quoteAsset);// remember launchAsset/Quote as a pair to launchASset address
    }
    ...
    function addRewards(address token0, address token1, uint128 amount0, uint128 amount1) external {//@audit L ignore token order does not work. It only work if quote token is ETH. not WETH
        (address launchAsset, address quoteAsset, uint128 launchAssetAmount, uint128 quoteAssetAmount) =
            (token0, token1, amount0, amount1);
        RewardPoolData storage rs = RewardsTrackerStorage.getRewardPool(token0);//@audit-ok safety check if pool exist.can addRewards before a pool even exists. This allow anyone to transfer nothing to rewards if Token not launched yet
        //@audit-ok M this is free memory reading anywhere. it use keccak(address,uint) so it cannot preimage attack.
        if (rs.quoteAsset == address(0)) {//token0 pointing to nothing. check token1 as launchAsset instead
            ...
            if (rs.quoteAsset == address(0)) revert RewardsDoNotExist();//@note this prevent non-launched token to be added
        ...
        }//@audit-ok H01 no check if rs.quoteAsset == quoteAsset. allow multiple tokens as rewards. lead to DOS by inflate other totalPending token -> inflate getAccRewardsPerShare value -> drain rewards token

        if (rs.totalShares == 0) revert NoSharesToIncentivize();//@share increase when someone buy()
        ...
        if (launchAssetAmount > 0) {
            ...
            launchAsset.safeTransferFrom(msg.sender, address(this), uint256(launchAssetAmount));//@audit-ok still need a pair exist.H01 how to add rewards from distrobutor before launch though?
        }

        if (quoteAssetAmount > 0) {
            rs.addQuoteRewards(launchAsset, quoteAsset, quoteAssetAmount);//self.pendingQuoteRewards += amount;
            _increaseTotalPending(quoteAsset, quoteAssetAmount);//unchecked {totalPendingRewards[asset] += amount;}
