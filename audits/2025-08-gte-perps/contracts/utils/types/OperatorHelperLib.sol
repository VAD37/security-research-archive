...
library OperatorHelperLib {
    ...
    function onlySenderOrOperator(IOperatorPanel operator, address account, SpotOperatorRoles requiredRole)
        ...
        view
    {
        if (msg.sender == account) return;//@account is user input in some cases.

        uint256 rolesPacked = operator.getOperatorRoleApprovals(account, msg.sender);//@note operators can be anyone as long as user approve them
        assertHasRole(rolesPacked, uint8(requiredRole));
    }
