...
contract VaultRegistry is AccessControl, IVaultRegistry {//@audit-ok I Pause is not used in vaultRegistry despite added
    ...
    function _removeVaultFromList(ICDPVault vault) private {
        ...
        for (uint256 i = 0; i < vaultLen; ) {
            if (vaultList[i] == vault) {
                vaultList[i] = vaultList[vaultLen - 1];//@note vault order can be changed halfway. Ntohing should use index
                vaultList.pop();
                break;
            }
