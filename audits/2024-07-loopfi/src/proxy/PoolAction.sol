...
contract PoolAction is TransferAction {
    ...
    function transferAndJoin(
        address from,
        PermitParams[] calldata permitParams,//@permit can be empty
        ...
        if (from != address(this)) {
            if (poolActionParams.protocol == Protocol.BALANCER) {
                ...
                for (uint256 i = 0; i < assets.length; ) {
                    ...
                    }//@audit-ok H If permit2 allowance not spend all tokens. Anyone can use empty permit to transfer token from other user to them.
                    //@ most likely usecase is frontrun other user transaction to spend all approval.
                    unchecked {
                        ++i;
                    }
                }
    ...
    function _balancerJoin(PoolActionParams memory poolActionParams) internal {
        (bytes32 poolId, address[] memory assets, uint256[] memory assetsIn, uint256[] memory maxAmountsIn) = abi
            .decode(poolActionParams.args, (bytes32, address[], uint256[], uint256[]));//@audit-ok M maxAmountIn !=  assetsIn. amountIn is approval allowance. it still there when user not use all allowance. 
        //@this seem like a features to support fee-on-transfer token.
        for (uint256 i = 0; i < assets.length; ) {
            if (maxAmountsIn[i] != 0) {
                IERC20(assets[i]).forceApprove(address(balancerVault), maxAmountsIn[i]);//@safety forceApprove(0) with UNI and USDT token.
            }
