...
abstract contract PositionAction is IERC3156FlashBorrower, ICreditFlashBorrower, TransferAction, BaseAction {
    ...
    function increaseLever(
        ...
        if (upFrontAmount > 0) {
            if (collateralizer == address(this)) {
                ...
                _transferFrom(upFrontToken, collateralizer, self, upFrontAmount, permitParams);//@audit-ok R wtf self?? this is delegatecall. it transfer money to original contract.
            }
        }
        ...
        flashlender.flashLoan(
            ...
            abi.encode(leverParams, upFrontToken, upFrontAmount)//@audit-ok M upFrontAmount must == leverParams.primarySwap.amount. Otherwise stuck token
        );
    ...
    function onFlashLoan(
        address /*initiator*/,
        address /*token*/,
        uint256 amount,//primaryswap.amount
        uint256 fee,
        bytes calldata data
        ...
        }//@ uniswap swap with fee and unpredictable amount. recipient is user input.
        ...
        uint256 swapAmountOut = abi.decode(swapData, (uint256));//@audit-ok already check token type early H do not check which type of token here. Resulting in hack
        ...
        uint256 addDebt = amount + fee;//@audit L it is unclear primarySwap.Amount here is correctly same as swapAmountOut.
    ...
    function _deposit(
        ...
        } else if (collateralParams.collateralizer != address(this)) {//@audit-ok not msg.sender signature but owner passed signature. H signatureTransfer require msg.sender signature check.
            _transferFrom(
