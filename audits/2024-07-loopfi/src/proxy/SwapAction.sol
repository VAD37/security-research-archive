...
contract SwapAction is TransferAction {
    ...
    function transferAndSwap(
        address from,//@ if anyone can choose any input address. Anyone can steal token.
        PermitParams calldata permitParams,
        SwapParams calldata swapParams
    ) external returns (uint256) {
        if (from != address(this)) {//@use permit to transfer token from UniswapPermit2
        ...
        }//@audit-ok delegateCall to signature check hashed (msg.sender) == address(PrbProxy)  .H anyone can hjack signature signing and replace this with their own transaction. It can replace transfer amount, and swap action
