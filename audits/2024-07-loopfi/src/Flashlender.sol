...
contract Flashlender is IFlashlender, ReentrancyGuard {
    ...
    function flashLoan(
        ...
        if (token != address(underlyingToken)) revert Flash__flashLoan_unsupportedToken();//WETH
        uint256 fee = wmul(amount, protocolFee);
        uint256 total = amount + fee;

        pool.lendCreditAccount(amount, address(receiver));//receiver get WETH
        //@underneath it write debt to FlashLender account. update avaiable liquidity from pool. BaseInterestRate is updated after WETH is transfered to receiver. Which can reach 100% instantly
        emit FlashLoan(address(receiver), token, amount, fee);

        if (receiver.onFlashLoan(msg.sender, token, amount, fee, data) != CALLBACK_SUCCESS)
            revert Flash__flashLoan_callbackFailed();//@receiver handle their stuff.

        // reverts if not enough Stablecoin have been send back
        underlyingToken.transferFrom(address(receiver), address(pool), total);//transfer weth back to pool
        pool.repayCreditAccount(total - fee, fee, 0);//@audit H flashloan can be used to stole weth from receiver address. Without repay credit back
        //@after WETH transfer back, remove previous debt from CreditManager which is flashlender.
        return true;
    }
