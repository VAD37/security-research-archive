...
contract CDPVault is AccessControl, Pause, Permission, ICDPVaultBase {//@also CreditManager control PoolV3
    /*//////////////////////////////////////////////////////////////
                               LIBRARIES
    //////////////////////////////////////////////////////////////*/
    ...
    IERC20 public immutable poolUnderlying;//WETH
    ...
    struct VaultConfig {
        ...
        uint64 liquidationRatio;//@ 1.25e18
    }
    ...
    struct LiquidationConfig {
        ...
        uint64 liquidationPenalty;//@ 1e18
        /// @notice Discount on collateral during liquidation [wad]
        uint64 liquidationDiscount;//1.05e18
    }
    ...
    constructor(CDPVaultConstants memory constants, CDPVaultConfig memory config) {
        pool = constants.pool;//PoolV3
        oracle = constants.oracle;//@it is unclear the oracle here is balancer or chainlink. balancer oracle is TWAP
        token = constants.token;//pendle stETH
        tokenScale = constants.tokenScale;//token decimals

        poolUnderlying = IERC20(pool.underlyingToken());//WETH
    ...
    /// @param amount Amount of tokens to deposit [tokenScale] //@ USDC still send 100e6 USDC as input.no exploit int ->uint trick.
    ...
    function deposit(address to, uint256 amount) external whenNotPaused returns (uint256 tokenAmount) {
        tokenAmount = wdiv(amount, tokenScale);//@amount * 1e18 / tokenScale
    ...
    function _modifyPosition(
        ...
        uint256 newCumulativeIndex,//_calDebt.culumulativeIndexLastUpdate
        int256 deltaCollateral,//amount * 1e18/ tokenScale
        uint256 totalDebt_//@previous totalDebt
    ) internal returns (Position memory) {
        uint256 currentDebt = position.debt;
        // update collateral and debt amounts by the deltas
        position.collateral = add(position.collateral, deltaCollateral);//@safe add
        ...
        if (position.debt != 0 && position.debt < uint256(vaultConfig.debtFloor))//@debt floor = 0
            revert CDPVault__modifyPosition_debtFloor();//@audit R why it is not possible to repay all debt through modify position

        // store the position's balances
        positions[owner] = position;//not updating quota part here? Quota was updated through _calcDebt()
        ...
        if (address(rewardController) != address(0)) {//@whoami? masterchef update rewards to owner
    ...
    function modifyCollateralAndDebt(//@deposit
        address owner,//              input
        address collateralizer,//     msg.sender, PositionAction.onFlashLoan()
        address creditor,//           msg.sender
        int256 deltaCollateral,//     amount * 1e18 / tokenScale
        int256 deltaDebt//            0
    ) public {
        if (//@msg.sender bypass all these check.
            // position is either more safe than before or msg.sender has the permission from the owner
            ((deltaDebt > 0 || deltaCollateral < 0) && !hasPermission(owner, msg.sender)) || //@increase debt without permission
            // msg.sender has the permission of the collateralizer to collateralize the position using their cash
            (deltaCollateral > 0 && !hasPermission(collateralizer, msg.sender)) ||//@deposit without permission
            // msg.sender has the permission of the creditor to use their credit to repay the debt
            (deltaDebt < 0 && !hasPermission(creditor, msg.sender))//@borrow without permission
        ...
        uint256 profit;
        int256 quotaRevenueChange;//@quote must be growth rate value like rate per second.
        if (deltaDebt > 0) {
            ...
            console2.log("deltaDebt > 0: %e", deltaDebt);
            console2.log("newDebt: %e", newDebt);
            console2.log("newCumulativeIndex: %e", newCumulativeIndex);
            console2.log("debtData.cumulativeQuotaInterest: %e", debtData.cumulativeQuotaInterest);
            console2.log("debtData.cumulativeQuotaIndexNow: %e", debtData.cumulativeQuotaIndexNow);
            ...
            console2.log("quotaRevenueChange: %e", quotaRevenueChange);
            ...
            newDebt = position.debt;//@ this should be cdd.debt. but it is the same value
            newCumulativeIndex = debtData.cumulativeIndexLastUpdate;
        }

        if (deltaCollateral > 0) {//@deposit
            uint256 amount = wmul(deltaCollateral.toUint256(), tokenScale);//(amount * 1e18 / tokenScale) * tokenScale / 1e18 = amount in [tokenScale]
            console2.log("deposit collateral: %e", amount);
            ...
            console2.log("withdraw collateral: %e", amount);
        ...
        uint256 collateralValue = wmul(position.collateral, spotPrice_);//newCollateral_wad * spotPrice_wad / 1e18

        if (//@check borrow, withdraw collateral need to check for ratio liquidation
            (deltaDebt > 0 || deltaCollateral < 0) && //@position already modified here
        ...
        if (quotaRevenueChange != 0) {//@this update internally?
            console2.log("_quotaRevenueChange: %e", quotaRevenueChange);
            IPoolV3(pool).updateQuotaRevenue(quotaRevenueChange); // U:[PQK-15] //@this is force update to storage. No safe check for int cast.
        }
        emit ModifyCollateralAndDebt(owner, collateralizer, creditor, deltaCollateral, deltaDebt);
    }//@audit H vault accept stETH from pendle do have external rewards. how does it handle this?
    ...
    function _calcDebt(Position memory position) internal view returns (DebtData memory cdd) {
        console2.log("__calculateDebtPosition: timestamp delta:",(block.number - position.lastDebtUpdate) );
        uint256 index = pool.baseInterestIndex();//> 1e27
        ...
        console2.log("baseInterestIndex: %e", index);
        console2.log("cumulativeQuotaIndexLU: %e", position.cumulativeQuotaIndexLU);
        ...
        console2.log("cumulativeQuotaInterest: %e", cdd.cumulativeQuotaInterest);
        console2.log("cumulativeQuotaIndexNow: %e", cdd.cumulativeQuotaIndexNow);

        cdd.accruedInterest = CreditLogic.calcAccruedInterest(cdd.debt, cdd.cumulativeIndexLastUpdate, index);
        console2.log("accruedInterest1: %e", cdd.accruedInterest);
        cdd.accruedInterest += cdd.cumulativeQuotaInterest;
        console2.log("accruedInterest2: %e", cdd.accruedInterest);
    }
