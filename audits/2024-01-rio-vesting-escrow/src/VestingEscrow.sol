...
contract VestingEscrow is IVestingEscrow, Clone {
    ...
    function initialize(bool _isFullyRevokable, bytes calldata _initialDelegateParams) external {
        ...
        IERC20 _token = token();
        //@audit H fail if(block.timestamp > _endTime) emit AssertionFailed("block.timestamp > _endTime");        
    ...
    function unclaimed() public view returns (uint256) {
        ...
        //@note disabledAt == endTime or block.timestamp when revoked. 
    ...
    function locked() public view returns (uint256) {
        if (block.timestamp >= disabledAt) return 0;//@note when revoked. locked become 0
    ...
    function revokeUnvested() external onlyOwnerOrManager {
        uint256 revokable = locked();//@note locked = original token transfered in - vested at current time
        if (revokable == 0) revert NOTHING_TO_REVOKE();

        disabledAt = uint40(block.timestamp);//@audit revoked could make disableAt before startTime.
        if(revokable < token().balanceOf(address(this))) {
            emit AssertionFailed("revokable < balanceOf(address(this))");
        }
    ...
    function _totalVestedAt(uint256 time) internal pure returns (uint256) {
        uint40 _startTime = startTime();//@time == block.timestamp or disabledAt
        uint256 _totalLocked = totalLocked();
        if (time < _startTime + cliffLength()) {//@note when time is less than cliff. vested return 0 always
            return 0;//@note afer cliff over. vested become linear to from starttime to endtime. based on original locked token
        }
        return Math.min(_totalLocked * (time - _startTime) / (endTime() - _startTime), _totalLocked);
    }
    function totalVestedAt(uint256 time) external view returns (uint256) {
        return _totalVestedAt(time);
    }

            event AssertionFailed(uint256);
    event AssertionFailed();
    event AssertionFailed(string);
    event AssertionFailed(string,uint,uint);
}
