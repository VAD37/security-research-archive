...
contract VestingEscrowTest is TestUtil {
    function setUp() public {
        ...
        deployVestingEscrow(
            VestingEscrowConfig({
                amount: 10.123123123 ether,
                recipient: address(this),
                vestingDuration: 3 days,
                vestingStart: uint40(block.timestamp + 0),
                cliffLength: 3 days,
    ...
    function testCustom() public {
        ...
        console.log("totalLocked", deployedVesting.totalLocked());
        console.log("locked", deployedVesting.locked());
        console.log("vested", deployedVesting.totalVestedAt(block.timestamp));

        vm.warp(block.timestamp+ 1 days);
        
        
        deployedVesting.recoverERC20(address(token),  1e29 );

        console.log("totalLocked", deployedVesting.totalLocked());
        console.log("locked", deployedVesting.locked());
        console.log("vested", deployedVesting.totalVestedAt(block.timestamp));
        console.log("recipient balance", token.balanceOf(recipient));
        console.log("escrow balance", token.balanceOf(address(deployedVesting)));
    ...
    //     deployedVesting.recoverERC20(address(token), extra + 1e29 );
