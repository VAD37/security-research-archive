...
contract V3Oracle is IV3Oracle, Ownable, IErrors {
    uint16 public constant MIN_PRICE_DIFFERENCE = 200; // div by 10000 so 200 = 2%
    ...
    function getValue(uint256 tokenId, address token)//@ oracle check 3 price difference 3 times. 1 for each token. 1 for pool price in comparision with derived price from each token
        external
        view
        override
        returns (uint256 value, uint256 feeValue, uint256 price0X96, uint256 price1X96)//@audit oracle getValue must return price in USDC price not USD. Vault use USDC
    {
        (address token0, address token1, uint24 fee,, uint256 amount0, uint256 amount1, uint256 fees0, uint256 fees1) =
            getPositionBreakdown(tokenId);//@ok liquidity of pool token always exist. wrong INIT_CODE .getValue always fail for USDC token. because its uniswap pool not exist. for USDC/USD
        // console.log("getValue token %s",token);
        // console.log("token0: %s, token1: %s",IERC20Metadata(token0).symbol(),IERC20Metadata(token1).symbol());
        uint256 cachedChainlinkReferencePriceX96;//@USDC/USD priceXQ96. ~0.9999e8 

        (price0X96, cachedChainlinkReferencePriceX96) =//@note price from oracle read directly from one of chainlink or TWAP but not both. verifyPrice only check if one of the price have move too far.
        ...
        uint256 priceTokenX96;//@ token price in USD or atleast in USDC.
        if (token0 == token) {//Q96 ~= 8e28
            priceTokenX96 = price0X96;//NFT 123 with DAI token e18. price0X96 = 7.9219491309416672e16
        } else if (token1 == token) {//USDC price1X96 = 7.9228162514264337593543950336e28
            priceTokenX96 = price1X96;// 0.3e18 DAI and 9.436e6 USDC
        } else {
            (priceTokenX96,) = _getReferenceTokenPriceX96(token, cachedChainlinkReferencePriceX96);
        }//@token amount0,fee0 all in original token decimals. but what about price? it is e96 base to USD.
        // console.log("-----------------");
        // console.log("Q96: %e",Q96);//  Q96: 7.9228162514264337593543950336e28
        // console.log("derivedPoolPriceX96_original: %e",price0X96 * Q96 / price1X96);
        // console.log("price0X96: %e",price0X96);
        // console.log("price1X96: %e",price1X96);
        // console.log("priceTokenX96: %e",priceTokenX96);
        // console.log("liquidity0: %e",amount0+ fees0);
        // console.log("liquidity1: %e",amount1+ fees1);
        value = (price0X96 * (amount0 + fees0) / Q96 + price1X96 * (amount1 + fees1) / Q96) * Q96 / priceTokenX96;//@ value is total liquidity value in request token price
        feeValue = (price0X96 * fees0 / Q96 + price1X96 * fees1 / Q96) * Q96 / priceTokenX96;//@ same with fee. if fee is 10 USDC in request WETH. then it worth 0.005 ETH with 2000$ ETH
        price0X96 = price0X96 * Q96 / priceTokenX96;//This is just price converted to request token Price
        price1X96 = price1X96 * Q96 / priceTokenX96;
        // console.log("value: %e",value);
        // console.log("feeValue: %e",feeValue);
        // // console.log("price0X96 modified: %e",price0X96);
        // // console.log("price1X96 modified: %e",price1X96);
        // console.log("value0 modified: %e",price0X96 * (amount0 + fees0) / Q96 );
        // console.log("value1 modified: %e",price1X96 * (amount1 + fees1) / Q96 );
        // console.log("value modified: %e",price0X96 * (amount0 + fees0) / Q96 + price1X96 * (amount1 + fees1) / Q96);
        ...
        uint256 derivedPoolPriceX96 = price0X96 * Q96 / price1X96;//@this derived Pool price is modified after convert price into request token price. there is slight loss of conversion?
        // console.log("derivedPoolPriceX96_loss: %e",derivedPoolPriceX96);//@ok the loss is minimal
        _checkPoolPrice(token0, token1, fee, derivedPoolPriceX96); //@audit double pool price check? This does not work with oracle mode Chainlink oracle alone
    }
        ...
        view
    {
        ...
        // console.log("differenceX10000: %s , maxX10000:",differenceX10000, maxDifferenceX10000);
        if (differenceX10000 >= maxDifferenceX10000) {// 200/10000 = 2%
    ...
    {
        if (token == referenceToken) {
            return (Q96, chainlinkReferencePriceX96);//@audit-ok oracle value return USDC price M USDC does not mean it always 1 USD. This oracle does not consult chainlink oracle for USDC/USD price at all
        }
        ...
        if (usesChainlink) {
            ...
            if (feedConfig.mode == Mode.TWAP_CHAINLINK_VERIFY) {
                verifyPriceX96 = chainlinkPriceX96; //@ later priceX96 =  twapPriceX96
            } else {//@CHAINLINK || CHAINLINK_TWAP_VERIFY
                priceX96 = chainlinkPriceX96; //@ later verifyPriceX96 =  twapPriceX96 
            }
        }
        ...
        if (usesTWAP) {
            ...
            } else {//@TWAP || TWAP_CHAINLINK_VERIFY
                priceX96 = twapPriceX96;
            }
        }

        if (feedConfig.mode == Mode.CHAINLINK_TWAP_VERIFY || feedConfig.mode == Mode.TWAP_CHAINLINK_VERIFY) {
            // console.log("priceX96: %e",priceX96);
            // console.log("verifyPriceX96: %e",verifyPriceX96);
            _requireMaxDifference(priceX96, verifyPriceX96, feedConfig.maxDifference);//@ 200 is max different
        }
    }

    // calculates chainlink price given feedConfig
    function _getChainlinkPriceX96(address token) internal view returns (uint256) {
        if (token == chainlinkReferenceToken) {//@audit what happen when chainlinkReferenceToken is USDC instead of address(0)
            return Q96;
        }
    ...
    function _getFeeGrowthInside(
        ...
        // allow overflow - this is as designed by uniswap - see PositionValue library (for solidity < 0.8)
        unchecked {//@audit-ok R overflow not exist in modern solidity. This might be vulneribility fixed.
