...
abstract contract Automator is Swapper, Ownable {
    ...
    function _validateSwap(
        ...
        if (swap0For1) {
            amountOutMin = FullMath.mulDiv(amountIn * (Q64 - maxPriceDifferenceX64), priceX96, Q96 * Q64);//@audit M calculate amountOutMin with slot0 Price is not accurate. If liquidity is really low for current tick
        } else {//@ Swap with this price does not we gain same amount expected. Price can be moved both way. But then does amountOut zero-block work most of the time?
