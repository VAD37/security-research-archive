...
//@note vesting only have 4 public function for Auth VESTING_MANAGER_ROLE. And we have 1M token to start with
    ///@notice the VVV token being vested
    IERC20 public VVVToken;
//@note vesting just receive token directly
    /**
    ...
    */
    ...
    function withdrawVestedTokens(
        ...
        }//@getVestedAmount roundUp release token. there is nothing wrong with calculation
    ...
    function _setVestingSchedule(SetVestingScheduleParams memory _params) private {
        ...
        if (_params.vestingScheduleIndex == userVestingSchedules[_params.vestedUser].length) {
            userVestingSchedules[_params.vestedUser].push(newSchedule);//@add new
        } else if (_params.vestingScheduleIndex < userVestingSchedules[_params.vestedUser].length) {
            userVestingSchedules[_params.vestedUser][_params.vestingScheduleIndex] = newSchedule;//@replace
        } else {
            revert InvalidScheduleIndex();
        }
...
//@vestingSchedule can be empty. vestingSchedule is full struct take 2 memory slot. when delete the whole thing reset to 0
        if (
            ...
            return 0;//@when empty it will return 0
        } else if (block.timestamp < vestingSchedule.cliffEndTime) {
            return vestingSchedule.tokensToVestAtStart;//@when vesting not reach cliff. return initial token.
        } else {//@ when block.timestamp > cliffEndTime
            uint256 elapsedIntervals;
            uint256 vestedAmount;

            unchecked {//@audit unchecked math does not convert block.timestamp to uint32
                elapsedIntervals =
                    (block.timestamp - vestingSchedule.cliffEndTime) /   //@ (block.timestamp - vestingSchedule.cliffEndTime) >= 0
                    vestingSchedule.intervalLength;
                elapsedIntervals = elapsedIntervals > vestingSchedule.maxIntervals //@clamp max intervals
                    ? vestingSchedule.maxIntervals
                    : elapsedIntervals;
                vestedAmount =//@audit-ok  overflow only when math operation with pre hardcoded valued .uint88 overflow? This only happen with hardcoded value. 
                    vestingSchedule.tokensToVestAtStart +
    ...
    function calculateVestedAmountAtInterval(
        uint256 _firstIntervalAccrual,//tokensToVestAfterFirstInterval  100e18
        uint256 _elapsedIntervals,//delta time / interval    // 0 ->100
        uint256 _growthRateProportion//growthRateProportion // 0 but it should be range 0-1e18
    ) public pure returns (uint256) {
        if (_growthRateProportion == 0 || _elapsedIntervals == 0) {
            return _firstIntervalAccrual * _elapsedIntervals;
        } else {//@_growthRateProportion < 1e40
            uint256 r;//@audit r must > 1e18 or WAD
            uint256 rToN;
            uint256 Sn;
            unchecked {
                // Convert growth rate proportion to a fixed-point number with 1e18 scale
                r = FixedPointMathLib.divWadDown(// r = (_growthRateProportion + FixedPointMathLib.WAD) * 1e18 / 1e18
                    _growthRateProportion + FixedPointMathLib.WAD,
                    FixedPointMathLib.WAD
                );//@audit when growth rate is 0 it just increase first interval accrual repeatedly.

                // Calculate r^n
                rToN = FixedPointMathLib.rpow(r, _elapsedIntervals, FixedPointMathLib.WAD);//if r == 1e18. r still 1e18
    ...
    */
    function setVestingSchedule(
        ...
        newSchedule.tokensToVestAtStart = _tokensToVestAtStart;//@ 1000e18
        newSchedule.tokensToVestAfterFirstInterval = _tokensToVestAfterFirstInterval;//100e18
        newSchedule.tokenAmountWithdrawn = _vestingScheduleAmountWithdrawn;//0
        newSchedule.scheduleStartTime = _vestingScheduleStartTime;// 2 days from now
        newSchedule.cliffEndTime = _vestingScheduleCliffEndTime;// 365 days vest cliff
        newSchedule.intervalLength = _vestingScheduleIntervalLength;// 12
        newSchedule.maxIntervals = _vestingScheduleMaxIntervals;// 100
    ...
    function removeVestingSchedule(
        ...
        delete userVestingSchedules[_vestedUser][_vestingScheduleIndex];//@audit delete vesting schedule does not reduce length of array
