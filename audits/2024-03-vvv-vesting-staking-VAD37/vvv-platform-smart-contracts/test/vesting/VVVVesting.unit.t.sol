...
contract VVVVestingUnitTests is VVVVestingTestBase {
    ...
    function testdebugVesting() public {
        uint120 tokensToVestAfterFirstInterval = 200000.00 ether; 
        uint64 growthRateProportion = 176505576804629530; 

        VestingParams memory params = VestingParams({
            vestingScheduleIndex: 0,
            tokensToVestAtStart: 1_000_000 * 1e18, 
            tokensToVestAfterFirstInterval: uint120(tokensToVestAfterFirstInterval), 
            amountWithdrawn: 0,
            scheduleStartTime: uint32(block.timestamp + 60 * 60 * 24 * 2),
            cliffEndTime: uint32(block.timestamp + 60 * 60 * 24 * 365),
            intervalLength: 1 days,
            maxIntervals: 100,
            growthRateProportion: uint64(growthRateProportion)
        });
        ...
        for (uint i = 0; i < 100; i++) {
            skip(60 * 60 * 24 );
            uint vestedAmount = VVVVestingInstance.getVestedAmount(sampleUser,0);
            console.log("index %s, vestedAmount %e", i, vestedAmount);
        }
    }

    function testdebugVesting2() public {
