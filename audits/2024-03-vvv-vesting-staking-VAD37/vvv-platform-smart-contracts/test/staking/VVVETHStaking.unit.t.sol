...
import {VVVETHStakingTestBase, console} from "test/staking/VVVETHStakingTestBase.sol";
...
contract VVVETHStakingUnitTests is VVVETHStakingTestBase {
    ...
    function setUp() public {
        ...
        AuthRegistry.setPermission(address(EthStakingInstance), setVvvTokenSelector, ethStakingManagerRole);
    ...
    function testInitialization() public {
        assertTrue(EthStakingInstance.durationToSeconds(VVVETHStaking.StakingDuration.ThreeMonths) == 90 days);
        assertTrue(EthStakingInstance.durationToSeconds(VVVETHStaking.StakingDuration.SixMonths) == 180 days);
        assertTrue(EthStakingInstance.durationToSeconds(VVVETHStaking.StakingDuration.OneYear) == 360 days);
    }
    ...
    function testStakeEthMultiple() public {
        ...
        uint256 stakeIdBefore = EthStakingInstance.stakeId();
        EthStakingInstance.stakeEth{value: stakeEthAmounts[0]}(VVVETHStaking.StakingDuration.ThreeMonths);
    ...
    function _testDebugRestake() public {


        vm.startPrank(sampleUser, sampleUser);
        uint256 stakeEthAmount = 1 ether;
        EthStakingInstance.stakeEth{value: 1}(VVVETHStaking.StakingDuration.ThreeMonths);
        ...
        // restake
        VVVETHStaking.StakingDuration restakeDuration = VVVETHStaking.StakingDuration.SixMonths;
        uint256 newStakeId = EthStakingInstance.restakeEth(stakeId, restakeDuration);

        vm.stopPrank();
        address addr = sampleUser;
        
        emit log_named_bytes32("slot 3", keccak256(abi.encode(addr, 3)));
        emit log_named_bytes32("slot 4", keccak256(abi.encode(addr , 4)));
        emit log_named_bytes32("slot 5", keccak256(abi.encode(addr , 5)));
        emit log_named_bytes32("slot 6", keccak256(abi.encode(addr , 6)));
        emit log_named_bytes32("slot 7", keccak256(abi.encode(addr , 7)));

        emit log_named_bytes32(
            "slot 4 mapping", 
            keccak256(abi.encode(stakeId , keccak256(abi.encode(addr , 4)) ))
        );
        emit log_named_bytes32(
            "slot 4 mapping new", 
            keccak256(abi.encode(newStakeId , keccak256(abi.encode(addr , 4)) ))
        );

        emit log_named_bytes32(
            "slot 5 mapping", 
            keccak256(abi.encode( keccak256(abi.encode(addr , 5)) ))
        );


        (bytes32[] memory reads, bytes32[] memory writes) = vm.accesses(address(EthStakingInstance));
        for (uint256 i = 0; i < writes.length; i++) {
            emit log_bytes32(writes[i]);
        }
    }
    ...
    function testWithdrawDifferentOrderMultiple() public {
        ...
        for (uint256 i = 0; i < stakeEthAmounts.length; i++) {
            ...
            EthStakingInstance.stakeEth{value: stakeEthAmounts[i]}(VVVETHStaking.StakingDuration.OneYear);
        }
    ...
    function testClaimVvv() public {
        ...
        uint256 expectedClaimedVvv = (
            stakeEthAmount * EthStakingInstance.ethToVvvExchangeRate()
    ...
    function testClaimVvvBeforeAndAfterWithdrawStakeMultipleClaims() public {
        ...
        advanceBlockNumberAndTimestampInSeconds(
            EthStakingInstance.durationToSeconds(VVVETHStaking.StakingDuration.ThreeMonths) / stakingDurationDivisor
        );
    ...
    function testCalculateAccruedVvvAmountSingleStake() public {
        ...
        uint256 expectedAccruedVvv = (
            stake.stakedEthAmount * EthStakingInstance.ethToVvvExchangeRate()
                * EthStakingInstance.durationToMultiplier(VVVETHStaking.StakingDuration.ThreeMonths)
        ) / EthStakingInstance.DENOMINATOR();
    ...
    function testsetDurationMultipliers() public {
        ...
        assertTrue(EthStakingInstance.durationToMultiplier(VVVETHStaking.StakingDuration.ThreeMonths) == 20_000);
        assertTrue(EthStakingInstance.durationToMultiplier(VVVETHStaking.StakingDuration.SixMonths) == 27_000);
        assertTrue(EthStakingInstance.durationToMultiplier(VVVETHStaking.StakingDuration.OneYear) == 33_000);
    }
    ...
    function testEmitWithdrawStake() public {
        ...
        uint256 stakeId = EthStakingInstance.stakeEth{value: stakedEthAmount}(VVVETHStaking.StakingDuration.ThreeMonths);
