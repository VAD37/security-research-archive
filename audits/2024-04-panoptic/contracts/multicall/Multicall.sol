...
abstract contract Multicall {//@nothing change from previous version
    ...
    function multicall(bytes[] calldata data) public payable returns (bytes[] memory results) {
        ...
        for (uint256 i = 0; i < data.length; ) {
            ...
            results[i] = result;//@note ok it revert on first call fail still return result from success call until broken one .do multicall does not revert on first fail but still run all the calls?

            unchecked {
                ++i;
            }//@note multicall use self delegatecall and its function is public. Is this work with proxy contract
        }
    }
}
