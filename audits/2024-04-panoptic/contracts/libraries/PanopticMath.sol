...
library PanopticMath {
    ...
    function getPoolId(address univ3pool) internal view returns (uint64) {
        unchecked {
            int24 tickSpacing = IUniswapV3Pool(univ3pool).tickSpacing();//@audit M duplicate PoolId can be generated uniswap poolID is 12 char. Seaport managed to generate address with 13 char
            uint64 poolId = uint64(uint160(univ3pool) >> 112);//https://github.com/tintinweb/smart-contract-sanctuary-ethereum/blob/master/contracts/mainnet/00/0000000000000aD24e80fd803C6ac37206a45f15_Seaport.sol
    ...
    function incrementPoolPattern(uint64 poolId) internal pure returns (uint64) {//@ok
        unchecked {
            // increment
            return (poolId & TICKSPACING_MASK) + (uint48(poolId) + 1);//@audit L poolId overflow when address is uniswapPool address is fffffffff
        }
    }
    ...
    function numberOfLeadingHexZeros(address addr) external pure returns (uint256) {//@ok
        unchecked {
    ...
    function updatePositionsHash(
        ...
        TokenId tokenId,//@ XOR edit last 18 hex
        bool addFlag//@change first 2 hex, 8 bits
    ) internal pure returns (uint256) {
        // add the XOR`ed hash of the single option position `tokenId` to the `existingHash`
        // @dev 0 ^ x = x

        unchecked {//@this flip 0^1=1, 0^0 = 0, 1^1=0, 1^0=1
            // update hash by taking the XOR of the new tokenId
            uint248 updatedHash = uint248(existingHash) ^
                (uint248(uint256(keccak256(abi.encode(tokenId)))));//@hex value flip is kinda weird with flip inverse of pair like 0xef->0xfe 0xab->0xba
            // increment the top 8 bit if addflag=true, decrement otherwise
            return
                addFlag
                    ? uint256(updatedHash) + (((existingHash >> 248) + 1) << 248)
                    : uint256(updatedHash) + (((existingHash >> 248) - 1) << 248);//@audit M updatePositionHash allow overflow hashing.
        }
    }
