...
contract ARCDVestingVault is IARCDVestingVault, HashedStorageReentrancyBlock, BaseVotingVault {
    ...
    function addGrantAndDelegate(
        ...
        // set the new grant
        grant.allocation = amount;//@delete here does not work as this is reading directly from hashed storage
        grant.cliffAmount = cliffAmount;
        grant.withdrawn = 0;
        grant.created = startTime;
        grant.expiration = expiration;
        grant.cliff = cliff;
        grant.latestVotingPower = newVotingPower;//@audit-ok do withdraw allocation from grant remove voting power?
        grant.delegatee = delegatee;

        // update the amount of unassigned tokens
        unassigned.data -= amount;//@ unassigned = token transfered in. Does user can withdraw allocation to prevent lost token?

        // update the delegatee's voting power
        History.HistoricalBalances memory votingPower = _votingPower();//@audit-ok M this should be storage? Not write back to storage?
    ...
    function revokeGrant(address who) external virtual onlyManager {
        ...
        // update the delegatee's voting power
        _syncVotingPower(who, grant);//@audit e test for voting power desync.
    ...
    function debug_getVotingPower(address who) public view returns(uint){
        History.HistoricalBalances memory votingPower = _votingPower();
        return votingPower.loadTop(who);
    }
    // =========================================== HELPERS ==============================================
