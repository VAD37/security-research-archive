...
contract ReputationBadge is ERC1155, AccessControl, ERC1155Burnable, IReputationBadge {
    ...
    function mint(
        ...
        if (msg.value < mintPrice) revert RB_InvalidMintFee(mintPrice, msg.value);//@audit M mint token allow user to send more eth than necessary.
        ...
        }//@audit M does not support multiple address in same MerkleRoot. Using totalAmount logic based on one leaf.

        // increment amount claimed
        amountClaimed[recipient][tokenId] += amount;//@note mint NFT token is safe. but user can be force gifted
    ...
    function publishRoots(ClaimData[] calldata _claimData) external onlyRole(BADGE_MANAGER_ROLE) {
        ...
        for (uint256 i = 0; i < _claimData.length; i++) {
            ...
            //@gas cache
            claimRoots[_claimData[i].tokenId] = _claimData[i].claimRoot;//@audit L admin can replace old merkle root
