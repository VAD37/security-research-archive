...
contract GSCVault is Authorizable, IVotingVault {
    ...
    constructor(
        ...
        uint256 _votingPowerBound,//110_000,test is 50
        address _owner
    ) {
    ...
    function proveMembership(
        ...
        bytes[] calldata extraData //@audit L no range check
    ) external {
        ...
        for (uint256 i = 0; i < votingVaults.length; i++) {
            ...
            uint256 votes =//@ vault here is NFTBoostVault
                IVotingVault(votingVaults[i]).queryVotePower(//@note membership query itsown vault vote?
                    msg.sender,
                    block.number - 1,
                    extraData[i]//@unused
                );
        ...
        require(totalVotes >= votingPowerBound, "Not enough votes");//@audit only owner can prove membership? How else other user can prove membership
        ...
        if (members[msg.sender].joined != 0) {
            ...
            members[msg.sender] = Member(votingVaults, block.timestamp);//@note member can change register vault anytime .
        }
    ...
    function kick(address who, bytes[] calldata extraData) external {
        ...
        require(totalVotes < votingPowerBound, "Not kick-able");//@audit admin change votingPowerBound allow kick user
        // Delete the member
        delete members[who];//@audit-ok M struct is dynamic address[] still got deleted 
    ...
    function queryVotePower(
        ...
        // a protocol upgrade.
        if (who == owner) {//@audit L - Vote is hardcoded. this prevent admin to refuse vote if we have 200K user 
            return 100000;
        }
