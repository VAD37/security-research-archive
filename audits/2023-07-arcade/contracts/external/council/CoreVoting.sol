...
contract CoreVoting is Authorizable, ReentrancyBlock, ICoreVoting {
    ...
    mapping(address => bool) public override approvedVaults;//GSC vaults
    ...
    constructor(
        ...
        setOwner(address(_timelock));//@ok weird but work
        _authorize(_gsc);
    }
    ...
    function proposal(
        ...
        bytes32 proposalHash = keccak256(abi.encode(targets, calldatas));//@audit proposolHash can still be duplicated. where is the verification
        ...
        for (uint256 i = 0; i < targets.length; i++) {
            ...
            uint256 unitQuorum = _quorums[targets[i]][selector];//@audit L this should have use default getFunction quorum?
        ...
        proposals[proposalCount] = Proposal(
            ...
            uint128(block.number - 1),//created
            uint128(block.number + lockDuration),//unlock
            uint128(block.number + lockDuration + extraVoteTime),//expiration
            uint128(quorum),//max quorum in list
            proposals[proposalCount].votingPower,//@audit R this is empty?
            uint128(lastCall)//8 days higher than expiration
        );
        ...
        // the execution
        uint256 minPower =//@note quorum is set by admin. lowest is always 1. Quorum is highest value in operations so it always 3.
    ...
    function vote(//@audit M lack unvote function
        address[] memory votingVaults,
        ...
        require(block.number <= proposals[proposalId].expiration, "Expired");// this still allow lastcall execution
        ...
        for (uint256 i = 0; i < votingVaults.length; i++) {
            ...
            }//@gas this can be moved to proposol
            require(approvedVaults[votingVaults[i]], "unverified vault");
            votingPower += uint128(//@audit-ok M votingPower rely on unsafe selfcheck VotingVault.
                IVotingVault(votingVaults[i]).queryVotePower(
                    msg.sender,
                    proposals[proposalId].created,//@note voteVault.queryVotePower do not use 2nd or 3rd variable. this look weird
                    extraVaultData[i]
                )//vote return 1 or 10000 if admin
            );
        }
        ...
        if (votes[msg.sender][proposalId].votingPower > 0) {
            proposals[proposalId].votingPower[
                uint256(votes[msg.sender][proposalId].castBallot)//@gas cache lol???
            ] -= votes[msg.sender][proposalId].votingPower;//@admin can remove their own vote from 10k votePower?
        }//@audit-ok M how admin can block vote proposol though? This was reference as admin have power to block vote.
        votes[msg.sender][proposalId] = Vote(votingPower, ballot);//@ override vote ballot?
    ...
    function execute(
        ...
        require(
            keccak256(abi.encode(targets, calldatas)) ==//@audit-ok now abi.encode is quite interesting with how bytes work. Execution might have quite a surprise.
        ...
        bool passesQuorum =
            results[0] + results[1] + results[2] >=//0 = yes, 1 = no, 2 = maybe
                proposals[proposalId].quorum;
        bool majorityInFavor = results[0] > results[1];//@audit maybe vote count as yes by abstaining
        ...
        // are never deleted
        delete proposals[proposalId];//@ok delete normal struct
    }
