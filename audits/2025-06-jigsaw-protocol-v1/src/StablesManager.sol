...
contract StablesManager is IStablesManager, Ownable2Step, Pausable {
     ...
     */
    function borrow(
        address _holding,//msg.sender holding
        address _token,//@whitelisted token
        uint256 _amount,//@collateral amount take away from user
        uint256 _minJUsdAmountOut,//@input
        bool _mintDirectlyToUser//@input
    ) external override onlyAllowed whenNotPaused returns (uint256 jUsdMintAmount) {
        require(_amount > 0, "3010");//@holdingManager, LiquidityManager, strategy
        require(shareRegistryInfo[_token].active, "1201");//@admin must register token for borrowing like USDC, WETH

        BorrowTempData memory tempData = BorrowTempData({
            registry: ISharesRegistry(shareRegistryInfo[_token].deployedAt),
            exchangeRatePrecision: manager.EXCHANGE_RATE_PRECISION(),//1e18
            amount: 0,
            amountValue: 0
        });

        // Ensure amount uses 18 decimals.//@ 600e6 USDC -> 600e18
        ...
        tempData.amountValue = //@usd_value_e18 = amount_e18 * exchangeRate_e18 /1e18
            tempData.amount.mulDiv(tempData.registry.getExchangeRate(), tempData.exchangeRatePrecision);//@oracle peek

        // Get the jUSD amount based on the provided collateral's USD value.
        jUsdMintAmount = tempData.amountValue.mulDiv(tempData.exchangeRatePrecision, manager.getJUsdExchangeRate());//1:1 ratio in USD
        ...
        totalBorrowed[_token] += jUsdMintAmount;//@tracking only
    ...
    function repay(
        ...
        address _token,
        uint256 _amount,
        address _burnFrom//@possible any. no check
    ) external override onlyAllowed whenNotPaused {//@repay from strategy manager
        require(shareRegistryInfo[_token].active, "1201");
    ...
    function isSolvent(address _token, address _holding) public view override returns (bool) {
        ...
        //getRatio return value of collateral in USD_e18 * collateralRate(75%)
        return getRatio({ _holding: _holding, registry: registry, rate: registry.getConfig().collateralizationRate })
            >= registry.borrowed(_holding).mulDiv(manager.getJUsdExchangeRate(), manager.EXCHANGE_RATE_PRECISION());//@JUSD exchange rate is 1:1
    }//@just collateral * 75% > borrowing * e18/e18
    ...
    function isLiquidatable(address _token, address _holding) public view override returns (bool) {
        ...
        uint256 threshold = registryConfig.collateralizationRate + registryConfig.liquidationBuffer;// 85% + 5%

        // Returns true when the ratio is below the liquidation threshold
        return getRatio({ _holding: _holding, registry: registry, rate: threshold })//allow 105% of original collateral ??
            <= registry.borrowed(_holding).mulDiv(manager.getJUsdExchangeRate(), manager.EXCHANGE_RATE_PRECISION());
    }//@audit-ok The logic just look inverted.liquidatable seem like it rate much smaller than collateral rate. Threshold = collateralRate - liquidationBuffer
    ...
    function getRatio(address _holding, ISharesRegistry registry, uint256 rate) public view returns (uint256) {
        ...
        uint256 exchangeRate = registry.getExchangeRate();//@collateral rate in USD_e18
        // Get holding's available collateral amount.
        uint256 colAmount = registry.collateral(_holding);//@collateral only increase by user deposit directly, no balanceOf
        ...
        // Transform to 18 decimals if needed. //collateral value in USD_e18
