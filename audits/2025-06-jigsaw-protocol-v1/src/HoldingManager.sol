...
contract HoldingManager is IHoldingManager, Ownable2Step, Pausable, ReentrancyGuard {
    ...
    function createHolding() external override nonReentrant whenNotPaused returns (address) {
        ...
        if (msg.sender != tx.origin) {//@audit EIP7702 account abstraction
            require(manager.isContractWhitelisted(msg.sender), "1000");//@no clue who is whitelisted contracts, should be strategy right?
        }
    ...
    function deposit(
        ...
        nonReentrant
        whenNotPaused
    {//@cash from user to holding address
        _deposit({ _from: msg.sender, _token: _token, _amount: _amount });
    }//SharesRegistry  collateral[holding] += amount
        ...
        whenNotPaused
    {
        ...
        (uint256 userAmount, uint256 feeAmount) = _withdraw({ _token: _token, _amount: _amount });//@audit-ok Emergency Invoker must be whitelisted by manager M anyone can bypass withdrawal fee by using emergency function from HOlding contract. Just empty collateral and funds
        ...
        holding.transfer({ _token: _token, _to: msg.sender, _amount: userAmount });//@audit I get withdraw more token from holding than what is collateral. Is this used as a way to get extra cash token from interest?
    }
    ...
    function borrowMultiple(
        ...
        for (uint256 i = 0; i < _data.length; i++) {
            uint256 jUsdMinted = _getStablesManager().borrow({
                ...
                _mintDirectlyToUser: _mintDirectlyToUser
            });//@all borrowing have post check for collateral, internal and modular.
    ...
    function _deposit(address _from, address _token, uint256 _amount) private {
        ...
        _getStablesManager().addCollateral({ _holding: holding, _token: _token, _amount: _amount });// collateral[holding] += amount
    }//@audit I stablesManager can be changed and old collateral info will be lost
    ...
    function _withdraw(address _token, uint256 _amount) private returns (uint256, uint256) {
        ...
        if (_tokenRegistry != address(0) && ISharesRegistry(_tokenRegistry).collateral(holding) > 0) {//@audit I update registryInfo or change Registry from StablesManager. Also allow user to withdraw without collateral lost
            _getStablesManager().removeCollateral({ _holding: holding, _token: _token, _amount: _amount });//@also check collateral ratio based on registry from manager.
        }//@audit-ok This mechanism exist because Holding might never hold any collateral but try to withdraw it anyway based on user demand.M withdraw too much, share got rounded down???? collateral got rounded down here
        uint256 withdrawalFee = manager.withdrawalFee();
        uint256 withdrawalFeeAmount = 0;
        if (withdrawalFee > 0) withdrawalFeeAmount = OperationsLib.getFeeAbsolute(_amount, withdrawalFee);//max 8% withdrawal fee.no fee on init
