...
contract WETHUSDCLoansForkTest is BaseLoansForkTest {
    function setUp() public override {
        ...
        vm.label(address(pair.loansContract), "Loans Contract");
        vm.label(address(pair.providerNFT), "Provider NFT");
        vm.label(address(pair.takerNFT), "Taker NFT");
        vm.label(address(pair.rollsContract), "Rolls Contract");
        vm.label(address(pair.escrowNFT), "Escrow NFT");
        vm.label(address(pair.cashAsset), "Cash Asset");
        vm.label(address(pair.underlying), "Underlying Asset");
        vm.label(address(pair.oracle), "Oracle");
        vm.label(address(pair.swapperUniV3), "Swapper UniV3");
                
        console.log("CollarProviderNFT: ", address(pair.providerNFT));
        console.log("CollarTakerNFT: ", address(pair.takerNFT));
        console.log("LoansNFT: ", address(pair.loansContract));
        console.log("Rolls: ", address(pair.rollsContract));
        console.log("cashAsset: ", address(pair.cashAsset));
        console.log("underlying: ", address(pair.underlying));
        console.log("EscrowSupplierNFT: ", address(pair.escrowNFT));
        if(router == address(0)) {
            router = 0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45;
        }
        oracleTWAP = new OracleUniV3TWAP(underlying, cashAsset, swapPoolFeeTier, 300, router, sequencerFeed);
    }
    ...
    function _setParams() internal virtual {
        ...
        vm.label(underlying, "WETH");
        vm.label(cashAsset, "USDC");
    }
}
