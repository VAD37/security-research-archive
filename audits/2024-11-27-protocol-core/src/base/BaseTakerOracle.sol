...
abstract contract BaseTakerOracle is ITakerOracle {
    ...
    constructor(address _baseToken, address _quoteToken, address _sequencerChainlinkFeed) {
        baseToken = _baseToken;//USDC , WETH
        quoteToken = _quoteToken;//0xff, 0xff
        baseUnitAmount = _unitAmount(baseToken);// e6 , e18
        quoteUnitAmount = _unitAmount(quoteToken);// e18
    ...
    function sequencerLiveFor(uint atLeast) public view virtual returns (bool) {
        ...
        */
        return answer == 0 && block.timestamp - startedAt >= atLeast;//1 hour
    }
    ...
    }//@quoteTokenAmount= oracle.currentPrice()
