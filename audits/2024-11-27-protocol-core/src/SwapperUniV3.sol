...
contract SwapperUniV3 is ISwapper {
    ...
    constructor(address _uniV3SwapRouter, uint24 _feeTier) {
        ...
        );// https://vscode.blockscan.com/42161/0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45
        uniV3SwapRouter = IV3SwapRouter(payable(_uniV3SwapRouter));//0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45 SwapRouter02
        swapFeeTier = _feeTier;//500 = 0.05% WETH/USDC with very low liquidity
    }
    ...
    function swap(IERC20 assetIn, IERC20 assetOut, uint amountIn, uint minAmountOut, bytes calldata extraData)
        external
        returns (uint amountOut)// @ 1e18 WETH -> USDC , minimum: 0.3 USDC out, extraData can be anything
    {
        ...
        assetIn.safeTransferFrom(msg.sender, address(this), amountIn);//@pull from LoansNFT. LoansNFT already pull 1e18 from user and force approve to this contract
        ...
        uint senderBalanceBefore = assetOut.balanceOf(msg.sender); //@cache USDC balance of LoansNFT
        ...
        uint amountOutRouter = uniV3SwapRouter.exactInputSingle(
            IV3SwapRouter.ExactInputSingleParams({
                tokenIn: address(assetIn),// WETH
                tokenOut: address(assetOut),// USDC
                fee: swapFeeTier,//0.03%
                recipient: msg.sender,// LoansNFT receive all USDC output
                amountIn: amountIn, // 1e18 WETH
                amountOutMinimum: minAmountOut, // 0.3 USDC
                sqrtPriceLimitX96: 0
            })
        );
