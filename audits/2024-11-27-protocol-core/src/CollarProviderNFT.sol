...
contract CollarProviderNFT is ICollarProviderNFT, BaseNFT {
    ...
    constructor(
        ...
        address _taker,// WETH/USDC with same config
        string memory _name,//Provider WETH/USDC
        string memory _symbol
    ) BaseNFT(initialOwner, _name, _symbol) {
        cashAsset = _cashAsset;
        underlying = _underlying;
        taker = _taker;//@audit-ok fancy check On Taker contract. shared by multiple contracts
    ...
    function protocolFee(uint providerLocked, uint duration) public view returns (uint fee, address to) {
        ...
        fee = to == address(0) //@ 1year = 31536000 * 10000 =31536000_0000
            ? 0//@fee = 1% = 100
            // rounds up to prevent avoiding fee using many small positions.
            : Math.ceilDiv(providerLocked * configHub.protocolFeeAPR() * duration, BIPS_BASE * YEAR);//@ default fee = amount * 3_153_600_000
    }// 5min  = 0.0000095129 . small amount of USDC always have fee even with 5 min durations.
    ...
    function createOffer(
        uint callStrikePercent,// 11_000 range:100% - 1000%
        uint amount, // 100_000e6
        uint putStrikePercent,//9000 range: 0 - 99.99%
        uint duration,// 5min
        uint minLocked//0
    ) external whenNotPaused returns (uint offerId) {//@audit-ok M Known issues. Provider missing active price Range when their offer is available. Provider only gain on LONG position. They do not want their offer is active pass certain price range
        // sanity checks
        require(callStrikePercent >= MIN_CALL_STRIKE_BIPS, "provider: strike percent too low");// callStrikePercent 100% - 1000%
        require(callStrikePercent <= MAX_CALL_STRIKE_BIPS, "provider: strike percent too high");
        require(putStrikePercent <= MAX_PUT_STRIKE_BIPS, "provider: invalid put strike percent");//99.99% max
        
        offerId = nextOfferId++;//@revert later if putStrike = 0%
        liquidityOffers[offerId] = LiquidityOfferStored({ //@note some live config from configHub check for limit later on. no early check for offer?
            provider: msg.sender,
            putStrikePercent: SafeCast.toUint24(putStrikePercent),//@25% <  putStrikePercent < 99%
            callStrikePercent: SafeCast.toUint24(callStrikePercent),
            duration: SafeCast.toUint32(duration),//@ 5min < duration < 1year
            minLocked: minLocked,//0
            available: amount
        });//@offer not become NFT. but how web can read user offer? This is just deposit
        cashAsset.safeTransferFrom(msg.sender, address(this), amount);//USDC transfer from user to contract as provider cash for loan
        emit OfferCreated(
    ...
    function updateOfferAmount(uint offerId, uint newAmount) external whenNotPaused {
        ...
        if (newAmount > previousAmount) {
            ...
            offer.available += toAdd;//@audit-ok M Can still put money in offer no longer valid. due to live config change.
            cashAsset.safeTransferFrom(msg.sender, address(this), toAdd);
        } else if (newAmount < previousAmount) {
            // withdraw
            uint toRemove = previousAmount - newAmount;//@audit-ok can withdraw all
            offer.available -= toRemove;
            cashAsset.safeTransfer(msg.sender, toRemove);
        } else { } // no change
        emit OfferUpdated(offerId, msg.sender, previousAmount, newAmount);//@offer can change half way
    }//@note can reuse old offer.
    ...
    function mintFromOffer(uint offerId, uint providerLocked, uint takerId)//loan 1WETH =  3600 USDC. 360 takerLocked. 40 providerLocked
        external//@note how much provider gain/lost, is depend on Taker contract. All calculations are on Taker contract.
        whenNotPaused
        onlyTaker//@taker is whitelisted. 
        returns (uint positionId)//@audit-ok takerId here point to NFT not even created yet. it still point correctly after Taker call openPariedPosition().
    {
        ...
        require(configHub.canOpenPair(underlying, cashAsset, msg.sender), "provider: unsupported taker");//@ok redundancy check
        require(configHub.canOpenPair(underlying, cashAsset, address(this)), "provider: unsupported provider");
        console2.log("Provider mint new offer: ", offerId, takerId);
        LiquidityOffer memory offer = getOffer(offerId); //@offer created from createOffer. already valid non zero check from caller Taker contract 
        // check terms
        uint ltv = offer.putStrikePercent; // assumed to be always equal
        require(configHub.isValidLTV(ltv), "provider: unsupported LTV"); //@audit-ok live config check. but not check during offer creation. no way to know if current offer is still valid with new config
        require(configHub.isValidCollarDuration(offer.duration), "provider: unsupported duration");//@live config check.
        //@audit-ok R can user withdraw offer if config change?
        // calc protocol fee to subtract from offer (on top of amount)
        (uint fee, address feeRecipient) = protocolFee(providerLocked, offer.duration); // 5min duration
        console2.log("Provider pay fee: %e",fee);
        // check amount //fee almost never 0, always tiny fee even with USDC rounded down
        require(providerLocked >= offer.minLocked, "provider: amount too low");// minimum 0 as default. assume as user they would want locked 1USDC as minimum
        uint prevOfferAmount = offer.available;//100k USDC
        require(providerLocked + fee <= prevOfferAmount, "provider: amount too high");
        uint newAvailable = prevOfferAmount - providerLocked - fee;//@locked to position, fee transfered out
        console2.log("leftOver balance: %e", newAvailable);
        // storage updates
        ...
        console2.log("Provider Lock money %e", providerLocked);
        ...
        _mint(offer.provider, positionId);// Provider lost 40$, gain a position of 40$ with 5 min expiration. Taker get 360$ locked somewhere else
    ...
    function cancelAndWithdraw(uint positionId) external whenNotPaused onlyTaker returns (uint withdrawal) {
        ...
        */ //@msg.sender here is Taker contract. Rolls give approval to Taker before cancel its providerNFT
    ...
    function withdrawFromSettled(uint positionId) external whenNotPaused returns (uint withdrawal) {
        ...
        emit WithdrawalFromSettled(positionId, withdrawal);//ok
    }
}
