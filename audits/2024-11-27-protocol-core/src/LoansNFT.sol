...
contract LoansNFT is ILoansNFT, BaseNFT {
    ...
    IERC20 public immutable underlying;//WETH
    ...
    {
        takerNFT = _takerNFT;//@audit-ok L Taker have  fancy setup check. it is correct fancy check.
        cashAsset = _takerNFT.cashAsset();// USDC
        underlying = _takerNFT.underlying();//WETH
        _setConfigHub(_takerNFT.configHub());//@fancy version check
    }
     ...
     */
    function openLoan(
        uint underlyingAmount,//1e18
        uint minLoanAmount,//0.3e6
        SwapParams calldata swapParams,//minAmountOut = 0, defaultSwapper, "" . There is callback
        ProviderOffer calldata providerOffer //WETH/USDC providerNFT , id with createOffer . 
        ...
        returns (uint underlyingOut)
    {
        ...
        address borrower = ownerOf(loanId);//@what happen if loan never close though? any can call settlePairedPosition() after duration expire
        require(_isSenderOrKeeperFor(borrower), "loans: not NFT owner or allowed keeper");//@ok, must be auth bot by both owner and user.

        // burn token. This prevents any other (or reentrant) calls for this loan
        _burn(loanId);//@note cannot close Loan early before expiration.

        // @dev will check settle, or try to settle - will revert if cannot settle yet (not expired)
        uint takerWithdrawal = _settleAndWithdrawTaker(loanId);//@gain cash from Taker long bet. include any profit if price increase.
        ...
        uint repayment = loan.loanAmount;//@original CASH sent to loaner. 90% of collateral in CASH.

        // @dev assumes approval
        cashAsset.safeTransferFrom(borrower, address(this), repayment);//@note when closeLoan that taker lose so bad. Due to price drop. full term repayment will try to swap full 
        //@audit-ok H04 fucking abuse to stole token from another user through Escrow optional system. transfer negative LoansNFT to another user who aprove bot keeper. Bot close loan and take CASH money from NFT owner to repay huge escrow fee.
        // total cash available
        uint cashAmount = repayment + takerWithdrawal;//@90% original cash loan when created. + profit from Taker long bet. could be zero too.
        // @dev Reentrancy assumption: no user state writes or reads AFTER the swapper call in _swap. //@cashAmount swap here could be low as 10% original amount of position.
        uint underlyingFromSwap = _swap(cashAsset, underlying, cashAmount, swapParams);//@ swap CASH back to ETH. 0.05% to uniswap and exchange rate varied too.
        //@audit-ok M02 no swap price deviation protection here. Escrow might receive really lower value as output of swap. To avoid paying lateFee. There is no reason/benefit to do this. But it is an option.
        ...
        underlyingOut = _conditionalReleaseEscrow(loan, underlyingFromSwap);//@audit-ok Escrow for normal Loans have no purpose. it does not used in anything. Just cost borrower money more WETH.
        //@underlyingOut = interestRefund - lateFee + underlyingFromSwap = underlyingFromSwap - lateFee
        underlying.safeTransfer(borrower, underlyingOut);//@here there is no early fee refund so only pay lateFee is closeLoan late for like 1 days.
        
        emit LoanClosed(loanId, msg.sender, borrower, repayment, cashAmount, underlyingFromSwap);
    }//@note OpenLoan with escrow pay fee to escrower. But when close loan. they still have to pay lateFee to escrower. there is no benefit to using escrow yet.
        ...
        returns (uint newLoanId, uint newLoanAmount, int toUser)
    {
        ...
        require(block.timestamp <= _expiration(loanId), "loans: loan expired");//@same as Provider duration

        // burn token. This prevents any further calls for this loan
        _burn(loanId);

        // pull and push NFT and cash, execute roll, emit event
        (uint newTakerId, int _toUser, int rollFee) = _executeRoll(loanId, rollOffer, minToUser);//@receive new takerNFT with different amount, must create new LoansNFT
        toUser = _toUser; // convenience to allow declaring all outputs above
        //@toUser = toTaker = prevTakerBalance + prevTakerGain - newTakerLocked(depend on price gain/reduce) - rollFee
        Loan memory prevLoan = getLoan(loanId);
        // calculate the updated loan amount (changed due to the roll)
        newLoanAmount = _loanAmountAfterRoll(toUser, rollFee, prevLoan.loanAmount);//@audit-ok R newLoan look a bit shady. Where rollFees coming from? who pay it. Provider offer can choose any fee amount to make it positive and inflate Loans value
        //@under Rolls.sol all money taken from Taker,Provider though. So rollsFee amount is covered and sent to Rolls contract and its position.
        // set the new ID from new taker ID
        newLoanId = _newLoanIdCheck(newTakerId);//@Rolls pretty tight calculation and no leech.

        // switch escrows if escrow was used because collar expiration has changed
        // @dev assumes interest fee approval
        uint newEscrowId = _conditionalSwitchEscrow(prevLoan, newEscrowOfferId, newLoanId, newEscrowFee);//@new escrow created with same underlying amount. same fee amount. Although the value of loan already shift and not reflect correct amount.

        // store the new loan data
        loans[newLoanId] = LoanStored({
            underlyingAmount: prevLoan.underlyingAmount,//@should be different underlyingAmount? because of price change and winning position profit limit by cap 100%. 
            loanAmount: newLoanAmount,//@this new loan != underlingAmount * 90% and 10% to taker. so Escrow actually profit from using higher escrowed Amount or lower than expected. Compare to open new loan
     ...
     */
    function unwrapAndCancelLoan(uint loanId) external whenNotPaused onlyNFTOwner(loanId) {//@only called before loan expired to allow Taker get their deposit APRfee back from escrower
        // release escrow if needed with refunds (interest fee) to sender
        _conditionalCheckAndCancelEscrow(loanId, msg.sender);//@call endEscrow if before loan duration end. To refund 95% original fee amount to user. depend on time left. 5% of original amount + interest fee send to escrower offer.
        //@when open loan. only sent in fee to escrower. Escrow lock their own funds same amount as LoanNFT. This fund do nothing. Just stuck there waiting to gain fee also locked when loan open.
        // burn token. This prevents any further calls for this loan
        _burn(loanId);//@refund fee transfer directly to msg.sender too
        //@audit-ok L LoansNFT can avoid paying lateFee and losing escrow interest fee by making sure escrow call lastResortSeize first. This is possible with another attack path. But also this function allow LoansNFT can still cancel and withdraw takerPair.
        // unwrap: send the taker NFT to user
        takerNFT.transferFrom(address(this), msg.sender, _takerId(loanId));//@taker position still not cancel yet.

        emit LoanCancelled(loanId, msg.sender);//@cannot called after escrow released and loan pass expiration.
    }
    ...
    function forecloseLoan(uint loanId, SwapParams calldata swapParams) external whenNotPaused {
        ...
        uint gracePeriodEnd = _expiration(loanId) + escrowGracePeriod(loanId);//@escrowGracePeriod is unique function calculate time
        require(block.timestamp > gracePeriodEnd, "loans: cannot foreclose yet");
        //@note Escrow can force close loan after expiration if loan not have enough money to pay lateFee. or simply date pass the grace period.
        ...
        _burn(loanId);//@revert if already burned.
        ...
        underlying.safeTransfer(borrower, toBorrower);//@audit-ok L03 underlying WBTC,WETH have blacklist on L2 network. if LoanNFT temporary transfer to blacklist account. foreclose loan not possible. even when zero amount
    ...
    function setSwapperAllowed(address swapper, bool allowed, bool setDefault) external onlyOwner {
        ...
        allowedSwappers[swapper] = allowed;//@note can use multiple pair swapper. 0.3% WETH/USDC is allowed and 0.05% WETH/USDC too.
    ...
    function _openLoan(
        uint underlyingAmount, //1e18 . nonzero check
        uint minLoanAmount,//0.3e6
        SwapParams calldata swapParams, //minAmountOut = 0, defaultSwapper, "" . There is callback
        ProviderOffer calldata providerOffer, // WETH/USDC providerNFT , id with CollarProvider.createOffer() .
        bool usesEscrow, // noEscrow /@escrow fee is user input
        EscrowOffer memory escrowOffer, //empty for no escrow
        uint escrowFee//@interesting fee is user input. fee from underlyingAmount. the asset amount
    ) internal returns (uint loanId, uint providerId, uint loanAmount) {
        require(configHub.canOpenPair(underlying, cashAsset, address(this)), "loans: unsupported loans");//@open WETH/USDC as whitelist. so wrong provider address still allowed?
        ...
        escrowFee = usesEscrow ? escrowFee : 0;//@escrow fee is user input
        // @dev pull underlyingAmount and escrowFee
        underlying.safeTransferFrom(msg.sender, address(this), underlyingAmount + escrowFee); //@provider have USDC cash, user open loan provide asset
        
        // handle optional escrow, must be done first, to use "supplier's" underlying in swap
        (EscrowSupplierNFT escrowNFT, uint escrowId) =
            _conditionalOpenEscrow(usesEscrow, underlyingAmount, escrowOffer, escrowFee); // @no escrow = skip
        ...
        // stack too deep
        {
            ...
            (takerId, providerId, loanAmount) = //@ 2 new NFT minted to this Loans contract
                _swapAndMintCollar(underlyingAmount, providerOffer, swapParams);//@address whitelist check inside
            // despite the swap slippage check, explicitly check loanAmount, to avoid coupling and assumptions
            require(loanAmount >= minLoanAmount, "loans: loan amount too low");//1WETH excahnge for 3600 USDC. 360 USDC locked to taker position. 40$ of position from provider. fee taken right away
            //@90% Put. so 10% go to locked, provider locked same amount too. loan user bet 10% of their loan against provider
            // validate loanId. @redundency check? takerId is unique and immutable. provider is not checked
            loanId = _newLoanIdCheck(takerId);//this loan still have 3240 USDC after swap
        }//@ 90% of WETH swap to CASH now available.

        // @dev these checks can only be done in the end of _openLoan, after both escrow and taker
        // positions exist
        if (usesEscrow) _escrowValidations(loanId, escrowNFT, escrowId);//@note there is check escrow duration = TakerNFT/ProviderNFT duration

        // store the loan opening data. 
        loans[loanId] = LoanStored({//@each loan is paired with takerId. Taker also pair with unique ProviderId positionId
            underlyingAmount: underlyingAmount, //@but Taker not necessary paired with Loan. Loan just periphery
            loanAmount: loanAmount,//@loanAmount = swapResult * offer.putStrikePercent / 10000 ~= 3240 USDC
            usesEscrow: usesEscrow,
            escrowNFT: escrowNFT,
            escrowId: SafeCast.toUint64(escrowId)
        });//@note loanId same as takerId. so Loan storage do not cache TakerNFT id. Each loan hold single takerNFT matched with other Provider pair.
        console2.log("Create LoanNFT: %d", loanId);
        ...
        console2.log("Send %e CASH to msg.sender %s", loanAmount , msg.sender);
        // transfer the full loan amount on open
        cashAsset.safeTransfer(msg.sender, loanAmount);//@caller lose 1WETH, gain 3239 USDC. 1USDC to uniswap fee. 
        //@ taker gain position with 359.9 USDC. provider/loan user lock 359.9 USDC , 0.1 USDC fee APR to protocol. 
        emit LoanOpened(loanId, msg.sender, underlyingAmount, loanAmount);//@caller gain loan position of 3240 USDC and 1 WETH. 
    }

    /// @dev swaps underlying to cash and mints collar position
    function _swapAndMintCollar(
        uint underlyingAmount,//1e18 WETH
        ProviderOffer calldata offer, // WETH/USDC providerNFT , id with CollarProvider.createOffer() from different user
        SwapParams calldata swapParams // minAmountOut = 0, defaultSwapper, "" . There is callback. unused
        ...
        require(configHub.canOpenPair(underlying, cashAsset, address(takerNFT)), "loans: unsupported taker");//@ok whitelist check seem properly setup
        ...
        );//@this check provider have same underlying and cashAsset as Loan and takerNFT
        ...
        console2.log("Swap WETH: %e ", underlyingAmount);
        // swap underlying
        ...
        console2.log("Get USDC: %e", cashFromSwap);//1WETH = 3600e6 USDC
        ...
        _checkSwapPrice(cashFromSwap, underlyingAmount);// USDC balance change check //@note unclear why allow extreme splippage of 10% swap drop. when this condition become true? 

        // split the cash to loanAmount and takerLocked //@audit-ok under which extreme liquidity condition of uniswap that large price failed to swap?
        ...
        uint ltvPercent = providerNFT.getOffer(offerId).putStrikePercent;//@ 9000 . range: 0% ->99.99%. Short position demanded by ProviderNFT offer.
        loanAmount = ltvPercent * cashFromSwap / BIPS_BASE;// swap 1e18 WETH from user, and provide 3600*90% loan? This loan can also be 0%
        // everything that remains is locked on the taker side in the collar position
        uint takerLocked = cashFromSwap - loanAmount;// 3600 USDC cash from swap. 3240 as loan. 360 as locked away?
        //@note if Provider require 10% put|collateral. 10% WETH -> 90% USDC become takerPosition. 10% USDC cash return to loan user.
        // open the paired taker and provider positions.//@note loan user benefit when price of WETH increase. their collateral gain value. When duration end, they gain CASH not their collateral back.
        cashAsset.forceApprove(address(takerNFT), takerLocked);//@anyone can use TakerNFT to open another position
        (takerId, providerId) = takerNFT.openPairedPosition(takerLocked, providerNFT, offerId);//@ offerId put in 1WETH as offer
    }//@note TakerNFT also check provider have same IERC20 underlying and cash asset address.
    //@taker can be called by everyone
    ...
    function _checkSwapPrice(uint cashFromSwap, uint underlyingAmount) internal view {
        ITakerOracle oracle = takerNFT.oracle();// chainlink combined oracle WETH/USD/USDC
        // get the equivalent amount of underlying the cash from swap is worth at oracle price
        uint underlyingFromCash = oracle.convertToBaseAmount(cashFromSwap, oracle.currentPrice()); // cash * 1e18 / price = cash *1e18 /3590.69785992e6 ~= 1e18
        console2.log("Oracle: %e UNDERLYING == %e CASH ", underlyingAmount, oracle.convertToQuoteAmount(underlyingAmount, oracle.currentPrice()));        // calculate difference
        (uint a, uint b) = (underlyingAmount, underlyingFromCash);//compare original amount vs output convert to base amount
        uint absDiff = a > b ? a - b : b - a;
        uint deviation = absDiff * BIPS_BASE / underlyingAmount; // checked on open to not be 0
        require(deviation <= MAX_SWAP_PRICE_DEVIATION_BIPS, "swap and oracle price too different");//@ deviation always > 0
    }//@ok
    ...
    function _swap(IERC20 assetIn, IERC20 assetOut, uint amountIn, SwapParams calldata swapParams)
        internal
        returns (uint amountOut)//@1e18 WETH -> USDC 
    {
        // check swapper allowed
        require(allowedSwappers[swapParams.swapper], "loans: swapper not allowed"); //@whitelsit WETH/USDC 0.05% and 0.3%
        //@ contract SwapperUniV3 as only swapper
        uint balanceBefore = assetOut.balanceOf(address(this));//@cache USDC balance
        // approve the swapper
        assetIn.forceApprove(swapParams.swapper, amountIn);//approve 1e18 WETH for swap
        ...
        */
        uint amountOutSwapper = ISwapper(swapParams.swapper).swap(//@ok no special going on here
            assetIn, assetOut, amountIn, swapParams.minAmountOut, swapParams.extraData
        );//@note swapper is future upgrade proofsafety. it is VERY safe.
        // Calculate the actual amount received
        amountOut = assetOut.balanceOf(address(this)) - balanceBefore; //@check second times. This also checked inside SwapperUniV3
    ...
    function _settleAndWithdrawTaker(uint loanId) internal returns (uint withdrawnAmount) {
        ...
        if (!settled) {
            ...
            takerNFT.settlePairedPosition(takerId);//@NFT owner become withdrawable for locked amount. same as providerNFT
        }

        /// @dev because taker NFT is held by this contract, this could not have been called already
        withdrawnAmount = takerNFT.withdrawFromSettled(takerId);//@burn NFT, all CASH from taker win/lose position to this contract.
    }//@Provider also take their share too. so whatever money receive here is just result of long position. it can range from 0->200% original
        ...
        returns (uint newTakerId, int toTaker, int rollFee)
    {
        ...
        rollFee = preview.rollFee;//@toTaker already include rollFee
    ...
    function _conditionalOpenEscrow(bool usesEscrow, uint escrowed, EscrowOffer memory offer, uint fee)
        internal//@msg.sender already transfer in 1 WETH + escrowFee any amount
        returns (EscrowSupplierNFT escrowNFT, uint escrowId)//escrowed = 1WETH, offer: any input, fee: any input
    {
        if (usesEscrow) {
            escrowNFT = offer.escrowNFT;
            // check asset matches
            require(escrowNFT.asset() == underlying, "loans: escrow asset mismatch");//escrow hold WETH and whitelisted
            // whitelisted only
            require(configHub.canOpenSingle(underlying, address(escrowNFT)), "loans: unsupported escrow"); //@check escrowNFT address is whitelisted
            console2.log("--Open Escrow: escrowed: %e, fee: %e", escrowed, fee);
            // @dev underlyingAmount and fee were pulled already before calling this method
            underlying.forceApprove(address(escrowNFT), escrowed + fee);//@trasnfer 1WETH + escrow amount. getting back 1WETH for swapping.
            escrowId = escrowNFT.startEscrow({//@this loans address must be whitelisted by EscrowNFT.
                offerId: offer.id,
                escrowed: escrowed,//@audit-ok duration is synced and checked. M escrow Loan duration not in sync between Escrow Offer and Provider offer. This can result in loan forced to pay grace fee or close early before Provider turn.
                fee: fee,
                loanId: takerNFT.nextPositionId() // @dev checked later in _escrowValidations
             });//@note only LoansNFT can open escrow
    ...
    function _releaseEscrow(EscrowSupplierNFT escrowNFT, uint escrowId, uint fromSwap)//@a1: from closeLoan(). must pass expiration. so there is late fee
        internal
        returns (uint underlyingOut)
    {
        // get owing and late fee (included in totalOwed)
        (uint totalOwed, uint lateFee) = escrowNFT.currentOwed(escrowId);//totalOwed = borrowed + latefee
        ...
        uint fromEscrow = escrowNFT.endEscrow(escrowId, toEscrow);//a1: withdrawal = escrowed + interestHeld + lateFee - interestRefund || escrow.escrowed + escrow.interestHeld + fromLoans . So escrower lose nothing here.
        // @dev no balance checks because contract holds no funds, mismatch will cause reverts
        //@fromEscrow = toEscrow + interestRefund - lateFee. 
        // the released and the leftovers to be sent to borrower. Zero-value-transfer is allowed
        underlyingOut = fromEscrow + leftOver;//@there is also locked value still inside escrowNFT too.
        //@underlyingOut = interestRefund - lateFee + fromswap
    ...
    function _conditionalCheckAndCancelEscrow(uint loanId, address refundRecipient) internal {
        Loan memory loan = getLoan(loanId);//@called by loan owner. aka Taker position owner.
    ...
    */
        ...
        returns (uint newLoanAmount)
    {
        ...
        int loanChange = fromRollsToUser + rollFee;//@loanChange = profit/loss from Taker + fee pay by provider or take from LoansNFT. Taker can take on malicious fee and reduce their original loan amount
        if (loanChange < 0) {
