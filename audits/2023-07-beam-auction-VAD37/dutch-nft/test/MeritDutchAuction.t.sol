...
contract MeritDutchAuctionTest is Test {
    ...
    function testPriceAtAuction() public {
        uint count = 14;
        ...
        for(uint256 i = 0; i < count; i++) {
            ...
            uint256 price = auction.getPriceAt(block.timestamp);
            console.log("price at %s: %s", i, price/1e15);
    ...
    function privateViewData() private view returns (bool done) {
        MeritDutchAuction.AuctionData memory data = auction.getAuctionData();
        //console.log everydata inside data
        //         return AuctionData({
        ...
        //     timeToEnd: timeToEnd
        // });
        console.log("startPrice:   %s", data.startPrice);
        console.log("endPrice:     %s", data.endPrice);
        console.log("currentPrice: %s", data.currentPrice);
        console.log("totalRaised:  %s", data.totalRaised);
        console.log("minted:       %s", data.minted);
        console.log("cap:          %s", data.cap);
        console.log("startTime:    %s", data.startTime);
        console.log("endTime:      %s", data.endTime);
        console.log("stepSize:     %s", data.stepSize);
        for(uint256 i = 0; i < data.priceSteps.length; i++) {
            console.log("priceSteps[%s]:          %s", i, data.priceSteps[i]);
        }
        for(uint256 i = 0; i < data.priceStepTimestamps.length; i++) {
            console.log("priceStepTimestamps[%s]: %s", i, data.priceStepTimestamps[i]);
        }
        
        console.log("currentStep:    %s", data.currentStep);
        console.log("timeToNextStep: %s", data.timeToNextStep);
        console.log("timeToStart:    %s", data.timeToStart);
        console.log("timeToEnd:      %s", data.timeToEnd);
