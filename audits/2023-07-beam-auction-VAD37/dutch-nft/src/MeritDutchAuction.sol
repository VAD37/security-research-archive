...
contract MeritDutchAuction is Ownable {
    ...
    constructor(
        uint256 startPrice_,//1e18
        uint256 endPrice_,//0.1e18
        uint256 startTime_,//block.timestamp + 1 days;
        uint256 endTime_,//startTime + 2 hours
        uint256 stepSize_//12 minutes step. 10 step total
    ) { 
    ...
    function setNFT(address nft_) external onlyOwner {
        ...
        nft = MeritNFT(nft_);//@audit L why not constructor.
    }
    ...
    function getPriceAt(uint256 time) public view returns(uint256) {
        ...
        return price;//@note price drop 0.09ETH every 12 minutes
    }
    ...
    function mint(uint256 amount) external payable {
        ...
        for(uint256 i = 0; i < amount; i++) {
            ...
            nft.mint(mintedBefore + i + 1, msg.sender);// ID: 0 + 1234 -> 10000
        }

        if(msg.value > totalPaid) {//@audit reentrancy mint more than 1 NFT at a time?
