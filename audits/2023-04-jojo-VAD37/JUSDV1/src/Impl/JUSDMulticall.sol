    ...
    function multiCall(
        ...
        for (uint256 i; i < callData.length; i++) {
            ...
            if (success == false) {
                assembly {
                    let ptr := mload(0x40)
                    let size := returndatasize()//@audit-ok multicall false revert assembly code look funny. Follow solidity memory safe sample
                    returndatacopy(ptr, 0, size)
                    revert(ptr, size)
                }//@audit-ok callstack depth error with multiCall. Can this prevent dealer or trader
            }
            returnData[i] = res;
        }
    }
