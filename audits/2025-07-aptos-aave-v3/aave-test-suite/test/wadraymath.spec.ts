...
describe("WadRayMath", () => {
  ...
  test("Plain getters", async () => {
    expect(wrapper.wad().eq(consts.WAD)).toBeTruthy();
    expect(wrapper.halfWad().eq(consts.HALF_WAD)).toBeTruthy();
    expect(wrapper.ray().eq(consts.RAY)).toBeTruthy();
    expect(wrapper.halfRay().eq(consts.HALF_RAY)).toBeTruthy();
  });
  ...
  test("wadMul()", async () => {
    ...
    const tooLargeA = BigNumber.from(consts.MAX_UINT_AMOUNT)
      .sub(consts.HALF_WAD)
      .div(b)
      .add(1);
    expect(() => {
  ...
  test("rayMul()", async () => {
    ...
    expect(
      wrapper.rayMul(BigNumber.from(0), b).eq(BigNumber.from(0)),
    ).toBeTruthy();
    expect(
      wrapper.rayMul(a, BigNumber.from(0)).eq(BigNumber.from(0)),
    ).toBeTruthy();
  ...
  test("rayToWad()", async () => {
    ...
    const tooLarge = BigNumber.from(consts.MAX_UINT_AMOUNT).sub(half).add(1);
  ...
  test("rayToWad()", async () => {
    ...
    const tooLarge = BigNumber.from(consts.MAX_UINT_AMOUNT).div(ratio).add(1);
    expect(() => {
