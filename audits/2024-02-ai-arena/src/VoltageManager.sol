...
contract VoltageManager {
    ...
    function adjustAllowedVoltageSpenders(address allowedVoltageSpender, bool allowed) external {
        ...
        allowedVoltageSpenders[allowedVoltageSpender] = allowed;//@ranked battle contract can spend voltage
    }
    ...
    function useVoltageBattery() public {
        ...
        require(_gameItemsContractInstance.balanceOf(msg.sender, 0) > 0);//@audit-ok M non stake NFT can fight infinite amount by transfer away to another holder.
        _gameItemsContractInstance.burn(msg.sender, 0, 1);//@audit-ok There is nothing to add new item R how new items will be added to the game. The contract is fixed and no UUPS. How it will be interact with RankedBattle or Staking?
    ...
    function spendVoltage(address spender, uint8 voltageSpent) public {
        require(spender == msg.sender || allowedVoltageSpenders[msg.sender]);//@RankedBattle allowed to become spender
        if (ownerVoltageReplenishTime[spender] <= block.timestamp) {
            _replenishVoltage(spender);
        }
        ownerVoltage[spender] -= voltageSpent;//@audit-ok L user can spent their own voltage for nothing at all. User can pretty much burning away their battery
