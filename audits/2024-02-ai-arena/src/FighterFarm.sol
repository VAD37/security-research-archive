...
//@admin initialize functions
    ...
    function incrementGeneration(uint8 fighterType) external returns (uint8) {
        require(msg.sender == _ownerAddress);
        generation[fighterType] += 1;//@audit-ok H admin increase generation of fighter type will destroy minting new fighter of that type due to element not exist for new generation
    ...
    function claimFighters(
        uint8[2] calldata numToMint,//@note there is only 2 types of fighter. So this is the number of fighter to mint for each type.
        bytes calldata signature,
        string[] calldata modelHashes,//@userinput this is not verified?
        string[] calldata modelTypes//@userinput 
    ) 
        external 
    {
        bytes32 msgHash = bytes32(keccak256(abi.encode(//@why not encodepacked. 24 bytes does not hit keccack bug limitation
            msg.sender, //uint160 = 20 bytes
            numToMint[0], //uint8 = 1 byte
            numToMint[1],//uint8 = 1 byte
            nftsClaimed[msg.sender][0],//1byte //@note hash verification use current total amount minted NFT by user to prevent replay attack
            nftsClaimed[msg.sender][1]//1byte
        )));
        ...
        nftsClaimed[msg.sender][1] += numToMint[1];//@this prevent replay hash attack
        for (uint16 i = 0; i < totalToMint; i++) {//@ R what? this mint loop from 0 -> 512 max. Then fighter types is based on mint index i?
            _createNewFighter(
                msg.sender, //@to
                uint256(keccak256(abi.encode(msg.sender, fighters.length))),//@audit-ok L DNA can be crafted using user address
                modelHashes[i], //@string hash
                modelTypes[i],//@string hash
                i < numToMint[0] ? 0 : 1,//@ what is going on here? what is fighter types
                0,//icon type
                [uint256(100), uint256(100)]//@ custom attributes
            );
        }
    }
        ...
        external 
    {
        ...
        for (uint16 i = 0; i < mintpassIdsToBurn.length; i++) {
            ...
            _createNewFighter(
                msg.sender, 
                uint256(keccak256(abi.encode(mintPassDnas[i]))), //@audit-ok L dna redeem allow user input any DNA
                modelHashes[i], 
                modelTypes[i],
                fighterTypes[i],
                iconsTypes[i],
        ...
        external
    {
        ...
        fighters[tokenId].modelHash = modelHash;//@audit-ok M there is nothing to stop user to use someone else model fighting for them. As all modelHash and type is public. There need to be some offchain verification purpose
        ...
        public 
    {
        ...
        _createNewFighter(
            to, 
            uint256(keccak256(abi.encode(msg.sender, fighters.length))), //@audit-ok length increase after each mint .M dna not unique for mint from mergepool
            modelHash, 
            modelType,
            0,//@audit-ok M merge pool destroy fighter type 
            0,
            customAttributes
        );
    }
    ...
    function reRoll(uint8 tokenId, uint8 fighterType) public {
        require(msg.sender == ownerOf(tokenId));//@audit-ok can reroll despite wrong fighter type
        ...
        if (success) {
            ...
            uint256 dna = uint256(keccak256(abi.encode(msg.sender, tokenId, numRerolls[tokenId])));//@audit-ok M there exist reroll function that provide income to dev. Any DNA compromise is an issue.
    ...
    {
        uint256 element = dna % numElements[generation[fighterType]];//@audit-ok H numElements only exist for index generation 0. There is no way to increase generation. Create new fighter always fail due to increasing generationf or fighter type
        uint256 weight = dna % 31 + 65;
        uint256 newDna = fighterType == 0 ? dna : uint256(fighterType);//@audit-ok M fighterbase use fighterType as DNA if non zero fighterType??????
    ...
    function _createNewFighter(
        ...
        uint8 fighterType,//0 or 1 //@audit-ok M redeem user can use non exist fighter type to bypass DNA _createFighterBase()
        uint8 iconsType,//0
    ...
    {  
        ...
        uint256 element; //@note element range 0-2 . numElements cannot change.
        uint256 weight;//@note weight range 65-95 
        uint256 newDna;
        if (customAttributes[0] == 100) {
            (element, weight, newDna) = _createFighterBase(dna, fighterType);//@dna can be 1 if fighter type is 1
        }
        else {
            element = customAttributes[0];
            weight = customAttributes[1];//@audit R no custom attribute weight validation
            newDna = dna;
        }
        uint256 newId = fighters.length;

        bool dendroidBool = fighterType == 1;//@note normal fighterType ID dendroid is 1.
        FighterOps.FighterPhysicalAttributes memory attrs = _aiArenaHelperInstance.createPhysicalAttributes(
            newDna,//@ rarity = dna divided by [2, 3, 5, 7, 11, 13] and % 100
            generation[fighterType],//gen 0 during deployment
            iconsType,
            dendroidBool//@if dendroid all attributes are 99
        );
        fighters.push(
            FighterOps.Fighter(
                weight,
