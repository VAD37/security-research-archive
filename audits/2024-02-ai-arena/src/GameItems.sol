...
contract GameItems is ERC1155 {
    ...
    function mint(uint256 tokenId, uint256 quantity) external {
        ...
        uint256 price = allGameItemAttributes[tokenId].itemPrice * quantity;//@audit-ok L input validation. quantity can be zero can the code still run
        ...
        require(//@reset timer max 10 items per account per day
            dailyAllowanceReplenishTime[msg.sender][tokenId] <= block.timestamp || 
            quantity <= allowanceRemaining[msg.sender][tokenId]//@timestamp allow bypass quantity check. akkiw mint any amount but still revert later
        );

        _neuronInstance.approveSpender(msg.sender, price);//@allow GameItem to transferFrom token from user to treasury
        bool success = _neuronInstance.transferFrom(msg.sender, treasuryAddress, price);
        if (success) {//@OZ ERC20 never return false. it throw error
            if (dailyAllowanceReplenishTime[msg.sender][tokenId] <= block.timestamp) {
                _replenishDailyAllowance(tokenId);
            }
            allowanceRemaining[msg.sender][tokenId] -= quantity;//@can mint more quantity than available but it revert here.
            if (allGameItemAttributes[tokenId].finiteSupply) {
                allGameItemAttributes[tokenId].itemsRemaining -= quantity;
            }
            _mint(msg.sender, tokenId, quantity, bytes("random"));//@mint have callback.
    ...
    function createGameItem(
        string memory name_,//battery
        string memory tokenURI,
        bool finiteSupply,//true
        bool transferable,//true
        uint256 itemsRemaining,//10000
        uint256 itemPrice,//1e18
        uint16 dailyAllowance//10
    ) 
        public //@audit-ok L admin have no way to remove unique item. Who is spending these gameItem
    {
        require(isAdmin[msg.sender]);
        allGameItemAttributes.push(//@audit-ok L lack of admin check. itemsRemaining must always > daily allowance otherwise it will be revert.
            GameItemAttributes(
                name_,
                finiteSupply,
                transferable,
