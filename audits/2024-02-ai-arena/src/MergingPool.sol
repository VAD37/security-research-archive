...
contract MergingPool {
    ...
    function updateWinnersPerPeriod(uint256 newWinnersPerPeriodAmount) external {
        ...
        winnersPerPeriod = newWinnersPerPeriodAmount;//@3
    }    
        ...
        external 
    {
        ...
        uint32 claimIndex = 0;
        uint32 lowerBound = numRoundsClaimed[msg.sender];//@audit-ok user with no rewards can claim rewards but receive no rewards.
        for (uint32 currentRound = lowerBound; currentRound < roundId; currentRound++) {
            numRoundsClaimed[msg.sender] += 1;//@audit-ok M out of gas 2M gas on loop after 80~ rounds
            winnersLength = winnerAddresses[currentRound].length;
            for (uint32 j = 0; j < winnersLength; j++) {
                if (msg.sender == winnerAddresses[currentRound][j]) {//@audit-ok L loop out of gas
                    _fighterFarmInstance.mintFromMergingPool(
                        msg.sender,
                        modelURIs[claimIndex],//@audit-ko L claim rewards revert if transaction pending during admin call winners
