...
contract RankedBattle {
    ...
    function setRankedNrnDistribution(uint256 newDistribution) external {
        ...
        rankedNrnDistribution[roundId] = newDistribution * 10**18;//@audit-ok L admin can override current ranking distribution
    }
    ...
    function stakeNRN(uint256 amount, uint256 tokenId) external {//@audit can stake for ongoing round. This mean when battle going for halfway. User can still stake for winner?
        ...
        require(_fighterFarmInstance.ownerOf(tokenId) == msg.sender, "Caller does not own fighter");//@audit-ok disable staked token transfer through updateFighterStaking() L user can stake then transfer NFT away and still stake for them
        ...
        if (success) {
            ...
            stakingFactor[tokenId] = _getStakingFactor(//@this update staking factor for this token only
                tokenId, 
                _stakeAtRiskInstance.getStakeAtRisk(tokenId)//@this is zero? only nonzero after admin call update staking round
            );
    ...
    function unstakeNRN(uint256 amount, uint256 tokenId) external {
        ...
        amountStaked[tokenId] -= amount;
        globalStakedAmount -= amount;
        stakingFactor[tokenId] = _getStakingFactor(//@audit-ok L can refresh staking factor by unstake or stake 0 amount
            tokenId, 
            _stakeAtRiskInstance.getStakeAtRisk(tokenId)//@note stake at risk always current round sync with ranked battle
        );
        _calculatedStakingFactor[tokenId][roundId] = true;
        hasUnstaked[tokenId][roundId] = true;//@note if user unstake any amount even 0. they can't stake again for this round
    ...
    function claimNRN() external {
        require(numRoundsClaimed[msg.sender] < roundId, "Already claimed NRNs for this period");//@audit-ok M out of gas claimNRN after 4000 rounds. Tested through foundry
    ...
    function updateBattleRecord(//@audit-ok M you can game offchain service. Make sure gameserver send failed transaction by spending all voltage.
        uint256 tokenId, 
        uint256 mergingPortion,//@50
        uint8 battleResult,//@0 for win, 1 for tie, 2 for loss
        uint256 eloFactor,//@ 1500
        bool initiatorBool//@true
    ) //@audit-ok H What happen when update 2 battle record but one transaction fail? it is better for the game server to make 1 function update 2 token battles instead of 2 calls
        external 
    {
        ...
        require(
            ...
            _voltageManagerInstance.ownerVoltage(fighterOwner) >= VOLTAGE_COST//@audit-ok L this check is useless. just spend directly and revert
        );

        _updateRecord(tokenId, battleResult);//@audit-ok M missing admin verficitation check for non support enum battle result
        uint256 stakeAtRisk = _stakeAtRiskInstance.getStakeAtRisk(tokenId);//@stake always 0 after 1st stake
        if (amountStaked[tokenId] + stakeAtRisk > 0) {//@audit-ok M frontrun battle result. unstake lost battle.
            _addResultPoints(battleResult, tokenId, eloFactor, mergingPortion, fighterOwner);
        }//@audit-ok H user with no stake cannot add resultPoints. There is never points increase.
        if (initiatorBool) {
            _voltageManagerInstance.spendVoltage(fighterOwner, VOLTAGE_COST);//@audit-ok M for nonstake users. They can transfer NFT away to bypass voltage cost spending.This give user MMR ranking but no stake
        }
        totalBattles += 1;
    }
    ...
    function _addResultPoints(
        uint8 battleResult, //@ 0 for win, 1 for tie, 2 for loss
        uint256 tokenId, //@can be both loser or winner token
        uint256 eloFactor, //@1500 //@audit-ok L no onchain check to make sure elo factor is reasonable
        ...
        private 
    {
        ...
        //@update staking only for user with no stake
        /// Calculate the staking factor if it has not already been calculated for this round 
        if (_calculatedStakingFactor[tokenId][roundId] == false) {//@ this only reached in new round when user still have previous stake
        ...
        if (battleResult == 0) {//@audit-ok lose 0.1% token after 1 lost. Next lost lose another 0.1% of original R stake at risk compound after multiple round?
            ...
            if (curStakeAtRisk > amountStaked[tokenId]) {
                curStakeAtRisk = amountStaked[tokenId];//@audit-ok M user lose all stake when losing and not lose some points.
            }
        ...
        returns (uint256) 
    {
      uint256 stakingFactor_ = FixedPointMathLib.sqrt(
          (amountStaked[tokenId] + stakeAtRisk) / 10**18//@audit-ok  H getstaking factor seem manipulated by staking <1e18
      );
      if (stakingFactor_ == 0) {
        stakingFactor_ = 1;
      }
