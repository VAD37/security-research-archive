...
In `DebitaIncentives.sol`, function `incentivizePair()` will skip caching new bribes token when that bribe token on that epoch already exists [(Reference)](https://github.com/sherlock-audit/2024-11-debita-finance-v3-VAD37/blob/877aca0c5bf364eff2a8b3b0a72df9927b784720/Debita-V3-Contracts/contracts/DebitaIncentives.sol#L257).
But each epoch have unique principle, each unique principle have its own list of bribe tokens ([Ref1](https://github.com/sherlock-audit/2024-11-debita-finance-v3-VAD37/blob/877aca0c5bf364eff2a8b3b0a72df9927b784720/Debita-V3-Contracts/contracts/DebitaIncentives.sol#L57-L63)).
And when query bribes info, it return total bribes for each bribe token for each unique principle per epoch [(Ref)](https://github.com/sherlock-audit/2024-11-debita-finance-v3-VAD37/blob/877aca0c5bf364eff2a8b3b0a72df9927b784720/Debita-V3-Contracts/contracts/DebitaIncentives.sol#L364-L396).

The underlying logic issue is: unique principle did not add new bribes token to its own index list when that bribes token already exist on another list. Causing wrong index and return wrong total bribes.

For detailed explaination, see "@" Comments. Focus on `SpecificBribePerPrincipleOnEpoch` and `bribeCountPerPrincipleOnEpoch` mapping.
...
List return for principle USDC will be empty, because `bribeCountPerPrincipleOnEpoch[epoch][USDC] = 0`
When it should return 1000e6 USDC bribes info.
