...
contract DutchAuction_veNFT is ERC721Holder {
    ...
    constructor(
        ...
        uint curedInitAmount = _initAmount * (10 ** difference);//_initAmount >= _floorAmount
        uint curedFloorAmount = _floorAmount * (10 ** difference);

        s_CurrentAuction = dutchAuction_INFO({
            auctionAddress: address(this),
            nftAddress: _veNFTAddress,//any NFT
            nftCollateralID: _veNFTID,
            sellingToken: sellingToken,//any token, $AERO
            owner: owner,
            initAmount: curedInitAmount,
            floorAmount: curedFloorAmount,
            duration: _duration,
            endBlock: block.timestamp + _duration,
            tickPerBlock: (curedInitAmount - curedFloorAmount) / _duration,  //@audit-ok H02 no limit on duration. it revert with 0 seconds
            isActive: true,//@tick always in e18 decimals to prevent loss.
            initialBlock: block.timestamp,
            isLiquidation: _isLiquidation,
            differenceDecimals: difference
        });

        s_ownerOfAuction = owner;//msg.sender
        factory = msg.sender;
    }
    /**
    ...
    function buyNFT() public onlyActiveAuction {
        // get memory data
        dutchAuction_INFO memory m_currentAuction = s_CurrentAuction; //@audit-ok  Price only decrease. no price racket protection 
        // get current price of the auction
        uint currentPrice = getCurrentPrice();//@ ?
        // desactivate auction from storage
        s_CurrentAuction.isActive = false;
        uint fee;
        if (m_currentAuction.isLiquidation) { //Aggregator.isSenderALoan(msg.sender)
            fee = auctionFactory(factory).auctionFee();// 0.5% - 4%, 50 -> 400
        } else {
            fee = auctionFactory(factory).publicAuctionFee(); //0%-1%, 0 -> 100 
        }
        ...
        auctionFactory(factory)._deleteAuctionOrder(address(this));//@audit-ok H03 nft ERC721 safetransfer reentrancy
    ...
    function editFloorPrice(
        ...
        uint curedNewFloorAmount = newFloorAmount *
            (10 ** s_CurrentAuction.differenceDecimals);//100e6 USDC -> 100e18
        require(
            s_CurrentAuction.floorAmount > curedNewFloorAmount, //@ previous floor must higher than new floor
            "New floor lower"
        );
        ...
        uint newDuration = (m_currentAuction.initAmount - curedNewFloorAmount) /
            m_currentAuction.tickPerBlock;// (init - newFloor) / (init -oldFloor) * oldDuration . 
        //@ new duration can be longer. speed still the same. 
        uint discountedTime = (m_currentAuction.initAmount -
            m_currentAuction.floorAmount) / m_currentAuction.tickPerBlock;//discountedTime = previousDuration
        //@audit-ok H02 tick perBlock can be zero. revert with 0 seconds. Make it impoosible to edit Floor price
        if (
            (m_currentAuction.initialBlock + discountedTime) < block.timestamp
        ) {//@if already reach floor price. reset initial block time so price continue to drop from now.
    ...
    function getCurrentPrice() public view returns (uint) {
        dutchAuction_INFO memory m_currentAuction = s_CurrentAuction;
        uint floorPrice = m_currentAuction.floorAmount;//@curedFloorAmount = inputFloor * 10^difference
        ...
        uint decreasedAmount = m_currentAuction.tickPerBlock * timePassed; //tick = (curedInitAmount - curedFloorAmount) / _duration
        ...
        currentPrice =
            currentPrice /
            (10 ** m_currentAuction.differenceDecimals); //if init_e18, then reduce token by other token decimals?
        return currentPrice;// 100e6 USDC ->  100e6 * 10^12 = 100e18 floor price. 100e18 / 10^12 = 100e6
    }
