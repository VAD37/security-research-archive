...
interface veNFR {
    struct receiptInstance {
        ...
        address vault;
        address underlying;
    }//@audit-ok still parse with missing field return. this receiptInstance is missing last boolean field
...
contract BuyOrder is Initializable {
    ...
    function initialize(
        ...
        address _token,//AERO any
        address wantedToken,//veNFTAerodrome
        address factory,//address(this) creator
        uint _amount,//any 100e18 . received 100e18 token from factory right away after init
        uint ratio//any 0.7e18
    ) public initializer {
    ...
    function deleteBuyOrder() public onlyOwner {
        ...
        SafeERC20.safeTransfer(
            IERC20(buyInformation.buyToken),//AERO any
            buyInformation.owner,//msg.sender
            amount
        );
    ...
    function sellNFT(uint receiptID) public {
        ...
        IERC721(buyInformation.wantedToken).transferFrom(//veNFTAerodrome
            msg.sender,
            address(this),//@audit-ok H05 this is buyer/owner. not address(this)
            receiptID
        );//@audit-ok H05 does not transfer NFT to owner who bought it?
        veNFR receipt = veNFR(buyInformation.wantedToken);//@it does not looking good when request external information
        veNFR.receiptInstance memory receiptData = receipt.getDataByReceipt(
            receiptID
        );//@audit-ok H07 receipt have vault manager address that does not change on transfer. This require special attentions
        uint collateralAmount = receiptData.lockedAmount;//@audit-ok locked never changed with ownership change. M does not tell how much locked remaining after previous owner withdraw or extend
        ...
        buyInformation.availableAmount -= amount;//reduce token offer for sale left.
        buyInformation.capturedAmount += collateralAmount;
        uint feeAmount = (amount *
            IBuyOrderFactory(buyOrderFactory).sellFee()) / 10000;// 0.2-1% fee
        SafeERC20.safeTransfer(
        ...
        SafeERC20.safeTransfer(
            ...
            feeAmount
        );//@audit-ok buytoken reentrancy?

        if (buyInformation.availableAmount == 0) {
            buyInformation.isActive = false;
            IBuyOrderFactory(buyOrderFactory).emitDelete(address(this));//@audit-ok H03 reentrancy duplicate delete order. Reset active order count to zero
            IBuyOrderFactory(buyOrderFactory)._deleteBuyOrder(address(this));
        } else {
