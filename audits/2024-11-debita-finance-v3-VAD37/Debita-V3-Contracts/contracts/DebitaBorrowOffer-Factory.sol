...
contract DBOFactory {
    ...
    address aggregatorContract;//DebitaV3 Aggregator
    address implementationContract;
    address public owner;

    constructor(address _implementationContract) {
        owner = msg.sender;
        implementationContract = _implementationContract;//DebitaBorrowOffer-Implementation
    }
     ...
     */
    function createBorrowOrder(//@user can pass in NFT receiptID or use ERC20 token. any token will also be sufficient
        bool[] memory _oraclesActivated,    //[false]
        uint[] memory _LTVs,                // [0]
        uint _maxInterestRate,              // 1000
        uint _duration,                     // 864000 10 days
        address[] memory _acceptedPrinciples,//[AERO]
        address _collateral,                //AERO or NFT receipt address with underlying token
        bool _isNFT,                        //false
        uint _receiptID,                    //0 
        address[] memory _oracleIDS_Principles,// [0x0]
        uint[] memory _ratio,               // [1e18]
        address _oracleID_Collateral,       //(0x0)
        uint _collateralAmount              //10e18 overrided by NFT lockedAmount
    ) external returns (address) {
        if (_isNFT) {//NFT means this is receipt
            require(_receiptID != 0, "Receipt ID cannot be 0");
            require(_collateralAmount == 1, "Started Borrow Amount must be 1");
        }//@audit-ok nonNFT on sale can have non-zero receiptID
