...
contract DBOImplementation is ReentrancyGuard, Initializable {
    ...
    function initialize(
        ...
        address _owner,                             //msg.sender to Factory
        address[] memory _acceptedPrinciples,       // input AERO
        address _collateral,                        // input AERO
        bool[] memory _oraclesActivated,            // input false
        bool _isNFT,                                // input false
        uint[] memory _LTVs,                        // input 0
        uint _maxApr,                               // input 1000
        uint _duration,                             // input 10 days
        uint _receiptID,                            // input 0
        address[] memory _oracleIDS_Principles,     // input 0x
        uint[] memory _ratio,                       // input 1e18
        address _oracleID_Collateral,               // input 0x
        uint _startedBorrowAmount                   // input 10e18 _collateralAmount
    ) public initializer {
        ...
        if (_isNFT) {
            ...
            _startedBorrowAmount = nftData.lockedAmount;//@audit-ok no problem with zero amount.accept NFT with zero locked amount
    ...
    function acceptBorrowOffer(
        uint amount
    ) public onlyAggregator nonReentrant onlyAfterTimeOut {//@audit-ok accept borrow or lending offer missing defined amount protection first. Is this belong to aggregator
        ...
        if (m_borrowInformation.isNFT) {
            IERC721(m_borrowInformation.collateral).transferFrom(
                address(this),
                aggregatorContract,//@note all tokens transfer to aggregator first
        ...
        uint percentageOfAvailableCollateral = (borrowInformation
            .availableAmount * 10000) / m_borrowInformation.startAmount;//nft this is locked amount
