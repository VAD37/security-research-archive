...
contract DebitaV3Loan is Initializable, ReentrancyGuard {
    ...
    function initialize(
        address _collateral,//from borrower NFT or ERC20
        address[] memory _principles,
        bool _isCollateralNFT,
        uint _NftID,
        uint _collateralAmount, // collateral amount calculated by aggregator
        uint _valuableCollateralAmount, //original borrower valuableAssetAmount/collateral amount from their order
        uint valuableCollateralUsed, // 
        address valuableAsset,
        uint _initialDuration, // duration from borrower order
        uint[] memory _principlesAmount,
        uint _borrowerID,
        infoOfOffers[] memory _acceptedOffers, //offer from lenders
        address m_OwnershipContract, // Debita Ownership
        uint feeInterestLender, // 15% to Debita
        address _feeAddress
