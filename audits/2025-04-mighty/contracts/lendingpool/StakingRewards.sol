...
contract StakingRewards is Ownable, IStakingRewards {
    ...
    modifier updateReward(address user) {
        for (uint256 i; i < rewardTokens.length; i++) {
            address rewardToken = rewardTokens[i];//@global index refresh
            ...
            if (user != address(0)) {
                ...
                userRewardPerTokenPaid[user][rewardToken] = rewardData[rewardToken].rewardPerTokenStored;//@set latest index
            }
        }
        _;
    }
    ...
    }//@the more token stake, the less rewards index accumulate
    ...
    }//@audit-ok rewardRate and global index compensate for this M hardcore rounded down too much. deposit high value wETH or USDC token will never get reward. Ex 1 USDC never earned anything

    function setReward(address rewardToken, uint256 startTime, uint256 endTime, uint256 totalRewards)
        public
        onlyOwner//@owner here is Admin and not LendingPool
        nonReentrant
        updateReward(address(0))
    {
        ...
        if (block.timestamp > startTime && totalStaked > 0) {//@audit-ok Early user abuse seem unlikely R missing startTime > block.timestamp. this fastforward rewards mechanism
        ...
        if (block.timestamp > startTime && totalStaked == 0) {//@replace old rewards
            // If this happens, there is no staked tokens from startTime to now
            // The rewards in this period should be removed
            uint256 dt = block.timestamp - startTime;
            totalRewards -= rewardData[rewardToken].rewardRate * dt;//@audit-ok M but rewardRate still being set. And new user deposit still get the same rate?
        }
