...
contract LendingPool is ILendingPool, OwnableUpgradeable, PaymentsUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initReserve(address asset) external onlyOwner notPaused {
        ...
        initReserve(reserveData, asset, eTokenAddress, type(uint256).max, id);//@setup reserveData. all token e18 precision. default 15% reserve rate
    ...
    function depositAndStake(uint256 reserveId, uint256 amount, address onBehalfOf, uint16 referralCode)
        ...
        returns (uint256 eTokenAmount)
    {
        eTokenAmount = _deposit(reserveId, amount, address(this));//@audit-ok M no minimal deposit open up inflation attack

        address stakingPool = reserves[reserveId].stakingAddress;
        require(stakingPool != address(0), "Address=0");
        IERC20(getETokenAddress(reserveId)).approve(stakingPool, eTokenAmount);//@deposit mint eToken to here.
        IStakingRewards(stakingPool).stake(eTokenAmount, onBehalfOf);//@audit R staking rewards does not have dela/timelock.. Does eToken have secondary purpose other than stake
        ...
        returns (uint256)
    {
        ...
        uint256 underlyingTokenAmount = _redeem(reserveId, eTokenAmount, to, receiveNativeETH);//@this does not redeem all eToken inside this contract
        ...
        returns (uint256)
    {
        ...
        IStakingRewards(stakingPool).withdrawByLendingPool(eTokenAmount, _msgSender(), address(this));//force reduce staked balance. eToken transfer from staked contract to this
    ...
    function _deposit(uint256 reserveId, uint256 amount, address onBehalfOf) internal returns (uint256 eTokenAmount) {
        ...
        // update states
        reserve.updateState(getTreasury());//@refresh global index. Also force print cash to treasury using current index

        // validate
        reserve.checkCapacity(amount);//@audit-ok check totalDebt and available funds no more than max limit R unclear mechanism to get total borrowed and liquidity?

        uint256 exchangeRate = reserve.reserveToETokenExchangeRate();

        // Transfer the user's reserve token to eToken contract
        pay(reserve.underlyingTokenAddress, _msgSender(), reserve.eTokenAddress, amount);//@send underlying token to eToken (wrap ETH if needed)

        // Mint eTokens for the user
        eTokenAmount = amount.mul(exchangeRate).div(Precision.FACTOR1E18);//@audit-ok I 1 wei exchange rate drift. Due to not rounding up and down like in ERC4626

        IExtraInterestBearingToken(reserve.eTokenAddress).mint(onBehalfOf, eTokenAmount);//@allow zero amount

        // update the interest rate after the deposit
        reserve.updateInterestRates();//@ refresh currentBorrowingRate based on utilization of new assets
    }
        ...
        returns (uint256)
    {
        DataTypes.ReserveData storage reserve = getReserve(reserveId);
        // update states
        reserve.updateState(getTreasury());//@this increase totalBorrowing, also increase exchange Rate slightly when refresh new interest rate into borrowing

        // calculate underlying tokens using eTokens
        uint256 underlyingTokenAmount =
            reserve.eTokenToReserveExchangeRate().mul(eTokenAmount).div(Precision.FACTOR1E18);//@do exchange rate change based on reduced liquidity reduced after redeem? This is straighline rate
        ...
        }//@audit-ok I no dust zero mint or burn preventation. Samething with redeem
    ...
    }//@ new debt position can be used to force global refresh
    ...
    function borrow(address onBehalfOf, uint256 debtId, uint256 amount) external notPaused nonReentrant {
        ...
        require(_msgSender() == debtPosition.owner, Errors.VL_INVALID_DEBT_OWNER);//@user cannot manually borrow without pool. there is check for credit below. Only vault allowed to borrow
        ...
        // update states
        reserve.updateState(getTreasury());
        updateDebtPosition(debtPosition, reserve.borrowingIndex);//@refresh global index into each debt storage
        ...
        require(amount <= credit, Errors.VL_OUT_OF_CREDITS);//@vault only
    ...
    {//@must be vault
        ...
        uint256 credit = credits[debtPosition.reserveId][_msgSender()];
        credits[debtPosition.reserveId][_msgSender()] = credit.add(amount);//@audit-ok M admin can set credit to infinity. Causing this to revert and no user can repay
        ...
        reserve.totalBorrows = reserve.totalBorrows.sub(amount);//@audit-ok M totalBorrowed != sum of borrowed. one is rounded down constantly, other one fly skip time to end index
    ...
    function createStakingPoolForReserve(uint256 reserveId) internal {
        ...
        reserves[reserveId].stakingAddress = StakingRewardsDeployer.deploy(eTokenAddress);//@new StakingRewards(eToken)
    ...
    function latestBorrowingIndex(uint reserveId) public view returns (uint256) {
        DataTypes.ReserveData storage reserve = reserves[reserveId];
