...
library Perp {
    ...
    struct AssetPoolStatus {
        address token; //quote/target token
        address supplyTokenAddress; // new deployed contract SupplyToken. Contract addr predictable
        ScaledAsset.AssetStatus tokenStatus; //generated empty, this is different from Perp.SqrtPerpAssetStatus
        InterestRateModel.IRMParams irmParams; //input //modified by poolOwner
        uint256 accumulatedProtocolRevenue; //0 on init
        uint256 accumulatedCreatorRevenue; //0
    }

    struct AssetRiskParams {
        uint128 riskRatio; //109544511 , ivr: 1e8 < riskRatio <= 10e8
        uint128 debtRiskRatio; // 2000  , no limit check
        int24 rangeSize; //ivr: > 0  , 1000  , ivr: 4096 max
        int24 rebalanceThreshold; //ivr: > 0  , 500 ,
        uint64 minSlippage; //1005000 , user input , no limit check
        uint64 maxSlippage; //1005000
    }
        ...
        returns (SqrtPerpAssetStatus memory)
    {
        return SqrtPerpAssetStatus(
            uniswapPool, //safe user input
            tickLower, //@ negative and inversed of upper
            tickUpper, //asset Risk params rangeSize 4096 max
            0,
            0,
            0,
            0,
            ...
            0,
            0,
            ScaledAsset.createUserStatus(), //empty struct (0,0)
    ...
    function updateFeeAndPremiumGrowth(uint256 _pairId, SqrtPerpAssetStatus storage _assetStatus) internal {
        ...
        unchecked {
            ...
            f1 = feeGrowthInside1X128 - _assetStatus.lastFee1Growth; //@audit how feeGrowth work. what is it variable
        }
        ...
        //spread = 1600 * (util -10%) ~= range (16_000e18,160_000e18)
