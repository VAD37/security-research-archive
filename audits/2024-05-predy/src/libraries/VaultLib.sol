...
library VaultLib {
    ...
    function createOrGetVault(GlobalDataLibrary.GlobalData storage globalData, uint256 vaultId, uint256 pairId)
        ...
        returns (uint256)
    {
        address quoteToken = globalData.pairs[pairId].quoteToken; //@quote can be empty because there is only 4 pairs
        ...
        if (vaultId == 0) {
            ...
            vault.openPosition.pairId = pairId; //if (pairId <= 0 || globalData.pairsCount <= pairId)
