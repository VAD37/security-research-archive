...
library AddPairLogic {
     ...
     */
    function addPair(
        GlobalDataLibrary.GlobalData storage _global, //feed  from  storage directly
        mapping(address => bool) storage allowedUniswapPools,
        AddPairParams memory _addPairParam //@pure uint struct
        ...
        address stableTokenAddress = _addPairParam.quoteToken; //@audit-ok there is check quoteToken exist in uniswap pool
        ...
        if (
            ...
            revert InvalidUniswapPool(); //@audit-ok revert when it return empty bytes. uniswap zero address pool still working
        }
        ...
        _storePairStatus(
            stableTokenAddress, //source token0
            _global.pairs, //sub storage
            pairId, //4 == current count start from 1.
            isQuoteZero ? uniswapPool.token1() : uniswapPool.token0(), //target token1
            isQuoteZero, //true
            _addPairParam //memory location
        );

        allowedUniswapPools[_addPairParam.uniswapPool] = true;

        _global.pairsCount++; //id 1 , quote: 0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8 bridge USDC.e , pool 0xC31E54c7a869B9FcBEcc14363CF510d1c41fa443 USDC/WETH UniswapV3Pool
        //id:2 , quote: 0xaf88d065e77c8cC2239327C5EDb3A432268e5831  USDC , pool 0xC6962004f452bE9203591991D15f6b388e09E8D0 USDC/WETH
        emit PairAdded(pairId, _addPairParam.quoteToken, _addPairParam.uniswapPool); //id 3 hacker pool, quote 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1 WETH, 0xC6962004f452bE9203591991D15f6b388e09E8D0
    }
    ...
    function updateIRMParams(
        ...
        //@audit M updateIRMParams() have IRM validation but not exist during registerPair().
        _pairStatus.quotePool.irmParams = _quoteIrmParams;
        _pairStatus.basePool.irmParams = _baseIrmParams;
        //@audit-ok this reset child struct not whole struct. H update IRMParams reset accrued token profit from fee.
        emit IRMParamsUpdated(_pairStatus.id, _quoteIrmParams, _baseIrmParams);
    }

    //CF: registerPair
    function _storePairStatus(
        address quoteToken, //source input token 0
        mapping(uint256 => DataType.PairStatus) storage _pairs, //globaldata.pairs mapping
        uint256 _pairId, //4 pairsCount manually index
        address _tokenAddress, //target token1
        bool _isQuoteZero, //true
        AddPairParams memory _addPairParam
    ) internal {
        validateRiskParams(_addPairParam.assetRiskParams); //@ 1e8 < riskRatio <= 10e8 . Test: 1.095e8 . Also check int24 variable > 0. which is kinda weird
        validateFeeRatio(_addPairParam.fee); // 0 fee <= 20

        require(_pairs[_pairId].id == 0, "AAA"); //check pairId exist. this is uncessary as index already increased. unless reset

        _pairs[_pairId] = DataType.PairStatus(
            _pairId,
            quoteToken,
            _addPairParam.poolOwner, //@audit M can create pool with zero address during registration.
            Perp.AssetPoolStatus(
                quoteToken,
                deploySupplyToken(quoteToken),
                ScaledAsset.createAssetStatus(), // AssetStatus(0, 0, 0, Constants.ONE, 0, 0);
            ...
            Perp.createAssetStatus( //empty struct with rangesize as lowerTick,upperTick
                _addPairParam.uniswapPool,
                -_addPairParam.assetRiskParams.rangeSize,
                _addPairParam.assetRiskParams.rangeSize
            ),
            _addPairParam.priceFeed, //@note pool owner have absolute power. Over oracle, pricefeed,params config hot reload.
            _isQuoteZero, //true
            _addPairParam.allowlistEnabled, //true
            _addPairParam.fee, //0
            block.timestamp
        );
    ...
    function deploySupplyToken(address _tokenAddress) internal returns (address) {
        ...
        //@ deploy both source/target token
        return address( //@gas no proxy clone
            new SupplyToken( //@note each new pool have 2 new contract wrapToken is very to predict address.
                address(this),
                string.concat("Predy6-Supply-", erc20.name()),
                string.concat("p", erc20.symbol()),
                erc20.decimals()
            )
        ); //@note there is no stopping user from generate malicious pool. with fake token
    }
    ...
    function validateIRMParams(InterestRateModel.IRMParams memory _irmParams) internal pure {
        require(
            ...
            "C4"
        ); //@audit M IRM missing-validation for slope2 > slope1. interest model can increase more slowly pass kink rate
    }
}
