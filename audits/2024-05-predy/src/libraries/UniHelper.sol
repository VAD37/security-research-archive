...
library UniHelper {
    ...
    function getSqrtPrice(address uniswapPoolAddress) internal view returns (uint160 sqrtPrice) {
        (sqrtPrice,,,,,,) = IUniswapV3Pool(uniswapPoolAddress).slot0(); //this return sqrtPriceX96 not sqrtPrice
    }
    ...
    function convertSqrtPrice(uint160 sqrtPriceX96, bool isQuoteZero) internal pure returns (uint160) {
        if (isQuoteZero) {
            return uint160((Constants.Q96 << Constants.RESOLUTION) / sqrtPriceX96); //2^96 * 2^96 / sqrtPriceX96 = sqrtPrice
        } else {
            return sqrtPriceX96; //@audit UniHelper library convertSqrtPrice function is wrong for isQuoteZero = false
        }
    }
