...
contract PredyPool is IPredyPool, IUniswapV3MintCallback, Initializable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address uniswapFactory) public initializer {
        __ReentrancyGuard_init(); //@note pool already have pair start from 1. with 3 pairs. and 200 vaults. 1 pair belong to hacker with unknown property
        AddPairLogic.initializeGlobalData(globalData, uniswapFactory); //reset pairs,vault count to 1. set uniswapFactory
        //@audit on mainnet globalData already been initialized by eearlier version.
        operator = msg.sender;
    } //@ok
    ...
    function uniswapV3MintCallback(uint256 amount0, uint256 amount1, bytes calldata) external override {
        ...
        require(allowedUniswapPools[msg.sender]); //@ pool was checked with uniswap factory
    ...
    /**
     * @notice Adds a new trading pair. //@storage have write operation with cound and boolean mapping. so keccak storage slot jumping is not available unless there is array provided.
    ...
    function registerPair(AddPairLogic.AddPairParams memory addPairParam) external onlyOperator returns (uint256) {
        return AddPairLogic.addPair(globalData, allowedUniswapPools, addPairParam); //address generated from token not possible to jumping destination
    }
    //@audit mainnet hack still have hacker poool added as safety pool.
    /**
    ...
    {
        //@note pool owner can change riskParams at anytime.
        //@ok same validation as addPair
    ...
    } //@ok missing some validation only
    ...
    } //@ok nothing special except oracle might not work
    ...
    function withdrawProtocolRevenue(uint256 pairId, bool isQuoteToken) external onlyOperator {
        Perp.AssetPoolStatus storage pool = _getAssetStatusPool(pairId, isQuoteToken); //read nothign will revert
    ...
    function withdrawCreatorRevenue(uint256 pairId, bool isQuoteToken) external onlyPoolOwner(pairId) {
        ...
        pool.accumulatedCreatorRevenue = 0; //@audit R can pool owner can manipulate accumulatedCreatorRevenue to signifactly high amount with 1 fake token on uniswap pool?
    ...
    } //@ok nothing special except pool owner can manipulate how much they gain
    ...
    function trade(TradeParams memory tradeParams, bytes memory settlementData)
        external
        nonReentrant //@audit trade new nonReentrant. Would this block UniswapV3MintCallback?
