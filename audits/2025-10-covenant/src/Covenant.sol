...
contract Covenant is ICovenant, NoDelegateCall, Ownable2Step {
    ...
    function createMarket(
        MarketParams calldata marketParams,//point to Lex deployed contract, curator is Oracle
        bytes calldata initData
    ) external noDelegateCall returns (MarketId) {//@open use. create any token pair.
        // Calculate marketId
        MarketId marketId = marketParams.id();//@no base/quote token order check.

        // validate marketParams
        ValidationLogic.checkMarketParams(marketParams, idToMarketParams[marketId], isLexEnabled, isCuratorEnabled);//@whitelist address check lex,curator oracle
        //@any token.
        ...
        (SynthTokens memory synthTokens, bytes memory lexData) = ILiquidExchangeModel(marketParams.lex).initMarket(
            marketId,
            marketParams,//@audit R do Lex and oracle curator support getting same token quote? like USDC/USDC
            _defaultProtocolFee,
            initData//@unused
        );//@revert if init twice.

        idToMarketParams[marketId] = marketParams;//@audit create new market. no oracle verification support token. No token sort. Lex does not care about same duplicate token too. Can be Base/Base
    ...
    function mint(
        MintParams calldata mintParams//@inlcude minimum out
        ...
        // check payment
        _checkPayment(mintParams.msgValue);//@multicall check

        // Validate mint params. check amountIn> 0, non zero receiver
        ValidationLogic.checkMintParams(mintParams);//@check id, params match. not sure why not just reading from id directly.
...
//@audit M if this have reentrancy. It could booost ERC4626 value, devalue input token in. because it use different oracle value. Which is very unlikely
        // Validate mint amounts
        ValidationLogic.checkMintOutputs(mintParams, aTokenAmountOut, zTokenAmountOut);//@revert if no share
        ...
        if (protocolFees > 0) ms.protocolFeeGrowth += protocolFees;//@before + after. no balance check

        // Transfer base asset in
        IERC20(mp.baseToken).safeTransferFrom(_msgSender(), address(this), mintParams.baseAmountIn);//Could be vault token. underlying vault value is calculated internally.
//@audit M  All ERC4626 vault as Base token. Must transfer share. baseAmountIn is consider share token of Vault.
        return (aTokenAmountOut, zTokenAmountOut);
    }//@note Covenant hold all token deposit by user. This include fake market with weird property
        ...
        )
    {
        ...
        (aTokenAmountOut, zTokenAmountOut, protocolFees, oracleUpdateFee, tokenPrices) = ILiquidExchangeModel(mp.lex)
            .quoteMint(mintParams, _msgSender(), localBaseSupply);//@ need to check this too same as normal mint.
    ...
    function multicall(bytes[] calldata data) external payable returns (bytes[] memory results) {
        ...
        // perform multicall
        _isMulticall = true;
        results = MulticallLib.multicall(data);//@note multicall have post-balance check and toggle multicall on.
        _isMulticall = false;
