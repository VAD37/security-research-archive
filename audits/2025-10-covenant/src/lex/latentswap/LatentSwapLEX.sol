...
import "./interfaces/ILatentSwapLEX.sol";
...
contract LatentSwapLEX is ILatentSwapLEX, TokenData, Ownable2Step {
    ...
    constructor(
        ...
        uint160 limHighSqrtPriceX96_,//95%
        uint160 limMaxSqrtPriceX96_,//99%
        int64 initLnRateBias_,//5012540000000000 WAD 0.05 = ~5%
        uint32 debtDuration_,//30 days
        uint8 swapFee_// 0
    ...
    function setQuoteTokenDecimalsOverrideForNewMarkets(address asset, uint8 newDecimals) external onlyOwner {
        _updateAssetDecimals(asset, newDecimals);//@audit M update new asset decimal does not revert old market already created. These new market also conflicting with old market. Wonder what happen with duplicate Lex market params, what happen if misconfigure token decimal happen?
    }
    ...
    /// @inheritdoc ILiquidExchangeModel
    function initMarket(
        MarketId marketId,//keccak from marketParams. decided by Covenant
        MarketParams calldata marketParams,//user input. must be whitelist
        uint32 protocolFee,
        ...
        // e.g., Symbol: ETHx2.USDT  Name: ETH x2 Leverage Coin (USDT/3M). BaseCoin x2  Leverage Coin (QuoteTToken/3Months)
        ...
        (uint256 currentBasePrice, ) = LatentSwapLogic.readBasePriceAndCalculateLiqRatio(
            ...
            false
        );//@audit so much fancy decimal conversion for price here. unknown how it would look like for USDC or WETH/ USDC or USDC/WETH
        ...
        returns (uint256 aTokenAmountOut, uint256 zTokenAmountOut, uint128 protocolFees, TokenPrices memory tokenPrices)
    {
        ...
        _updateOraclePrice(mintParams.marketParams, mintParams.data); //@pass data to oracle and msg.value.
//@audit no pyth oracle update if data is empty. User can choose to call update oracle or not. Unlcear how pyth oracle work. Delay up to 15 minutes to new live data
        ///////////////////////////////
        // Mint logic
        ...
        (currentState, tokenPrices, aTokenAmountOut, zTokenAmountOut) = LatentSwapLogic.mintLogic(
            ...
            baseTokenSupply,//@total user deposit - fees
            false
        );
