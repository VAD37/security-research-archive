...
import {
    ...
    MarketParams,
    LexParams
} from "../interfaces/ILatentSwapLEX.sol";
...
library LatentSwapLogic {
    ...
    function mintLogic(
        ...
        uint256 baseTokenSupply, //total user deposit
        bool isPreview
    )
        external
        view
        ...
        )
    {
        ...
        currentState =
            _calculateMarketState(mintParams.marketParams, lexParams, lexConfig, lexState, baseTokenSupply, isPreview);
    ...
    function redeemLogic(
        ...
        currentState = _calculateMarketState(
            redeemParams.marketParams, lexParams, lexConfig, lexState, baseTokenSupply, isPreview
        );

        ///////////////////////////////
        // Calculate redeem
        (amountOut, currentState.lexState.lastSqrtPriceX96) =
            _calcRedeem(lexParams, currentState, redeemParams.aTokenAmountIn, redeemParams.zTokenAmountIn);
        ...
        returns (LexFullState memory currentState, TokenPrices memory tokenPrices, uint256 amountCalculated)
    {
        ...
        // Calculate market state (storage read)
        currentState =
            _calculateMarketState(swapParams.marketParams, lexParams, lexConfig, lexState, baseTokenSupply, isPreview); ///////////////////////////////
    ...
    function calcRatio(LexParams memory lexParams, LexFullState memory marketState, AssetType base, AssetType quote)
        external
        pure
    ...
    function getInitMarketInfo(
        ...
        else console.log(" ---------- FANCY synth Decimals non 18? ---", info.synthDecimals);// @USDC is one case
        // Get actual quote decimals (as would be returned by IPriceOracle)
        (success, info.quoteDecimals) = IERC20(marketParams.quoteToken).tryGetDecimals();//@this ignore override
        if (!success) info.quoteDecimals = 18;//@audit why admin need to override decimal? synth decimal can get override by admin. this quote Decimal here is original token
        ...
        if (info.noCapLimit == 0) {
            ...
            if (!success2) {
                ...
                info.noCapLimit = (FixedPointMathLib.log2(10 ** baseDecimals) + 1).toUint8();
            }//@audit How noCapLimit work? why it need such fancy math setup
        }
        console.log("info.noCapLimit: ", info.noCapLimit);
        ...
        info.aTokenName =
            string.concat(info.baseName, " x", info.levStr, " Leverage Coin (", info.quoteSymbol, "/", info.durStr, ")"); // prettier-ignore
        info.aTokenSymbol = string.concat(info.baseSymbol, "x", info.levStr, ".", info.quoteSymbol);
        info.zTokenName = string.concat(
            info.quoteSymbol, " Yield Coin - backed by ", info.baseSymbol, " (x", info.levStr, "/", info.durStr, ")"
        ); // prettier-ignore
    ...
    function _calcMint(LexParams memory lexParams, LexFullState memory marketState, uint256 baseTokenAmountIn)
        internal
        pure
        returns (uint256 aTokenAmountOut, uint256 zTokenAmountOut)
    {
        ///////////////////////////////
        // Validate inputs
        ...
        uint160 liquidityIn =
            _synthToDex(marketState, baseTokenAmountIn, AssetType.BASE, Math.Rounding.Floor).toUint160();
            ...
            if (marketState.underCollateralized || marketState.liquidity == 0) {
                if (marketState.supplyAmounts[DEBT] > 0) {
                    ...
                    nextSqrtPriceX96 = (amountOut < marketState.baseTokenSupply)
                        ? lexParams.edgeSqrtPriceX96_B
                        : (marketState.supplyAmounts[LVRG] > 0) ? lexParams.edgeSqrtPriceX96_A : uint160(FixedPoint.Q96);
                } else {
                    ////////////////////////////////////
    ...
    function _calcSwap(
        ...
        if ((assetIn != AssetType.BASE) && (assetOut != AssetType.BASE)) {
            ...
            vars.inputDexAmount = _synthToDex(
                marketState, swapAmount, vars.fixedSynth, isExactIn ? Math.Rounding.Floor : Math.Rounding.Ceil
            );
            ...
            // Block minting while still allowing market value to appreciate.
            _checkSynthMintCap(
                marketState.supplyAmounts[assetOut == AssetType.DEBT ? DEBT : LVRG], isExactIn ? calcAmount : swapAmount
            );
            ...
            vars.newDexTokenAmount += Math.ternary(
                assetOut == AssetType.LEVERAGE, vars.aDexTokenAmount, vars.zDexTokenAmount
            );
        ...
        if (
            (assetIn != AssetType.BASE)
                && ((isExactIn ? swapAmount : calcAmount)
                        > marketState.supplyAmounts[(assetIn == AssetType.DEBT) ? 0 : 1])
    ...
    function _calcRatio(LexParams memory lexParams, LexFullState memory marketState, AssetType base, AssetType quote)
        internal
        pure
        returns (uint256 price)
    {
        ...
        price = _dexToSynth(
            marketState, _synthToDex(marketState, dexPrice, base, Math.Rounding.Ceil), quote, Math.Rounding.Ceil
        );
    }
    ...
    function _calculateMarketState(
        ...
        console.log("--- Calculate market State ---");
        ////////////////////////////////////////////////////////////////////////////////
        // Read external values
        // read synth token supplies (external call to trusted protocol)
        marketState.supplyAmounts[DEBT] = IERC20(marketState.lexConfig.zToken).totalSupply(); // debt token. USDT back by ETH
        marketState.supplyAmounts[LVRG] = IERC20(marketState.lexConfig.aToken).totalSupply(); // leverage token. ETH x2
        ...
        (marketState.lexState.lastBaseTokenPrice, marketState.liquidityRatioX96) = _readBasePriceAndCalculateLiqRatio(
            marketParams, lexParams.targetXvsL, marketState.lexConfig.scaleDecimals, isPreview
        );
        console.log("LastBaseTokenPrice: %e", marketState.lexState.lastBaseTokenPrice);
        console.log("LastSqrtPriceX96: %e %e", toPrice(marketState.lexState.lastSqrtPriceX96), marketState.lexState.lastSqrtPriceX96);
        ////////////////////////////////////////////////////////////////////////////////
        ...
        if (block.timestamp > marketState.lexState.lastUpdateTimestamp) {
            ...
            vars.spotLnRateBias = int256(marketState.lexState.lastLnRateBias)
                - int256(
            ...
            if (
                ...
                if (vars.tvlFee > 0) {
                    ...
                    vars.feeX96 = DebtMath.calculateLinearAccrual(
                        baseTokenSupply, uint256(vars.tvlFee) << FixedPoint.RESOLUTION, vars.elapsedTime
                    );
                }
                if ((vars.yieldFee > 0) && (vars.newDebtNotionalPrice > marketState.lexState.lastDebtNotionalPrice)) {
                    ...
                    vars.yieldInBaseUnits = marketState.baseTokenSupply
                        .mulDiv(
                    ...
                    vars.feeX96 += vars.yieldInBaseUnits
                        .mulDiv(uint256(vars.yieldFee) << FixedPoint.RESOLUTION, FixedPoint.PERCENTAGE_FACTOR);
                }
                ...
                } else if (vars.fee > type(uint96).max) {
                    vars.fee = type(uint96).max;
                }
            ...
            vars.invUpdateFactor = DebtMath.calculateApproxExponentialUpdate(LN2, vars.elapsedTime, ETWAP_MIN_HALF_LIFE);

            marketState.lexState.lastETWAPBaseSupply =
                marketState.lexState.lastETWAPBaseSupply.mulDiv(FixedPoint.RAY, vars.invUpdateFactor)
                    + marketState.baseTokenSupply
                        .mulDiv(
                            FixedPoint.RAY - (FixedPoint.RAY * FixedPoint.RAY) / vars.invUpdateFactor, FixedPoint.RAY
                        );
        }
        ...
        uint256 divScaleFactorX96 =
            marketState.liquidityRatioX96.saturatingMulShr(marketState.baseTokenSupply.max(FixedPoint.Q96), 152);
        ...
        marketState.synthAmountsScaled[uint8(AssetType.BASE)] =
            Math.ternary(scaledLiquidity, divScaleFactorX96, FixedPoint.Q96);
        ...
        marketState.synthAmountsScaled[uint8(AssetType.DEBT)] = scaledLiquidity
            ? (marketState.lexState.lastDebtNotionalPrice < FixedPoint.Q160)
                ? (divScaleFactorX96 < FixedPoint.Q192) ? FixedPoint.WAD * divScaleFactorX96 : divScaleFactorX96
        ...
        marketState.liquidity =
            _synthToDex(marketState, marketState.baseTokenSupply, AssetType.BASE, Math.Rounding.Floor).toUint160();
        ...
        if (marketState.liquidity > 0) {
            ...
            marketState.dexAmounts[DEBT] =
                _synthToDex(marketState, marketState.supplyAmounts[DEBT], AssetType.DEBT, Math.Rounding.Floor);

            // Check max value for debt, given availablie liquidity in market.
            vars.maxDebtValue = LatentMath.computeMaxDebt(
                lexParams.edgeSqrtPriceX96_A, lexParams.edgeSqrtPriceX96_B, marketState.liquidity
            );
            ...
            if (marketState.dexAmounts[DEBT] > vars.maxDebtValue) {
                ...
                (marketState.dexAmounts[LVRG], marketState.lexState.lastSqrtPriceX96) =
                    LatentMath.getMarketStateFromLiquidityAndDebt(
                        lexParams.edgeSqrtPriceX96_A,
                        lexParams.edgeSqrtPriceX96_B,
                        marketState.liquidity,
                        marketState.dexAmounts[DEBT]
        ...
        marketState.dexAmountsScaled[uint8(AssetType.LEVERAGE)] =
            Math.ternary(marketState.supplyAmounts[LVRG] > 0, Math.max(marketState.dexAmounts[LVRG], 1), 1);
        marketState.synthAmountsScaled[uint8(AssetType.LEVERAGE)] =
            Math.ternary(marketState.supplyAmounts[LVRG] > 0, marketState.supplyAmounts[LVRG], 1);
    ...
    function _getDebtPriceDiscount(
        ...
        uint256 current_dXdL_X96 =
            LatentMath.get_XvsL(currentSqrtPriceX96, edgeSqrtPriceX96_A, edgeSqrtPriceX96_B, AssetType.DEBT);
    ...
    function _checkMintCap(
        ...
        unchecked {
            if (marketBaseTokenSupply <= (1 << noCapLimit)) {
                ...
                (mintAmount > marketBaseTokenSupply) || ((type(uint256).max - mintAmount) < marketBaseTokenSupply)
    ...
    function _checkRedeemCap(
        ...
        unchecked {
            if (
                (marketBaseTokenSupply > (1 << noCapLimit)) && (marketBaseTokenSupply > redeemAmount)
    ...
    function _synthToDex(
        ...
        return synthAmount.mulDiv(
            marketState.dexAmountsScaled[uint8(assetType)], marketState.synthAmountsScaled[uint8(assetType)], rounding
        );
    }
    ...
    function _dexToSynth(
        ...
        return dexAmount.saturatingMulDiv(
            marketState.synthAmountsScaled[uint8(assetType)], marketState.dexAmountsScaled[uint8(assetType)], rounding
        );
    }
    ...
    function _readBasePriceAndCalculateLiqRatio(
        ...
        console.log("scaledLiquidityConcentrationX96: %e %e", toPrice(scaledLiquidityConcentrationX96), scaledLiquidityConcentrationX96);
        ...
        liqRatioX96 = (isPreview)
            ? IPriceOracle(marketParams.curator)
                .previewGetQuote(scaledLiquidityConcentrationX96, marketParams.baseToken, marketParams.quoteToken)
            : liqRatioX96 = IPriceOracle(marketParams.curator)
                .getQuote(scaledLiquidityConcentrationX96, marketParams.baseToken, marketParams.quoteToken);
    ...
    function _calculateTokenPrices(LexParams memory lexParams, LexFullState memory marketState)
        internal
        pure
        returns (TokenPrices memory tokenPrices)
    {
        ...
        tokenPrices.aTokenPrice = (marketState.underCollateralized && marketState.dexAmounts[LVRG] > 0)
            ? 0
            : tokenPrices.baseTokenPrice
                .mulDiv(_calcRatio(lexParams, marketState, AssetType.LEVERAGE, AssetType.BASE), FixedPoint.WAD);
        ...
        tokenPrices.zTokenPrice = (marketState.underCollateralized && marketState.dexAmounts[DEBT] > 0)
            ...
            : tokenPrices.baseTokenPrice
                .mulDiv(_calcRatio(lexParams, marketState, AssetType.DEBT, AssetType.BASE), FixedPoint.WAD);
    }
    ...
    function _squareUnsafe(uint256 value) internal pure returns (uint256) {
        ...
        function toPrice(uint256 sqrtRatioX96) internal pure returns (uint256 price) {
