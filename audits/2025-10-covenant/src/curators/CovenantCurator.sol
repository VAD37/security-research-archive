...
contract CovenantCurator is Ownable2Step, IPriceOracle {
    ...
    function getQuote(uint256 inAmount, address base, address quote) external view returns (uint256) {
        ...
        if (base == quote) return inAmount;//@oracle here seem ignore BASE_ERC4626/BASE_ERC4626 case
        ...
        returns (uint256, /* resolvedAmount */ address, /* base */ address, /* quote */ address /* oracle */)
    {
        ...
        if (oracle != address(0)) return (inAmount, base, quote, oracle); //@return oracle set by config. order of token got fixed. Oracle must handle it later.
        // 3. Recursively resolve `base`.
        address baseAsset = resolvedVaults[base];//@baseToken must a be vault contract. baseAsset is just vault deposit token. Just any Stub ERC4626. Unclear who is actual vault here? Could be vault ETH maybe
        if (baseAsset != address(0)) {
            inAmount = IERC4626(base).convertToAssets(inAmount);//@hope vault does not include rewards asset and vault faithfully share price never decrease.
            return resolveOracle(inAmount, baseAsset, quote);//@direct loop get oracle again. What happen if vault token is vault? weird but unlikely.
        }
    ...
    function updatePriceFeeds(address base, address quote, bytes calldata updateData) external payable {
        ...
        return IPriceOracle(oracle).updatePriceFeeds{value: msg.value}(base, quote, updateData);//@note only Pyth Oracle require payment. and pull new data from off-chain using user data as merkle verification
    }
