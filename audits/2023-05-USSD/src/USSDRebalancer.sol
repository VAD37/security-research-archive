...
contract USSDRebalancer is AccessControlUpgradeable, IUSSDRebalancer {
    ...
    // main USSD/DAI pool
    IUniswapV3Pool public uniPool;//admin can change pool

    // USSD token
    address public USSD;

    // boundary to make rebalancing
    uint256 private threshold;//1e4
    ...
    function getOwnValuation() public view returns (uint256 price) {//@audit sqrtPriceX96 is not used for price uniswap v3
      (uint160 sqrtPriceX96,,,,,,) =  uniPool.slot0();//https://blog.uniswap.org/uniswap-v3-math-primer
      if(uniPool.token0() == USSD) {//price=(sqrtPriceX96/2^96)^2 * (10^(token0_decimal-token1_decimal))  //price = token1 / token0 == exchangeRate token0 amount to 1 token1
        price = uint(sqrtPriceX96)*(uint(sqrtPriceX96))/(1e6) >> (96 * 2);//@audit H this always return 0
      } else {//@audit L for else case. the other token might not be 1e18 decimal
        price = uint(sqrtPriceX96)*(uint(sqrtPriceX96))*(1e18 /* 1e12 + 1e6 decimal representation */) >> (96 * 2);
        // flip the fraction. @audit M Precision loss?
    ...
    function getSupplyProportion() public view returns (uint256, uint256) {
      ...
      uint256 vol2 = IERC20Upgradeable(uniPool.token1()).balanceOf(address(uniPool));//@audit supply portion is not flashloan resistant.
    ...
    function rebalance() override public {
      ...
      if (ownval < 1e6 - threshold) {//999977 for DAI/USDC
        // peg-down recovery
        ...
        SellUSSDBuyCollateral();//@audit attacker can add their own position and steal 0.5% fee. then swap back to DAI
      }
    }

    function BuyUSSDSellCollateral(uint256 amountToBuy) internal {//amount = half the different
      ...
      for (uint256 i = 0; i < collateral.length; i++) {
        ...
        if (collateralval > amountToBuyLeftUSD) {//collateralval = 100e8 *1e18 / 1e8 * 20000e18 / 1e18 = 100e18 * 20000
          // sell a portion of collateral and exit
          if (collateral[i].pathsell.length > 0) {
            uint256 amountBefore = IERC20Upgradeable(baseAsset).balanceOf(USSD);
            uint256 amountToSellUnits = IERC20Upgradeable(collateral[i].token).balanceOf(USSD) * ((amountToBuyLeftUSD * 1e18 / collateralval) / 1e18) / 1e18;//@audit amountToSellUnits div too much 1e18. It always 0 for token like WBTC. The last division is unnecessary
