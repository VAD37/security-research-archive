...
contract FaultDisputeGame is IFaultDisputeGame, Clone, ISemver {
    ...
    Claim internal immutable ABSOLUTE_PRESTATE;// 0x03617abec0b255dc7fc7a0513a2c2220140a1dcd7a1c8eca567659bd67e05cea 

    /// @notice The max depth of the game.
    uint256 internal immutable MAX_GAME_DEPTH;// 73
    ...
    uint256 internal immutable SPLIT_DEPTH;// 30

    /// @notice The maximum duration that may accumulate on a team's chess clock before they may no longer respond.
    Duration internal immutable MAX_CLOCK_DURATION;// 302400 or 3.5 days

    /// @notice An onchain VM that performs single instruction steps on a fault proof program trace.
    IBigStepper internal immutable VM;// 0x0f8EdFbDdD3c0256A80AD8C0F2560B1807873C9c MIPS

    /// @notice The game type ID.
    GameType internal immutable GAME_TYPE;// 0

    /// @notice WETH contract for holding ETH.
    IDelayedWETH internal immutable WETH;// Delayed WETH 0xE497B094d6DbB3D5E4CaAc9a14696D7572588d14

    /// @notice The anchor state registry.
    IAnchorStateRegistry internal immutable ANCHOR_STATE_REGISTRY; // 0x18DAc71c228D1C32c99489B7323d441E1175e443

    /// @notice The chain ID of the L2 network this contract argues about.
    uint256 internal immutable L2_CHAIN_ID;// 10
    ...
    Duration internal immutable CLOCK_EXTENSION;// 10800 or 3 hours

    /// @notice The global root claim's position is always at gindex 1.
    Position internal constant ROOT_POSITION = Position.wrap(1);// uint128 ROOT_POSITION = 1. @it is unclear how this wrap work
    ...
    constructor(
        GameType _gameType,//0
        Claim _absolutePrestate,//0x03617abec0b255dc7fc7a0513a2c2220140a1dcd7a1c8eca567659bd67e05cea 
        uint256 _maxGameDepth,//73
        uint256 _splitDepth,//30
        Duration _clockExtension,//10800 or 3 hours
        Duration _maxClockDuration,//302400 or 3.5 days
        IBigStepper _vm,//0x0f8EdFbDdD3c0256A80AD8C0F2560B1807873C9c MIPS
        IDelayedWETH _weth,//Delayed WETH 0xE497B094d6DbB3D5E4CaAc9a14696D7572588d14
        IAnchorStateRegistry _anchorStateRegistry,//0x18DAc71c228D1C32c99489B7323d441E1175e443 
        uint256 _l2ChainId//10
    ) {
    ...
    function initialize() public payable virtual {
        ...
        startingOutputRoot = OutputRoot({ l2BlockNumber: rootBlockNumber, root: root });//@could be fake root, but block number always increasing
        ...
        //
        // Expected length: 0x7A or 122
        // - 0x04 selector //4 bytes
        // - 0x14 creator address //@20 msg.sender call create()
        // - 0x20 root claim //@32 bytes user input unique hash from L2 chain
        // - 0x20 l1 head //@32 blockhash(block.number - 1)
        // - 0x20 extraData //@ 32 bytes user input?  _extraData is encoded without length this is 64 bytes 0 length and data in 32 bytes not fully written 0x00000000000000000000000000000000000000000000000000000000075324e8
        // - 0x02 CWIA bytes // 2 bytes of calldata reserved by clone library?
        assembly {
        ...
        if (l2BlockNumber() <= rootBlockNumber) revert UnexpectedRootClaim(rootClaim());//@l2BlockNumber read 32bytes from extraData.
        ...
        claimData.push(
            ClaimData({
                ...
                claimant: gameCreator(),//anyone as msg.sender
                bond: uint128(msg.value),//0.08ETH
                claim: rootClaim(),//user input
                position: ROOT_POSITION,//uint128 Position.wrap(1) = 1; @magic function
                clock: LibClock.wrap(Duration.wrap(0), Timestamp.wrap(uint64(block.timestamp)))//abi.encodePacked (Duration uint64 , Timestamp uint64)
            })
        );
        ...
        // Deposit the bond.
        WETH.deposit{ value: msg.value }();//delayed deposit
    ...
    function gameCreator() public pure returns (address creator_) {
        creator_ = _getArgAddress(0x00);//80 bit, 20 bytes or 0x14
    }

    /// @inheritdoc IDisputeGame
    function rootClaim() public pure returns (Claim rootClaim_) {
        rootClaim_ = Claim.wrap(_getArgBytes32(0x14));//32 bytes
    }

    /// @inheritdoc IDisputeGame
    function l1Head() public pure returns (Hash l1Head_) {
        l1Head_ = Hash.wrap(_getArgBytes32(0x34));//32 bytes
    }
    ...
    function extraData() public pure returns (bytes memory extraData_) {
        ...
        // is 32 bytes long.
        extraData_ = _getArgBytes(0x54, 0x20);//@include length when reading bytes data too //0x000000000000000000000000075324e8
    }
    ...
    function gameData() external view returns (GameType gameType_, Claim rootClaim_, bytes memory extraData_) {
        ...
        extraData_ = extraData();//0x00000000000000000000000000000000000000000000000000000000075324e8
    }
