...
contract DisputeGameFactory is OwnableUpgradeable, IDisputeGameFactory, ISemver {
    ...
    function create(
        GameType _gameType,
        Claim _rootClaim,//6ff6a009f01329c506678dd305cd25224f1d51e9f502bc2fe20dc32ec3e8c0b5
        bytes calldata _extraData//0x00000000000000000000000000000000000000000000000000000000075324e8
    )
        external
        payable
        returns (IDisputeGame proxy_)
    {
        ...
        IDisputeGame impl = gameImpls[_gameType];//2 game type. 0: FaultDisputeGame, 1: PermissionedDisputeGame

        // If there is no implementation to clone for the given `GameType`, revert.
        if (address(impl) == address(0)) revert NoImplementation(_gameType);

        // If the required initialization bond is not met, revert.
        if (msg.value != initBonds[_gameType]) revert IncorrectBondAmount();//0.08 ETH

        // Get the hash of the parent block.
        bytes32 parentHash = blockhash(block.number - 1);//@audit what happen if ethereum re-org though?
        ...
        // └──────────────┴────────────────────────────────────┘ //@clone immutable arguments with bytes data
        proxy_ = IDisputeGame(address(impl).clone(abi.encodePacked(msg.sender, _rootClaim, parentHash, _extraData)));//@only FaultDisputeGame exist
        proxy_.initialize{ value: msg.value }();//@clone cloneDeterministic. unique per transaction, for each address.
        //@initialize only accept extraData is 32 bytes. with 0 length
    ...
    function getGameUUID(
        ...
        pure
        returns (Hash uuid_)
    {//@audit ?? wrap for Hash(bytes32) function not exist in any library? Where is this coming from?
