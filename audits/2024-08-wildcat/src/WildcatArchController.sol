...
contract WildcatArchController is SphereXConfig, Ownable {
  ...
  constructor() SphereXConfig(msg.sender, address(0), address(0)) {
    _initializeOwner(msg.sender);//unique owner for wildcatController.
  }
  ...
  /**
   * @dev Update SphereX engine on registered contracts and add them as //@2 step process: add, enable
   *      allowed senders on the engine contract.
   */
  function updateSphereXEngineOnRegisteredContracts(//@ someone must add these address to enumSet bedfore it can be enabled. 
    address[] calldata controllerFactories,//@this enabled address can be called not add them new. 
  ...
  function _updateSphereXEngineOnRegisteredContractsInSet(
    ...
    for (uint256 i = 0; i < contracts.length; i++) {
      ...
      _callWith(account, changeSphereXEngineCalldata);//@changeSphereXEngine(), update new sphereX address
      if (engineAddress != address(0)) {
        assembly {
          mstore(add(addAllowedSenderOnChainCalldata, 0x24), account)//@audit-ok this mstore look weird. It override wrong slot?. addAllowedSenderOnChainCalldata is 64 bytes. 4 bytes seecltor, 20 bytes address take up to 1 or 2 slot.
        }//0x24 = 36bytes. Did this skip the address slot? bytes memory include 1 slot for lenght. 1 slot for location to read. then calldata slot with 32 bytes. 8 selctor,20 for address. = 28 bytes taken
        _callWith(engineAddress, addAllowedSenderOnChainCalldata);//tell sphereX I just registered
        emit_NewAllowedSenderOnchain(account);//@0k addAllowedSenderOnChain() emit event still right. local address in stack is still 32 bytes not 20 bytes address. mstore replace entire slot with local stack address slot with new slot
      }
    }
  }
  ...
  function _callWith(address target, bytes memory data) internal {
    assembly {
      if iszero(call(gas(), target, 0, add(data, 0x20), mload(data), 0, 0)) {//@audit-ok M malicious crafting for abi.encoding vs manual mload and validator
        returndatacopy(0, 0, returndatasize())//mload(data) here is double mload. data pass offset to mload. mload(data)= mload(data_offset+offset_read). it read 2nd slot from bytes memory which is bytes size.
        revert(0, returndatasize())//@ this also why data only skip 32 bytes not 64 bytes.
      }
    }
  }
