...
contract WildcatArchControllerIntegrationTest is BaseMarketTest {
    function deployControllersAndMarkets(uint256 numControllers, uint256 numMarketsPerController)
        internal
        returns (address[] memory controllers, address[] memory markets)
    {
        controllers = new address[](numControllers);
        markets = new address[](numControllers * numMarketsPerController);
        for (uint256 i = 0; i < numControllers; i++) {
            MarketInputParameters memory parameters;
            controllers[i] = address(deployHooksInstance(parameters, false));
            for (uint256 j = 0; j < numMarketsPerController; j++) {
    ...
    function test_updateSphereXEngineOnRegisteredContracts() external asAccount(SphereXOperator) {
        ...
        archController.updateSphereXEngineOnRegisteredContracts(controllerFactories, controllers, markets);
    }

    function test_updateSphereXEngineOnRegisteredContracts_NullEngine() external asAccount(SphereXOperator) {
        address oldSphereXEngine = SphereXEngine;
    ...
    function test_updateSphereXEngineOnRegisteredContracts_MarketDoesNotExist() external asAccount(SphereXOperator) {
    ...
    function test_updateSphereXEngineOnRegisteredContracts_SphereXOperatorOrAdminRequired(address account) external {
