...
contract Allo is IAllo, Native, Transfer, Initializable, Ownable, AccessControl, ReentrancyGuardUpgradeable, Errors {
    ...
    uint256 private percentFee;//0% for init
    ...
    uint256 internal baseFee;// 0 for init
    ...
    ) external payable returns (uint256 poolId) {//@audit R this missing nonreentrant for custom strategy?
    ...
    function createPool(
        ...
        return _createPool(
            _profileId,
            IStrategy(Clone.createClone(_strategy, _nonces[msg.sender]++)),//@audit H all strategies are proxyBytes of create3.  You cannot clone a clone .clone address predictable. Donation resistant? Pool frontrun?
            _initStrategyData,
            _token,
            _amount,
            _metadata,
    ...
    function recoverFunds(address _token, address _recipient) external onlyOwner {
        ...
        uint256 amount = _token == NATIVE ? address(this).balance : IERC20Upgradeable(_token).balanceOf(address(this));//@audit M balanceOf might not always work for ERC20
    ...
    function _createPool(
        ...
        bytes32 POOL_ADMIN_ROLE = keccak256(abi.encodePacked(poolId, "admin"));//@audit poolId is not unique. Duplicate authentication giving to user
    ...
    function _isPoolAdmin(uint256 _poolId, address _address) internal view returns (bool) {
        return hasRole(pools[_poolId].adminRole, _address);//@note poolManager have access to revoke,renounce,transfer control of the pool
    }
