...
contract Transfer is Native {
    ...
    function _transferAmountsFrom(address _token, TransferData[] memory _transferData) internal returns (bool) {
        uint256 msgValue = msg.value;//@audit Allo remove some baseFee from msg.value beforehand. why use this again?
        ...
        for (uint256 i; i < _transferData.length;) {
            ...
            if (_token == NATIVE) {
                ...
                SafeTransferLib.safeTransferETH(transferData.to, transferData.amount);//@audit User can steal ETH fee from contract if bypassed .transferFrom native token it have msg.sender send msg.value to contract beforehand.
            } else {
    ...
    function _transferAmountFrom(address _token, TransferData memory _transferData) internal returns (bool) {
        ...
        if (_token == NATIVE) {
            // Native Token
            if (msg.value < amount) revert AMOUNT_MISMATCH();//@audit M _transferAmountFrom take more native token than necessary
