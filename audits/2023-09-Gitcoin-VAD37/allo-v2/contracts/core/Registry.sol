...
contract Registry is IRegistry, Native, AccessControl, Transfer, Initializable, Errors {
    ...
    function createProfile(
        ...
        bytes32 profileId = _generateProfileId(_nonce);//@note profileID based on msg.sender. But ownership can be someone else
    ...
    function updateProfileName(bytes32 _profileId, string memory _name)
        external
        onlyProfileOwner(_profileId)//@audit profile owner can be given to anyone
        returns (address anchor)
    {
        // Generate a new anchor address
        anchor = _generateAnchor(_profileId, _name);//@audit anchor address collide in any possible way
    ...
    function _generateAnchor(bytes32 _profileId, string memory _name) internal returns (address anchor) {
        bytes32 salt = keccak256(abi.encodePacked(_profileId, _name));//_profileId = keccak256(abi.encodePacked(_nonce, msg.sender));

        address preCalculatedAddress = CREATE3.getDeployed(salt);//salt here with deployed code of smallCloneProxy

        // check if the contract already exists and if the profileId matches
        if (preCalculatedAddress.code.length > 0) {//@audit-ok this is reupdate name .how create3 deployed conflict beforehand work?
            ...
            anchor = CREATE3.deploy(salt, creationCode, 0);//@audit do CREATE3 constructor work?
        }//@audit lack check if anchor address generated is zero.
    }
