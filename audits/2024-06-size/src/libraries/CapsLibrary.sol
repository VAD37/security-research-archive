...
library CapsLibrary {
    ...
    function validateVariablePoolHasEnoughLiquidity(State storage state, uint256 amount) public view {
        ...
        uint256 liquidity = state.data.underlyingBorrowToken.balanceOf(address(aToken));//@audit L It still not possible to reach max liquidity value, limit by how money deposited by Size vs normal User .USDC.balanceOf(aToken) is not actual liquidity of AAVE pool. It might include accidental balance send by someone else
        if (liquidity < amount) {
