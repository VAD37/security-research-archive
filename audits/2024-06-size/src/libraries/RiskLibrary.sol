...
library RiskLibrary {
    ...
    /// @return The collateral ratio //@ratio should be in e18 decimals
    function collateralRatio(State storage state, address account) public view returns (uint256) {
        uint256 collateral = state.data.collateralToken.balanceOf(account);// szWETH
        uint256 debt = state.data.debtToken.balanceOf(account);//szDebt_USDC debt, This is not the same as  borrowATokenV1_5 which is just aave deposit USDC
        uint256 price = state.oracle.priceFeed.getPrice();

        if (debt != 0) {//@ collateral * 1e6 * price / (debt * 1e18)
            return Math.mulDivDown(
                collateral * 10 ** state.data.underlyingBorrowToken.decimals(),
                price,
                debt * 10 ** state.data.underlyingCollateralToken.decimals()//@debtWad = amount * 10 ** (18 - underlyingBorrowToken);
            );//@audit collateral seem weird, some test is needed to verify. this is far different from previous version. So the use case of lib must also edited too.
        } else {
