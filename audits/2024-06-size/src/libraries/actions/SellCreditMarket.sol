...
library SellCreditMarket {
    ...
    function validateSellCreditMarket(State storage state, SellCreditMarketParams calldata params) external view {
        ...
        if (params.creditPositionId == RESERVED_ID) {//@borrower create new loan to lender
            tenor = params.tenor;
            ...
            if (!state.isCreditPositionTransferrable(params.creditPositionId)) { //@note borrower can have multiple loan. Collateral ratio is shared among all loans
        ...
        try state.getBorrowOfferAPRByTenor(params.lender, tenor) returns (uint256 borrowAPR) {//@try catch return can be used as local variable?
            if (borrowAPR >= loanAPR) {
                revert Errors.MISMATCHED_CURVES(params.lender, tenor, loanAPR, borrowAPR);
            }//@audit R this look weird need testing. this verify that lender minimum rate is higher than borrower maximum rate? 
        } catch (bytes memory) {
            // N/A
        }
    }
