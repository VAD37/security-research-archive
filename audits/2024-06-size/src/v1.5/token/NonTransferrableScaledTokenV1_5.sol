...
contract NonTransferrableScaledTokenV1_5 is IERC20Metadata, IERC20Errors, Ownable2StepUpgradeable, UUPSUpgradeable {
    ...
    function deposit(address, address to, uint256 amount) external onlyMarket {
        underlyingToken.safeTransferFrom(msg.sender, address(this), amount);//take USDC from Size, Size already receive from user

        IAToken aToken = IAToken(variablePool.getReserveData(address(underlyingToken)).aTokenAddress);// USDC from aave
        ...
        _mintScaled(to, scaledAmount);//@same as before
    }
