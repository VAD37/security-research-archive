...
contract Size is ISize, SizeView, Initializable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    {
        uint128 oldBorrowRate = state.oracle.variablePoolBorrowRate;//@L no check < 100% rate
    ...
    /// @inheritdoc ISize
    function deposit(DepositParams calldata params) public payable override(ISize) whenNotPaused {//@ aave debt token is transfer to middlemen Token craeted through factory
        state.validateDeposit(params);//@no change, only middleman layer changed to different contract instead of handling through Size directly.
        state.executeDeposit(params);//@event change
    }

    /// @inheritdoc ISize
    function withdraw(WithdrawParams calldata params) external payable override(ISize) whenNotPaused {
        state.validateWithdraw(params);//@no change. aave token withdraw from middle layer
        state.executeWithdraw(params);//@event change. @ risk calculation change a little bit. fomular still the same
    }

    /// @inheritdoc ISize //this create an static offer setting by user
    function buyCreditLimit(BuyCreditLimitParams calldata params) external payable override(ISize) whenNotPaused {
        state.validateBuyCreditLimit(params);//@OfferLibrary become for more fancy.
        state.executeBuyCreditLimit(params);//@no change. just store user offer based on tenors with curve
    }

    /// @inheritdoc ISize
    function sellCreditLimit(SellCreditLimitParams calldata params) external payable override(ISize) whenNotPaused {
        state.validateSellCreditLimit(params);//@same as above
    ...
    function buyCreditMarket(BuyCreditMarketParams calldata params) external payable override(ISize) whenNotPaused {
        ...
        uint256 amount = state.executeBuyCreditMarket(params);//@amount = cashIn - swap APR fee -fragmentation fee
        if (params.creditPositionId == RESERVED_ID) {
            state.validateUserIsNotBelowOpeningLimitBorrowCR(params.borrower);
        }
        state.validateVariablePoolHasEnoughLiquidity(amount);//@ok
    }
    ...
    function copyLimitOrders(CopyLimitOrdersParams calldata params) external payable override(ISize) whenNotPaused {
        state.validateCopyLimitOrders(params);//@new function v1.7
