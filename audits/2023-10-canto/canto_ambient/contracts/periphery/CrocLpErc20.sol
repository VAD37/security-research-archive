...
contract CrocLpErc20 is ERC20, ICrocLpConduit {
    ...
    function withdrawCrocLiq (address sender, bytes32 pool,//@old interface 0x0f382c0e
                              int24 lowerTick, int24 upperTick, uint128 seeds,//@ new version 0x668e2cdb
                              uint72) public override returns (bool) {//@audit H crocLPERC20 change method variable also change method signature. does this effect CrocSwap anyway
        require(pool == poolHash, "Wrong pool");
        require(lowerTick == 0 && upperTick == 0, "Non-Ambient LP Deposit");
        _burn(sender, seeds);
        return true;
