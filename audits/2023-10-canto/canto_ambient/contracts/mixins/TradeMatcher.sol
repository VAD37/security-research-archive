 ...
 *           5) Swap                                                     */
contract TradeMatcher is LiquidityMining, LiquidityCurve, KnockoutCounter,//@note Trade matcher update from PositionRegistar to LiquidityMining.
    ProxyCaller {//@audit old storage from PositionRegistar still exist. positions_ -> positions72. What happen to old position though?
    ...
    function burnAmbient (CurveMath.CurveState memory curve, uint128 liqBurned, 
        ...
        console.log("burnAmbient before:" , uint(curve.ambientSeeds_) );
        (uint128 base, uint128 quote) = liquidityPayable(curve, liqSeeds);
        console.log("burnAmbient after: " , uint(curve.ambientSeeds_) );
    ...
    function mintRange (CurveMath.CurveState memory curve, int24 priceTick,
        ...
        mintPosLiq(lpOwner, poolHash, lowTick, highTick,//@audit here is access old position_ storage. it is unknown if user can still access old position?
    ...
    function burnRange (CurveMath.CurveState memory curve, int24 priceTick,
        ...
        console.log("burnConcen before:" , uint(curve.concLiq_) );
        (uint128 base, uint128 quote) = liquidityPayable(curve, liquidity, rewards,
                                                         lowTick, highTick);
        console.log("burnConcen after: " , uint(curve.concLiq_) );
        return signBurnFlow(base, quote);
    }
    ...
    function claimKnockout (CurveMath.CurveState memory curve, 
        ...
        uint128 liquidity = lots.lotsToLiquidity();//@audit claim knockout does not accrue anything
    ...
    function harvestRange (CurveMath.CurveState memory curve, int24 priceTick,
        ...
        accrueConcentratedGlobalTimeWeightedLiquidity(poolHash, curve);//@audit this thing bump ambient not cencentrated
        console.log("burnAmbient before:" , uint(curve.ambientSeeds_) );
