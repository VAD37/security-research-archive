...
contract StorageLayout {
    ...
    struct RangePosition72 {//@note range position 72 is new. but what happen to old range position
        uint128 liquidity_;
        uint72 feeMileage_;
        uint32 timestamp_;
        bool atomicLiq_;
    ...
    mapping(bytes32 => RangePosition) internal positions_;//@audit old range positions_ not used anymore? Or it just hide behind other side operation in diamond contracts
    ...
    //@note all storage below to serve new sidecar update liquidity mining
    ...
    mapping(bytes32 => mapping(uint32 => uint64)) concRewardPerWeek_;//@ admin stuff
    mapping(bytes32 => mapping(uint32 => uint64)) ambRewardPerWeek_;//@ week % 604800 == 0 
...
library CrocSlots {
    ...
    uint16 constant MICRO_PROXY_LEGACY_IDX = 5;//@note what happen to legacy method?
