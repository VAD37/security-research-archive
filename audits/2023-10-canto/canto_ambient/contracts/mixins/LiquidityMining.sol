...
contract LiquidityMining is PositionRegistrar {
    ...
    function initTickTracking(bytes32 poolIdx, int24 tick) internal {
        ...
        tickTracking_[poolIdx][tick].push(tickTrackingData);//@audit M tick array can grow really big to pass around
    }
    ...
    function accrueAmbientGlobalTimeWeightedLiquidity(
        bytes32 poolIdx,//@audit can accrue non exist pool
        CurveMath.CurveState memory curve
    ) internal {
    ...
    function accrueAmbientPositionTimeWeightedLiquidity(
        ...
        if (lastAccrued != 0) {
            ...
            uint256 liquidity = pos.seeds_;//@audit-ok M can pos.seeds_ change midway. if it can then manipulate reward accrue
            uint32 time = lastAccrued;
            while (time < block.timestamp) {
                uint32 currWeek = uint32((time / WEEK) * WEEK);
                uint32 nextWeek = uint32(((time + WEEK) / WEEK) * WEEK);//@gas
                uint32 dt = uint32(
                    nextWeek < block.timestamp
                        ? nextWeek - time//@audit-ok H time==lastAccrued is not clamp to week. user lose some reward if first accrue is the end of the week
                        : block.timestamp - time
                );
                timeWeightedWeeklyPositionAmbLiquidity_[poolIdx][posKey][
                    currWeek
                ] += dt * liquidity;
                time += dt;//@if (nextweek >= block.timestamp) break;
            }//@1st loop give reward from lasttime to the end of the week.
        }//@2 time skip to next week. give reward of current week then loop to next week. 
        timeWeightedWeeklyPositionAmbLiquidityLastSet_[poolIdx][
            posKey
        ] = uint32(block.timestamp);//@3 give final reward of current timestamp to beginning of the week
    }

    function claimAmbientRewards(//@user operation
        address owner,//msg.sender through delegatecall to Dex
        bytes32 poolIdx,//user
        uint32[] memory weeksToClaim//user
    ) internal {
        CurveMath.CurveState memory curve = curves_[poolIdx];
        ...
        for (uint256 i; i < weeksToClaim.length; ++i) {
            ...
            if (overallTimeWeightedLiquidity > 0) {
                uint256 rewardsForWeek = (timeWeightedWeeklyPositionAmbLiquidity_[
                    ...
                    overallTimeWeightedLiquidity;//@audit M user can exploit timeweighted weekly to very small value to get more reward
        ...
        if (rewardsToSend > 0) {
            (bool sent, ) = owner.call{value: rewardsToSend}("");
            require(sent, "Sending rewards failed");
        }//@audit-ok reward to send external call before relayer
    }
}
