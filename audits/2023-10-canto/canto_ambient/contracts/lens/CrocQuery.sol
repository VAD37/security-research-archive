...
contract CrocQuery {//@note croc lens like query and impact is not important. only for offchain view.
    ...
    function queryConcRewards (address owner, address base, address quote, uint256 poolIdx,
        ...
        unchecked {
            uint72 odometer = uint72(type(uint64).max) + uint72(feeUpper) - uint72(feeLower);//@audit change fee uptick from utin64 to uint74 to prevent float fixedpoint value error

            if (odometer < feeStart) {
                return (0, 0, 0);
            }

            uint64 accumFees = uint64(odometer - feeStart);//@ do uint72 - uint64 change float point when convert back? uncheck value too. this look quite dangerous.
