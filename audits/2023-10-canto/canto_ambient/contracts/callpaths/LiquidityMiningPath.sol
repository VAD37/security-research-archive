...
contract LiquidityMiningPath is LiquidityMining {
    ...
    function protocolCmd(bytes calldata cmd) public virtual {//@audit-ok protocolCMD missing payable when command say it need payable
    ...
    function userCmd(bytes calldata input) public payable {
        (uint8 code, bytes32 poolHash, int24 lowerTick, int24 upperTick, uint32[] memory weeksToClaim) =
            abi.decode(input, (uint8, bytes32, int24, int24, uint32[]));//@note dynamic input decoding detected.
