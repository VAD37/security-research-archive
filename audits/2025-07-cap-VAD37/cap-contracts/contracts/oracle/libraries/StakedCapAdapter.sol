...
library StakedCapAdapter {
    ...
    function price(address _asset) external view returns (uint256 latestAnswer, uint256 lastUpdated) {
        address capToken = IERC4626(_asset).asset();// asset is staked cap token
        (latestAnswer, lastUpdated) = IOracle(msg.sender).getPrice(capToken);//@call CapTokenAdapter.price()
