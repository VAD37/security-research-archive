...
abstract contract RateOracle is IRateOracle, Access, RateOracleStorageUtils {
    ...
    {//@only 0: USDC 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48. adapter AAVEAdapter. paylaod 0x50e3004200000000000000000000000087870bca3f3fd6335c3f4ce8392d69350b4fa4e2000000000000000000000000a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48
        ...
        checkAccess(this.setUtilizationOracleData.selector)
    {
        ...
        emit SetUtilizationOracleData(_asset, _oracleData);//@no utilization oracle??
    }
    ...
    function setRestakerRate(address _agent, uint256 _rate) external checkAccess(this.setRestakerRate.selector) {
        getRateOracleStorage().restakerRate[_agent] = _rate;// 0.05e27 is 5% per year
    ...
    /// @inheritdoc IRateOracle
    function utilizationRate(address _asset) external returns (uint256 rate) {//@used by debt token
    ...
    /// @inheritdoc IRateOracle
    function restakerRate(address _agent) external view returns (uint256 rate) { //@viewLogic.accruedRestakerInterest()
    ...
    function _getRate(address _adapter, bytes memory _payload) private returns (uint256 rate) {
        (bool success, bytes memory returnedData) = _adapter.call(_payload);//aave adapter 0x50e3004200000000000000000000000087870bca3f3fd6335c3f4ce8392d69350b4fa4e2000000000000000000000000a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48
        if (success) rate = abi.decode(returnedData, (uint256));
    }//@rate: 18.446744072323900000 in ray e27
}//accrued index += rate * delta time / year ?? Questionable
