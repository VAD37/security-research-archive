...
contract ConfigureAccessControl {
    function _initInfraAccessControl(InfraConfig memory infra, UsersConfig memory users) internal {
        ...
        accessControl.grantAccess(Lender.borrow.selector, infra.lender, users.lender_admin);//@audit M it seem like project forget to give only Delegations monopoly on borrow with lending
