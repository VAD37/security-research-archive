    ...
    SymbioticNetworkMiddlewareStorageUtils
{
    ...
    function initialize(
        address _accessControl,//@AccessControl.sol 
        address _network,//proxy of SymbioticNetwork.sol 
        address _vaultRegistry,//fork middleware network registry
        address _oracle,
    ...
    function distributeRewards(address _agent, address _token) external checkAccess(this.distributeRewards.selector) {
        ...
        IERC20(_token).forceApprove(address(IStakerRewards(stakerRewarder)), _amount);//@audit M StakerRewards fail when amount == 0
    ...
    function _verifyVault(address _vault) internal view {
        ...
        if (!IRegistry($.vaultRegistry).isEntity(_vault)) {//@there are 194 vaults on mainnet, this check if vault is init by Symbioltic VaultFactory 3rd party
            revert NotVault();
        }

        if (!IVault(_vault).isInitialized()) revert VaultNotInitialized();//@vault from 0xdd649adab2e67cadc2ec29d75abe73f3df08065c
        ...
        address slasher = IVault(_vault).slasher();//@can be empty . 0x3221b31f169Ac6891c79f4Fc352911F6605C427a aka contract
        uint64 slasherType = IEntity(slasher).TYPE();
        if (slasher == address(0)) revert NoSlasher();
        if (slasherType != uint64(ISymbioticNetworkMiddleware.SlasherType.INSTANT)) revert InvalidSlasher();//@no clue why it must instant slash? it is default slasher type
        ...
        if (delegatorType != uint64(ISymbioticNetworkMiddleware.DelegatorType.NETWORK_RESTAKE)) {//allow restaking across multiple networks but restrict operators from being restaked within the same network
