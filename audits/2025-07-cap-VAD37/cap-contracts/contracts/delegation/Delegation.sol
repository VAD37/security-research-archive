...
contract Delegation is IDelegation, UUPSUpgradeable, Access, DelegationStorageUtils {
    ...
    /// @inheritdoc IDelegation
    function distributeRewards(address _agent, address _asset) external {//@called from BorrowLogic. also money received from BorrowLogic Vault too
        DelegationStorage storage $ = getDelegationStorage();
        ...
        checkAccess(this.addAgent.selector)
    {
        ...
        $.agentData[_agent].ltv = _ltv;//5e26? = 0.5e27 = 50%
        $.agentData[_agent].liquidationThreshold = _liquidationThreshold;//0.7e27 = 70%
