...
library ViewLogic {
    ...
    {
        ILender.ReserveData storage reserve = $.reservesData[_asset];
        uint256 totalDebt = IERC20(reserve.debtToken).balanceOf(_agent);//@this is inaccurate, should use refreshed index
        uint256 rate = IOracle($.oracle).restakerRate(_agent);// 0.05e27 is 5% per year
