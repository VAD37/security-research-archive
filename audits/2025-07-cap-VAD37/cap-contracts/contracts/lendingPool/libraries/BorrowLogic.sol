...
library BorrowLogic {
        ...
        returns (uint256 borrowed)
    {
        ...
        IDelegation($.delegation).setLastBorrow(params.agent);//@just update timestamp. this update slashable time
        ...
        returns (uint256 repaid)
    {
        ...
        if (remaining > reserve.unrealizedInterest[params.agent]) {
            console.log("repay more than unrealized interest %e, %e", reserve.unrealizedInterest[params.agent] , remaining);
            restakerRepaid = reserve.unrealizedInterest[params.agent];
            remaining -= restakerRepaid;
        } else {
            console.log("repay less than unrealized interest %e, %e", reserve.unrealizedInterest[params.agent] , remaining);
        ...
        if (restakerRepaid > 0) {
            ...
            IDelegation($.delegation).distributeRewards(params.agent, params.asset);//@this will revert? it send some cash over
    ...
    function realizeRestakerInterest(ILender.LenderStorage storage $, address _agent, address _asset)//@repay/borrow or Lender directly
        public
        returns (uint256 realizedInterest)
    {
        ILender.ReserveData storage reserve = $.reservesData[_asset];
        uint256 unrealizedInterest;
        (realizedInterest, unrealizedInterest) = maxRestakerRealization($, _agent, _asset);//@audit-ok it still use latest timestamp M unrealized interest does not use latest Debt index
        ...
        console.log("- realizeRestakerInterest %e, %e", realizedInterest, unrealizedInterest);
        IDebtToken(reserve.debtToken).mint(_agent, realizedInterest + unrealizedInterest);//@audit-ok M not true. this ignore zero minting cannot realizeRestakerInterest on same block due to minting 0 amount
    ...
    {
        ILender.ReserveData storage reserve = $.reservesData[_asset];
        uint256 accruedInterest = ViewLogic.accruedRestakerInterest($, _agent, _asset);//@total debt of this agent
        uint256 reserves = IVault(reserve.vault).availableBalance(_asset);
