...
//@note debtToken is fixed erc20. just a normal tracking contract for debt per user.
    ...
    }//@audit M balance and supply do not use latest index.
    ...
    function _updateIndex() internal {
        ...
        console.log("DebtToken: Updated interest rate to %e", $.interestRate);
    }
    ...
    function _nextInterestRate() internal returns (uint256 rate) {
        DebtTokenStorage storage $ = getDebtTokenStorage();
        address _oracle = $.oracle;//@audit H interest depend on aave pool directly, this can instantly shooting up for flashing. this use previous interest rate between update.
        uint256 marketRate = IOracle(_oracle).marketRate($.asset);//aave pool.getReserveData().variableBorrowRate?,  variableBorrowIndex:1.181485246827007965855609814, borrowRate: 0.049216574274132035332852702
        uint256 benchmarkRate = IOracle(_oracle).benchmarkRate($.asset);//15%
        uint256 utilizationRate = IOracle(_oracle).utilizationRate($.asset);//@Vault.utilization(_asset). max 100%. if single asset not whole pool

        rate = marketRate > benchmarkRate ? marketRate : benchmarkRate;
        rate += utilizationRate;//@audit verify interest rate calculation with aave code
    }
