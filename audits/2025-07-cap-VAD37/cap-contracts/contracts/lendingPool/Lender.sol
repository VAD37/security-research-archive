...
contract Lender is ILender, UUPSUpgradeable, Access, LenderStorageUtils {
    ...
    function initialize(
        ...
        uint256 _grace,//1 hours
        uint256 _expiry,// 1 days
        uint256 _bonusCap,// 0.1e27
        uint256 _emergencyLiquidationThreshold //0.9e27
    ...
    function borrow(address _asset, uint256 _amount, address _receiver) external returns (uint256 borrowed) {
        borrowed = BorrowLogic.borrow(//@only whitelist agent have LTV can borrow.
            getLenderStorage(),
            BorrowParams({
                agent: msg.sender,//@this have access check
                asset: _asset,
                amount: _amount,
                receiver: _receiver,
    ...
    function repay(address _asset, uint256 _amount, address _agent) external returns (uint256 repaid) {
        ...
        repaid = BorrowLogic.repay(//@this does not. this allow anyone to refresh oracle rate and pay agent debt.
    ...
    /// @inheritdoc ILender
    function addAsset(AddAssetParams calldata _params) external checkAccess(this.addAsset.selector) {//@called from deployVault
        LenderStorage storage $ = getLenderStorage();//each asset have their own FeeReceiver, Auction
