...
library VaultLogic {
        ...
        updateIndex($, params.asset)
    {
        ...
        $.totalSupplies[params.asset] += params.amountIn; //@MinterLogic depend on totalSupplies to calculate rates
    ...
    }//@audit L paused only work on mint/borrow. what happen with user repay or redeem/burn
    ...
    function _updateIndex(IVault.VaultStorage storage $, address _asset) internal {
        ...
        $.utilizationIndex[_asset] += utilization($, _asset) * (block.timestamp - $.lastUpdate[_asset]);//@audit M utilization update once, does not refresh on sameblock?
