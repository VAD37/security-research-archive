...
contract LenderBorrowTest is TestDeployer {
    ...
    function test_lender_borrow_and_repay() public {
        ...
        lender.repay(address(usdc), 500e6, user_agent);
        lender.repay(address(usdc), 500e6, user_agent);
    ...
    function test_realize_restaker_interest_debug() public {
        vm.startPrank(user_agent);

        lender.borrow(address(usdc), lender.maxBorrowable(address(user_agent), address(usdc)), user_agent);
        

        _timeTravel(100 days);
