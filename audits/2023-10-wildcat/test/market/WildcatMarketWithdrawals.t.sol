...
contract WithdrawalsTest is BaseMarketTest {
  ...
  function test_queueWithdrawal_AuthorizedWithdrawOnly_Double() public asAccount(bob) {
    _deposit(bob, 1e18);
    startPrank(address(controller));
    market.updateAccountAuthorization(bob, false);
    stopPrank();
    _requestWithdrawal(bob, 0.1e18);
    vm.warp(block.timestamp + 87600);
    market.updateState();
    _requestWithdrawal(bob, 0.1e18);
  }
