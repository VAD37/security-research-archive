...
contract WildcatArchController is Ownable {
  ...
  function registerControllerFactory(address factory) external onlyOwner {
    if (!_controllerFactories.add(factory)) {
      revert ControllerFactoryAlreadyExists();//@audit-ok L lack check if factory is correct contract and have wildcat market as controller
    }
  ...
  function registerMarket(address market) external onlyController {//@audit-ok R market whitelist allow nuke and create escrow for lender. Is this quite weak power? review whitepaper
  ...
  function removeMarket(address market) external onlyOwner {//@audit-ok L owner cannot reverse removed markets?
