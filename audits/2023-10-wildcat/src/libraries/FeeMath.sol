...
library FeeMath {
  ...
  function calculateLinearInterestFromBips(
    uint256 rateBip,//@ user input max 1e4 . annualInterestBips|| delinquencyFeeBips
    uint256 timeDelta//timestamp - state.lastInterestAccruedTimestamp || timestamp - expiry
  ) internal pure returns (uint256 result) {
    uint256 rate = rateBip.bipToRay();// 1e4 => 1e27.
  ...
  function updateScaleFactorAndFees(
    MarketState memory state,
    uint256 protocolFeeBips,//1000
    uint256 delinquencyFeeBips,//1000
    uint256 delinquencyGracePeriod,//2000
    uint256 timestamp// state.pendingWithdrawalExpiry || block.timestamp
  )
    internal
    pure
    returns (uint256 baseInterestRay, uint256 delinquencyFeeRay, uint256 protocolFee)
  {
    ...
    state.lastInterestAccruedTimestamp = uint32(timestamp);//@ this is not latest time block? duplicate call too
  }
}
