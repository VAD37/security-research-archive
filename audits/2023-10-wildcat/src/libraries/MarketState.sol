...
using MarketStateLib for Account global;//@audit what the heck global library?
...
library MarketStateLib {
  ...
  function hasPendingExpiredBatch(MarketState memory state) internal view returns (bool result) {
    ...
    assembly {
      ...
      result := gt(timestamp(), sub(expiry, 1))//@intentional overflow?
    }//@audit what happen when pending expire time == blockstamp. user can set expire time to zero? Actually there is no set pending expiry over zero
  }
