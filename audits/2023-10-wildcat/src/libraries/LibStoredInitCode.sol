...
library LibStoredInitCode {
  ...
  function deployInitCode(bytes memory data) internal returns (address initCodeStorage) {
    assembly {
      let size := mload(data)//@ mload load the length of data. first 32 byte of data bytes
      let createSize := add(size, 0x0b)
      // Prefix Code
      //
      // Has trailing STOP instruction so the deployed data
      // can not be executed as a smart contract.  //@audit how deployinit code cannot work as smart contract? Why it cannot be executed?
      //
      // Instruction                | Stack
      // ----------------------------------------------------
      // PUSH2 size                 | size                  |
  ...
  function calculateCreate2Address(//@audit library dirty bits not cleaned for pure function.
