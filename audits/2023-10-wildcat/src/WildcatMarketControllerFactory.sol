...
contract WildcatMarketControllerFactory {
  ...
  uint256 public immutable ownCreate2Prefix = LibStoredInitCode.getCreate2Prefix(address(this));
  ...
  constructor(
    ...
    MinimumDelinquencyGracePeriod = constraints.minimumDelinquencyGracePeriod;// @ 0
    MaximumDelinquencyGracePeriod = constraints.maximumDelinquencyGracePeriod;// @ 86400
    MinimumReserveRatioBips = constraints.minimumReserveRatioBips;// @ 1000
    MaximumReserveRatioBips = constraints.maximumReserveRatioBips;// @ 10000
    MinimumDelinquencyFeeBips = constraints.minimumDelinquencyFeeBips;// @ 1000
    MaximumDelinquencyFeeBips = constraints.maximumDelinquencyFeeBips;// @ 10000
    MinimumWithdrawalBatchDuration = constraints.minimumWithdrawalBatchDuration; // @ 0
    MaximumWithdrawalBatchDuration = constraints.maximumWithdrawalBatchDuration; // @ 365 days
    MinimumAnnualInterestBips = constraints.minimumAnnualInterestBips; // @ 0
    MaximumAnnualInterestBips = constraints.maximumAnnualInterestBips; // @ 10000
    ...
    returns (address initCodeStorage, uint256 initCodeHash)
  {
    ...
    initCodeStorage = LibStoredInitCode.deployInitCode(controllerInitCode);//@audit does LibStoredInitCode cause dirtybits?
    // WildcatMarketController(initCodeStorage).test_initialize();//@test init instead of constructor
  }
  ...
  function setProtocolFeeConfiguration(
    address feeRecipient,
    address originationFeeAsset,
    uint80 originationFeeAmount,//@audit-ok 1e4 .M no hardcap on protocol fee
    uint16 protocolFeeBips
  ...
  function deployController() public returns (address controller) {
    ...
    controller = LibStoredInitCode.calculateCreate2Address(//@audit library view method possible assembly code poison.
      ownCreate2Prefix,
      salt,
