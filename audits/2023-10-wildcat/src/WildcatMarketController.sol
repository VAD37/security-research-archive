...
contract WildcatMarketController is IWildcatMarketControllerEventsAndErrors {//@note borrower is whitelist account. What happen when blacklisted again?
  ...
  address public immutable borrower;//@audit-ok borrower is fixed address everrytime create new market.
  ...
  constructor() {
    controllerFactory = IWildcatMarketControllerFactory(msg.sender);//@there was 5 immutable value different when self deploy vs clone
    MarketControllerParameters memory parameters = controllerFactory.getMarketControllerParameters();//@audit-ok constructor parameters is empty. so clone new contract also empty too
  ...
  function authorizeLenders(address[] memory lenders) external onlyBorrower {
    for (uint256 i = 0; i < lenders.length; i++) {
      address lender = lenders[i];//@audit-ok  .why borrower must whirelist lender? It suppose to be other way around
      if (_authorizedLenders.add(lender)) {//@note lenders authorized by borrower default have deposit and withdraw role
  ...
  function deauthorizeLenders(address[] memory lenders) external onlyBorrower {
    for (uint256 i = 0; i < lenders.length; i++) {
      ...
      }//@audit-ok why not update lender authorization during add/remove?
      // get  _controlledMarkets.values() and updating all
   ...
   *      status.
   */
  function updateLenderAuthorization(address lender, address[] memory markets) external {//@note borrower have power to remove lender from market. what happen to lender afterward though?
    for (uint256 i; i < markets.length; i++) {
      address market = markets[i];
      if (!_controlledMarkets.contains(market)) {
        revert NotControlledMarket();
      }
      WildcatMarket(market).updateAccountAuthorization(lender, _authorizedLenders.contains(lender));//@audit-ok revert _getaccount() when blocked .lender can use this to override blocked status
    }
  }
  ...
  function getMarketParameters() external view returns (MarketParameters memory parameters) {
    parameters.asset = _tmpMarketParameters.asset; //@user
    parameters.namePrefix = _tmpMarketParameters.namePrefix;//@user
    parameters.symbolPrefix = _tmpMarketParameters.symbolPrefix;//@user
    parameters.borrower = borrower;//immutable
    parameters.controller = address(this);//immutable
    parameters.feeRecipient = _tmpMarketParameters.feeRecipient;//@factory config
    parameters.sentinel = sentinel;//immutable
    parameters.maxTotalSupply = _tmpMarketParameters.maxTotalSupply;//user
    parameters.protocolFeeBips = _tmpMarketParameters.protocolFeeBips;//factory config
    parameters.annualInterestBips = _tmpMarketParameters.annualInterestBips;//user
    parameters.delinquencyFeeBips = _tmpMarketParameters.delinquencyFeeBips;//user
    parameters.withdrawalBatchDuration = _tmpMarketParameters.withdrawalBatchDuration;//user
    parameters.reserveRatioBips = _tmpMarketParameters.reserveRatioBips;//user
    parameters.delinquencyGracePeriod = _tmpMarketParameters.delinquencyGracePeriod;//user
  }
  ...
  function deployMarket(
    address asset,//@note multiple market  share same asset per borrower is ok
    string memory namePrefix,
    string memory symbolPrefix,
    uint128 maxTotalSupply,
    uint16 annualInterestBips,
    ...
    (
      ...
      parameters.protocolFeeBips//1000
  ...
  /**
   * @dev Returns      constraints on market parameters that
  ...
  function setAnnualInterestBips(
    address market,
    uint16 annualInterestBips//@ hidden limit max 1e4
  ) external virtual onlyBorrower onlyControlledMarket(market) {//@audit-ok borrower can change interest rate?? at anytime?
    // If borrower is reducing the interest rate, increase the reserve
    // ratio for the next two weeks.
    if (annualInterestBips < WildcatMarket(market).annualInterestBips()) {
      TemporaryReserveRatio storage tmp = temporaryExcessReserveRatio[market];

      if (tmp.expiry == 0) {//@audit-ok M is it possible for state reserveRatioBips change during locked expiry duration?
        tmp.reserveRatioBips = uint128(WildcatMarket(market).reserveRatioBips());//@audit-ok temporary value set but no reset? Who is reading this? It can change between market operation

        // Require 90% liquidity coverage for the next 2 weeks
        WildcatMarket(market).setReserveRatioBips(9000);//@hidden limit to 1e4
      }
