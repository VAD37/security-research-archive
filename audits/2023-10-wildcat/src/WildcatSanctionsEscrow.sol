...
contract WildcatSanctionsEscrow is IWildcatSanctionsEscrow {
  ...
  function releaseEscrow() public override {
    ...
    uint256 amount = balance();//@audit-ok if market token view vs updated state does not sync then escrow does not release all token. which is a huge issue.

    IERC20(asset).transfer(account, amount);//@audit-ok missing safe transfer
