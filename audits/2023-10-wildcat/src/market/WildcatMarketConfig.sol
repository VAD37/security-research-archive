...
contract WildcatMarketConfig is WildcatMarketBase {
  ...
  function nukeFromOrbit(address accountAddress) external nonReentrant {
    ...
    MarketState memory state = _getUpdatedState();//@audit-ok can sanctions any random address already sanctioned by chainalysis? This manually update new state
    _blockAccount(state, accountAddress);//@audit-ok borrower indeed can favor another lenders over another lenders
    _writeState(state);
  }
  ...
  function stunningReversal(address accountAddress) external nonReentrant {
    ...
    _accounts[accountAddress] = account;//@audit-ok unreverse blocked account does not reverse escrow locked token.
  }
  ...
  function updateAccountAuthorization(
    ...
    if (_isAuthorized) {
      ...
      account.approval = AuthRole.WithdrawOnly;//@audit-ok anyone can be forced change role to withdrawonly which is non-null
    }
  ...
  function setAnnualInterestBips(uint16 _annualInterestBips) public onlyController nonReentrant {
    MarketState memory state = _getUpdatedState();//@audit which function fail to write state after call updateState()?
  ...
  function setReserveRatioBips(uint16 _reserveRatioBips) public onlyController nonReentrant {
    ...
    if (_reserveRatioBips < initialReserveRatioBips) {
      if (state.liquidityRequired() > totalAssets()) {//@audit R isnt this suppose to be reversed if?
