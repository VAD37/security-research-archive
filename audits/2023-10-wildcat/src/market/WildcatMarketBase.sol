...
contract WildcatMarketBase is ReentrancyGuard, IMarketEventsAndErrors {
  ...
  constructor() {
    ...
    if (parameters.annualInterestBips > BIP) {//0 -> 1e4 //user setting
      revert InterestRateTooHigh();
    }
    if (parameters.reserveRatioBips > BIP) {//1e3 -> 1e4 //user
      revert ReserveRatioBipsTooHigh();
    }
    if (parameters.protocolFeeBips > BIP) {// max 1e4. constant is 1e3 //factory config
      revert InterestFeeTooHigh();
    }
    if (parameters.delinquencyFeeBips > BIP) {//1e3 ->1e4 //user
      revert PenaltyFeeTooHigh();
    }

    // Set asset metadata
    asset = parameters.asset;//@user metadata unique
    ...
    _state = MarketState({
      isClosed: false,
      maxTotalSupply: parameters.maxTotalSupply,//user
      ...
      scaleFactor: uint112(RAY),//@1e27
      lastInterestAccruedTimestamp: uint32(block.timestamp)//@
    });

    sentinel = parameters.sentinel;//@immutable
    borrower = parameters.borrower;//@immutable
    controller = parameters.controller;// market controller @immutable
    feeRecipient = parameters.feeRecipient;//factory
    protocolFeeBips = parameters.protocolFeeBips;//factory
    delinquencyFeeBips = parameters.delinquencyFeeBips;//@user
    delinquencyGracePeriod = parameters.delinquencyGracePeriod;//@user max 86_400 seconds
    withdrawalBatchDuration = parameters.withdrawalBatchDuration;//@user
  }
  ...
  function _getAccount(address accountAddress) internal view returns (Account memory account) {
    ...
    if (account.approval == AuthRole.Blocked) {//@audit-ok internal view _getAccount is really weird. null is acceptable but revert on blocked?
   ...
   */
  function _blockAccount(MarketState memory state, address accountAddress) internal {//@ state already accrued
    ...
    if (account.approval != AuthRole.Blocked) {
      ...
      if (scaledBalance > 0) {
        ...
        address escrow = IWildcatSanctionsSentinel(sentinel).createEscrow(//@note can blocklist twice. reuse escrow again
          borrower,
          accountAddress,
          address(this)
        );
        emit Transfer(accountAddress, escrow, state.normalizeAmount(scaledBalance));
        _accounts[escrow].scaledBalance += scaledBalance;//@audit-ok escrow roll is null. what happen when borrower update escrow role?
   ...
   */
  function _getAccountWithRole(//@note badcode. _getAccountWithRole() also act as require check if account have enough permission.
    ...
    if (account.approval == AuthRole.Null) {
      if (IWildcatMarketController(controller).isAuthorizedLender(accountAddress)) {
        account.approval = AuthRole.DepositAndWithdraw;//@audit lender null account default to full access without overriding .can be approved if is lender
  ...
  function _getUpdatedState() internal returns (MarketState memory state) {
    ...
    if (state.hasPendingExpiredBatch()) {//@block.timestamp > pendingWithdrawalExpiry
      uint256 expiry = state.pendingWithdrawalExpiry;
      // Only accrue interest if time has passed since last update.
      // This will only be false if withdrawalBatchDuration is 0.
      if (expiry != state.lastInterestAccruedTimestamp) {//@audit R this revert when expiry < lastInterestAccruedTimestamp. this can happen when request withdrawal
        console.log("state debug update expiry  %s , last: %s", expiry, state.lastInterestAccruedTimestamp);
        (uint256 baseInterestRay, uint256 delinquencyFeeRay, uint256 protocolFee) = state
          .updateScaleFactorAndFees(
            protocolFeeBips,
            delinquencyFeeBips,
            delinquencyGracePeriod,
            expiry//@audit R lastInterestAccruedTimestamp replaced by expiry date here? then replace again down below
          );
    ...
    if (block.timestamp != state.lastInterestAccruedTimestamp) {      
      ...
      console.log("state debug update lastTimestamp  %s , last: %s", state.pendingWithdrawalExpiry, state.lastInterestAccruedTimestamp);
      emit ScaleFactorUpdated(state.scaleFactor, baseInterestRay, delinquencyFeeRay, protocolFee);
    }
  }
  ...
  function _calculateCurrentState()
    ...
    view
    returns (
      MarketState memory state,//local view
      uint32 expiredBatchExpiry,//@audit unused?
      WithdrawalBatch memory expiredBatch//unused?
    )
  {
    state = _state;//@copy to memory
    // Handle expired withdrawal batch
    if (state.hasPendingExpiredBatch()) {//@assembly library call. overflow?
  ...
  function _processExpiredWithdrawalBatch(MarketState memory state) internal {
    ...
    console.log("reset pendingWithdrawalExpiry");
    state.pendingWithdrawalExpiry = 0;
