...
contract WildcatMarketWithdrawals is WildcatMarketBase {
  ...
  function queueWithdrawal(uint256 amount) external nonReentrant {
    ...
    if (state.pendingWithdrawalExpiry == 0) {
      console.log("updating pending withdrawal from: %s to: %s ",state.pendingWithdrawalExpiry, uint32(block.timestamp + withdrawalBatchDuration));
    ...
    // Update stored state
    _writeState(state);
    console.log("state debug withdrawal: ",state.pendingWithdrawalExpiry, state.lastInterestAccruedTimestamp);
  }
