...
contract VestingEscrow is IVestingEscrow, Clone {
    ...
    function initialize(bool _isFullyRevokable, bytes calldata _initialDelegateParams) external {
        ...
        uint256 _totalLocked = totalLocked();//1 eth
        uint40 _endTime = endTime();//block.timestamp+1 year
        IERC20 _token = token();//        
        //@audit Need to check endTime > block.timestamp. there is no check for time validity. vesting can be broken.
    ...
    function locked() public view returns (uint256) {
        ...
        //@audit-ok never math.min do _totalVestedAt ever bigger than totalLocked?
    ...
    function _totalVestedAt(uint256 time) internal pure returns (uint256) { //@invariant time <= endtime

        uint40 _startTime = startTime();//@startTime can be in the super past
        uint256 _totalLocked = totalLocked();//@amount locekd 1 eth
        if (time < _startTime + cliffLength()) {//@cliff 90 days <= vesting duration 1 year
            return 0;//@time  ==block.timestamp or last claim time or endtime
        }//@note reward still accrued during cliff time
        return Math.min(_totalLocked * (time - _startTime) / (endTime() - _startTime), _totalLocked);
    }//@audit does not support token with 2 decimals or decimals smaller than block.timestamp
}
