...
contract Size is ISize, SizeView, Initializable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    }//@note onBehalf open up new multicall attack. One account can open/close multiple loan at the same times. If there is any bypass during multicall this would also work quite well.
    ...
    function compensateOnBehalfOf(CompensateOnBehalfOfParams memory externalParams)
        ...
        override(ISizeV1_7)
        whenNotPaused
        shouldNotEndUpUnderwater(externalParams.onBehalfOf)//@audit RH can bypass underwater check if user both underwater before and after. multicall onbehalf possible attack
    {
    ...
    }//@copy order is nothing more than oracle. allow user to use someone APR rate depend on market without constantly update their own limit order
}
