...
library AccountingLibrary {
    ...
    function createDebtAndCreditPositions(
        State storage state,
        address lender,//recipient any input
        address borrower,//any input borrower. credit must valid though
        uint256 futureValue,
        uint256 dueDate
        ...
        state.data.debtToken.mint(borrower, futureValue);//@audit R possible liquidation attack, assume borrower can be any address
    }
    ...
    function createCreditPosition(
        ...
        if (exitCreditPosition.credit == credit) {//@credit position same amount as input by user
            exitCreditPosition.lender = lender;//@change new lender
            exitCreditPosition.forSale = forSale;//true, always sale
            ...
            uint256 creditPositionId = state.data.nextCreditPositionId++;
            state.data.creditPositions[creditPositionId] = creditPosition;
            state.validateMinimumCreditOpening(creditPosition.credit);//@note it is possible to have multiple credit positions linking to single debt
