...
library OfferLibrary {
        ...
        returns (uint256)
    {
        ...
        if (isNull(limitOrder)) {//@limitOrder is pointer to somebody else order, copy is original copy when user call
            revert Errors.INVALID_OFFER(user);//@copy can also seen as acceptable limit by user to prevent shenanigans
        }
        ...
        returns (LimitOrder memory limitOrder, CopyLimitOrder memory copyLimitOrder)
    {
        ...
        } else {//@copy/mirror offer from somebody, other account
