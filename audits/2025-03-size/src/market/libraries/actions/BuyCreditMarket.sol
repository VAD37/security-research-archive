...
library BuyCreditMarket {
        ...
        returns (SwapDataBuyCreditMarket memory swapData)
    {
        ...
        } else {//@split credit position
            DebtPosition storage debtPosition = state.getDebtPositionByCreditPositionId(params.creditPositionId);
            swapData.creditPosition = state.getCreditPosition(params.creditPositionId);

            swapData.borrower = swapData.creditPosition.lender;//@replace borrower to previous lender.
        ...
        returns (uint256 netCashAmountIn)
    {
        BuyCreditMarketParams memory params = externalParams.params;
        address onBehalfOf = externalParams.onBehalfOf;
        address recipient = externalParams.recipient;//@note the only shenanigan with onBehalf is lender,borrower can be the same person. It still allowed to happen before. 
        ...
        if (params.creditPositionId == RESERVED_ID) {
            ...
            state.createDebtAndCreditPositions({ //@debt position created link to borrower
                lender: recipient,//@ credit position created link to recipient and debt position above
                borrower: swapData.borrower,
                futureValue: swapData.creditAmountOut,
                dueDate: block.timestamp + swapData.tenor
            });//@borrower gain debt token equal to total amount of whole loan duration
        } else {
            state.createCreditPosition({
                ...
                forSale: true
            });//@audit if credit position is for sale. It is expected split credit position will give some credit cash to original lender? Currently there is no check whose credit position is this
        }
        //@ cash send to previous lender if split. swapData.borrower is replaced above. Confusing yeah
    ...
    }//@check borrower collateralratio right later execution. But only for RESERVED_ID or createDebtAndCreditPositions(). Fragment credit position does not check for ratio
}
