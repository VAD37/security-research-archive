...
library Compensate {
    ...
    function validateCompensate(State storage state, CompensateOnBehalfOfParams memory externalParams) external view {
        ...
        }//@note msg.sender != borrower. Before direct call, now delegator can force to repay
    ...
    function executeCompensate(State storage state, CompensateOnBehalfOfParams memory externalParams) external {
        ...
        if (params.creditPositionToCompensateId == RESERVED_ID) {
            creditPositionToCompensate = state.createDebtAndCreditPositions({
                lender: onBehalfOf,
                borrower: onBehalfOf,//@audit R compensate still look like angle to exploit minting debt to someone account to force liquidation for both?
        ...
        );//@burn debt from borrower, this increase CR. remove some credit from previous lender credit position
