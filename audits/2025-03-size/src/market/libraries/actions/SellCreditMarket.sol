...
library SellCreditMarket {
        ...
        returns (uint256)
    {
        SellCreditMarketParams memory params = externalParams.params;
        address onBehalfOf = externalParams.onBehalfOf;
        address recipient = externalParams.recipient;//@audit R now we can send certain token accounting to any account. Would this open up liquidation attack?
        ...
        }//@debt to onBehalfOf, new credit position to onBehalfOf
        ...
        state.createCreditPosition({//@split credit position to lender or replace owner of  newly created position to lender?
        ...
        emit Events.SwapData(
            ...
            onBehalfOf,//@only check Collateral ratio of onBehalfOf user
            params.lender,
