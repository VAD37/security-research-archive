...
enum Action {
    DEPOSIT,//@audit action Deposit is null enum. So anyone can deposit onbehalf of anyone
    WITHDRAW,
    BUY_CREDIT_LIMIT,
    SELL_CREDIT_LIMIT,
    BUY_CREDIT_MARKET,
...
library Authorization {
    ...
    function isValid(ActionsBitmap actionsBitmap) internal pure returns (bool) {
        ...
        return toUint256(actionsBitmap) <= maxValidBitmap;//@ok
    }
    ...
    function getActionsBitmap(Action[] memory actions) internal pure returns (ActionsBitmap) {
        ...
        return toActionsBitmap(actionsBitmapUint256);//@ok only used by external view
    }
}
