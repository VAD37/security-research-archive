    ...
    UUPSUpgradeable
{
    ...
    function setAuthorization(address operator, ActionsBitmap actionsBitmap) external override(ISizeFactoryV1_7) {
        ...
        if (!Authorization.isValid(actionsBitmap)) {//@only check bitmap < maxActionsBitmap
        ...
        authorizations[nonce][operator][msg.sender] = actionsBitmap;//@audit-ok keccak(address,keccak(address)) is impossible for collision exploit. R can this allow storage collision exploit?
    }
