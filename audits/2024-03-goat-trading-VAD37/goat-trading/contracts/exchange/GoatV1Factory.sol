...
contract GoatV1Factory {
    ...
    //@note createPair just create new contract and init with user params.
    function createPair(address token, GoatTypes.InitParams memory params) external returns (address) {
        ...
        GoatV1Pair pair = new GoatV1Pair();//@audit L new pair address is predictable
        pair.initialize(token, weth, baseName, params);
        pools[token] = address(pair);//@audit M each token can only have one pair with WETH. This seem quite inefficient.
        emit PairCreated(token, weth, address(pair));
        return address(pair);//@audit user cant query all pair have been created
    }
    ...
    function removePair(address token) external {
        ...
        }//@audit can user interact with removedPair?
        delete pools[token];//@note only pair token can remove itself from factory. This is weird
    ...
    function setFeeToTreasury(uint256 _minimumCollectibleFees) external {
        if (msg.sender != treasury) {
            revert GoatErrors.Forbidden();
        }//@audit no limit on collectable fees
