...
library GoatLibrary {
    ...
    function quote(uint256 amountA, uint256 reserveA, uint256 reserveB) internal pure returns (uint256 amountB) {
        ...
        amountB = (amountA * reserveB) / reserveA;//@ok same as uniswapV2 Quote 
    }
    ...
    //@note fee is hardcoded 99 bps
    function getTokenAmountOutAmm(uint256 amountWethIn, uint256 reserveEth, uint256 reserveToken)
        internal//@note token order is fixed. WETH is always first
        pure
        returns (uint256 amountTokenOut)
    {//@audit L getTokenOut does not verify reserve > 0 or amount > 0 like in uniswap library
        // 99 bps for fees
        uint256 actualWethIn = amountWethIn * 9901;// amount after fee

        uint256 numerator = actualWethIn * reserveToken; //@reserveToken is reserveOut
    ...
    function getTokenAmountOut(
        uint256 amountWethIn,
        uint256 virtualEth,//@user input or take over
        uint256 reserveEth,
        uint32 vestingUntil,
        uint256 bootStrapEth,//@user input or take over // Amounts of eth needed to turn pool into an amm
        uint256 reserveToken,
        uint256 virtualToken,
        uint256 reserveTokenForAmm
    ) internal pure returns (uint256 amountTokenOut) {//@ k formular amountOut = amountInWithFee * reserveOut / (reserveIn + amountInWithFee)
        ...
        vars.actualWethIn = amountWethIn * 9901;//@take fee 99bps . later in denominator need to multiple by 10000

        if (vestingUntil != type(uint32).max) { //@basic uniswapV2 logic
            // amm logic
            ...
            vars.actualWethIn = vars.actualWethIn / 10000; //@audit L small precision lost round down WETH by 10000

            if (reserveEth + vars.actualWethIn > bootStrapEth) {//@clamp what?
