...
contract BorrowerOperations is LiquityBase, AddRemoveManagers, IBorrowerOperations {
    ...
    */
    ...
    function openTrove(
        ...
        _requireValidAnnualInterestRate(_annualInterestRate);//0.5%-250%

        OpenTroveVars memory vars;

        vars.troveId = _openTrove(//@audit R BOLD token send to msg.sender. but NFT send to owner. there must be router or manager class somewhere.
            _owner,
            _ownerIndex,
            _collAmount,//write to vars.collIncrease
            _boldAmount,//write to vars.debtIncrease
            _annualInterestRate,
            address(0),
            0,
            0,
        ...
        troveManager.onOpenTrove(_owner, vars.troveId, vars.change, _annualInterestRate);//@NFT minting to owner, cache rewards index,

        sortedTroves.insert(vars.troveId, _annualInterestRate, _upperHint, _lowerHint);//@hint allow quicker search.
    ...
    function _openTrove(
        address _owner,//@all any
        uint256 _ownerIndex,
        uint256 _collAmount,
        uint256 _boldAmount,
        uint256 _annualInterestRate,//0.5-250%
        address _interestBatchManager,//0
        uint256 _batchEntireDebt,//0
        uint256 _batchManagementAnnualFee,//0
        uint256 _maxUpfrontFee,
        address _addManager,
        address _removeManager,
        address _receiver,
        TroveChange memory _change//reference
        ...
        _change.upfrontFee = _calcUpfrontFee(_change.debtIncrease, vars.avgInterestRate);//@note 7 days fee is global interestrate. Input huge debt with smaller interest also drag down fee have to pay due to new interest after new debt is decrese significantly
        _requireUserAcceptsUpfrontFee(_change.upfrontFee, _maxUpfrontFee);//@note have slippage protection for upfrontfee

        vars.entireDebt = _change.debtIncrease + _change.upfrontFee;//@new debt = borrowed amount + prepaid interest 7 days
        ...
        if (_interestBatchManager == address(0)) {
            _change.newWeightedRecordedDebt = vars.entireDebt * _annualInterestRate;//@note debt grow slightly exponentially along with interestRate. due to (7 days fee) * rate add to debt.
        } else {//@reasonable trade off. there is no attack exploit depend on printing more fee into debt
        ...
        vars.ICR = LiquityMath._computeCR(_collAmount, vars.entireDebt, vars.price);//collateral ratio = in coll * price/ new debt
        _requireICRisAboveMCR(vars.ICR);// > 110% minimum collateral ratio

        vars.newTCR = _getNewTCRFromTroveChange(_change, vars.price);//total collateral * price  / total debt
        _requireNewTCRisAboveCCR(vars.newTCR);//@audit L it is possible to bypass TCR TotalCollateralRatio check. By abuse pending interest rate swing wildly. 

        // --- Effects & interactions ---

        // Set add/remove managers
        _setAddManager(vars.troveId, _addManager);//unique ID, changed by NFT owner later
        _setRemoveManagerAndReceiver(vars.troveId, _removeManager, _receiver);// can be both address(0) //@audit L no receiver == address(0) check. can be changed later by NFT owner
        ...
        _pullCollAndSendToActivePool(vars.activePool, _collAmount);//@also tell active pool they just receive some collateral too
        //@activePool increase its own internal balance. Ignore actual collateral balance. Just trust Borrower operations
        // Mint the requested _boldAmount to the borrower and mint the gas comp to the GasPool
        vars.boldToken.mint(msg.sender, _boldAmount);//mint debt to msg.sender and not NFT owner
        WETH.transferFrom(msg.sender, gasPoolAddress, ETH_GAS_COMPENSATION);//@audit R interesting that msg.sender pay collateral, debt. But trove ownership can be created/transfer to anyone.
    ...
    */
        ...
        returns (uint256 newTCR)
    {
        ...
        uint256 totalDebt = getEntireSystemDebt(); //@note All trove ratio > 150% also include acrued interest. This use previous interest rate to calculate pending
