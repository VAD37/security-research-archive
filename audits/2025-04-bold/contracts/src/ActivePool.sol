...
contract ActivePool is IActivePool {
    ...
    */
    ...
    }//@note quite interesting way to calculate whole system pending interest. aggWeightedDebt literaly sum of all (borrowed * rate-1year). somehow create a system interest rate.
        ...
        external
    {
        ...
        if (_batchAddress != address(0)) {//@default open trove ignore this
        ...
        newAggRecordedDebt += _mintAggInterest(_troveChange.upfrontFee); //@ acrrued. adds minted agg. interest + upfront fee
        newAggRecordedDebt += _troveChange.appliedRedistBoldDebtGain;//@only batch manager and adjust trove
        newAggRecordedDebt += _troveChange.debtIncrease;//@ignore fee
        newAggRecordedDebt -= _troveChange.debtDecrease;
    ...
    function _mintAggInterest(uint256 _upfrontFee) internal returns (uint256 mintedAmount) {
        ...
        if (mintedAmount > 0) {
            uint256 spYield = SP_YIELD_SPLIT * mintedAmount / DECIMAL_PRECISION;//75%
            uint256 remainderToLPs = mintedAmount - spYield;//@audit verify whitepaper fomular and interest money flow to actual immutable source code

            boldToken.mint(address(interestRouter), remainderToLPs);//@note interest Router is fixed address (GOV). whom is receiving 25% of all interest? 0x636deb767cd7d0f15ca4ab8ea9a9b26e98b426ac

            if (spYield > 0) {
                boldToken.mint(address(stabilityPool), spYield);//@TODO check how interest money flow to pool
