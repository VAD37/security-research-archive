...
contract TroveManager is LiquityBase, ITroveManager, ITroveEvents {
    ...
    */
    ...
    function _computeNewStake(uint256 _coll) internal view returns (uint256) {
        uint256 stake;
        if (totalCollateralSnapshot == 0) {//audit-ok first depositor exploit does not work. Minimum 1000$ cash deposit prevent share reduced to small value.
            stake = _coll;
        } else {
            /*
    ...
    */
    ...
    function onOpenTrove(address _owner, uint256 _troveId, TroveChange memory _troveChange, uint256 _annualInterestRate)
        external
    {//@id is unique. owner can be anyone.
        _requireCallerIsBorrowerOperations();//@troveChange edited by BorrowerOperation first


        uint256 newStake = _computeNewStake(_troveChange.collIncrease);//@1:1 collateral:stake. Unless Trove is undercollateral then it is. stake = totalStakesSnapshot / totalCollateral

        // Trove memory newTrove;
        Troves[_troveId].debt = _troveChange.debtIncrease + _troveChange.upfrontFee;// all debt
        ...
        totalStakes = newTotalStakes;//@total collateral, if undervalue then it represent share of all left over collateral

        // mint ERC721
        troveNFT.mint(_owner, _troveId);

        _updateTroveRewardSnapshots(_troveId);//@ cache trove latest L_coll, L_boldDebt when TroveManager is init
