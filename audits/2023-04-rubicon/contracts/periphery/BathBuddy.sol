...
contract BathBuddy is ReentrancyGuard, IBathBuddy, Pausable {
    ...
    modifier onlyBuddy() {
        require(
            msg.sender == myBathTokenBuddy &&
                msg.sender != address(0) &&//@audit L can skip this check
                friendshipStarted,
        ...
        return// reward = (time - lastTime) * rewardRate * 1e18 / totalSupply
            rewardsPerTokensStored[token].add(
                lastTimeRewardApplicable(token)
                    .sub(lastUpdateTime[token])//@note when this will be not zero - zero after init
    ...
    function earned(
        ...
        return
            IERC20(myBathTokenBuddy) // Care with this?
                .balanceOf(account)//@audit token rewards no checkpoint. transfer multiple account for rewards
                .mul(
    ...
    function getReward(
        IERC20 rewardsToken,
        address holderRecipient //always msg.sender
    )
        external//unsafe called by anyone
        override
        nonReentrant
        whenNotPaused
    ...
    function notifyRewardAmount(//@note already have boolean friendship check started
        uint256 reward,//1st admin call. buddyToken totalsupply is zero. no one migrate yet
        IERC20 rewardsToken
    ) external onlyOwner updateReward(address(0), address(rewardsToken)) {//modifier update rewardsPerTokensStored = 0, lastUpdateTime = 0 (1st call)
        if (block.timestamp >= periodFinish[address(rewardsToken)]) {
            ...
            uint256 leftover = remaining.mul(//leftover == reward from previous epoch
                rewardRates[address(rewardsToken)]
            );
            rewardRates[address(rewardsToken)] = reward.add(leftover).div(
                rewardsDuration[address(rewardsToken)]//@audit give new rewards will also refresh rewards duration
            );
        }
        ...
        require(//@audit this check (reward + leftover) < balance
            rewardRates[address(rewardsToken)] <=
                balance.div(rewardsDuration[address(rewardsToken)]),//@audit rewardRate does not include totalSupply of buddy. Why earned calculation include buddy.
            "Provided reward too high"
        );

        lastUpdateTime[address(rewardsToken)] = block.timestamp;//@note modifier also update this value.
        periodFinish[address(rewardsToken)] = block.timestamp.add(
            rewardsDuration[address(rewardsToken)]
        );
        emit RewardAdded(reward);//@audit notifyRewardAmount look pretty ok. Only check modifier for faulty address(0) logic
    }
    ...
    /* ========== MODIFIERS ========== */
    // lastTimeRewardApplicable = block.timestamp < periodFinish[token]? block.timestamp: periodFinish[token]
    // Rewards set here
    modifier updateReward(address account, address token) {
        rewardsPerTokensStored[token] = rewardPerToken(token);// zero first call
        lastUpdateTime[token] = lastTimeRewardApplicable(token); // 1st update call return zero and always return periodFinish[token]
