...
contract VaultManagerV2 is IVaultManager, Initializable {
      ...
      isDNftOwner(id)
  {
    ...
    if (!keroseneManager.isLicensed(vault))                 revert VaultNotLicensed();//@audit accept deprecated vault. old version vault through licenser
  ...
  function remove(
    ...
    external
      isDNftOwner(id)
  {
    if (Vault(vault).id2asset(id) > 0) revert VaultHasAssets();//@audit what happen when vault not licensed anymore?
  ...
  function deposit(
    ...
    external 
      isValidDNft(id)//@audit there is validNFT check only modifier. Anyone can call this as long as NFT is valid. include reentrancy.
  {
    idToBlockOfLastDeposit[id] = block.number;
    Vault _vault = Vault(vault);//@audit deposit didnt validate vault is licensed or exist with id. What if user send money to deprecated vault. or NFT owner never add this vault. or NFT owner already remove this vault.
    _vault.asset().safeTransferFrom(msg.sender, address(vault), amount);
    _vault.deposit(id, amount);//@audit anyone deposit allow dust token inside vault on L2. to prevent remove vault
  }
      ...
      isDNftOwner(id)
  {
    ...
    uint dyadMinted = dyad.mintedDyad(address(this), id);//@audit H different vault allow
      ...
      isDNftOwner(id)
  {
    ...
    if (getNonKeroseneValue(id) < newDyadMinted)     revert NotEnoughExoCollat();//@audit L trust Vault to find correct oracle price. this is bad
  ...
  function burnDyad(
    ...
    external 
      isValidDNft(id)
  {
    dyad.burn(id, msg.sender, amount);//@audit anyone can remove DYAD stable coin from circulation from any NFT ID. This allow someone to reduce other debt.
