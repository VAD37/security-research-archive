# No Minimum Deposit Allows DoS Attack on Backend Infrastructure. Resulting in Rewards Theft

The contract relies on a centralized offchain bot for token balance validation and campaign enforcement.

With no minimum deposit and unlimited repeated reallocation exploit, an attacker can overload the bot using only tiny deposits—denial-of-servicing it—and potentially drain all rewards with minimal funds.

While high API call costs might contribute to the issue, the core flaw is the reliance on offchain validation/monitoring.

A backup failsafe (similar to Chainlink’s last-online checks) is needed to secure onchain operations if the bot fails.

## Finding description and impact

The contract relies on a centralized offchain bot to validate token balances and enforce campaign rules. Users must maintain a sufficient token balance to participate; otherwise, the operator bot triggers `invalidateParticipations()`, preventing reward claims and reallocating rewards back to the campaign pool for other users.

This offchain process may need to monitor millions of addresses, incurring high API costs and degrading performance—a challenge that will only worsen as the system scales to Layer 2.

An attacker can exploit this design by generating millions of small deposits to overwhelm the bot. Such an overload could trigger backend crashes from API cost surges, range errors, or memory overflows.

Below POC demonstrates infinite claiming rewards that by repeatedly calling `handleReallocation()`, an exploiter can drain all campaign rewards from the pending pool at minimal expense and then wait until the escrow period ends. If the bot fails to come back online or an admin does not intervene in time to pause the campaign, the exploiter can bypass the requirement of holding sufficient tokens and steal all rewards.

## Impact

- **High Operational Costs:** Continuously checking millions of user balances leads to significant API costs for Nudge developers.
- **DoS Vulnerability:** The offchain bot's validation process can be overwhelmed, causing service disruption.
- **Unrestricted Reward Claims:** With the bot offline, an attacker can repeatedly claim rewards, effectively draining the entire campaign until the bot is restored.


## Proof of Concept

POC show one account can drain all rewards with small balance.
...
- Enforce Minimum Deposit for campaign

- **Reduce Single Point of Failure:** The system heavily depends on backend code being online 100% of the time. Claiming rewards should also be managed by bot system directly. Or better it check oracle for last online time by Operator.

- **Automated Availability Checks:** Consider using off-the-shelf solutions like OpenZeppelin Defender combined with an Operator oracle. Require the operator to send a transaction every hour to prove system availability. If the operator fails to validate users during downtime, automatically pause all campaigns.
