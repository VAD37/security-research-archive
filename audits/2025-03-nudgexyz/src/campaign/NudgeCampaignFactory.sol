...
contract NudgeCampaignFactory is INudgeCampaignFactory, AccessControl {
    ...
    function deployCampaign(
        uint32 holdingPeriodInSeconds,//7 days
        address targetToken,//@any
        address rewardToken,//@any
        uint256 rewardPPQ,// 5e13 = 5%
        address campaignAdmin,//@any
        uint256 startTimestamp,//0 or any
        address alternativeWithdrawalAddress,//@any
        uint256 uuid//@any
        ...
        bytes32 salt = keccak256( //@ salt reuse-able. No nonce
            abi.encode(
        ...
        bytes memory bytecode = abi.encodePacked(type(NudgeCampaign).creationCode, constructorArgs);//@no surprise bytecode
        campaign = Create2.deploy(0, salt, bytecode);//@no callback during deploy
