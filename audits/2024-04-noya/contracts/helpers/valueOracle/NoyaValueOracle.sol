...
contract NoyaValueOracle is INoyaValueOracle {
    ...
    function updatePriceRoute(address asset, address base, address[] calldata s) external onlyMaintainer {
        ...
        emit UpdatedPriceRoute(asset, base, s);//@audit-ok L QA should have check to make sure priceRoutes is ending with baseToken
    }
    ...
    function getValue(address asset, address baseToken, uint256 amount) public view returns (uint256) {
        ...
        console.log("oracle asset: %s, baseToken: %s, amount: %s", asset, baseToken, amount);
        ...
        returns (uint256 value)
    {
        ...
        for (uint256 i = 0; i < sources.length; i++) {
            initialValue = _getValue(asset, sources[i], initialValue);//this allow getOracle value for BTC/WETH/USDC/BaseToken
            quotingToken = sources[i];//@audit-ok H2 _getvalue should be quotingToken not asset
        }
    ...
    function _getValue(address quotingToken, address baseToken, uint256 amount) internal view returns (uint256) {
        ...
        }//uniswapOracle is just swap token to token. using UniswapV3 1800s period
        return oracle.getValue(quotingToken, baseToken, amount);//@weth oracle would return 1e18
    }
}
