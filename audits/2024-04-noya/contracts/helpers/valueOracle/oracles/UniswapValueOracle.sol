...
contract UniswapValueOracle is INoyaValueOracle {
    ...
    function addPool(address tokenIn, address baseToken, uint24 fee) external onlyMaintainer {
        ...
        assetToBaseToPool[tokenIn][baseToken] = pool;//@uniswap oracle might depend on pool with little funds.
    ...
    function getValue(address tokenIn, address baseToken, uint256 amount) public view returns (uint256 _amountOut) {
        ...
        (int56[] memory tickCumulatives,) = IUniswapV3Pool(pool).observe(secondsAgos);//@audit-ok deposit,withdraw have delay L uniswap fixed 1800s period look quite easy for price swing. this require user must locked token before withdraw
        ...
        _amountOut = OracleLibrary.getQuoteAtTick(timeWeightedAverageTick, amountIn128, tokenIn, baseToken);//@audit-ok no swap happening H uniswapV3 oracle price inaccurate ignore liquidity problem. price will just swing heavily and affected due to liquidity price change.
    }
}
