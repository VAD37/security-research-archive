...
contract ChainlinkOracleConnector is INoyaValueOracle {
    ...
    */
    ...
    address public constant USD = address(840); //@audit-ok M4 no decimal for default address for USD is address(840). get 840 meaning get current token in USD like USDC to USD.
    ...
    */
    function updateChainlinkPriceAgeThreshold(uint256 _chainlinkPriceAgeThreshold) external onlyMaintainer {
        if (_chainlinkPriceAgeThreshold <= 1 hours || _chainlinkPriceAgeThreshold >= 10 days) {
            //@audit-ok M1 M chainlink 1 hours age threshold does not work for some aggregators who is slow to update
    ...
    */
    function setAssetSources(address[] calldata assets, address[] calldata baseTokens, address[] calldata sources)
        ...
        onlyMaintainer
    {
        //asset: USDC - baseToken: USD 840 - USDC_USDC_feed
    ...
    */
    function getValue(address asset, address baseToken, uint256 amount) public view returns (uint256) {
        ...
        //primarySource = USDC/USD  feed . inversed = false. isPrimaryInverse also config by admin like when USD/USDC feed
        (address primarySource, bool isPrimaryInverse) = getSourceOfAsset(asset, baseToken); //@audit-ok yes depend on admin config .M does this work for USDC/ETH feeds on mainnet
        ...
        decimalsSource = decimalsSource == ETH || decimalsSource == USD ? primarySource : decimalsSource; //@audit-ok empty address not happing. L this was config correctly or not depend on admin. To prevent case of calling empty address
    ...
    */
    function getValueFromChainlinkFeed(
        ...
        return (amountIn * uintprice) / (sourceTokenUnit); //@audit-ok H3 chainlink oracle did not convert price correctly for all types of oracle. This ignore oracle feed 6 or 8 decimals
    }
    ...
    //@ this if/else allow USDC/USD or USD/USDC will work. because price inverse swap taking place
    ...
    function getSourceOfAsset(address asset, address baseToken) public view returns (address source, bool isInverse) {
        if (assetsSources[asset][baseToken] != address(0)) {
            return (assetsSources[asset][baseToken], false); //@audit-ok oracle depend on admin get correct config. missing onchain check token 0 and token1
