...
contract BaseConnector is NoyaGovernanceBase, IConnector, ReentrancyGuard {
    ...
    SwapAndBridgeHandler public swapHandler;//@lifi router handler
    INoyaValueOracle public valueOracle;

    uint256 public MINIMUM_HEALTH_FACTOR = 15e17;//@15%
    ...
    constructor(BaseConnectorCP memory params) NoyaGovernanceBase(params.registry, params.vaultId) {
        swapHandler = params.swapHandler;//@SwapAndBridgeHandler
        valueOracle = params.valueOracle;//@NoyaValueOracle
        minimumHealthFactor = MINIMUM_HEALTH_FACTOR;
    }//@note vault maintainer have absolute power over BaseConnectors contracts and how to add/remove them
    ...
    function updateMinimumHealthFactor(uint256 _minimumHealthFactor) external onlyMaintainer {
        if (_minimumHealthFactor < MINIMUM_HEALTH_FACTOR) {//@15%
        ...
        returns (uint256)
    {
        ...
        if (msg.sender == accountingManager) {//@called from AM.retrieveTokensForWithdraw()
            (,,,, address watcherContract,) = registry.getGovernanceAddresses(vaultId);

            (uint256 newAmount, bytes memory newData) = abi.decode(data, (uint256, bytes));//@data is manager input
            Watchers(watcherContract).verifyRemoveLiquidity(amount, newAmount, newData);//@empty function

            IERC20(token).safeTransfer(address(accountingManager), newAmount);//@have post check with amount returned
            amount = newAmount;
        } else if (registry.isAnActiveConnector(vaultId, msg.sender) || msg.sender == registry.flashLoan()) {//if called from another connector
            IERC20(token).safeTransfer(address(msg.sender), amount);//@flashloan contract check if caller is keeper contract.
        } else {//@lifi._forward to caller|msg.sender that call executeSwap() which is this connector too
            uint256 routeId = abi.decode(data, (uint256));
            swapHandler.verifyRoute(routeId, msg.sender);//@audit-ok accept call from lifi Implementation only. LifiImple only accept from handler|SwapHandler.SwapHandler only accept from Connectors
            if (caller != address(this)) revert IConnector_InvalidAddress(caller);//@prevent other connector from swap this contract token
            IERC20(token).safeTransfer(msg.sender, amount);//send to lifi implementation
        }
        _updateTokenInRegistry(token);//@note all tokens must be updated when there is event transfer/revice stuff
        return amount;
    }
    /**
    ...
    function transferPositionToAnotherConnector(
        ...
        if (registry.isAnActiveConnector(vaultId, connector)) {//@audit-ok AM have retrieve over withdrawal function. M cannot transfer token back to AM. How manager withdraw token from UniswapCoonector back to AM for user withdraw?
            IConnector(connector).addLiquidity(tokens, amounts, data);
        }
    }
    //@ audit if position not added then add it. if position exist and no token balance then call remove.
    ...
    function _updateTokenInRegistry(address token, bool remove) internal {
        ...
        bytes32 positionId = registry.calculatePositionId(accountingManager, 0, abi.encode(token));//@base connector calculate its own position ID again.
        ...
        if ((positionIndex == 0 && !remove) || (positionIndex > 0 && remove)) {// check for isPositionUsed
            emit UpdateTokenInRegistry(token, remove);
            registry.updateHoldingPosition(vaultId, positionId, abi.encode(address(this)), "", remove);
        }//@audit-ok invalid token, invalid trusted position automatic fail  M connector is trying update non-exist, non-trusted position. As the only trusted is coming from AM. Registry parse positionID from msg.sender directly.
    }
    /**
    ...
    function addLiquidity(address[] memory tokens, uint256[] memory amounts, bytes memory data)
        ...
        nonReentrant
    {
        if (!registry.isAddressTrusted(vaultId, msg.sender)) {//@account Manager or Connectors
            revert IConnector_InvalidAddress(msg.sender);
        }
//@note Connector request back to AccountingManager to send token to them. Initiator must be accountManager for this to work.
        for (uint256 i = 0; i < tokens.length; i++) {
            // gather all of the tokens
            uint256 _balance = IERC20(tokens[i]).balanceOf(address(this));//@audit-ok post balance check .R sendTokensToTrustedAddress return 0 if failed to transfer due to permission cehck.
            ITokenTransferCallBack(msg.sender).sendTokensToTrustedAddress(tokens[i], amounts[i], msg.sender, "");//@audit-ok L gas this can reduced to caching only. unnecessary loop
            uint256 _balanceAfter = IERC20(tokens[i]).balanceOf(address(this));
            if (_balanceAfter < amounts[i] + _balance) {//@revert if not enough token received.
                revert IConnector_InsufficientDepositAmount(_balanceAfter - _balance, amounts[i]);
            }
        }//@nothing happen here? no override yet
        ...
        for (uint256 i = 0; i < tokens.length; i++) {//this refresh holdingPositions and timestamp.
            _updateTokenInRegistry(tokens[i]); // update the token in the registry
        }//@note _updateTokenInRegistry never update most of the time. It just push new holdingPositionId if this is new token holded by connector. 
        emit AddLiquidity(tokens, amounts, data);// update only called 2nd time if there was no token balance.
    }
     ...
     */
    function swapHoldings(//@audit-ok flashloan loan address can make call swapHoldings. swap vault token.
        ...
        for (uint256 i = 0; i < tokensIn.length; i++) {
            ...
            _updateTokenInRegistry(tokensIn[i]);//@audit-ok L vault maintainer still interact with disable vault or connector
            _updateTokenInRegistry(tokensOut[i]);
            emit SwapHoldings(tokensIn[i], tokensOut[i], amountsIn[i], swapData[i]);
        }//@audit-ok update token will failed due to non trusted token M maintainer can swap non exist tokens. causing drop in TVL. This token will be updated registry holdingPosition too. But these token possible have no oracles.
    }

    function _executeSwap(SwapRequest memory swapRequest) internal returns (uint256 amountOut) {
        amountOut = swapHandler.executeSwap(swapRequest);//@lifi swap.
    }
    /**
