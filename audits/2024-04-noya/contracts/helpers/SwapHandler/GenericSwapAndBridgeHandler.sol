...
contract SwapAndBridgeHandler is NoyaGovernanceBase, ISwapAndBridgeHandler, ReentrancyGuard {
        ...
        returns (uint256 _amountOut)
    {
        ...
        if (_swapRequest.checkForSlippage && _swapRequest.minAmount == 0) {
            ...
            _swapRequest.minAmount = (((1e6 - _slippageTolerance) * _outputTokenValue) / 1e6);//@audit-ok M slippage check is drunk.min amount never used. swap using bytesdata anyway
        }
//@audit-ok slippage check with minAmount work M lifi swap allow uniswap V2 swap . which is price sensitive
        _amountOut = ISwapAndBridgeImplementation(swapImplInfo.route).performSwapAction(msg.sender, _swapRequest);//@audit L swap does not handle msg.value from maintainer
    ...
    function verifyRoute(uint256 _routeId, address addr) external view onlyExistingRoute(_routeId) {
        if (routes[_routeId].route != addr) {//@LifiImplementation or omnichain not exist yet
