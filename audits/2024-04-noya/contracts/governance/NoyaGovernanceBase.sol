...
//@note GovernanceBase only asking registry for msg.sender permission. Vault permission address is unique for each vault. Also non-immutable and not unique VaultId.Multiple
contract NoyaGovernanceBase {
    ...
    constructor(PositionRegistry _registry, uint256 _vaultId) {//@vaultID and registry not immutable for proxy
    ...
    modifier onlyManager() {//@audit-ok bypass stuff R gov onlyManager include flashloan which is BalancerFlashLoan
