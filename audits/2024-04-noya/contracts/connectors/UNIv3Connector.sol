...
contract UNIv3Connector is BaseConnector, ERC721Holder {
    ...
    function openPosition(MintParams memory p) external onlyManager nonReentrant returns (uint256 tokenId) {
        ...
        _updateTokenInRegistry(p.token0); //@audit-ok this is 2 different token with abi.encode . H duplicate different positionID but same address. This possible inflate TVL because double counting
        _updateTokenInRegistry(p.token1); //registry.updateHoldingPosition(vaultId, registry.calculatePositionId(accountingManager, 0, abi.encode(token)), abi.encode(address(this)), "", balance()==0);
...
//     function getUncollectFee() returns(uint128 fees0, uint128 fees1) {
//         MintParams p;// cached from openPosition()
