...
struct Vault {
    ...
    address baseToken;
    mapping(address => ConnectorData) connectors;//@note vault connectors is vaultManager by default.
    mapping(bytes32 => PositionBP) trustedPositionsBP;//@audit-ok positionID generated from msg.sender which are AM and connectors .L how do you  get view list of PositionBP? Using every tokens?
    mapping(bytes32 => uint256) isPositionUsed;//@position index //only used for holdingPositions to keep track what connectors is holding. which is different from positionId which point to AM/connector address.
    HoldingPI[] holdingPositions;//@non-sorted.array can be swap. 
    address governer;
    address maintainer;
    address maintainerWithoutTimeLock;//@zero possible
    address keeperContract;
    address watcherContract;
    address emergency;//@ emergency address is zero possible
    bool enabled;//@enabled when call addVault
}
