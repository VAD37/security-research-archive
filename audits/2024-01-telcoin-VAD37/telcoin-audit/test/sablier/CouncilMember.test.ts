...
describe("CouncilMember", () => {
    ...
    describe("mutative", () => {
        ...
        describe("mint3", () => {
            it("mint a single NFT", async () => {
                await expect(councilMember.mint(member.address)).emit(councilMember, 'Transfer');
                await expect(councilMember.mint(support.address)).emit(councilMember, 'Transfer');
                await expect(councilMember.mint(admin.address)).emit(councilMember, 'Transfer');
                expect(await councilMember.balanceOf(member.address)).to.equal(1);
                expect(await councilMember.ownerOf(0)).to.equal(member.address);
                ...
                expect(await councilMember.balanceOf(admin.address)).to.equal(1);
                expect(await councilMember.ownerOf(2)).to.equal(admin.address);
                expect(await councilMember.totalSupply()).to.equal(3);
            });
        });
