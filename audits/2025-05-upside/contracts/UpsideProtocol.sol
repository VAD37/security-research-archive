...
contract UpsideProtocol is Ownable {
    ...
    struct MetaCoinInfo {
        ...
        bool isFreelyTransferable;// false by default when init
        uint256 liquidityTokenReserves;// 10000 USDC
        uint256 metaCoinReserves;// 10_000_000e18
        uint256 createdAtUnix;// block.timestamp
    }

    struct FeeInfo {
        address tokenizeFeeDestinationAddress;//owner
        uint32 swapFeeDecayInterval;// 6 seconds
        bool tokenizeFeeEnabled;//true
        uint16 swapFeeStartingBp;//99% 9900
        uint16 swapFeeDecayBp;// 1%
        uint16 swapFeeFinalBp;//1%
        uint16 swapFeeSellBp;//1%
        uint16 swapFeeDeployerBp;//10%
    }
    ...
    address public liquidityTokenAddress;//USDC base 0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913
    ...
    mapping(address tokenizeFeeAddress => uint256 tokenizeFeeAmount) public tokenizeFeeMap;//5 USDC fee
    ...
    function tokenize(string calldata _url, address _tokenizeFeeAddress) external returns (address metaCoinAddress) {
        ...
        metaCoinAddress = address(
            new UpsideMetaCoin(
                ...
                address(this)
            )//@audit L new coin address with no salt. predictable address
        );//@ 10_000_000e18 token send back to this address
        urlToMetaCoinMap[_url] = metaCoinAddress;//unique URL

        metaCoinInfoMap[metaCoinAddress] = MetaCoinInfo(//@URL with https can be stolen by anyone with http
            msg.sender,
            false,
        ...
        IUpsideStaking(stakingContractAddress).whitelistStakingToken(metaCoinAddress);//@note Staking contract must set owner is UpsideProtocol
    ...
    function swap(
        ...
        MetaCoinInfo storage metaCoinInfo = metaCoinInfoMap[_metaCoinAddress];//@Info by any user pay 5USDC
    ...
    function withdrawLiquidity(address[] calldata _metaCoinAddresses) external onlyOwner {
        ...
        if (withdrawLiquidityTimerStartTime == 0) {
            ...
            emit WithdrawLiquidityTimerStarted(block.timestamp + WITHDRAW_LIQUIDITY_COOLDOWN);//@audit withdrawal wait time is global and not per token
            return;
        }
