...
library Payload {
  ...
  function toEIP712(
    ...
    console.log("hash wallets %s parents wallets", _decoded.parentWallets.length);
    console.log("kind: %s", _decoded.kind);
    if (_decoded.kind == KIND_TRANSACTIONS) {
      console.log("check hash %s calls", _decoded.calls.length);
      ...
      console.log("check hash %s message");
      ...
      console.log("check hash config update imageHash");
      console.logBytes32(_decoded.imageHash);
      ...
      console.log("check hash digest");
      console.logBytes32(_decoded.digest);
      return keccak256(abi.encode(MESSAGE_TYPEHASH, _decoded.digest, walletsHash));
    } else {
      // Unknown kind
      console.log("invalid kind");
      revert InvalidKind(_decoded.kind);//@audit invariant, all operation must check transaction decoded.kind. Signature only check certain part of packed payload based on type of transaction.
    }
  }
  ...
  function hash(
    ...
    bytes32 domain = domainSeparator(_decoded.noChainId, address(this));//@audit wallet allow multichain approval. Sign key ignore chainId.
    bytes32 structHash = toEIP712(_decoded);
    console.log("----hash structHash");
    console.logBytes32(structHash);
    return keccak256(abi.encodePacked("\x19\x01", domain, structHash));
  }//@audit no nonce revert for hash operations too. It just hash nonce sent in by users. no local storage nonce check.
  //@nonce is checked with space, nonce in payload struct. Then it was check separately on parent contract?
  function hashFor(Decoded memory _decoded, address _wallet) internal view returns (bytes32) {
    ...
    bytes32 structHash = toEIP712(_decoded);
    console.log("----hashFor structHash");
    console.logBytes32(structHash);
    return keccak256(abi.encodePacked("\x19\x01", domain, structHash));
  }
