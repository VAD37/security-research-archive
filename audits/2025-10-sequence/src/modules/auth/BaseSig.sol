...
library BaseSig {
  ...
  function recover(
    ...
    bytes calldata _signature,//0x04 00 01 1197c4a952b46becad0663f76357d3776ba11566e1801c5b52498d74901f2ab5af572eaaef36dc1a508cbb43cf86f55bc0686ec24de1
    bool _ignoreCheckpointer,//@signatureValidation false
    address _checkpointer//@signatureValidation zero
    ...
    console.log("signatureFlag: %x, rindex: %x",signatureFlag, rindex);
    ...
    console.log("weird checkpointer error: %s",_checkpointer);
    ...
    if (signatureFlag & 0x40 == 0x40 && _checkpointer == address(0)) { //@note pass-through checkpointer is unimplemented. Always have checkpointer contract inside signature flags
      // Override the checkpointer
      // not ideal, but we don't have much room in the stack
      (_checkpointer, rindex) = _signature.readAddress(rindex);//checkpointer is empty and can by any address
      console.log("checkpointer from signature: %s, rindex: %s",_checkpointer , rindex);
    ...
    console.log("no chain id:", _payload.noChainId);
    {
    ...
    console.log("checkpoint: %s, %s", checkpoint, rindex);
    // Recover the threshold, using the flag for the size
    {
      uint256 thresholdSize = ((signatureFlag & 0x20) >> 5) + 1;
      (threshold, rindex) = _signature.readUintX(rindex, thresholdSize);//@audit signature sign threshold decided by user input. this was not rehash or check against config. Is there anyway to bypass this?
    }
    console.log("threshold: %s, %s", threshold, rindex);
    console.logBytes(_signature[rindex:]);
    // Recover the tree
    console.log("--hasing opHash");
    opHash = _payload.hash();
    console.logBytes32(opHash);
    console.log("--recovering branch");
    (weight, imageHash) = recoverBranch(_payload, opHash, _signature[rindex:]);//@ reading from signatures array. get address weight, hash addr+weight into merkleroot.
    console.log("weight: %s", weight);
    console.log("imageHash:");
    console.logBytes32(imageHash);
    imageHash = LibOptim.fkeccak256(imageHash, bytes32(threshold));//@imagehash here is combination of hash addr+weight or custom hash.
    ...
    console.logBytes32(imageHash); //0x9e605e57b90f14aa986a54ec32e1b58d87d06dc90c8018a281752166916d4e21
    ...
    if (snapshot.imageHash != bytes32(0) && snapshot.imageHash != imageHash && checkpoint <= snapshot.checkpoint) {
      revert UnusedSnapshot(snapshot);//@audit R snapshot can be anything from signature. this bypass stuff.
    }
  }
  ...
  function recoverBranch(
    Payload.Decoded memory _payload,
    bytes32 _opHash,
    bytes calldata _signature//0x1197c4a952b46becad0663f76357d3776ba11566e1 80 1c5b52498d74901f2ab5af572eaaef36dc1a508cbb43cf86f55bc0686ec24de1
    ...
    unchecked {
      ...
      while (rindex < _signature.length) {
        ...
        // Read next item type
        uint256 firstByte;
        (firstByte, rindex) = _signature.readUint8(rindex);// - 0000 00XX (bits [1..0]): signature type (00 = normal, 01/11 = chained, 10 = no chain id)

        // The top 4 bits are the flag
        uint256 flag = (firstByte & 0xf0) >> 4;
        console.log("flag: %s ,loop index: %s", flag, rindex);
        // Signature hash (0x00)
        if (flag == FLAG_SIGNATURE_HASH) {
          ...
          if (addrWeight == 0) {
            ...
            console.log("addr Weight ==0, read weight from user input??");
          }
          ...
          console.log("found signature addr %s , weight %s",addr,addrWeight);
          weight += addrWeight;
          bytes32 node = _leafForAddressAndWeight(addr, addrWeight);//keccak256(abi.encodePacked("Sequence signer:\n", _addr, _weight))
        ...
        if (flag == FLAG_ADDRESS) {
          ...
          console.log("Flag1 addr %s , weight %s",addr,addrWeight);
          // Compute the merkle root WITHOUT adding the weight
          bytes32 node = _leafForAddressAndWeight(addr, addrWeight);//just keccak(addr, weight)
          root = root != bytes32(0) ? LibOptim.fkeccak256(root, node) : node;
          continue;
        }
        ...
        if (flag == FLAG_NODE) {
          ...
          root = root != bytes32(0) ? LibOptim.fkeccak256(root, node) : node;
          continue;
        }//@audit flag node seem like a possible point of attack as it doesnt even check opHash of current operations. no weight increase either.
        ...
        if (flag == FLAG_BRANCH) {
          ...
          weight += nweight;
          root = LibOptim.fkeccak256(root, node);//@branch skip zero check? always expect non zero?
          continue;
        }
        ...
        if (flag == FLAG_SIGNATURE_ANY_ADDRESS_SUBDIGEST) {
          ...
          bytes32 hardcoded;
          (hardcoded, rindex) = _signature.readBytes32(rindex);
          console.log("hardcoded pos:", rindex);
          console.logBytes32(hardcoded);//0x1c5b52498d74901f2ab5af572eaaef36dc1a508cbb43cf86f55bc0686ec24de1
          bytes32 anyAddressOpHash = _payload.hashFor(address(0));//@this also include hashing payload calls so it must be recalculated
          if (hardcoded == anyAddressOpHash) {
            weight = type(uint256).max;
            console.log("anyAddressOpHash weight max");
          }
          console.logBytes32(hardcoded);
          bytes32 node = _leafForAnyAddressSubdigest(hardcoded);//@just keccak(string,hardcoded)
          root = root != bytes32(0) ? LibOptim.fkeccak256(root, node) : node;
          continue;
        }
        ...
        if (flag == FLAG_SIGNATURE_SAPIENT) {
          ...
          // - XX00 : Signature size size (00 = 0 byte, 01 = 1 byte, 10 = 2 bytes, 11 = 3 bytes)
          // - 00XX : Weight (00 = dynamic, 01 = 1, 10 = 2, 11 = 3)
          console.log("-Signature Sapient-");
          // Read signer and weight
          uint8 addrWeight = uint8(firstByte & 0x03);
          if (addrWeight == 0) {
            (addrWeight, rindex) = _signature.readUint8(rindex);
            console.log("addrWeight read from user input??", addrWeight);
          }

          address addr;
          (addr, rindex) = _signature.readAddress(rindex);
          console.log("sapient addr:", addr);
          ...
          uint256 size;
          {
            uint256 sizeSize = uint8(firstByte & 0x0c) >> 2;
            (size, rindex) = _signature.readUintX(rindex, sizeSize);
            console.log("sapient size:", size, sizeSize);
          }
          ...
          rindex = nrindex;
          console.log("recovered sapient image hash:");
          console.logBytes32(sapientImageHash);
          // Add the weight and compute the merkle root
          weight += addrWeight;
