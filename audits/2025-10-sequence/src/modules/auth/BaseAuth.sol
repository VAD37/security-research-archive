...
abstract contract BaseAuth is IAuth, IPartialAuth, ISapient, IERC1271, SelfAuth {
  ...
  }//@static sig is like approve for specific tx call,operation
  ...
  //@ called by Calls.sol:execute before executing any payload
  function signatureValidation(
    Payload.Decoded memory _payload, //Payload.fromPackedCalls(_payload);
    bytes calldata _signature//input by user
  ) internal view virtual returns (bool isValid, bytes32 opHash) {
    // Read first bit to determine if static signature is used
    bytes1 signatureFlag = _signature[0];//0x0400011197c4a952b46becad0663f76357d3776ba11566e1801c5b52498d74901f2ab5af572eaaef36dc1a508cbb43cf86f55bc0686ec24de1
//init code hash 0x573fd524dc86b426cf228cb23f874693e22c686ed86eee5bc6720a772e660608 
    if (signatureFlag & 0x80 == 0x80) {//@no static signature. init sig can still use static sig?
      opHash = _payload.hash();//@audit RH static signature bypass payload kind(type) check. Allow manipulated payload calls. Does docs allow static sig for any kind of tx?
      console.log("static signature used");
    ...
    (threshold, weight, imageHash,, opHash) = BaseSig.recover(_payload, _signature, false, address(0));//@audit view function recover have external call to another contract. Allow reentrancy injection
    console.log("recovered threshold,weight: ", threshold , weight); //imageHash 0x9e605e57b90f14aa986a54ec32e1b58d87d06dc90c8018a281752166916d4e21
    // Validate the weight
    if (weight < threshold) {
      revert InvalidSignatureWeight(threshold, weight);
    }
    console.log("-valid image from signature-");
  ...
  function isValidSignature(bytes32 _hash, bytes calldata _signature) external view returns (bytes4) {
    ...
    (bool isValid,) = signatureValidation(payload, _signature);//@digest singature branch only
    if (!isValid) {
      return bytes4(0);
    }
