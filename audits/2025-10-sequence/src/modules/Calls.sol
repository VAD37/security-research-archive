...
abstract contract Calls is ReentrancyGuard, BaseAuth, Nonce {
  ...
  function execute(bytes calldata _payload, bytes calldata _signature) external payable virtual nonReentrant {//@this is manual call by developer. Or approved signer by wallet
    uint256 startingGas = gasleft();
    Payload.Decoded memory decoded = Payload.fromPackedCalls(_payload);

    _consumeNonce(decoded.space, decoded.nonce);//@ mapping(space=>nonce) , nonce++
    console.log("-Nonce consumed-", decoded.space, decoded.nonce);
  ...
  ) external payable virtual onlySelf {//@only from EntryPoint Account Abstraction. validate call through this contracts -> get paid -> execute
    uint256 startingGas = gasleft();
    Payload.Decoded memory decoded = Payload.fromPackedCalls(_payload);
  ...
  function _execute(uint256 _startingGas, bytes32 _opHash, Payload.Decoded memory _decoded) private {
    ...
    for (uint256 i = 0; i < numCalls; i++) {
      ...
      if (call.onlyFallback && !errorFlag) {//@if we have execution0(succeed), fallback1(skip), fallback2(skip),fallback3(skip).
      ...
      if (call.delegateCall) {
        ...
        );//@note wallet only accept delegate call to contract using very specific interface
      } else {
