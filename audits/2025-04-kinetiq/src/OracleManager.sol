...
contract OracleManager is IOracleManager, Initializable, AccessControlEnumerableUpgradeable {
    ...
    function initialize(
        ...
        maxPerformanceBound = _maxPerformanceBound;//@10000
    ...
    function generatePerformance(address validator) external whenNotPaused onlyRole(OPERATOR_ROLE) returns (bool) {
        ...
        require(
            block.timestamp >= lastValidatorUpdate[validator] + MIN_UPDATE_INTERVAL,//@1 hour oracle
            "Update too frequent for validator"
        );
        require(validator != address(0), "Invalid validator address");

        // Check no pending rebalance
        require(!validatorManager.hasPendingRebalance(validator), "Validator has pending rebalance");//@note Manager have power to prevent validator from being updated if it have pending actions like rebalance
//@audit-ok It is check later when report performance .M missing check if validator already deactived by ValidatorManager.
        uint256 oracleCount = authorizedOracles.length();//single default oracle for now
        ...
        for (uint256 i = 0; i < oracleCount; ) {
            ...
            if (!activeOracles[oracle]) {
                ...
                continue; // Skip inactive oracles .@also revert if not enough active oracles
            }
            ...
            try IOracleAdapter(oracle).getPerformance(validator) returns (
                ...
                if (
                    ...
                    totalUptimeScore += uptimeScore;//10000
                    totalSpeedScore += speedScore;//10000
                    totalIntegrityScore += integrityScore;//10000
                    totalSelfStakeScore += selfStakeScore;//10000 score always basisPoint < 10000
                    totalRewardAmount += accRewardAmount;//0 or any
                    totalSlashAmount += accSlashAmount;//0 or any
                    validOracleCount++;
                }
            } catch {
