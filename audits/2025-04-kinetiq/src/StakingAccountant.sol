...
contract StakingAccountant is IStakingAccountant, Initializable, AccessControlEnumerableUpgradeable {
    ...
    function initialize(address admin, address manager, address _validatorManager) public initializer {
        ...
        _grantRole(MANAGER_ROLE, manager);//EOA

        validatorManager = IValidatorManager(_validatorManager);
    }//@note initialize() here missing kHYPE token address. this seem like refactor code
    ...
    function authorizeStakingManager(address manager, address kHYPEToken) external onlyRole(MANAGER_ROLE) {
        ...
        _authorizedManagers.set(manager, kHYPEToken);//@stakingManager and kHYPE token called by admin after all init
    ...
    function deauthorizeStakingManager(address manager) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        uint256 length = _authorizedManagers.length();//length could be 0, 

        for (uint256 i = 0; i < length; i++) {//@loop through list and ignore removed manager
    ...
    function recordStake(uint256 amount) external override onlyAuthorizedManager {
        totalStaked += amount;//@audit-ok single kHYPE token, all use HYPE to stake. Just multiple stakingManager for each validator M. conflict of AuthorizedManager. We have multiple Token. How to track which Token is which?
        emit StakeRecorded(msg.sender, amount);//staked is msg.value or L2 HYPE token 1e18
    }

    function recordClaim(uint256 amount) external override onlyAuthorizedManager {
        totalClaimed += amount;//e18
    ...
    }//@lower exchange -> kHYPE worthless. exchange for nothing
    ...
    function HYPEToKHYPE(uint256 HYPEAmount) public view override returns (uint256) {
        ...
        return Math.mulDiv(HYPEAmount, 1e18, exchangeRatio);//@new hype does not decrease exchangeRate
    }//@lower exchange -> hype can get more kHYPE
    ...
    function _getExchangeRatio() internal view returns (uint256) {
        ...
        uint256 rewardsAmount = validatorManager.totalRewards();//@unclear where kHYPE is minted and new flow on Hype come from
        uint256 slashingAmount = validatorManager.totalSlashing();//@audit-ok R reward/slashing in e8 decimal, while staked token in e18 decimal
        uint256 totalHYPE = totalStaked + rewardsAmount - totalClaimed - slashingAmount;

        // Calculate ratio with 18 decimals precision
        return Math.mulDiv(totalHYPE, 1e18, totalKHYPESupply);//@validator can decrease totalHype. decrease exchangeRate
    }
}
