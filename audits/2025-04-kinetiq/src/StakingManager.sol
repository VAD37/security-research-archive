    ...
    ReentrancyGuardUpgradeable
{
    ...
    bool public whitelistEnabled;//@audit-ok L whitelist not enable as default mode during init. Allow anyone to join staking before manager can disable it first. Allow first mover advantage
    ...
    function initialize(
        address admin,//EOA
        address operator,//EOA
        address manager,//EOA
        address _pauserRegistry,// PauserRegistry
        address _kHYPE,// kHYPE
        address _validatorManager,// ValidatorManager
        address _stakingAccountant,// StakingACcountant
        address _treasury,//EOA
        uint256 _minStakeAmount,//0.1 ether
        uint256 _maxStakeAmount,//1 ether
        uint256 _stakingLimit,//10_000 ether
        uint64 _hypeTokenId// 0 //@audit-ok R. Hype chain have this unique features of all ERC20 token have ordering ID from 0. So linking token might be exploited?
    ) public initializer {//@note ignore 1105 init variable in test file
        // Validate addresses
    ...
    function stake() public payable nonReentrant whenNotPaused whenStakingNotPaused {
        ...
        if (stakingLimit > 0) {
            ...
            uint256 rewardsAmount = validatorManager.totalRewards();//@pending rewards? HYPE token?report by offchain admin/operator
            uint256 netStaked = totalStaked + rewardsAmount - totalClaimed;//@unclear where claimed go from
            require(netStaked + msg.value <= stakingLimit, "Staking limit reached");
        }//@this only check for max staked global amount. not important

        totalStaked += msg.value;

        // Convert HYPE to kHYPE amount using exchange ratio //@audit M rate conversation abuse with first depositor method. but the delay is 7 days. so this attack is unlikely
        uint256 kHYPEAmount = stakingAccountant.HYPEToKHYPE(msg.value);//@note we might have multiple kHYPE token. HYPE in e18 decimals
        ...
        stakingAccountant.recordStake(msg.value);//@totalStaked += msg.value. exchangeRatio is likely the same. as both increase by same distance

        emit StakeReceived(address(this), msg.sender, msg.value);//send HYPE L2 token to L1 stake . receive kHYPE token based on conversion rate.
    }//133 HYPE,100kHYPE. mint 200 HYPE, get 150.37 kHYPE. 333HYPE/250.37 kHYPE. ratio still 1.33
    ...
    function queueWithdrawal(uint256 kHYPEAmount) external nonReentrant whenNotPaused whenWithdrawalNotPaused {
        ...
        uint256 withdrawalId = nextWithdrawalId[msg.sender];//from 0

        // Calculate fee in kHYPE using mulDiv for precision
        // kHYPEAmount is in 8 decimals
        uint256 kHYPEFee = msg.sender == treasury ? 0 : Math.mulDiv(kHYPEAmount, unstakeFeeRate, BASIS_POINTS);//0.1%
        ...
        uint256 hypeAmount = stakingAccountant.kHYPEToHYPE(postFeeKHYPE);//amount * rate / 1e18
        ...
        require(currentDelegation != address(0), "No delegation set");//@delegation set by admin manager. just EOA address
        _withdrawFromValidator(currentDelegation, hypeAmount, OperationType.UserWithdrawal);//take cash from buffer. Otherwise take from L1 HYPE balance. write to pending list.
    ...
    function confirmWithdrawal(uint256 withdrawalId) external nonReentrant whenNotPaused {
        ...
        (bool success, ) = payable(msg.sender).call{value: amount}("");//@note HYPE L2 have balance as L1. it share same balance for both address.
    ...
    function processValidatorWithdrawals(
        ...
        for (uint256 i = 0; i < validators.length; ) {
            ...
            // Use RebalanceWithdrawal type //note there is pending order on ValidatorManager, there is second pending happening at StakingManager too
     ...
     */
    function processValidatorRedelegation(uint256 amount) external nonReentrant whenNotPaused {//@cancel previoud rebalance delegation by Manager
        require(msg.sender == address(validatorManager), "Only ValidatorManager");
        require(amount > 0, "Invalid amount");
        require(address(this).balance >= amount, "Insufficient balance");//@audit-ok M02 why redelegation take CASH from HYPE buffer balance. this suppose to be just undelegate
    ...
    function _processConfirmation(address user, uint256 withdrawalId) internal returns (uint256) {
        ...
        // Update state
        totalQueuedWithdrawals -= hypeAmount;
        totalClaimed += hypeAmount;//@audit-ok H. it seem like token in limbo, pending does not count toward exchange rate?
        delete _withdrawalRequests[user][withdrawalId];

        // Burn kHYPE tokens (excluding fee)
        kHYPE.burn(address(this), kHYPEAmount);//kHype taken from user. it was still in pending
    ...
    function _distributeStake(uint256 amount, OperationType operationType) internal {
        ...
        address validator = validatorManager.getDelegation(address(this));//@audit-ok rebalancing work L All users share same Deletgation EOA target. But admin can shuffle delegation. Resulting in mismatch balance between validator

        // For user deposits, handle buffer first
        if (operationType == OperationType.UserDeposit) {//@staking
            // Check that amount can be cleanly divided into 8 decimals
            require(amount % 1e10 == 0, "Amount must be divisible by 1e10");//minimum is //0.1 ether
//@audit-ok  L. it seem like hardcode 1e10 decimals have not been upgraded to e18 decimals of HypeToken
            // Handle buffer first
            uint256 currentBuffer = hypeBuffer;
            uint256 target = targetBuffer;//@audit R. starting targetBuffer = 0. It only increase by admin.
            if (amount > 0 && currentBuffer < target) {//@this never called if targetBuffer == 0
                uint256 bufferSpace = target - currentBuffer;
                uint256 amountToBuffer = Math.min(amount, bufferSpace);
                hypeBuffer = currentBuffer + amountToBuffer;//@audit-ok buffer take cash from user deposit. it should be write same amount of debt to user
            ...
            uint256 remainder = amount % 1e10;//@note unclear purpose of buffer. if targetBuffer exist, then remainder != 0.
            if (remainder > 0) {//@ remainder always 0 due to check for amount % 1e10 = 0 above
                // Add the remainder to the buffer
                hypeBuffer += remainder;//@1e10 to buffer
                amount -= remainder;//@ x - 1e10 left
            ...
            }//@audit R. Buffer features is untested
            ...
            (bool success, ) = payable(L1_HYPE_CONTRACT).call{value: amount}("");//@send all staked amount to StakingManager contract address on L1 Hypechain.
            ...
            l1Write.sendCDeposit(uint64(truncatedAmount));//@audit R StakingManager.sol stake HYPE token on L1. so it must have enough HYPE token first on StakingManager. With buffer on. there might not be enough token to stake

            // 3. Queue the delegation operation (8 decimals)
            _queueL1Operation(validator, truncatedAmount, operationType);//@pending deposit push, emit event. OPERATOR must manually check if operation is success
            ...
            l1Write.sendCDeposit(uint64(truncatedAmount));//@audit M. move Hype from unstaking 7 days lock back to staking balance. where HYPE token come from? this came from cancel withdraw
    ...
    function processL1Operations(uint256 batchSize) public onlyRole(OPERATOR_ROLE) whenNotPaused {
        ...
        if (batchSize == 0) {
            ...
            uint256 depositsProcessed = _processL1Deposits(0);//@audit-ok H01 must process deposit first or there might not be enough for withdrawal
    ...
    function _processL1Withdrawals(uint256 batchSize) internal returns (uint256) {
        ...
        if (length <= _withdrawalProcessingIndex) {//@not possible to happen unless reentrancy callback.
            return 0;
        }

        // Calculate how many operations we can process
        uint256 endIndex = _withdrawalProcessingIndex + batchSize;//@batchSize can be huge number
        if (endIndex > length || batchSize == 0) {
            endIndex = length;//@limit loop
        }
        ...
        for (uint256 i = _withdrawalProcessingIndex; i < endIndex; i++) {
            ...
            l1Write.sendTokenDelegate(op.validator, uint64(op.amount), true);//@undelegate from staking. Must also have enough staking token first

            // Only call sendCWithdrawal for user withdrawals
            if (op.operationType == OperationType.UserWithdrawal) {//@audit M this crap need L1Read for delegation summary
                l1Write.sendCWithdrawal(uint64(op.amount));
            }

            emit L1DelegationProcessed(address(this), op.validator, op.amount, op.operationType);//@audit M does not check validator is active during process. There is unique validator per StakingManager too.
            processedCount++;
        }
    ...
    function _processL1Deposits(uint256 batchSize) internal returns (uint256) {
        ...
        for (uint256 i = _depositProcessingIndex; i < endIndex; i++) {
            ...
            l1Write.sendTokenDelegate(op.validator, uint64(op.amount), false);//@note deposit will slowly stake token into previous validator. Validator only changed by admin
    ...
    function cancelWithdrawal(address user, uint256 withdrawalId) external onlyRole(MANAGER_ROLE) whenNotPaused {
        ...
        _cancelledWithdrawalAmount += hypeAmount;//@audit M unstake 7 days got cancel, that money must go somewhere. redelegate ignore 7 days wait time when calling. cancel withdrawal does not delete pending operation. pending operation might reset by admin too.
        //@ignore fee amount seem right //@ user withdraw: unstake from previous delegation
    ...
    function redelegateWithdrawnHYPE() external onlyRole(MANAGER_ROLE) whenNotPaused {
        ...
        _distributeStake(amount, OperationType.SpotDeposit);//@sendCDeposit . take HYPE from L1 and stake it. But this does not have HYPE
    ...
    function resetL1OperationsQueue() external onlyRole(MANAGER_ROLE) {
        ...
        delete _pendingWithdrawals;
        delete _pendingDeposits;
        _withdrawalProcessingIndex = 0;//@audit M reset Operation queue does not refund kHYPE send in by users. this include user deposit stake too
        _depositProcessingIndex = 0;
    ...
    function executeEmergencyWithdrawal(
        ...
        l1Write.sendTokenDelegate(validator, uint64(truncatedAmount), true);//@audit M emergency withdrawal take cash from delegation. But not from staking. This prevent user from withdraw their staking.
