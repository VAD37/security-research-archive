    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(address admin, address manager, address _oracle, address _pauserRegistry) external initializer {
        ...
        _grantRole(MANAGER_ROLE, manager);//admin address
        _grantRole(ORACLE_MANAGER_ROLE, _oracle);//OracleManager.sol
     ...
     */
    function rebalanceWithdrawal(//@audit-ok Manager can remove delegate vote from validator.R unclear purpose and role of Manager call rebalance/withdraw cash from validator
        address stakingManager,//@all this do is undelegate vote from validator. 
        ...
        for (uint256 i = 0; i < validators.length; ) {
            ...
            //@withdraw pending order for validator. Manager must have watching L1
            unchecked {
                ++i;
            }
        }

        // Trigger withdrawals through StakingManager. @queue withdrawal pending orders on L1 and stakingManager will slowly execute this order.
        IStakingManager(stakingManager).processValidatorWithdrawals(validators, withdrawalAmounts);//@all this do is undelegate validator from staking. More votes from staking available
    }
    ...
    function closeRebalanceRequests(
        ...
        if (totalAmount > 0) {//@create pending RebalanceDeposit order by Manager
