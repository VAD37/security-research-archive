...
abstract contract VCVerifier is IVCVerifier, EIP712 {
  ...
  function validateCred(
    uint256 agent,// AgentId. FactoryID counter. Each agent have unique ID increment
    bytes4 selector,//msg.sig first 4 bytes of msg.data
    SignedCredential memory sc
  ) public view {
    address issuer = recover(sc);
    if (
      issuer != sc.vc.issuer ||
      !isValidIssuer(issuer) ||//issuer is admin offchain
      sc.vc.subject != agent ||
      ...
      sc.vc.subject == 0//@audit vc cred does not check for value change,target, claim
