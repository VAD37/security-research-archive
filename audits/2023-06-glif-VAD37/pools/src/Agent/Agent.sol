...
contract Agent is IAgent, Operatable {
  ...
  constructor(
    uint256 agentID,//factoryId counter
    address agentRouter,
    address owner,
    address operator,
  ...
  function addMiner(
    ...
    sc.vc.target.changeOwnerAddress(address(this));//@audit-ok H is it possible to share multiple miner owner? as miner beneficiary expire
   ...
   */
  function migrateMiner(uint64 miner) external {//@audit-ok M cannot migrateMiner if upgrade agent twice in a row without calling migrate miner.
  ...
  function refreshRoutes() external {
    wFIL = GetRoute.wFIL(router);
    agentPolice = GetRoute.agentPolice(router);//@audit-ok M synchronize issue exploit user agent can bypass paused agentPolice state if they did not upgrade agentPolice. version check does not help
    minerRegistry = GetRoute.minerRegistry(router);
    poolRegistry = GetRoute.poolRegistry(router);
  }
    ...
    checkVersion
  {
    ...
    agentPolice.agentApproved(sc.vc);//rateModule.isApproved
  }
  ...
  function pay(
    ...
    onlyOwnerOperator
    validateAndBurnCred(sc)//@audit can still pay if defaulted
    checkVersion
