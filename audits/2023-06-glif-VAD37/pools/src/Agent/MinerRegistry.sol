...
contract MinerRegistry is IMinerRegistry {
  ...
  function addMiner(uint256 agentID, uint64 miner) external onlyAgent(agentID) { //addMiner have verification check. This need to be double checked
    bytes32 key = _createMapKey(agentID, miner);

    if (_minerRegistered[key]) revert InvalidParams();
    _minersByAgent[agentID].push(miner);
    _minerRegistered[key] = true;//@audit-ok M agent can share miner with other agents? Whitepaper allow more miner to get more borrow power
