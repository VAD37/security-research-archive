...
contract InfPoolSimpleRamp is IOffRamp {
    ...
    function _processExit(
        ...
        iFIL.transferFrom(owner, address(this), iFILToBurn);//@audit-ok L user have to approve both iFIL and wFIL to this contract. just to withdraw() and call processExit
    ...
    function refreshExtern() external {
        _refreshExtern();//@audit SimpleRamp allow withdraw token from pool while it is shutting down. pool can be migrated by transfering all tokens away
    }
