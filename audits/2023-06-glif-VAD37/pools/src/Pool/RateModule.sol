...
contract RateModule is IRateModule, Ownable {
    ...
    function getRate(
        ...
        return _getRate(vc.getGCRED(credParser));//ICredentials(credParser).getGCRED(vc.claim);
    }
     ...
     */
    function isApproved(
        Account calldata account,//@ account already been postProcessed by pool borrow. router accounts[] already updated to new state
        VerifiableCredential calldata vc
        ...
        if (computeLTV(totalPrincipal, collateralValue) > maxLTV) { //principal.divWadDown(collateralValue); == debt * 1e18 / asset == debt ratio
            return false;//@LTV < 100$ is safe
        }
        // if DTE is greater than `maxDTE`, Agent does not have sufficient skin in the game
        if (computeDTE(totalPrincipal, vc.getAgentValue(credParser)) > maxDTE) {//DTE == principal / equity||(totalValue - principal)
            return false;//@DTE < 100% is safe
        }

        // if DTI is greater than `maxDTI`, Agent cannot meet its payments solely from block rewards
        return computeDTI(
            vc.getExpectedDailyRewards(credParser),//offchain data
            _getRate(gcred),//gcred off chain data
            account.principal,
            totalPrincipal
        ) <= maxDTI;
    }
    ...
    function computeDTI(
        ...
        uint256 rate,//gcred financial risk rate
        uint256 accountPrincipal,//onchain data after borrow account.principal or total += vc.value in borrow()
        uint256 totalPrincipal// offchain data 
    ) public pure returns (uint256) {
        // equityPercentage now has an extra WAD factor for precision
        uint256 equityPercentage = accountPrincipal.divWadDown(totalPrincipal);// p * 1e18 / tp
        // compute the % of EDR this pool can rely on
        // mulWadDown here cancels the extra WAD factor in equityPercentage
        uint256 weightedEDR = equityPercentage.mulWadDown(expectedDailyRewards);// p * 1e18 / tp * d / 1e18 
        ...
        // so mulWadUp by EPOCHS_IN_DAY cancels the extra WAD factor in `rate` to get a daily rate in WAD // rate in e36
        uint256 dailyRate = accountPrincipal.mulWadUp(rate).mulWadUp(EPOCHS_IN_DAY);// p * r / 1e18 * t /1e18
        // compute DTI - divWadUp here adds the extra WAD back to compare with DTI level in storage
        return dailyRate.divWadUp(weightedEDR);//principal * rate * time passed / (pricipal / equity *daily)
    }// p * r / 1e18 * t /1e18 * 1e18 / (p * 1e18 / tp * d / 1e18)= 
     //  r / 1e18 * t / (  / tp * d)= tp * r * t /1e18 / d = tp * re18 * t / d
    /**
