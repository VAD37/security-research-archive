...
contract PoolRegistry is IPoolRegistry, Ownable {
  ...
  constructor(
    uint256 _treasuryFeeRate,//1e17
    address _owner,//sender
    address _router//router
  ) Ownable(_owner) {
    treasuryFeeRate = _treasuryFeeRate;//@audit-ok L does not check feeRate on constructor
    router = _router;
  }
  ...
  function upgradePool(
    ...
    uint256 borrowedAmount = oldPool.decommissionPool(newPool);//@audit-ok R does new pool transfer all of its token and everything else to new pool?
