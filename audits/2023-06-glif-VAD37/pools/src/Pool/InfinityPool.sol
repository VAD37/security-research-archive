...
contract InfinityPool is IPool, Ownable {
    ...
    fallback() external payable {
        _depositFIL(msg.sender);
    } //@audit-ok missing recoverDeposit? https://github.com/wadealexc/fevmate/blob/6a80e989847fd563df21360176cbfd5d08aaabbb/contracts/token/WFIL.sol#L125C45-L125C45
    ...
    constructor(
        address _owner,
        address _router,
        address _asset, //wFIL //@audit-ok why both asset and wFIL is warpFILeCoin. Why duplicate
        address _rateModule, //RateModule
        address _liquidStakingToken, //PoolToken(systemAdmin)
        address _preStake, //PreStake(systemAdmin, IWFIL(address(wFIL)), IPoolToken(address(liquidStakingToken)))
        uint256 _minimumLiquidity, // 0 for test
        uint256 _id //router poolLength
    ) Ownable(_owner) {
        router = _router;
        asset = IERC20(_asset);
    ...
    function totalAssets() public view override returns (uint256) {
        //@audit-ok totalAsset donation attack avaiable.
        return
            asset.balanceOf(address(this)) +
            totalBorrowed +
            preStake.totalValueLocked() -
            feesCollected;
    } //@ERC4626 totalAssets. Is it possible to inflate feesCollected?
     ...
     */
    function isApproved(//@audit-ok why agentPolice depend on pool to check if account is valid?
        Account calldata account,//@audit-ok does all pool share the same account? How multiple pool work? If it is then approval does not check if account have enought credit in multiple pool
        VerifiableCredential calldata vc
    ...
    function writeOff(
    ...
    ) external returns (uint256 totalOwed) {
        ...
        uint256 remainder = recoveredFunds > principalOwed
            ? recoveredFunds - principalOwed
            : 0;
        ...
        if (remainder > 0 && account.epochsPaid < block.number) {
            ...
            uint256 interestOwed = principalOwed.mulWadUp(rate).mulWadUp(
                epochsToPay
            );
            ...
            feesCollected += poolRegistry.treasuryFeeRate().mulWadUp(
                interestPaid
            );
        }

        // whatever we couldn't pay back
        uint256 lostAmt = principalOwed > recoveredFunds
            ? principalOwed - recoveredFunds
            : 0;
     ...
     */
    function borrow(
        VerifiableCredential calldata vc//@audit-ok agent address is predictable. so are agentID. can this be exploited through donation of some kind.
    ) external isOpen subjectIsAgentCaller(vc) {//@ok check if agent exist before allow borrow. vc.subject==msg.sender
        ...
        if (account.epochsPaid == 0) {
            ...
            poolRegistry.addPoolToList(vc.subject, id);//@audit add pool can be duplicate. can getAccount be duplicate too? Does agent address is unique
        } else if (account.epochsPaid + maxEpochsOwedTolerance < block.number) {//@audit M this also be checked inside agentPolice. Duplicate function
            ...
            revert PayUp();
        }//@note  everyone can only borrow once per day.
        //@audit M does not calculate old interest after borrow 2nd time again.
        account.principal += vc.value;
        account.save(router, vc.subject, id);//@audit H defaulted account can still borrow again?
        ...
        asset.transfer(msg.sender, vc.value);//@audit-ok in agent contract. H where the heck is non-approved agent function from borrowing?
    }
        ...
        )
    {
        ...
        if (!account.exists()) revert AccountDNE();//startEpoch!= 0 
        // compute a rate based on the agent's current financial situation
        rate = getRate(vc);//@audit-ok H rate is user input. agent do not have to pay interest
        ...
        if (account.epochsPaid < block.number) {//@audit-ok why else case not revert?
            // compute the number of epochs that are owed to get current
            uint256 epochsToPay = block.number - account.epochsPaid;//@note epochsPaid is same as startEpoch when borrow
            // multiply the rate by the principal to get the per epoch interest rate
            // the interestPerEpoch has an extra WAD to maintain precision
            interestPerEpoch = account.principal.mulWadUp(rate);//@ rate is e36
        ...
        }//@audit-ok it is not possible . can call pay twice same block to make interestOwed = 0. skip an if check
        // if the payment is less than the interest owed, pay interest only
        if (vc.value <= interestOwed) {//@audit-ok WTF?
            ...
            account.epochsPaid += epochsForward;//@audit-ok this look absolute banana
    ...
    function deposit(
        address receiver
    ) public payable isOpen returns (uint256 shares) {
    ...
    function previewWithdraw(uint256 assets) public view returns (uint256) {
        if (address(ramp) == address(0)) return 0; //@audit-ok Pool ERC4626 previewWithdraw is different from standard. What happen when you are the last one withdraw. Is there enough share
        // suppose to be asset divUp to share. But here is divDown. So there will be leftover 1e1 share. allow inflated attack
        return ramp.previewWithdraw(assets);//== pool.convertToShares(assets);
    }
    ...
    function previewRedeem(uint256 shares) public view returns (uint256) {
        ...
        return ramp.previewRedeem(shares);//== pool.convertToAssets(shares); // same as standard
    }
     ...
     */
    function harvestToRamp() external requiresRamp {//@audit-ok anyone can call harvestToRamp(). exitDemand always zero unless called by ramp
        // distribute funds to the offramp when we are below the liquidity threshold
        uint256 exitDemand = ramp.totalExitDemand();
        uint256 rampAssets = asset.balanceOf(address(ramp));//@audit-ok what happen when rampAsset/wFIl is zero. exitDemand can also be 1e1
        //@audit-ok Ramp will revert if there is not enough wFIL receive from pool to cover user share .H project allow user to withdraw even when pool does not have enough wFIL or filecoin to cover the share cost.
        // only send funds to the offramp if our liquidityReserves are less than the min liquidity reserve requirement
        if (rampAssets < exitDemand) {//@audit-ok L cant withdraw if rampAsset/ wFIL is bigger than exitDemand/assetsToReceive. Someone can prevent everyone else from withdrawing by transfer large amount of wFIL to ramp. Admin can recover by calling recoverFIL
            ...
            uint256 toDistribute = Math.min(
                ...
                getLiquidAssets()//@audit-ok when pool shutting down. can still withdraw through ramp?
            );//@audit-ok no wFIL can stuck in ERC4626 totalAsset() already cover this. M it is possible to have wFIL stuck here. If ramp have wFIL, and user call harvestToRamp only transfer a portion to make a full assetToReceive.
            asset.approve(address(ramp), toDistribute);
            ramp.distribute(address(this), toDistribute);//@audit-ok why ramp accept distribute smaller amount than expected?
        }//distribute == wFIL transfer from pool to ramp
    }
    ...
    function harvestFees(uint256 harvestAmount) public {
        feesCollected -= harvestAmount; //@audit-ok M totalAssets can change significantly after call harvestFees that can inflate totalAsset or share value
    ...
    function setRamp(IOffRamp _ramp) external onlyOwner {
        ramp = _ramp;//@audit-ok missing zero check. actually owner can prevent withdraw if ramp is zero address or pause contract
    }
     ...
     */
    function jumpStartAccount(
        address receiver,
        uint256 agentID,//unique kekccak agentId,poolId . Upgraded pool still have same ID
        ...
        totalBorrowed += accountPrincipal;//@audit-ok owner jumpStartAccount give agent share of the pool without transfering in token. at somepoint owner must transfering in more token wFIL to pay user
    }
     ...
     */
    function setMaxEpochsOwedTolerance(
        uint256 _maxEpochsOwedTolerance
    ) external onlyOwner {
    ...
    function refreshRoutes() external {
        ...
        agentPolice = GetRoute.agentPolice(router);
        agentFactory = GetRoute.agentFactory(router);
        wFIL = GetRoute.wFIL(router);//@audit-ok M infinity pool missing address ramp refresh. SimpleRamp can change pool address through router but not InfinityPool. Does it still possible to upgrade pool?
    }
    ...
    function _deposit(
        uint256 assets,
        address receiver
    ) internal returns (uint256 lstAmount) {
    ...
    function _subjectIsAgentCaller(
        VerifiableCredential calldata vc
    ) internal view {
        if (vc.subject == 0 || agentFactory.agents(msg.sender) != vc.subject)
            revert Unauthorized();
    }
