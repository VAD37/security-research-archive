...
import {IVault, FeatureSwitches, LpItem} from "../interfaces/IVault.sol";
...
contract VaultFacet is IVault, OnlySelf {
    ...
    function _itemValue(LibVault.AvailableToken storage at, uint256 tokenValue) private view returns (LpItem memory lpItem) {
    ...
    function totalValue() external view override returns (LpItem[] memory lpItems) {
        ...
        lpItems = new LpItem[](tokenAddresses.length);
    ...
    function getTotalValueUsd() public view override returns (int256) {
        LibVault.VaultStorage storage vs = LibVault.vaultStorage();
        uint256 totalValueUsd;
        for (uint256 i = 0; i < vs.tokenAddresses.length;) {
            address tokenAddress = vs.tokenAddresses[i];
            LibVault.AvailableToken storage at = vs.tokens[tokenAddress];
            uint256 price = IPriceFacade(address(this)).getPrice(tokenAddress);
            uint256 valueUsd = price * vs.treasury[tokenAddress] * 1e10 / (10 ** at.decimals);
    ...
    function maxWithdrawAbleUsd() external view returns (int256) {
        ...
        return getTotalValueUsd() + lpUnPnlUsd;
    }
}