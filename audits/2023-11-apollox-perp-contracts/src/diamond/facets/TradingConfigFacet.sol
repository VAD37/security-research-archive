...
contract TradingConfigFacet is ITradingConfig {
    ...
    function setExecutionFeeReceiver(address receiver) external override {
        LibAccessControlEnumerable.checkRole(Constants.ADMIN_ROLE);
        if (receiver == address(0)) revert ZeroAddress();
        LibTradingConfig.TradingConfigStorage storage tcs = LibTradingConfig.tradingConfigStorage();
        tcs.executionFeeReceiver = receiver;
        emit SetExecutionFeeReceiver(receiver);
    }

    function executionFeeReceiver() external view override returns (address) {
        return LibTradingConfig.tradingConfigStorage().executionFeeReceiver;
    }
    ...
    function getPairMaxTpRatio(address pairBase, uint256 leverage_10000) external view override returns (uint24) {
        ...
        if (tpRatios.length == 0) {
            ...
            for (uint256 i = 0; i < tpRatios.length;) {
                MaxTpRatioForLeverage storage tpRatio = tpRatios[i];
