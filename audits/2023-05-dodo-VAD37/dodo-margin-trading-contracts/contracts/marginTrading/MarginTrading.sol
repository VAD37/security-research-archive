...
contract MarginTrading is OwnableUpgradeable, IMarginTrading, IFlashLoanReceiver {
    ...
    function executeFlashLoans(
        ...
        // the various assets to be flashed   *   0 -> Don't open any debt, just revert if funds can't be transferred from the receiver

        // the amount to be flashed for each asset *   1 -> Open debt at stable rate for the value of the amount flash-borrowed to the `onBehalfOf` address

        // 0 = no debt, 1 = stable, 2 = variable    *   2 -> Open debt at variable rate for the value of the amount flash-borrowed to the `onBehalfOf` address
    ...
    ) external override onlyLendingPool returns (bool) {//@audit H griefing attack https://ethereum.stackexchange.com/questions/92391/explain-griefing-attack-on-aave-flash-loan/92457#92457
    ...
    function _closetrade(
        ...
        if (_swapParams.length > 0) {
            ...
            (bool success,) = _swapAddress.call(_swapParams);//@audit grief this can be aave action do anything they want
            require(success, "dodoswap fail");
        }
        uint256[] memory _tradeAmounts = new uint256[](_tradeAssets.length);
        if (_flag == 2) {
    ...
    function _lendingPoolRepay(address _repayAsset, uint256 _repayAmt, uint256 _rateMode, uint8 _flag) internal {
        ...
        lendingPool.repay(_repayAsset, _repayAmt, _rateMode, address(this));//1 for stable, 2 for variable
