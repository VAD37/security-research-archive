...
contract MarginTradingFactory is Ownable, IMarginTradingFactory {
    ...
    constructor(address _lendingPool, address _weth, address _DODOApprove, address _template) {
        LendingPool = _lendingPool;//aave lending pool v2 0x7d2768de32b0b80b7a3454c06bdac94a69ddc7a9
        WETH = IWETH(_weth);//@note each margin factory have their own lending pool
    ...
    function multicall(bytes[] calldata data) public payable returns (bytes[] memory results) {//@audit multicall payable with margin trading
    ...
    function createMarginTrading(
        uint8 _flag,//@audit flag can be 0 or 3 too. it just skip the check
        ...
        if (depositParams.length > 0) {
            ...
            if (_depositFlag == 2) {
                depositMarginTradingETH(marginTrading, false, uint8(1));//this affected by mutilcall. flag do nothing
            }
        }
    ...
    function depositMarginTradingETH(address _marginTradingAddress, bool _margin, uint8 _flag) public payable {
        ...
        if (_margin) {
            IMarginTrading(_marginTradingAddress).lendingPoolDeposit(address(WETH), msg.value, _flag);//@audit deposit external call
        }
    ...
    function _depositMarginTradingERC20(
        address _marginTradingAddress,// new init marginTrading contract address
        address _marginAddress,//_tokenAddres
        uint256 _marginAmount,//_depositAmount
        bool _margin,// false
        uint8 _flag//1
    ) internal {
        require(IMarginTrading(_marginTradingAddress).user() == msg.sender, "factory:caller is not the user");
        ...
        if (_margin) {
            IMarginTrading(_marginTradingAddress).lendingPoolDeposit(_marginAddress, _marginAmount, _flag);//@audit deposit external call
        }
