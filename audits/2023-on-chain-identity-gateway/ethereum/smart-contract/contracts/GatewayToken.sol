    ...
    ChargeHandler
{
    ...
    function initialize(
        ...
        address _superAdmin,//0x74489ac99ae3ce1428ce8c8dbe374cffc009081e
        address _flagsStorage,//0xe25c6c4fcaec2010bae4b05d863be2a5a5db940f
        address[] calldata _trustedForwarders//0x7d648a11AE84487526b7474b6A2BC0BE3a299BBb //FlexibleNonceForwarder
    ...
    function createNetwork(uint network, string calldata name, bool daoGoverned, address daoManager) external virtual {
        if (bytes(_networks[network]).length != 0) {//@audit is it possible to set string name with length zero?
    ...
    function renameNetwork(uint network, string calldata name) external virtual {
        if (bytes(_networks[network]).length == 0) {//@audit networks length can be reset to zero. People can steal role later as role is shared
