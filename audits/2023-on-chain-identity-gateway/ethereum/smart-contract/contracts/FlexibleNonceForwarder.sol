...
contract FlexibleNonceForwarder is IForwarder, EIP712, ReentrancyGuard {
    ...
    struct SigsForNonce {
        mapping(bytes => bool) sigs;//@audit this look like memory collision chance
    }
    ...
    uint256 private immutable _blockAgeTolerance;//100 blocks
    ...
    constructor(uint256 blockAgeTolerance) EIP712("FlexibleNonceForwarder", "0.0.1") {
        _blockAgeTolerance = blockAgeTolerance;//0x64 ==100 blocks
    }
    ...
    function execute(
        ...
        (bool success, bytes memory returndata) = req.to.call{gas: req.gas, value: req.value}(//@audit anyone can steal token from forwarder if eth stuck
            abi.encodePacked(req.data, req.from)
        );

        // Forward the revert message from the call if the call failed.
        if (success == false) {//@audit-ok forwarder have not been updated. this false does not revert on mainnet
            // solhint-disable-next-line no-inline-assembly//@0x7d648a11AE84487526b7474b6A2BC0BE3a299BBb
            assembly {
                revert(add(returndata, 32), mload(returndata))
            }//@audit-ok old forwarder still kicking. execute does not revert on false? What can I do with this?
        }//@audit-ok nonces does not share between forward. replay attack is possible

        // Validate that the relayer has sent enough gas for the call.
        // See https://ronan.eth.limo/blog/ethereum-gas-dangers/
        if (gasleft() <= req.gas / 64) {//@audit-ok on mainnet this is 63
    ...
    function _verifyFlexibleNonce(ForwardRequest calldata req, bytes calldata signature) internal {
        ...
        if (_nonces[req.from].currentNonce == req.nonce) {//@audit nonce on mainnet is incremental for every transaction. user can make custom nonce to bypass this
        ...
        _nonces[req.from].sigsForNonce[req.nonce].sigs[signature] = true;//@audit same nonce can have multiple signature
    }
