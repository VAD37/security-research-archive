...
contract ROUSGFactory is IMulticall {
  ...
  function deployRebasingOUSG(
    ...
    rOUSGProxyAdmin = new ProxyAdmin();//@audit-ok deploy only happen once rOUSG is not beacon proxy. This will require manually upgrade every deployed contract
    rOUSGProxy = new TokenProxy(//@audit-ok admin can read getImplementation from proxy
    ...
    rOUSGProxyAdmin.transferOwnership(guardian);//@proxy admin transfered to guardian. why still need multiexcall
   ...
   */
  function multiexcall(//@audit-ok L admin multicall allow token ETH stuck
