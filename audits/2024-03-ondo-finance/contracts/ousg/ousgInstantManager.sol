  ...
  IMulticall
{
  ...
  uint256 public immutable decimalsMultiplier;//@1e12 == 10**12
  ...
  uint256 public redeemFee = 0; //@ < 200
    ...
    returns (uint256 rousgAmountOut)
  {
    ...
    rousgAmountOut = rousg.getROUSGByShares(//@amount = (_shares * getOUSGPrice()) / (1e18 * OUSG_TO_ROUSG_SHARES_MULTIPLIER);
      ousgAmountOut * OUSG_TO_ROUSG_SHARES_MULTIPLIER
    );//@ share = (_rOUSGAmount * 1e18 * OUSG_TO_ROUSG_SHARES_MULTIPLIER) / getOUSGPrice();
    rousg.transfer(msg.sender, rousgAmountOut);//@audit InstantManager can receive share. what does this do to periphery contract like this
  ...
  function _mint(
    ...
    _checkAndUpdateInstantMintLimit(usdcAmountIn);//@prevent bring in too much USDC
  ...
  function redeemRebasingOUSG(
    uint256 rousgAmountIn//USD pegged
  )
    external
    override
    nonReentrant
    ...
    returns (uint256 usdcAmountOut)
  {
    ...
    rousg.transferFrom(msg.sender, address(this), rousgAmountIn);//@transfer USD converted to share.
    rousg.unwrap(rousgAmountIn);//@unwrap burn share from this contract and get some ousg  @audit-ok unweap prevent 0 .L allow redeem zero amount
  ...
  function _redeem(
    ...
    );//@audit what happen when ousg price drop so much and user cant no longer redeem?
    ...
    if (usdcAmountToRedeem >= minBUIDLRedeemAmount) {
      ...
      _redeemBUIDL(minBUIDLRedeemAmount);//@audit-ok redeem whole things above.how 250_000e6 USDC is enough to cover redeem?
