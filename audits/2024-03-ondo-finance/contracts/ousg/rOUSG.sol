  ...
  IERC20MetadataUpgradeable
{
  ...
  }//@audit-ok QA1 missing init of other openzeppelin upgradable contracts
  ...
  function __rOUSG_init_unchained(
    ...
    _grantRole(CONFIGURER_ROLE, guardian);//@audit-ok nothing wrong with access control setup yet
  }
  ...
  function totalSupply() public view returns (uint256) {
    return
      (totalShares * getOUSGPrice()) / (1e18 * OUSG_TO_ROUSG_SHARES_MULTIPLIER);//@mul 10_000 = 1e4
  }//@audit-ok oracle return price range from 99e18-101e18 with stale price acceptable  who is oracle? ERC4626 price depend on this
  ...
  function balanceOf(address _account) public view returns (uint256) {
    return//@share = (_rOUSGAmount * 1e18 * OUSG_TO_ROUSG_SHARES_MULTIPLIER) / getOUSGPrice();
      (_sharesOf(_account) * getOUSGPrice()) /
      (1e18 * OUSG_TO_ROUSG_SHARES_MULTIPLIER);
  }//@amount = (_shares * getOUSGPrice()) / (1e18 * OUSG_TO_ROUSG_SHARES_MULTIPLIER);
  ...
  function transfer(address _recipient, uint256 _amount) public returns (bool) {
    _transfer(msg.sender, _recipient, _amount);//@audit M cannot use ERC20 to transfer all token due to oracle price change delay
    return true;
  }
  ...
  function transferFrom(
    address _sender,
    address _recipient,
    uint256 _amount//@rOUSG token convert to share later to transfer
  ) public returns (bool) {
    uint256 currentAllowance = allowances[_sender][msg.sender];
    require(currentAllowance >= _amount, "TRANSFER_AMOUNT_EXCEEDS_ALLOWANCE");

    _transfer(_sender, _recipient, _amount);//@audit-ok L transferFrom to self
  ...
  ) public returns (uint256) {//@audit-ok L can transfer share to self
  ...
  function wrap(uint256 _OUSGAmount) external whenNotPaused {
    ...
    uint256 ousgSharesAmount = _OUSGAmount * OUSG_TO_ROUSG_SHARES_MULTIPLIER;//@audit-ok OUSG converted directly into share because OUSG is already share H wrap token does not use oracle?
    _mintShares(msg.sender, ousgSharesAmount);//@share = amount * OUSG_TO_ROUSG_SHARES_MULTIPLIER;
  ...
  }//@audit wrap using OUSG token as input, but unwrap use stableCoin USD rOUSG as input
   ...
   *
   * @dev KYC checks implicit in OUSG Transfer
   *///@audit you cant unwrap using share for 100% unwrap
  function unwrap(uint256 _rOUSGAmount) external whenNotPaused {//@audit-ok this is USD price convert to share.oracle price must return 1e18 otherwise wrap and unwrap will allow steal token from contract
  ...
  function _transfer(
    address _sender,
    address _recipient,
    uint256 _amount//@ this amount convert from share when call balanceOf. _amount = share * oraclePrice / (1e18 * 1e4)
  ) internal {
    uint256 _sharesToTransfer = getSharesByROUSG(_amount);//@ share = amount * 1e18 * OUSG_TO_ROUSG_SHARES_MULTIPLIER) / oraclePrice
    _transferShares(_sender, _recipient, _sharesToTransfer);//@share = share * oraclePrice / (1e18 * 1e4) * 1e18 * 1e4 / oraclePrice = share
   ...
   */
  function _approve(
    address _owner,//@transfrom sender 
    ...
    allowances[_owner][_spender] = _amount;//@audit-ok can approve self too
  ...
  function _mintShares(
    address _recipient,
    uint256 _sharesAmount//@_OUSGAmount * OUSG_TO_ROUSG_SHARES_MULTIPLIER
  ...
  function _beforeTokenTransfer(
    ...
    }//@audit-ok token transfer also check KYC of msg.sender but not txt.origin?
