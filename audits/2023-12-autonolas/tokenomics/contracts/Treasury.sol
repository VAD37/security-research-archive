...
contract Treasury is IErrorsTokenomics {
    ...
    receive() external payable {
        ...
        ETHOwned = uint96(amount);//@audit ETHOwned does not update with eth transfer through destroyable means.
    ...
    function changeManagers(address _tokenomics, address _depository, address _dispenser) external {
        ...
        if (_tokenomics != address(0)) {
            tokenomics = _tokenomics;//@current 0xc096362fa6f4A4B1a9ea68b1043416f3381ce300 proxy
            emit TokenomicsUpdated(_tokenomics);
        }
        // Change Depository contract address
        if (_depository != address(0)) {
            depository = _depository;//@ current 0xfF8697d8d2998d6AA2e09B405795C6F4BEeB0C81
            emit DepositoryUpdated(_depository);//@previous 	0x52A043bcebdB2f939BaEF2E8b6F01652290eAB3f
        }
        // Change Dispenser contract address
        if (_dispenser != address(0)) {
            dispenser = _dispenser;//@current 0xeED0000fE94d7cfeF4Dc0CA86a223f0F603A61B8
