...
contract veOLAS is IErrors, IVotes, IERC20, IERC165 {//@note voting escrow token veOLAS is ERC20 but have no abilities of ERC20. so it is just a voting contract not ERC20
    enum DepositType {
        DEPOSIT_FOR_TYPE,
        CREATE_LOCK_TYPE,
    ...
    function _depositFor(
        ...
        uint256 amount,//@user nonzero
        uint256 unlockTime,//@user rounded down limit between 4 years from now
        LockedBalance memory lockedBalance,//@memory
        DepositType depositType//@event only
    ) internal {
        ...
        if (unlockTime > 0) {
            lockedBalance.endTime = uint64(unlockTime);//@audit locktime have no minimum limit. so it can be 1 seconds.
        }//@audit-ok M createLockFor can be used to reset locktime to smaller amount by overflowing value to smaller value than previous locktime.
    ...
    /// @dev Deposits `amount` tokens for `account` and locks for `unlockTime`.
    /// @notice Tokens are taken from `msg.sender`'s balance.
    /// @param account Account address. @user
    /// @param amount Amount to deposit. @user non zero
    /// @param unlockTime Time when tokens unlock, rounded down to a whole week. @user overflowable exploit
    function _createLockFor(address account, uint256 amount, uint256 unlockTime) private {
        ...
        unchecked {
            unlockTime = ((block.timestamp + unlockTime) / WEEK) * WEEK;//@audit-ok revert if final time < timestamp. unlocktime can be overflowable.
        }
        LockedBalance memory lockedBalance = mapLockedBalances[account];
        // The locked balance must be zero in order to start the lock
        if (lockedBalance.amount > 0) {//@audit-ok createLockFor can replace time when amount == 0
    ...
    function increaseAmount(uint256 amount) external {
        ...
        // Check the lock expiry
        if (lockedBalance.endTime < (block.timestamp + 1)) {//@note revert all lock operation when locked expired timestamp > endTime but not ==
            revert LockExpired(msg.sender, lockedBalance.endTime, block.timestamp);
        }
    ...
    function increaseUnlockTime(uint256 unlockTime) external {//@audit-ok increaseUnlockTime require minimum 1 week increase.
        LockedBalance memory lockedBalance = mapLockedBalances[msg.sender];
        // Cannot practically overflow because block.timestamp + unlockTime (max 4 years) << 2^64-1
        unchecked {
    ...
    function withdraw() external {//@audit can withdraw zero amount
        LockedBalance memory lockedBalance = mapLockedBalances[msg.sender];
