...
import "./IERC721.sol";
//The Minter contract is used to mint an ERC721 token for a collection on the Core contract based on certain requirements that are set prior to the minting process.
contract NextGenMinterContract is Ownable {
    ...
    struct collectionPhasesDataStructure {
        ...
        bytes32 merkleRoot;//2nd @audit-ok why second collection phase setup later?
    ...
    function setCollectionCosts(uint256 _collectionID, uint256 _collectionMintCost, uint256 _collectionEndMintCost, uint256 _rate, uint256 _timePeriod, uint8 _salesOption, address _delAddress) public CollectionAdminRequired(_collectionID, this.setCollectionCosts.selector) {
        require(gencore.retrievewereDataAdded(_collectionID) == true, "Add data");//@setCollectionData() true
        collectionPhases[_collectionID].collectionMintCost = _collectionMintCost;
        collectionPhases[_collectionID].collectionEndMintCost = _collectionEndMintCost;//@audit L no overflow check for end mint cost
        collectionPhases[_collectionID].rate = _rate;
        collectionPhases[_collectionID].timePeriod = _timePeriod;//_timePeriod must smaller than sale time
    ...
    //@CollectionAdminRequired //@audit-ok M collection sale and item can change during sale and after sale end.
    function setCollectionPhases(uint256 _collectionID, uint _allowlistStartTime, uint _allowlistEndTime, uint _publicStartTime, uint _publicEndTime, bytes32 _merkleRoot) public CollectionAdminRequired(_collectionID, this.setCollectionPhases.selector) {
        ...
        collectionPhases[_collectionID].allowlistEndTime = _allowlistEndTime;//@audit L lack time verfication and public start and end time. should use timePeriod to verify otherwise tDIFF will just be zero
        collectionPhases[_collectionID].merkleRoot = _merkleRoot;
        collectionPhases[_collectionID].publicStartTime = _publicStartTime;
        collectionPhases[_collectionID].publicEndTime = _publicEndTime;//@audit-ok M can change mint time after sale ended
    }

    // airdrop function
    //@FunctionAdminRequired @note aidrop only for admin
    function airDropTokens(address[] memory _recipients, string[] memory _tokenData, uint256[] memory _saltfun_o, uint256 _collectionID, uint256[] memory _numberOfTokens) public FunctionAdminRequired(this.airDropTokens.selector) {
        require(gencore.retrievewereDataAdded(_collectionID) == true, "Add data");//@setCollectionData() true and collection exist. setup byCollection or artist maybe
        uint256 collectionTokenMintIndex;//@audit-ok L no length check
        for (uint256 y=0; y< _recipients.length; y++) {//index = (_collectionID * 10000000000) + mintedTokens + mint amount - 1
            ...
            require(collectionTokenMintIndex <= gencore.viewTokensIndexMax(_collectionID), "No supply");//@does index max no overeach max (_collectionID * 10000000000) + _collectionTotalSupply - 1
            for(uint256 i = 0; i < _numberOfTokens[y]; i++) {
                uint256 mintIndex = gencore.viewTokensIndexMin(_collectionID) + gencore.viewCirSupply(_collectionID);
                gencore.airDropTokens(mintIndex, _recipients[y], _tokenData[y], _saltfun_o[y], _collectionID);//@audit-ok L airdropTokens does not have abiltities to change tokenData unique for each token. This is a bug
            }
        }
    }
    ...
    function mint(uint256 _collectionID, uint256 _numberOfTokens, uint256 _maxAllowance, string memory _tokenData, address _mintTo, bytes32[] calldata merkleProof, address _delegator, uint256 _saltfun_o) public payable {
        require(setMintingCosts[_collectionID] == true, "Set Minting Costs");//@ collection must exist
        uint256 col = _collectionID;//@audit-ok where collectionID verification go?
        ...
        if (block.timestamp >= collectionPhases[col].allowlistStartTime && block.timestamp <= collectionPhases[col].allowlistEndTime) {
            phase = 1;//@phase one: presale allowlist timer set by collection owner
            bytes32 node;//@audit-ok merkleproof leaf is node
            if (_delegator != 0x0000000000000000000000000000000000000000) {
                ...
                require(_maxAllowance >= gencore.retrieveTokensMintedALPerAddress(col, msg.sender) + _numberOfTokens, "AL limit");//@audit-ok H can bypass maxAllowance limit quite easy with reentrancy
                mintingAddress = msg.sender;
            }
            require(MerkleProof.verifyCalldata(merkleProof, collectionPhases[col].merkleRoot, node), 'invalid proof');//@audit-ok H merkleroot check does not spend leaf after verify allow reuse merkle root to infinite minting NFT
        } else if (block.timestamp >= collectionPhases[col].publicStartTime && block.timestamp <= collectionPhases[col].publicEndTime) {
            phase = 2;//@public phase 2
            require(_numberOfTokens <= gencore.viewMaxAllowance(col), "Change no of tokens");//@note max token Mint per execution this can be changed during public sale. so is publicEndTime
            require(gencore.retrieveTokensMintedPublicPerAddress(col, msg.sender) + _numberOfTokens <= gencore.viewMaxAllowance(col), "Max");
            mintingAddress = msg.sender;
            tokData = '"public"';//@audit-ok H tokData set to public will cause javascript error
        } else {
            revert("No minting");
        }
        uint256 collectionTokenMintIndex;//@ viewMaxAllowance == maxCollectionPurchases //@ viewCirSupply == collectionCirculationSupply //@ setfinalSupply fix collectionCirculationSupply
        collectionTokenMintIndex = gencore.viewTokensIndexMin(col) + gencore.viewCirSupply(col) + _numberOfTokens - 1;
        require(collectionTokenMintIndex <= gencore.viewTokensIndexMax(col), "No supply");//@ max index  == collectionTotalSupply//@note cannot mint anymore after setFinalSupply called. maxIndex fixed
        require(msg.value >= (getPrice(col) * _numberOfTokens), "Wrong ETH");//@getPrice is complicated. //@ no refund for over sending ETH
        for(uint256 i = 0; i < _numberOfTokens; i++) {
            uint256 mintIndex = gencore.viewTokensIndexMin(col) + gencore.viewCirSupply(col);
            gencore.mint(mintIndex, mintingAddress, _mintTo, tokData, _saltfun_o, col, phase);//@audit-ok safemint have reentrancy
        }//@audit-ok H getPrice return wrong price for saleOptions 3. it use fixed price for all bulk sale.
        collectionTotalAmount[col] = collectionTotalAmount[col] + msg.value;
        // control mechanism for sale option 3.//@note saleOptions 3 increase price infinitely for each sale. only for auction feature
        if (collectionPhases[col].salesOption == 3) {
            uint timeOfLastMint;
            if (lastMintDate[col] == 0) {
                // for public sale set the allowlist the same time as publicsale
                timeOfLastMint = collectionPhases[col].allowlistStartTime - collectionPhases[col].timePeriod;//@audit M allowList start time need to before public sale otherwise it cause issue with saleOptions 3
            } else {
            ...
            require(tDiff>=1 && _numberOfTokens == 1, "1 mint/period");//@note sale3 auction only allow mint 1 token.
            lastMintDate[col] = collectionPhases[col].allowlistStartTime + (collectionPhases[col].timePeriod * (gencore.viewCirSupply(col) - 1));
        }//@ first mint. lastMintDate == allowlistStartTime. increase by timePeriod for each new mint.
    }

    // burn to mint function (does not require contract approval)
    //@note burn to mint. swap NFT only work for public sale. pair NFT choosen by admin
    function burnToMint(uint256 _burnCollectionID, uint256 _tokenId, uint256 _mintCollectionID, uint256 _saltfun_o) public payable {
        require(burnToMintCollections[_burnCollectionID][_mintCollectionID] == true, "Initialize burn");//@note burn work by burn useless collection NFT for another collection NFT
    ...
    function mintAndAuction(address _recipient, string memory _tokenData, uint256 _saltfun_o, uint256 _collectionID, uint _auctionEndTime) public FunctionAdminRequired(this.mintAndAuction.selector) {
        ...
        uint256 collectionTokenMintIndex;//@audit-ok L mintAndAuction should check if collection time and start time exist first.
        ...
        mintToAuctionData[mintIndex] = _auctionEndTime;//@audit M review how auction work for sale option 3 and sale option 2
    ...
    function getPrice(uint256 _collectionId) public view returns (uint256) {
        uint tDiff;
        if (collectionPhases[_collectionId].salesOption == 3) {//@audit-ok sale option can be change during sale period
            ...
            if (collectionPhases[_collectionId].rate > 0) {
                return collectionPhases[_collectionId].collectionMintCost + ((collectionPhases[_collectionId].collectionMintCost / collectionPhases[_collectionId].rate) * gencore.viewCirSupply(_collectionId));//@audit-ok M division order rate could be 0.1 ETH 
            } else {
                return collectionPhases[_collectionId].collectionMintCost;
            }//@audit-ok M saleOption2 use time limit between allowStart and public end time. Other time it return default price. can still mint right at the end of public time
            ...
            // decreases exponentially every time period //@audit-ok H should use >= and <=. mint still work right at equal start and end time
            ...
            tDiff = (block.timestamp - collectionPhases[_collectionId].allowlistStartTime) / collectionPhases[_collectionId].timePeriod;//@note getPrice support change price after sale period end
            uint256 price;
            uint256 decreaserate;//@audit-ok H mathetically decreaseRate always zero. the price drop in linear fashion
            if (collectionPhases[_collectionId].rate == 0) {
                price = collectionPhases[_collectionId].collectionMintCost / (tDiff + 1);//@ price divide by 1 then 2,3,4,5,6, ...
            ...
            } else { //decreaserate = (price - mintCost / (tD + 2) ) / timePeriod * (block.timestamp - (tD * timePeriod) - allowlistStartTime)
                if (((collectionPhases[_collectionId].collectionMintCost - collectionPhases[_collectionId].collectionEndMintCost) / (collectionPhases[_collectionId].rate)) > tDiff) {
                    price = collectionPhases[_collectionId].collectionMintCost - (tDiff * collectionPhases[_collectionId].rate);//@audit-ok H no rate protection? prevent revert
                } else {//@audit-ok L safety check rate < mintCost - endMintCost
