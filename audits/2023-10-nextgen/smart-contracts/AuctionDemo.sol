...
//The auctionDemo smart contract holds the current auctions after the mintAndAuction functionality is called. Users can bid on a token and the highest bidder can claim the token after an auction finishes.
contract auctionDemo is Ownable {
    ...
    modifier WinnerOrAdminRequired(uint256 _tokenId, bytes4 _selector) {//@audit this can revert if auctionInfoData == false. not inited 
    ...
    function participateToAuction(uint256 _tokenid) public payable {
        ...
        auctionInfoStru memory newBid = auctionInfoStru(msg.sender, msg.value, true);//@audit H auction does not revert when token already claimed right at block.timestamp equal endtime.
        auctionInfoData[_tokenid].push(newBid);//@audit auction not return money to user after failure?
    }
    ...
    function cancelBid(uint256 _tokenid, uint256 index) public {//@audit-ok M ETH cannot manually return after auction end.
