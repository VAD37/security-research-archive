...
//The RandomizerVRF contract is responsible for generating a random hash for each token during the minting process using the Chainlink's VRF service.
contract NextGenRandomizerVRF is VRFConsumerBaseV2, Ownable {
    ...
    function requestRandomWords(uint256 tokenid) public {
        ...
        uint256 requestId = COORDINATOR.requestRandomWords(
            keyHash,
            s_subscriptionId,
            requestConfirmations,//@ max request 200 
            callbackGasLimit,  //@audit-ok callback gaslimit 40k is so little. it might not have enough.
            numWords//@max num words 500
        );
        tokenToRequest[tokenid] = requestId;
        requestToToken[requestId] = tokenid;
    }

    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {//@audit-ok VRF random words is keccak256 of string uint256(keccak256(abi.encode(randomness, i)))
        gencoreContract.setTokenHash(tokenIdToCollection[requestToToken[_requestId]], requestToToken[_requestId], bytes32(abi.encodePacked(_randomWords,requestToToken[_requestId])));//@audit-ok VRF hash forget keccak256
