...
// custom randomizer The RandomizerNXT contract is responsible for generating a random hash for each token during the minting process using the NextGen's proposed approach.
contract NextGenRandomizerNXT {
    ...
    function calculateTokenHash(uint256 _collectionID, uint256 _mintIndex, uint256 _saltfun_o) public {
        ...
        gencoreContract.setTokenHash(_collectionID, _mintIndex, hash);//@audit-ok XRandomizer return same random on same block
    }
