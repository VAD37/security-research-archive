...
import "./Ownable.sol";
//The Admin contract is responsible for adding or removing global or function-based admins who are allowed to call certain functions in both the Core and Minter contracts.
contract NextGenAdmins is Ownable{
    ...
    modifier AdminRequired {
      require((adminPermissions[msg.sender] == true) || (_msgSender()== owner()), "Not allowed");//@audit L 2 different context
      _;
    }
    ...
    function registerAdmin(address _admin, bool _status) public onlyOwner {
        adminPermissions[_admin] = _status;//@note owner can add new admin contract only. admin can allow func selector or collection. 1 admin can DOS other admin function too.
    }
