...
abstract contract ERC20Gauges is ERC20MultiVotes, ReentrancyGuard, IERC20Gauges {
    ...
    function userUnusedVotes(address user) public view override returns (uint256) {
        return super.userUnusedVotes(user) - getUserWeight[user];//@audit super.userUnusedVotes is very different from balanceOf[user]
    }
    ...
    function _incrementGaugeWeight(address user, address gauge, uint112 weight, uint32 cycle) internal {
        ...
        IBaseV2Gauge(gauge).accrueBribes(user);//@audit accrure bribe here is new. what the heck is BaseV2Gauge
        ...
        returns (uint112 newUserWeight)
    {
        ...
        if (newUserWeight > getVotes(user)) revert OverWeightError();//@audit need to check if getVotes same as balanceOf
