...
abstract contract ERC4626MultiToken is ERC20, ReentrancyGuard, IERC4626MultiToken {
    ...
    function receiveAssets(uint256[] memory assetsAmounts) private {
        ...
        for (uint256 i = 0; i < length;) {
            assets[i].safeTransferFrom(msg.sender, address(this), assetsAmounts[i]);//@audit do fuzzing assembly code dirty bit cause issue here?
        ...
        returns (uint256 shares)
    {
        ...
        require((shares = previewDeposit(assetsAmounts)) != 0, "ZERO_SHARES");//@audit can duplicate token. What happen then?
    ...
    function convertToShares(uint256[] calldata assetsAmounts) public view virtual returns (uint256 shares) {
        ...
        for (uint256 i = 0; i < length;) {
            uint256 share = assetsAmounts[i].mulDiv(_totalWeights, weights[i]); //@audit share here is not ERC4626 fomular. This ignore whatever Assets worth in the pool
            if (share < shares) shares = share;//share = amount * supply / Totalasset
            unchecked {
                i++;
            }
        }
