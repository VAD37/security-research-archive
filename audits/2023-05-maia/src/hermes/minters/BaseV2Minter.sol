...
contract BaseV2Minter is Ownable, IBaseV2Minter {
    ...
    /// @inheritdoc IBaseV2Minter
    uint256 public override activePeriod;//start time rounded to week
    ...
    function weeklyEmission() public view returns (uint256) {
        return (circulatingSupply() * tailEmission) / base;//@ok why the more HERMES burned, the more HERMES minted weekly?
    }
    ...
    function updatePeriod() public returns (uint256) {
        ...
        if (block.timestamp >= _period + week && initializer == address(0)) {
            ...
            uint256 newWeeklyEmission = weeklyEmission();//0.02e18 %
            weekly += newWeeklyEmission;
            uint256 _circulatingSupply = circulatingSupply();

            uint256 _growth = calculateGrowth(newWeeklyEmission);// burn * new mint amount / total supply
            uint256 _required = _growth + newWeeklyEmission;
            /// @dev share of newWeeklyEmission emissions sent to DAO.
            uint256 share = (_required * daoShare) / base;//@note DAO share is 10% on top of new emission. the emission % is more than 2%. 2% + 2 * 0.75  = 3.5% * 0.1
            _required += share;// requried = _growth + newWeeklyEmission + share
            uint256 _balanceOf = underlying.balanceOf(address(this));
            if (_balanceOf < _required) {
                HERMES(underlying).mint(address(this), _required - _balanceOf);
            }

            underlying.safeTransfer(address(vault), _growth);//@audit is it possible for _required > growth+share

            if (dao != address(0)) underlying.safeTransfer(dao, share);//@audit M minting dao share even without DAO address
