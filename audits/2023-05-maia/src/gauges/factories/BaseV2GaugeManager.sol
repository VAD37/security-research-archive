...
contract BaseV2GaugeManager is Ownable, IBaseV2GaugeManager {
    ...
    function changebHermesGaugeOwner(address newOwner) external onlyAdmin {
        ...
        bHermesGaugeBoost.transferOwnership(newOwner);//@audit owner != admin. Admin have power to change bHermes Gauge owner

        emit ChangedbHermesGaugeOwner(newOwner);//bHermes Boost and Gauge Owner can do add/remove gauge
    }
