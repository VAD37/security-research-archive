...
contract FlywheelBribeRewards is FlywheelAcummulatedRewards, IFlywheelBribeRewards {
    ...
    function getNextCycleRewards(ERC20 strategy) internal override returns (uint256) {
        return rewardsDepots[strategy].getRewards();//@audit M FlywheelBribeRewards frontrun skip cycle. rewards address might not be the same.
    }

    /// @inheritdoc IFlywheelBribeRewards
    function setRewardsDepot(RewardsDepot rewardsDepot) external {//@ok - not missing onlyFlywheel. only _flywheel can call getNextCycleRewards
        /// @dev Anyone can call this, whitelisting is handled in FlywheelCore
        rewardsDepots[ERC20(msg.sender)] = rewardsDepot;//@audit R why only Bribe though? Whitelist by flywheel core? anyone can fake contracts depots.
