...
contract FlywheelGaugeRewards is Ownable, IFlywheelGaugeRewards {
    ...
    ERC20Gauges public immutable override gaugeToken;//@new
    ...
    /// @inheritdoc IFlywheelGaugeRewards
    function getAccruedRewards() external returns (uint256 accruedRewards) {//@audit what is the consequence of allowing anyone to transfer rewards balance to flywheel
        /// @dev Update minter cycle and queue rewars if needed.
        minter.updatePeriod();//@new

        QueuedRewards memory queuedRewards = gaugeQueuedRewards[ERC20(msg.sender)];//@audit can zero gaugeQueuedRewards bypass anything?
