...
abstract contract FlywheelCore is Ownable, IFlywheelCore {
    ...
    function _addStrategyForRewards(ERC20 strategy) internal {
        ...
        strategyIndex[strategy] = ONE;//@audit strategyIndex missing blockTimestamp
    ...
    /// @inheritdoc IFlywheelCore
    mapping(ERC20 => uint256) public strategyIndex;//@audit Flywheel replace strategyState with strategyIndex . Missing block.timestamp when strategy is added. This is delegate to booster and rewarder
    ...
    function accrueStrategy(ERC20 strategy, uint256 state) private returns (uint256 rewardsIndex) {
        ...
        if (strategyRewardsAccrued > 0) {
            ...
            if (supplyTokens != 0) {
                deltaIndex = ((strategyRewardsAccrued * ONE) / supplyTokens).toUint224();//@gas this can be ignored. no need to convert Uint224 to Uint256
            }
    ...
    function _getAccruedRewards(ERC20 strategy) internal virtual returns (uint256);//flywheelRewards.getAccruedRewards(strategy, state.lastUpdatedTimestamp);
}
