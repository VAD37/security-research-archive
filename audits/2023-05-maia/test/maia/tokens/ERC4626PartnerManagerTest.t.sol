...
import { IBaseVault} from "@maia/tokens/ERC4626PartnerManager.sol";
...
contract ERC4626PartnerManagerTest is DSTestPlus {
    ...
    //////////////////////////////////////////////////////////////*/
    function testFactoryManager() public {
        PartnerManagerFactory factory = new PartnerManagerFactory(ERC20(_bHermes), address(this));
        factory.addVault(IBaseVault(address(10)));
        factory.addVault(IBaseVault(address(11)));
        factory.addVault(IBaseVault(address(12)));
        factory.addVault(IBaseVault(address(13)));      
        console2.log("array length: %s", factory.getVaults().length);
        // print console log entire vaults array
        for (uint256 i = 0; i < factory.getVaults().length; i++) {
            IBaseVault v = factory.getVaults()[i];
            console2.log(address(v));
            console2.log(factory.getVaultId(address(v)));
        }
        console2.log(factory.getVaultId(address(11)));
        factory.removeVault(IBaseVault(address(11)));
        console2.log("array length: %s", factory.getVaults().length);
        factory.addVault(IBaseVault(address(14)));
        console2.log(factory.getVaultId(address(14)));
        console2.log(factory.getVaultId(address(13)));
    }
