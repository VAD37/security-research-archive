...
contract BaseV2MinterTest is DSTestPlus {
    ...
    //                          TESTS
    //////////////////////////////////////////////////////////////////
    function testTotalSupplyMineter() public {
        console.log("uint256 max", type(uint256).max);
        baseV2Minter.initialize(flywheelGaugeRewards);
        baseV2Minter.setDaoShare(300);
        baseV2Minter.setTailEmission(100);
        baseV2Minter.updatePeriod();
        rewardToken.mint(address(this),40_000_000 ether);
        rewardToken.mint(address(bHermesToken),80_000_000 ether);
        console.log("Total Supply:", rewardToken.totalSupply());
        console.log("vault Supply:", bHermesToken.totalAssets());
        uint weekCount = 52*5;
        for (uint i = 0; i < weekCount; i++) {
            hevm.warp(block.timestamp + 1 weeks);
            baseV2Minter.updatePeriod();
            console.log("new Supply     :", baseV2Minter.circulatingSupply());
            console.log("weekly Emission:", baseV2Minter.weeklyEmission());
            console.log("growth Emission:", baseV2Minter.calculateGrowth(baseV2Minter.weeklyEmission()));
            console.log("vault Supply   :", bHermesToken.totalAssets());
            //move all token supply to vault so 75% always burned.
        }
    }
