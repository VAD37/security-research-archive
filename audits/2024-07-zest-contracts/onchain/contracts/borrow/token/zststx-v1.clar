...
(define-read-only (get-balance (account principal))
  (let (
    ...
    (if (is-eq current-principal-balance u0)
      ...
      (let (
        (cumulated-balance
          (contract-call? .pool-0-reserve calculate-cumulated-balance ;; .contract-name just mean reading current address different contract deployed. 1 address can hold multiple contracts
            account
            u6 ;; @audit super low decimal for uint128
            asset-addr
...
(define-read-only (get-principal-balance (account principal))
  (ok 
    (+
      (unwrap-panic (contract-call? .zststx get-principal-balance account));;@audit new version upgrade read balance from old version + current new version balance.
      (ft-get-balance zststx account);;@audit is it possible to manipulate to get balance from v0?
    )
  )
)
