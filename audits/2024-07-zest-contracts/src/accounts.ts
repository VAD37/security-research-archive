...
import {
  generateNewAccount,
  generateWallet,
  restoreWalletAccounts, fetchWalletConfig,
...
export async function getWallets() {
  ...
  const borrowerWallet = await generateWallet({
    secretKey:
      "north swim broom surface easy gasp street sibling pond slab electric endorse comfort bargain valley news drive siege fatal sort current blur cover random",
    password: "password",
  });
...
//Print all wallet address and data

(async () => {
  let wallets = await getWallets();
  let mainnet: StacksNetwork = new StacksMainnet();
  console.log(wallets);
  const deployerWallet = wallets["deployer"];
  const lpWallet = wallets["lp"];
  ...
  wallet.accounts.forEach(async (account) => {
    //get stx address
    let stxAddress = getStxAddress({ account: account, transactionVersion: TransactionVersion.Mainnet });
