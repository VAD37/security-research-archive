...
contract DeployV2 is Script, Parameters {
  function run() public returns (Contracts memory) {
    ...
    vm.label(MAINNET_DYAD, "DYAD");
    vm.label(MAINNET_WETH, "WETH");
    vm.label(MAINNET_WSTETH, "WSTETH");
    vm.label(MAINNET_KEROSENE, "KEROSENE");
    vm.label(MAINNET_OWNER, "OWNER");
    vm.label(MAINNET_VAULT_MANAGER_LICENSER, "VAULT_MANAGER_LICENSER");
    vm.label(MAINNET_WETH_ORACLE, "WETH_ORACLE");
    vm.label(MAINNET_CHAINLINK_STETH, "CHAINLINK_STETH");
    
    vm.startBroadcast();  // ----------------------

    Licenser vaultLicenser = new Licenser();//@audit-ok This was pointed out in README. @ L deployment V2 still missing add new licenser vault from DYAD token licenser.
    ...
    kerosineManager.add(address(ethVault));//@audit-ok R did kerosineManager add wrong vault? This should be kerosineVault.
    ...
    vaultLicenser.add(address(unboundedKerosineVault));//@audit-ok H add wrong vault. this allow infinite minting DYAD
