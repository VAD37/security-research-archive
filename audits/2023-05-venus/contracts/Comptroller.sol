    ...
    MaxLoopsLimitHelper
{
    ...
    function initialize(uint256 loopLimit, address accessControlManager) external initializer {
        ...
        _setMaxLoopsLimit(loopLimit);//150
    }
     ...
     * @custom:access Not restricted
     */
    function exitMarket(address vTokenAddress) external override returns (uint256) {//@ok except internal check collateral> borrow
    ...
    function preMintHook(
        ...
        }//@audit preMint hook or mintAllowed() include new code check supplyCaps for vToken. prevent user from mint more cToken if reach maximum amount
     ...
     * @custom:access Not restricted
     */
    function preRedeemHook(//@audit redeem allow zero amount and zero tokens address. https://github.com/compound-finance/compound-protocol/blob/a3214f67b73310d547e00fc578e8355911c9d376/contracts/Comptroller.sol#L322
        address vToken,
        address redeemer,
        ...
        _checkRedeemAllowed(vToken, redeemer, redeemTokens);//@not ok yet . check liquidity

        // Keep the flywheel moving
        uint256 rewardDistributorsCount = rewardsDistributors.length;

        for (uint256 i; i < rewardDistributorsCount; ++i) {//@audit flywheel update all token
            rewardsDistributors[i].updateRewardTokenSupplyIndex(vToken);
    ...
    function preBorrowHook(
        ...
        oracle.updatePrice(vToken);//@audit borrow hook update oracle price but not mint.
        ...
        // Skipping the cap check for uncapped coins to save some gas
        if (borrowCap != type(uint256).max) {//@audit does not use zero as borrowCap. but max number instead
     ...
     * @custom:error PriceError is thrown if the oracle returns an incorrect price for some asset
     */
    function preLiquidateHook(//@ok
        address vTokenBorrowed,
        address vTokenCollateral,//no liquidator address was not used in original
        address borrower,
        uint256 repayAmount,
        bool skipLiquidityCheck//@audit S liquidity check have hook skip liquidity?
    ) external override {
        ...
        if (skipLiquidityCheck || isDeprecated(VToken(vTokenBorrowed))) {
            ...
            return;
        }//@audit can repay more than borrow. This is allowed in Compound

        /* The borrower must have shortfall and collateral > threshold in order to be liquidatable */
        AccountLiquiditySnapshot memory snapshot = _getCurrentLiquiditySnapshot(borrower, _getLiquidationThreshold);//@audit weight have 2 view function

        if (snapshot.totalCollateral <= minLiquidatableCollateral) {//@audit minLiquidatableCollateral might be too costly for WBTC along with closeFactor minimum 5% of borrow amount.
            /* The liquidator should use either liquidateAccount or healAccount */
            revert MinimalCollateralViolated(minLiquidatableCollateral, snapshot.totalCollateral);
        }//@audit include minimum liquidate amount for liquidity check.
        ...
        uint256 maxClose = mul_ScalarTruncate(Exp({ mantissa: closeFactorMantissa }), borrowBalance);//@note close factor to make sure huge portion of liquidity is repay
    ...
    function preSeizeHook(
        ...
        address seizerContract,//@audit seizeHook look vastly different. no cTokenBorrowed == seizerContract?
        address liquidator,
        address borrower
    ) external override {
        ...
        if (seizerContract == address(this)) {
            ...
            }//@note what is diff? ` if (CToken(cTokenCollateral).comptroller() != CToken(cTokenBorrowed).comptroller())
        } else {//@audit S this else case is original comptroller. can we exploit above if?
    ...
    function preTransferHook(
        ...
        for (uint256 i; i < rewardDistributorsCount; ++i) {//@audit with this update reward loop case. is there any external call?
            rewardsDistributors[i].updateRewardTokenSupplyIndex(vToken);
    ...
    function healAccount(address user) external {
        VToken[] memory userAssets = accountAssets[user];//@audit is it possible to bypass exitMarket while borrow still large
    ...
    function setCloseFactor(uint256 newCloseFactorMantissa) external {
        ...
        closeFactorMantissa = newCloseFactorMantissa;//0.05e18
    ...
    function setLiquidationIncentive(uint256 newLiquidationIncentiveMantissa) external {
        ...
        liquidationIncentiveMantissa = newLiquidationIncentiveMantissa;//1e18 test case
    ...
    function supportMarket(VToken vToken) external {
        _checkSenderIs(poolRegistry);//@audit poolRegistry is proxy of something and might not what dev expect
        ...
        for (uint256 i; i < rewardDistributorsCount; ++i) {//@audit new rewards distributor have init market? what about updaterewards?
    ...
    function setActionsPaused(
        ...
        _ensureMaxLoops(marketsCount);//@gas this was not needed
     ...
     * @custom:access Controlled by AccessControlManager
     */
    function setMinLiquidatableCollateral(uint256 newMinLiquidatableCollateral) external {//@audit missing check for maximum of minimum liquidation
    ...
    function addRewardsDistributor(RewardsDistributor _rewardsDistributor) external onlyOwner {
        ...
        rewardsDistributorExists[address(_rewardsDistributor)] = true;//@audit what is rewards distributer
     ...
     */
    function _addToMarket(VToken vToken, address borrower) internal {//@ok
     ...
     */
    function _checkRedeemAllowed(//@ok
        address vToken,
        address redeemer,
        uint256 redeemTokens
    ) internal view {
        ...
        AccountLiquiditySnapshot memory snapshot = _getHypotheticalLiquiditySnapshot(//@not ok yet
            redeemer,
            VToken(vToken),
            redeemTokens,
            0,
     ...
     * @return snapshot Account liquidity snapshot
     */
    function _getHypotheticalLiquiditySnapshot(//@note need double check math on liquidity snapshot. check if snapshot math is  wrong
        address account,
        VToken vTokenModify,
        uint256 redeemTokens,
        uint256 borrowAmount,
        function(VToken) internal view returns (Exp memory) weight // _getCollateralFactor ||  _getLiquidationThreshold
        ...
        for (uint256 i; i < assetsCount; ++i) {
            ...
            (uint256 vTokenBalance, uint256 borrowBalance, uint256 exchangeRateMantissa) = _safeGetAccountSnapshot(
                asset,
                account
            );//exchangeRateMantissa scaled 1e18. some 0.02e18. other much lower

            // Get the normalized price of the asset
            Exp memory oraclePrice = Exp({ mantissa: _safeGetUnderlyingPrice(asset) });//1e18 or 10 ** (18 + 18 - token decimal)

            // Pre-compute conversion factors from vTokens -> usd
            Exp memory vTokenPrice = mul_(Exp({ mantissa: exchangeRateMantissa }), oraclePrice);//weightedVTokenPrice =   markets[address(asset)].liquidationThresholdMantissa || collateralFactorMantissa * vTokenPrice
            Exp memory weightedVTokenPrice = mul_(weight(asset), vTokenPrice);//_getCollateralFactor(asset) * vTokenPrice. //markets[address(asset)].collateralFactorMantissa * vTokenPrice
            //weightedVTokenPrice = tokensToDenom =collateralFactor*exchangeRate*oraclePrice
            // weightedCollateral += weightedVTokenPrice * vTokenBalance
            snapshot.weightedCollateral = mul_ScalarTruncateAddUInt(
                weightedVTokenPrice,
                vTokenBalance,
                snapshot.weightedCollateral//weightedCollateral all in USD. unknown decimals
            );//weightedCollateral is totalCollateral * liquidationThresholdMantissa||collateralFactorMantissa /1e18
        ...
        unchecked {
            if (snapshot.weightedCollateral > borrowPlusEffects) {//weightedCollateral = sumCollateral
    ...
    function _safeGetUnderlyingPrice(VToken asset) internal view returns (uint256) {
        uint256 oraclePriceMantissa = oracle.getUnderlyingPrice(address(asset));//@audit oracle does not return fixed decimals. But each token have their own multiply decimals
        if (oraclePriceMantissa == 0) {//WBTC 8 decimals 21000e28. normal DAI 1e18
     ...
     */
    function _safeGetAccountSnapshot(VToken vToken, address user) //@audit-ok
        internal
        view
        returns (
