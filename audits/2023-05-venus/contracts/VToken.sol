    ...
    TokenErrorReporter
{
    ...
    function initialize(
        ...
        _initialize(
            ...
            name_,
            symbol_,
            decimals_,// 8 for all same as cToken
            admin_,
    ...
    function setProtocolSeizeShare(uint256 newProtocolSeizeShareMantissa_) external {
        ...
        if (newProtocolSeizeShareMantissa_ + 1e18 > liquidationIncentive) {//@audit seize share is new. include check too
    ...
    function _mintFresh(
        address payer,//msg.sender
        address minter,//@audit minter can be any address. this differ from original.
        uint256 mintAmount
    ) internal {
    ...
    function _redeemFresh(
        ...
        if (_getCashPrior() - totalReserves < redeemAmount) {//@audit redeem include total reserve. original does not have this
            revert RedeemTransferOutNotPossible();//underlying.balanceOf(this) > redeemAmount
        }
    ...
    function _doTransferIn(address from, uint256 amount) internal virtual returns (uint256) {
        ...
        token.safeTransferFrom(from, address(this), amount);//@audit can ERC721 have callback?
    ...
    function _initialize(
        ...
        initialExchangeRateMantissa = initialExchangeRateMantissa_;//1e18
        ...
        _setReserveFactorFresh(reserveFactorMantissa_);//30% //@audit _setReserveFactorFresh this is new
        ...
        protocolSeizeShareMantissa = 5e16; // 5% //@note protocolSeizeShareMantissa is new
    ...
    function _exchangeRateStored() internal view virtual returns (uint256) {
        ...
        if (_totalSupply == 0) {
             ...
             */
            return initialExchangeRateMantissa;//@audit new vToken can always be front run. through pool registry
        } else {
            /*
             * Otherwise:
             *  exchangeRate = (totalCash + totalBorrows + badDebt - totalReserves) / totalSupply
             */
            uint256 totalCash = _getCashPrior();//@audit what is badDebt. Why exchange rate included this?
            uint256 cashPlusBorrowsMinusReserves = totalCash + totalBorrows + badDebt - totalReserves;//@audit-ok totalReserve freely controlled by user
