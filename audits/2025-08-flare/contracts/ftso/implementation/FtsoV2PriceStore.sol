    ...
    function publishPrices(FeedWithProof[] calldata _proofs) external {
        ...
        for (uint256 i = 0; i < _proofs.length; i++) {
            ...
            if (i == 0) {
                ...
                require(feed.votingRoundId == votingRoundId, VotingRoundIdMismatch());//@all feed same voting round
            }
            bytes21 feedId = feedIds[i];
            require(feed.id == feedId, FeedIdMismatch());//@cannot duplicate feed
            require(feed.value >= 0, ValueMustBeNonNegative());

            bytes32 feedHash = keccak256(abi.encode(feed));// every feed in list must share same hash
            bytes32 merkleRoot = relay.merkleRoots(ftsoProtocolId, votingRoundId);//@note FTSO oracle, must submit feed same as relay. Relay is unknown but realted to centralized feed
