...
contract HypoVault is ERC20Minimal, Multicall, Ownable {
    ...
    function requestDeposit(uint128 assets) external {
        ...
        queuedDeposit[msg.sender][currentEpoch] += assets;//@epoch only increase by admin, when they call fulfill order

        depositEpochState[currentEpoch].assetsDeposited += assets;

        SafeTransferLib.safeTransferFrom(underlyingToken, msg.sender, address(this), assets);//take user asset and hold it inside queue
