...
contract RamsesV3Pool is IRamsesV3Pool {
    ...
    constructor() {
        PoolStorage.PoolState storage $ = PoolStorage.getStorage();//@note fee is not immutable inside Ram Pool
    ...
    function balance0() internal view returns (uint256) {
        return IERC20(token0).balanceOf(address(this));//@audit UniswapV3 pool.balanceOf allow weird erc20 token with non standard return value.
    }
    ...
    {
        checkTicks(tickLower, tickUpper);//@ok
    ...
    /// @dev init
    function initialize(uint160 sqrtPriceX96) external {//@note factory might never call initialize on pool. prevent getting good tick  and oracle observation
        PoolStorage.PoolState storage $ = PoolStorage.getStorage();
    ...
    function _modifyPosition(//@ same except not using tokenId anymopre
        ...
        position = Position._updatePosition(//@note position logic have major change. no Boost and no NFT
    ...
    function _advancePeriod() public {
        ...
        if ((_blockTimestamp() / 1 weeks) != _lastPeriod) {
            ...
            uint160 secondsPerLiquidityCumulativeX128 = Oracle.newPeriod(
                ...
                _slot0.observationIndex,
                period
            );//@note remove reset boost liquidity from advancePeriod
