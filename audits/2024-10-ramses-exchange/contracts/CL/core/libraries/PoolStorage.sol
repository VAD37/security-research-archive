...
library PoolStorage {
    ...
    struct PoolState {
        ...
        uint128 liquidity;
        mapping(int24 => TickInfo) _ticks;//@tick info include new periodSecondsPerLiquidityOutsideX128. Periods is new
        mapping(int16 => uint256) tickBitmap;
        mapping(bytes32 => PositionInfo) positions;
        Observation[65535] observations;
        mapping(uint256 => PeriodInfo) periods;
        uint256 lastPeriod;
        mapping(bytes32 => PositionCheckpoint[]) positionCheckpoints;//@audit M position Checkpoints allow memory array hopping.
        bool initialized;
        address nfpManager;
    }
