...
library Oracle {
    ...
    function snapshotCumulativesInside(
        ...
        TickInfo storage lower = $._ticks[tickLower];//@ new period not used here
        TickInfo storage upper = $._ticks[tickUpper];

        SnapShot memory snapshot;//@ok same as uniswap
    ...
    /// @param tickUpper The upper tick of the range
    /// @return secondsPerLiquidityInsideX128 The snapshot of seconds per liquidity for the range
    function periodCumulativesInside(//@note period oracle is new. It compare with snapshot from normal ticks.
        uint32 period,
        int24 tickLower,
        int24 tickUpper,
        uint32 _blockTimestamp
