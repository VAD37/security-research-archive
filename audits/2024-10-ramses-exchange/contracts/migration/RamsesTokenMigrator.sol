...
contract RamsesTokenMigrator {
    ...
    function migrateVe(uint256[] calldata _tokenId) external notPaused {
        for (uint256 i = 0; i < _tokenId.length; ++i) {//@audit M no safety check to prevent multisig to migrate its own NFT that was treated as liquid.
            require(msg.sender == _ve.ownerOf(_tokenId[i]), Unauthorized()); //@audit M shoulduse is approved or owner for NFT wallet
            require(
                !partnerNfts[_tokenId[i]],
                "use depositPartnerNft function"
            );
            require(!migratedNft[_tokenId[i]], Migrated());
            (int128 lockAmount, uint256 timeEnd) = _ve.locked(_tokenId[i]);//lockAmount= total old RAM token deposit
            uint256 _amount = uint256(int256(lockAmount));//@
            bool treatAsLiquid = (timeEnd <= unlockCutOff);
            //@ not possible move NFT already attached to gauge/booster or voted
            _ve.transferFrom(msg.sender, multisig, _tokenId[i]);//@transfer old NFT to new owner. clear approval,delegatee
            ...
            if (treatAsLiquid) {
                ...
                _newVe.createLock(msg.sender, _amount);//@burn RAM V3
                /// @dev update mapping
    ...
    function depositPartnerNft(uint256 _tokenId) external notPaused {
        ...
        emit PartnerNftDeposited(msg.sender, _tokenId, tx.origin);//@audit M didnt cache partner beside event?
    }
    ...
    function retrieve(
        ...
        if (_all) {//@audit M failed to clean eth token though. and can still withdraw unused RAM V3.
