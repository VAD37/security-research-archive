    ...
    AccessManagedUpgradeable
{
    ...
    function createGauge(address _pool) external returns (address) {
        ...
        if (gaugeForPool[_pool] != address(0)) {
            revert ActiveGauge(gaugeForPool[_pool]);//@audit-ok  legacyFactory prevent create new unknown pair .M can create Gauge for Pool not exist. or malicious Pool not come from pool deployer
        }
        /// @dev check if the pair exists //@audit-ok new version is createClGauge .M it is only possible to create new Gauge through legacy V2 code
        bool isPair = IPairFactory(legacyFactory).isPair(_pool);//@there is 2 type of pair with unique pool address. Stable and nonStable
        if (!isPair) revert NotPool();//@open permission. anyone can create new pair token

        (, , , , bool stable, address token0, address token1) = IPair(_pool)
            .metadata();
        /// @dev if the msg.sender is not the governor - revert before creation
        if (msg.sender != governor) { //@audit-ok whitelist during init .must PAUSE Voter after init so gov have time to blacklist tokens and migrate old Gauge
            if (isForbidden[token0] || isForbidden[token1])//@audit whitelist might not include new pair token created through old V2 legacy. old version should be paused first.
                revert IsForbidden();
...

        {
            ...
            if (mainGauge == address(0)) {
                ...
                killGauge(_gauge);//@audit this public func is restricted?
            }
        }
