...
contract ProfitManager is CoreRef {
    ...
    address public guild;//@immutable

    /// @notice reference to CREDIT token.
    address public credit;//@note external contract address are immutable. permission is not.
    ...
    struct ProfitSharingConfig {//max 4 294 967 296
    ...
    constructor(address _core) CoreRef(_core) {
        emit MinBorrowUpdate(block.timestamp, 100e18);//@ why hardcode?
    }
    ...
    function setProfitSharingConfig(
        ...
        );//@audit M rounding down divided by 1e9 not 1e18. 1e18 is 100%. 1e9 is 100%
    ...
    function donateToSurplusBuffer(uint256 amount) external {
        ...
        uint256 newSurplusBuffer = surplusBuffer + amount;//@audit anyone can inflate surplus buffer
    ...
    function donateToTermSurplusBuffer(address term, uint256 amount) external {
        CreditToken(credit).transferFrom(msg.sender, address(this), amount);
        uint256 newSurplusBuffer = termSurplusBuffer[term] + amount;//@note why there are 2 safety surplus buffer.
        termSurplusBuffer[term] = newSurplusBuffer;//@audit M Missing verification for term.can donate surplus buffer to any term? Can this be used to attack future term?
    ...
    function notifyPnL(//@note what is PNL? why profitmanager call PNL gauge to update surplus?
        address gauge,
        int256 amount
