...
abstract contract ERC20RebaseDistributor is ERC20 {
    ...
    uint256 internal constant START_REBASING_SHARE_PRICE = 1e30;//@note unique decimal fixed point 1e30 to prevent floating point precision error.
    ...
    uint256 public constant DISTRIBUTION_PERIOD = 30 days;//@note how share price increase 30 days evenly after a distribution.

    struct InterpolatedValue {
        uint32 lastTimestamp;
        uint224 lastValue;
        uint32 targetTimestamp;//@note targetTimestamp is final date of interpolate. should be +30 days
        uint224 targetValue;
    }
    ...
    function interpolatedValue(
        ...
        if (block.timestamp >= targetTimestamp) {
            ...
            uint256 elapsed = block.timestamp - lastTimestamp;//@audit M interpolate is not linear. lastTimeStamp is for x0. it is a x value between x0 x1
            uint256 delta = targetValue - lastValue;//@note invariant target value always > last value
            return
                lastValue +
                (delta * elapsed) /
