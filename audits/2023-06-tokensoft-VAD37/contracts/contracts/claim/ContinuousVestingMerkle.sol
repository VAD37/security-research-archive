...
contract ContinuousVestingMerkle is ContinuousVesting, MerkleSet {
  ...
  function initializeDistributionRecord(// init first without claim give user voteToken power
  ...
  {
    _initializeDistributionRecord(beneficiary, amount);//@audit M if merkleRoot have 2 same beneficiary but different totalAmount, they can reset claimed amount of each other and mint more
  }// mint voteToken = 2*amount which is this contract ERC20

  function claim(// claim remove user voteToken and give user real locked token
    uint256 index, // the beneficiary's index in the merkle root
    address beneficiary, // the address that will receive tokens
    ...
    nonReentrant
  {
    ...
    // interactions
    super._settleClaim(beneficiary, claimedAmount);//transfer token
  }
