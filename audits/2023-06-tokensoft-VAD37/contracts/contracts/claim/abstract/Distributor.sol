...
abstract contract Distributor is IDistributor, ReentrancyGuard {
  ...
  constructor(IERC20 _token, uint256 _total, string memory _uri, uint256 _fractionDenominator) {
    ...
    total = _total;
    uri = _uri;
    fractionDenominator = _fractionDenominator;//1e18
  ...
  function _executeClaim(
    ...
    // effects
    if (records[beneficiary].total != totalAmount) {//@audit why change fixed totalAmount if user want to claim more than necessary? Is this handle properlly on parent contract?
      // re-initialize if the total has been updated
      _initializeDistributionRecord(beneficiary, totalAmount);//@note Internal distributor reupdate total amount but not reset boolean claimed status
    }
    ...
    claimed += claimableAmount;//@audit increasing claimed amount without transfer token. What could go wrong
