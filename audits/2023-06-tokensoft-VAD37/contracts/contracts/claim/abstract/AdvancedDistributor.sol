  ...
  FairQueue// not that fair
{
  ...
  constructor(
    ...
    uint256 _total,
    string memory _uri,
    uint256 _voteFactor,//2e18
    uint256 _fractionDenominator,//1e18
  ...
  function _initializeDistributionRecord(
    ...
    _mint(beneficiary, tokensToVotes(totalAmount));//@note what user gonna use stored token voting for? before claiming?
  }
  ...
  function adjust(address beneficiary, int256 amount) external onlyOwner {
    ...
    if (amount < 0) {
      ...
      token.safeTransfer(owner(), diff);//@audit M adjust does not consider token already claimed? If user already claimed then owner will get some token that should belong to other user
      // reduce voting power
