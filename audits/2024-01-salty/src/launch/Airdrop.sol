...
contract Airdrop is IAirdrop, ReentrancyGuard
    ...
    function authorizeWallet( address wallet ) external
		...
		_authorizedUsers.add(wallet);//@gas for this use normal array is quite good enough
    	}
    ...
    function allowClaiming() external
		...
		saltAmountForEachUser = saltBalance / numberAuthorized();//@5_000_000 e18 / people
    ...
    function claimAirdrop() external nonReentrant
		...
		staking.stakeSALT( saltAmountForEachUser );//@note airdrop stake token and then transfered xSALT to user
