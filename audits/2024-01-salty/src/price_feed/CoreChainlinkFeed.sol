...
contract CoreChainlinkFeed is IPriceFeed
    {
    uint256 constant MAX_ANSWER_DELAY = 60 minutes;//@audit-ok H chainlink 60 minutes delay is too tight. Chainlink update round exact every 60 minutes.
//@https://etherscan.io/address/0xE62B71cf983019BFf55bC83B48601ce8419650CC  
	...
	function latestChainlinkPrice(AggregatorV3Interface chainlinkFeed) public view returns (uint256)
		...
		returns (
			...
			uint80 // _answeredInRound//@audit L chainlink missing stale round check. Chainlink update round every 60 minutes.
		)
			{
			...
			if ( answerDelay <= MAX_ANSWER_DELAY )
				price = _price;//@audit-ok yes it is huge problem.chainlink known to transmit price delay 1 block. is this a problem?
			else
				price = 0;//@audit-ok if chainlink down for 60s minutes. price always return 0. Because if 1 of other oracle can be easily manipulated. This is a problem.
			}
