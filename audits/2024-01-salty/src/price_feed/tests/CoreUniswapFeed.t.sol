...
contract TestCoreUniswapFeed is Deployment {
    ...
    constructor() {
        testUniswapFeed = new TestUniswapFeed(
            _testBTC,
            _testETH,
            _testUSDC,
            UNISWAP_V3_BTC_ETH,
    ...
    function testGetTwapFailure() public {
        ...
        IUniswapV3Pool UNISWAP_V3_WBTC_WETH = testUniswapFeed
            .UNISWAP_V3_WBTC_WETH();
        IUniswapV3Pool UNISWAP_V3_WETH_USDC = testUniswapFeed
    ...
    function testCoreUniswapFeedConstructor(
        address _wbtc,
        address _weth,
        address _usdc
    ) public {
        ...
        assertEq(
            testUniswapFeed.wbtc_wethFlipped(),
            expectedWbtcWethFlipped,
            "WBTC/WETH order incorrectly determined in constructor"
        );

        // Check if WETH/USDC order is correctly determined
        bool expectedWethUsdcFlipped = address(_usdc) < address(_weth);
        assertEq(
            testUniswapFeed.weth_usdcFlipped(),
            expectedWethUsdcFlipped,
            "WETH/USDC order incorrectly determined in constructor"
        );
        ...
        uint256 expectedTwapWBTC = (forcedTWAP_WETH_USDC * 10 ** 18) /
            forcedTWAP_WBTC_WETH;
        ...
        assertEq(
            twapWBTC,
            expectedTwapWBTC,
            "WBTC TWAP not as expected when token order is flipped"
        );
        assertEq(
            twapWETH,
            expectedTwapWETH,
            "WETH TWAP not as expected when token order is flipped"
        );
    }
    ...
    function testCoreUniswapFeedInitialization() public {
        ...
        TestUniswapFeed newUniswapFeed = new TestUniswapFeed(
            _testBTC,
            _testETH,
            _testUSDC,
            UNISWAP_V3_BTC_ETH,
        ...
        assertEq(
            address(newUniswapFeed.UNISWAP_V3_WBTC_WETH()),
            UNISWAP_V3_BTC_ETH,
            "WBTC/WETH pool address not set correctly in constructor"
        );
        assertEq(
            address(newUniswapFeed.UNISWAP_V3_WETH_USDC()),
            UNISWAP_V3_USDC_ETH,
            "WETH/USDC pool address not set correctly in constructor"
        );

        // Check if the ExchangeConfig address is set correctly
        assertEq(
            address(newUniswapFeed.wbtc()),
            address(exchangeConfig.wbtc()),
            "WBTC token address not set correctly in constructor"
        );
        assertEq(
            address(newUniswapFeed.weth()),
            address(exchangeConfig.weth()),
            "WETH token address not set correctly in constructor"
        );

        // Check if the WBTC/WETH and WETH/USDC orders are determined correctly
        bool expectedWbtcWethFlipped = address(exchangeConfig.weth()) <
            address(exchangeConfig.wbtc());
        bool expectedWethUsdcFlipped = address(testUniswapFeed.usdc()) <
            address(exchangeConfig.weth());

        assertEq(
            newUniswapFeed.wbtc_wethFlipped(),
            expectedWbtcWethFlipped,
            "WBTC/WETH order incorrectly determined in constructor"
        );
        assertEq(
            newUniswapFeed.weth_usdcFlipped(),
            expectedWethUsdcFlipped,
            "WETH/USDC order incorrectly determined in constructor"
        );
    }
