...
abstract contract PoolStats is IPoolStats
	...
	function updateArbitrageIndicies() public
		...
		for( uint256 i = 0; i < poolIDs.length; i++ )
			...
			(IERC20 arbToken2, IERC20 arbToken3) = poolsConfig.underlyingTokenPair(poolID);//@audit-ok arbToken is not sorted. is there any problem down the line?
	...
	function profitsForWhitelistedPools() external view returns (uint256[] memory _calculatedProfits)
		...
		_calculateArbitrageProfits( poolIDs, _calculatedProfits );//@audit-ok simulator show this copy memory pointer over. R How memory view can be passed into internal? H upkeep function will failed here. this view function is broken. it edit memory value and return nothing.
		}
