...
contract PoolsConfig is IPoolsConfig, Ownable
	...
	// Whitelist a given pair of tokens//@note PoolConfigs whitelist tokenPair always WBTC or WETH as second tokenB.
	function whitelistPool( IPools pools, IERC20 tokenA, IERC20 tokenB ) external onlyOwner
		...
		//@audit-ok proposal check for zero .M whitelist pool does not check token as non zero address.
		bytes32 poolID = PoolUtils._poolID(tokenA, tokenB);

		// Add to the whitelist and remember the underlying tokens for the pool
		_whitelist.add(poolID);//@audit-ok proposal check this .whitelistPool does not check if pool already added. This return false. arbitrageIndicided can be flipped because tokenPair is not sorted.
		underlyingPoolTokens[poolID] = TokenPair(tokenA, tokenB);//@audit-ok this always ordered from proposal .L TokenPair from PoolsConfig is ordered. It was implementer job to sort tokenA and tokenB

		// Make sure that the cached arbitrage indicies in PoolStats are updated
		pools.updateArbitrageIndicies();//@PollStats then call this contract for _whitelist array and underlyingTokenPair() which return underlyingPoolTokens[]
		//@audit-ok this will be reverted. dueto duplicate token address .M whitelistPool does not check if token is WETH. This allow WETH tobe middle token in arbitrage path which does not updated. tokenHasBeenWhitelisted() also check tokenpair is WETH or WBTC
	...
	function unwhitelistPool( IPools pools, IERC20 tokenA, IERC20 tokenB ) external onlyOwner
		{
		bytes32 poolID = PoolUtils._poolID(tokenA,tokenB);//@audit L DAO can remove all pools.

		_whitelist.remove(poolID);//@audit L DAO can remove pool not exist.
		delete underlyingPoolTokens[poolID];

		// Make sure that the cached arbitrage indicies in PoolStats are updated
		pools.updateArbitrageIndicies();//@audit audit-issue M-04 remove poolID from array but does not remove from  _arbitrageIndicies[] from poolStats
		//Update indice loop through whitelist array. which does not include removed element. so it only update previous known config poolID.
