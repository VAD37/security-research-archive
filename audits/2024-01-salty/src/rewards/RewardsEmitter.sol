...
contract RewardsEmitter is IRewardsEmitter, ReentrancyGuard
	...
	function addSALTRewards( AddedReward[] calldata addedRewards ) external nonReentrant
		...
		for( uint256 i = 0; i < addedRewards.length; i++ )
			...
			require( poolsConfig.isWhitelisted( addedReward.poolID ), "Invalid pool" );//@audit-ok L PoolUtils.STAKED_SALT pool must be whitelisted before launch. this was not checked

			uint256 amountToAdd = addedReward.amountToAdd;
			if ( amountToAdd != 0 )
				{
				// Update pendingRewards so the SALT can be distributed later
				pendingRewards[ addedReward.poolID ] += amountToAdd;//@note add rewards but does not update timer. Is this flywheel?
				sum += amountToAdd;
				}
			}//@audit L allow add salt to reward pools that does not support certain pools like staked salt.
	...
	function performUpkeep( uint256 timeSinceLastUpkeep ) external
		...
		//@A=Initial reward×(1−rate)^t . after 100 days we get 65%. 200 days 85%
		// Rewards to emit = pendingRewards * timeSinceLastUpkeep * rewardsEmitterDailyPercent / oneDay
		uint256 numeratorMult = timeSinceLastUpkeep * rewardsConfig.rewardsEmitterDailyPercentTimes1000();//@1000 = 1%
