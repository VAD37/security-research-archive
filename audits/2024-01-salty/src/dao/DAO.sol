...
contract DAO is IDAO, Parameters, ReentrancyGuard
	...
	function _executeApproval( Ballot memory ballot ) internal
		...
		if ( ballot.ballotType == BallotType.UNWHITELIST_TOKEN )
			...
			{
			ICalledContract(ballot.address1).callFromDAO( ballot.number1 );//@audit L DAO does not have arbitrary call.
	...
	function _finalizeTokenWhitelisting( uint256 ballotID ) internal
		...
		if ( proposals.ballotIsApproved(ballotID ) )
			...
			uint256 bestWhitelistingBallotID = proposals.tokenWhitelistingBallotWithTheMostVotes();//@audit-ok M whitelist cannot be success if there is another proposal with more yes vote and more no votes
...
//@note DAO contract have all permission so it is the one execute Ballot or proposal. Any user can execute this as long as ballot is finalized.
	...
	function finalizeBallot( uint256 ballotID ) external nonReentrant
		...
		//@audit M proposal execution and finalize can always revert. This left execution delayed and can be executed later.
		Ballot memory ballot = proposals.ballotForID(ballotID);
...
//@note DAO have several operation does not related to DAO. It is  upkeep operation
