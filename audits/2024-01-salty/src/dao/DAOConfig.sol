...
contract DAOConfig is IDAOConfig, Ownable
	...
	uint256 public bootstrappingRewards = 200000 ether;//@audit-ok what happen when new token listed and bootstrap reward is not enough for 200k token
	//@note bootstrappingrewards is double amount in config. There 2 pool created for each new token. so 200 ether rewards required 400 for both pool.
	...
	uint256 public baseBallotQuorumPercentTimes1000 = 10 * 1000; // Default 10% of the total amount of SALT staked with a 1000x multiplier
		//@audit do for quorum xSALT is calculated. what happen when not enough people join airdrop for DAO
	...
	uint256 public maxPendingTokensForWhitelisting = 5;//minimum 600K for 12 tokens bootstrapping, max 6M for 12 tokens bootstrapping
	//@audit how pending token whitelist work?. what is relationship with bootstrapping reward
