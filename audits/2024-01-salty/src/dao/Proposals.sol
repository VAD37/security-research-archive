...
contract Proposals is IProposals, ReentrancyGuard
	...
	//@ ballotName, BallotType.WHITELIST_TOKEN, address(token), 0, tokenIconURL, description 
	function _possiblyCreateProposal( string memory ballotName, BallotType ballotType, address address1, uint256 number1, string memory string1, string memory string2 ) internal returns (uint256 ballotID)
		...
		if ( msg.sender != address(exchangeConfig.dao() ) )//@audit-ok R How DAO is calling createProposal? Not every call go through DAO
			{
		...
		//@note ballot block multiple proposal mechanism is using name as preventation. This create issue where similar name cannot be executed. Does user can craft any name they want?
	...
	function createConfirmationProposal( string calldata ballotName, BallotType ballotType, address address1, string calldata string1, string calldata description ) external returns (uint256 ballotID)
		...
		return _possiblyCreateProposal( ballotName, ballotType, address1, 0, string1, description );//@note DAO proposalConfirmation ballot have double "_confirm" while checking internally
		}

	//@finalized called by DAO. with 3 different path and function.
	...
	function proposeParameterBallot( uint256 parameterType, string calldata description ) external nonReentrant returns (uint256 ballotID)
		...
		return _possiblyCreateProposal( ballotName, BallotType.PARAMETER, address(0), parameterType, "", description );//@audit L allow create proposal parameter Type outside Enum range.
		}//@audit-ok User can unstake for really long time. H execution of ballot can be failed. when that happen staked token is locked.
	...
	function proposeTokenWhitelisting( IERC20 token, string calldata tokenIconURL, string calldata description ) external nonReentrant returns (uint256 _ballotID)
		...
		//@note Why only token with uint112 can be added. Why?
		...
		//@audit L whitelisting token does not check if token is WETH or WBTC directly. This will fail silently later after executed.
		string memory ballotName = string.concat("whitelist:", Strings.toHexString(address(token)) );
		//@audit L DAO does not check if token address is contract or exist. This leave to DAO user.
	...
	function proposeTokenUnwhitelisting( IERC20 token, string calldata tokenIconURL, string calldata description ) external nonReentrant returns (uint256 ballotID)
		...
		string memory ballotName = string.concat("unwhitelist:", Strings.toHexString(address(token)) );//@audit R you can unwhitelist token immediately by stacking proposal. What happen to rewards then?
	...
	function proposeSendSALT( address wallet, uint256 amount, string calldata description ) external nonReentrant returns (uint256 ballotID)
		...
		string memory ballotName = "sendSALT";//@audit audit-issue M-02 DAO lack abilities to cancel proposal early if there is enough No votes. This disable emergency sending token when there is a limit for proposal.
	...
	function castVote( uint256 ballotID, Vote vote ) external nonReentrant
		{//@note castVote always cast entire user share.
		...
		//@audit voting does not reduce user voting share or power. Cast vote does not cache votingId in array.
		...
		}//@note no cancel vote mechanism. Only recast vote.
	...
	function requiredQuorumForBallotType( BallotType ballotType ) public view returns (uint256 requiredQuorum)
		...
		uint256 minimumQuorum = totalSupply * 5 / 1000;//@minimum quora is 0.5% of 100M = 500K. 4 days available = 5M airdrop + 8M rewards + 1M vesting + 3M emissions = ~15M token
	...
	function canFinalizeBallot( uint256 ballotID ) external view returns (bool)
        ...
        if (block.timestamp < ballot.ballotMinimumEndTime )
            return false;//@audit audit-issue  M-03 Ballot missing execution timer.ballot or proposal can last for infinite time. This is not good. Missing expire mechanism. Allow later staking user to vote yes on old expired ballot.
	...
	function tokenWhitelistingBallotWithTheMostVotes() external view returns (uint256)
		...
		for( uint256 i = 0; i < _openBallotsForTokenWhitelisting.length(); i++ )
			...
			//@audit-ok L this internal logic is wrong. this should be reverted
