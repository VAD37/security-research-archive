...
contract ManagedWallet is IManagedWallet
    ...
    receive() external payable
    	...
    	if ( msg.value >= .05 ether )//@audit L confirmationWallet can reset active timelock when send more money again.
    		activeTimelock = block.timestamp + TIMELOCK_DURATION; // establish the timelock
    	else
			activeTimelock = type(uint256).max; // effectively never
        }//@audit audit-issue M-01 wallet can send in token but cannot withdraw it back. This is not a wallet
	...
	function changeWallets() external
		...
		// confirmationWallet.call{value: address(this).balance, gas: 30000}(""); // refund original eth for confirmationWallet. 30k gas is safe amount to prevent gas exploit and safe transfer
		// Set the wallets
		mainWallet = proposedMainWallet;
		confirmationWallet = proposedConfirmationWallet;//@audit-ok mainWallet have no abilities to block the change of confirmationWallet. As confirmation wallet can keep sending money to reset the timelock.
