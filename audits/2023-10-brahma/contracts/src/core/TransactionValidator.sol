...
contract TransactionValidator is AddressProviderService {
    ...
    struct SafeTransactionParams {
        ...
        address from;//@audit abnormal. The transaction validation from always gnosis safe. 
        address to;
    ...
    function _isConsoleBeingOverriden(
        ...
        if (_from == _to && _value == 0 && _operation == Enum.Operation.Call) {
            if (SafeHelper._GUARD_REMOVAL_CALLDATA_HASH == keccak256(_data)) {//@audit-ok This only use for skipping policy check. this is not safe. as calldata can be extra length with zero.
                return true;
