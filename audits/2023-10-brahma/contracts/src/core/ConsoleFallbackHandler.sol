...
contract ConsoleFallbackHandler is AddressProviderService, DefaultCallbackHandler, ISignatureValidator {
    ...
    bytes4 internal constant UPDATED_MAGIC_VALUE = 0x1626ba7e;//@ 	isValidSignature(bytes32,bytes)
     ...
     */
    function isValidSignature(bytes memory _data, bytes memory _signature) public view override returns (bytes4) {//@audit this is different isValidSignature function from interface
        // Caller should be a Safe
        GnosisSafe safe = GnosisSafe(payable(msg.sender));
        bytes32 messageHash = getMessageHashForSafe(safe, _data);//@audit how this work? Is it the same as encodeMessageDataForSafe
        if (_signature.length == 0) {
            require(safe.signedMessages(messageHash) != 0, "Hash not approved");//@ no change
        } else {
            ...
            safe.checkSignatures(messageHash, _data, _signature);//@note validSignature include new policy check
        }
    ...
    function isValidSignature(bytes32 _dataHash, bytes calldata _signature) external view returns (bytes4) {
        ISignatureValidator validator = ISignatureValidator(msg.sender);//@audit this change significantly from original fallback.
        bytes4 value = validator.isValidSignature(abi.encode(_dataHash), _signature);//@audit this call into signature modifed check above. But why call onto msg.sender?
        return (value == EIP1271_MAGIC_VALUE) ? UPDATED_MAGIC_VALUE : bytes4(0);
    }//@audit-ok M fallbackHandler broke safe signature validation as it return very different safe module function. What happen when user include another gnosis module that does not support these fall back?
    ...
    }//@audit fallback missing warning view checkNSignatures() https://github.com/safe-global/safe-contracts/blob/810fad9a074837e1247ca24ac9e7f77a5dffce19/contracts/handler/CompatibilityFallbackHandler.sol#L160-L173
}
