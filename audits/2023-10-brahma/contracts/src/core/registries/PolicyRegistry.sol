...
contract PolicyRegistry is AddressProviderService {
    ...
    mapping(address account => bytes32 policyCommit) public commitments;//@audit policyCommit can be whatever user want. there is no whitelist on registry
    //@ policyCommit is singature hash for verfication. On PolicyValidator.sol, it will verify the signature of the policyCommit
    ...
    function updatePolicy(address account, bytes32 policyCommit) external {
        ...
        if (
            currentCommit == bytes32(0)
                && msg.sender == AddressProviderService._getAuthorizedAddress(_SAFE_DEPLOYER_HASH)//@ok. caller is safe
        ) {
            // In case invoker is safe  deployer //@ init account include policy. or empty policy by not calling
        } else if (walletRegistry.isOwner(msg.sender, account)) {
            //In case invoker is updating on behalf of sub account
        } else if (msg.sender == account && walletRegistry.isWallet(account)) {//@audit EOA can be wallet or ConsoleAccount
            // In case invoker is a registered wallet. //@ EOA can update their own policy
        } else {
