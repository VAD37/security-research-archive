 ...
 */
contract AddressProvider {//@note Address Provider have nothing important going on. It just a singleton for registry address. Owner is GOV
    ...
    constructor(address _governance) {
        ...
        governance = _governance;//@note all contracts have addressProvider in constructor. AddressProvider is singleton.
    }
    ...
    function setAuthorizedAddress(bytes32 _key, address _authorizedAddress, bool _overrideCheck) external {
        ...
        if (!_overrideCheck) {
            ...
            if (_authorizedAddress.code.length != 0) _ensureAddressProvider(_authorizedAddress);//@note consoleFactory.t.s have registry and permission to set everything
        }
