...
contract PolicyValidator is AddressProviderService, EIP712 {
        ...
        returns (bool)
    {
        ...
        address trustedValidator = AddressProviderService._getAuthorizedAddress(_TRUSTED_VALIDATOR_HASH);//@trusted is EOA owned by admin
        ...
        // Validate signature
        return SignatureCheckerLib.isValidSignatureNow(trustedValidator, txnValidityDigest, validatorSignature);//@audit policy signature is for signing.
    }
     ...
     */
    function _decompileSignatures(bytes calldata _signatures)//@audit L signatures send by user can have tidbit in between. what is the point?
        internal
        pure
