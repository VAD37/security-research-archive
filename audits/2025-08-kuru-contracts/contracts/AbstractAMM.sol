...
abstract contract AbstractAMM is IOrderBook, ReentrancyGuardTransient {
        ...
        returns (uint96 quoteLeft, uint96 sizeFilled, bytes memory makerCredits)
    {
        ...
        if (_price != vaultBestAsk) {
            console.log("-update candle: %e", vaultBestAsk);
            vaultBestAsk = _price;
            ...
            console.log("-new bestAsk: %e", vaultBestAsk);
            console.log("-new bestBid: %e", vaultBestBid);
            console.log("-new askOrderSize: %e", vaultAskOrderSize);
            console.log("-new bidOrderSize: %e", vaultBidOrderSize);
        }
        ...
        returns (uint96 sizeLeft, uint96 fundsConsumed, bytes memory makerCredits)
    {
        ...
        uint96 _availableSize = _cachedLastVaultSize - askPartiallyFilledSize;
        if (_availableSize <= sizeLeft) {//@ order size > available size at this price point
            askPartiallyFilledSize = 0;//@reset global fill for current price point
        }
        revert("fill vault for buy special condition reached");
        ...
        returns (uint96 sizeLeft, uint256 quoteOwedToUser, bytes memory makerCredits)
    {
        ...
        revert("fill vault for sell special condition reached");
