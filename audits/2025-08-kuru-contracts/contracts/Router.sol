...
contract Router is IRouter, Ownable, Initializable, UUPSUpgradeable {
    ...
    function initialize(
        address _owner,
        address _marginAccount,//pre deployed init contract
        address _orderbookImplementation,//impl
        address _kuruAmmVaultImplementation,//impl
        address _trustedForwarder//kuruForwarder
    ) public initializer {
    ...
    function deployProxy(
        ...
        uint96 _sizePrecision,//1e10
        uint32 _pricePrecision,//1e2
        uint32 _tickSize,//50
        uint96 _minSize,//2e8
        uint96 _maxSize,//10e12
        uint256 _takerFeeBps,//0
        uint256 _makerFeeBps,//0
        uint96 _kuruAmmSpread//100
    ) public returns (address proxy) { //@can create same pair market with different configure
        ...
        );//@can predict vault address before deployment

        verifiedMarket[proxy] = MarketParams(//@save hash salt
            _pricePrecision,
            _sizePrecision,
            _baseAssetAddress,
            _baseAssetDecimals,
        ...
        IMarginAccount(marginAccountAddress).updateMarkets(proxy);//@whitelist new market just created. anyone can create market that share same treasury pool though
        {
            IOrderBook(proxy).initialize(
                ...
                _minSize,
                _maxSize,
                _takerFeeBps,//@check during init
                _makerFeeBps,//@check
                address(_kuruAmmVault),
                _kuruAmmSpread,//@check
                TRUSTED_FORWARDER
            );
        }
