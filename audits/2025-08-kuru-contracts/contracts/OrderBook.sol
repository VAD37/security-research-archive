...
contract OrderBook is Initializable, UUPSUpgradeable, ERC2771Context, AbstractAMM {
    ...
    uint96 internal sizePrecision; //1e10
    uint32 internal pricePrecision; //100 must log10
     ...
     */
    function initialize(
        address _owner, //Router
        OrderBookType _type, // Default None or depend on base/quote is native
        address _baseAssetAddress,
        uint256 _baseAssetDecimals,
        address _quoteAssetAddress,
        uint256 _quoteAssetDecimals, //@what happen with non standard token decimals though?
        address _marginAccountAddress, //@MarginAccount.sol
        uint96 _sizePrecision, //1e10 , >1e1 and < 1e32
        uint32 _pricePrecision, //100
        uint32 _tickSize, //50 , tickSize > 0
        uint96 _minSize, // 2e8 . I think this is USDC minimum for order
        uint96 _maxSize, // 10e12
        uint256 _takerFeeBps, //0
        uint256 _makerFeeBps, // 0 no limit cap. < 10,000
        address _kuruAmmVault, // AMMVault proxy
        uint96 _kuruAmmSpread, //100 . < 500 also rounded to 10
        address __trustedForwarder // fixed KuruForwarder.sol
    ) public initializer {
        owner = _owner;

        require(_makerFeeBps <= _takerFeeBps && _takerFeeBps < BPS_MULTIPLIER, OrderBookErrors.MarketFeeError()); //max 10000 < 100%?
     ...
     * @param _price Price of the buy order. @price is non decimal based
     * @param size Size of the buy order. just input token amount. max 7e28 or 7Billions order
    ...
    function addBuyOrder(uint32 _price, uint96 size, bool _postOnly) public marketActive nonReentrant {
        //@price not multiplied by quote decimals
        ...
        require(_price % tickSize == 0, OrderBookErrors.TickSizeError()); //@tick can be non 10

        //The buy order is filled aggressively and the leftover size is added as a limit buy order
        (uint96 _remainingSize, uint96 _consumedFunds) = _matchAggressiveBuyWithCap(uint256(_price), size); //@return 0 ,0 on first call. since nothing is being filled
        //consumeFunds = price * size / 1e10
        _consumedFunds += _quoteAmountRoundedUp(_price, _remainingSize); //@audit price always limited by maximum 4Billions of something. depend on their decimals.

        _consumeFunds(quoteAsset, (((_consumedFunds) * quoteDecimalMultiplier) / pricePrecision), _msgSender());
        if (_postOnly) {
            require(size == _remainingSize, OrderBookErrors.PostOnlyError()); //@revert if order is fufilled. and user lose cash
        }
        if (_remainingSize == 0) {
            return;
        } else {
            //@create new chained order. internal AMM also track its own current candle amount, remaining unfill order
            uint40 _orderId = s_orderIdCounter + 1; //max 1099511627776
            s_orderIdCounter = _orderId; //@new order placed
            //@ orderId is created as linked list and flipped function. no clue what that does
            // Add price to the tree and update DLL of price point.
            uint40 _prevOrderId = OrderLinkedList.insertAtTail(s_buyPricePoints[_price], _orderId); //@this cache last person ID create order at this price point

            _addOrder(_price, _remainingSize, _orderId, true, _prevOrderId); //@just add order to tree. tell last order in queue to remember next order just placed
        }
    }
    ...
    function addSellOrder(uint32 _price, uint96 _size, bool _postOnly) public marketActive nonReentrant {
        ...
        require(_price % tickSize == 0, OrderBookErrors.TickSizeError()); //tick > 0. so it can be 1
        require(_price > 0 && _price < type(uint32).max, OrderBookErrors.PriceError());
        //@msg sender decode through forwarder require signed signature. so cannot steal transferFrom.
        _consumeFunds(baseAsset, ((_size * baseDecimalMultiplier) / sizePrecision), _msgSender()); //debitUser(). remove user balance from book. this orderbook is whitelisted
    ...
    function _addOrder(uint32 _price, uint96 _size, uint40 _orderId, bool _isBuy, uint40 _prevOrderId) internal {
        ...
        if (_prevOrderId != OrderLinkedList.NULL) {
            s_orders[_prevOrderId].next = _orderId; //@cache previous order with next order. 1st order will remember ID of 2nd order. when match, remove cash from 1st order?
        }
    ...
    function placeAndExecuteMarketBuy(uint96 _quoteSize, uint256 _minAmountOut, bool _isMargin, bool _isFillOrKill)
        ...
        returns (uint256)
    {
        OrderBookType _type = orderBookType; //@audit under what condition does msg.sender == 0?? during self delegate?
    ...
    function _matchAggressiveBuyWithCap(uint256 _limitPrice, uint96 _sizeToBeFilled)
        ...
        returns (
            uint96,
            uint96 //@must also use chained order here somewhere
        )
    {
        uint96 sizeToCreditTaker;
        uint96 sizeToCreditViaVault;
        uint96 fundsConsumed;
        bytes memory makerCredits; //@audit isVaultFill logic seem wrong. it return if price limit == current market sell
        (bool _isVaultFill, uint256 _bestAsk) = bestAsk(); //best price from sell list. on empty return (true,0). first call is false,bestAsk
        console.log("bestAsk %e", _bestAsk);
        uint32 _pricePrecision = pricePrecision; //@50 tick spread. tick could be unrounded like 17. the only requirement is >0. max 4e9
        _limitPrice = _limitPrice * vaultPricePrecision / _pricePrecision; // price *1e18/ pricePrecision. all price in tree already scaled by same precision and value too
        //@loop when buy order still have cash left, go to next sell order and remove cash. if no sell order left, just create new buy order
        while (_sizeToBeFilled > 0 && _bestAsk <= _limitPrice && _bestAsk != 0) {
            //@first loop never called though when order is empty. this open up abuse condition when empty orders
            uint96 sizeToBeFilledBefore = _sizeToBeFilled; // Capture the size before filling
            bytes memory priceUpdate;
            if (_isVaultFill) {//@audit H this condition never reached when no margin call ever called to update vaultBestAsk to other value
                //@order hit correct price point of current selling order.
                console.log("fill vault");
                uint96 _vaultFundsConsumed;
                (_sizeToBeFilled, _vaultFundsConsumed, priceUpdate) = //@this look like special code. ignore tree and order list from OrderBook.
                 _fillVaultForBuy(_getOBAsk() > _limitPrice ? _limitPrice + 1 : _getOBAsk(), _sizeToBeFilled);//@audit fill Vault depend on variable that onlyUpdated through matchBuy function. 
                fundsConsumed += _vaultFundsConsumed;
            } else {
                //@just loop through all list of sell order with small price point than limit. Fill a whole range of price point
                console.log("loop all");
                (_sizeToBeFilled, priceUpdate) = _fillSizeForPrice(
                    s_sellTree, //@why convert bestAsk back to limitPrice decimals?
                    toU32(_bestAsk * _pricePrecision / vaultPricePrecision), //@audit this does not work with USDC 6 decimals. it just reduce price to 0
    ...
    function _fillSizeForPrice(
        ...
        while (_size > 0 && _orderId != OrderLinkedList.NULL) {
            //@loop go from order head to tail
    ...
    /// Search for lowest price value in sell tree. aka lowest selling price

    function bestAsk() internal view returns (bool, uint256) {
        uint256 firstLeft = TreeMath.findFirstLeft(s_sellTree, 0) * vaultPricePrecision / pricePrecision; //@this search for lowest value from 0->max
        console.log("firstLeft %e", firstLeft);
        console.log("vaultBestAsk %e", vaultBestAsk);
        if (firstLeft != 0) {
            if (vaultBestAsk != type(uint256).max) {
                if (firstLeft == vaultBestAsk) {
                    return (false, firstLeft);
                }
                console.log("hidden ask");
                return (FixedPointMathLib.min(firstLeft, vaultBestAsk)) == vaultBestAsk
                    ? (true, vaultBestAsk)
                    : (false, firstLeft);
            }
            return (false, firstLeft); //@maximum case is not possible.
        }
        return (vaultBestAsk != type(uint256).max) ? (true, vaultBestAsk) : (false, 0);
    }
    /// Search for highest price value in buy tree. aka highest buying price

    function bestBid() internal view returns (bool, uint256) {
        //uint32.max * 1e18 /  uint32(100)
        uint256 firstRight = TreeMath.findFirstRight(s_buyTree, type(uint32).max) * vaultPricePrecision / pricePrecision; //@search highest value in tree. from max->0, if none found just return max
        if (firstRight != type(uint32).max * vaultPricePrecision / pricePrecision) {
            //@audit why here compare firstRight uint32.max with modified max value? This ignore condition where it failed to find anything
            if (vaultBestBid != 0) {
                //@vault best bid still 0
            ...
            return (false, firstRight); //@this always return on first call search when there is nothing in tree. Or nothing larger
        } //@else case is exploited case. when bid in tree == if check above
        return vaultBestBid != 0 ? (true, vaultBestBid) : (false, type(uint256).max); //@this condition never reached
    } //@vault best bid can be (uint32.max * 1e18 /  uint32(100)). this result in rare return condition
