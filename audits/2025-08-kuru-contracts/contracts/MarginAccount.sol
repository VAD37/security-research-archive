...
contract MarginAccount is IMarginAccount, ERC2771Context, Ownable, Initializable, UUPSUpgradeable {
        ...
        initializer
    {
        ...
        feeCollector = _feeCollector;//@ also router
        trustedForwarder = _trustedForwarder;//@random address but assume kuruforwarder
    }
    ...
    function updateMarkets(address _marketAddress) external onlyRouter protocolActive {
        verifiedMarket[_marketAddress] = true;//Router Deployed Proxy
    }
    ...
    function debitUser(address _user, address _token, uint256 _amount) external protocolActive {
        require(verifiedMarket[msg.sender], MarginAccountErrors.OnlyVerifiedMarketsAllowed());//@proxy Router deployed 
    ...
    function deposit(address _user, address _token, uint256 _amount) external payable protocolActive {
        ...
        if (_token == NATIVE) {
            ...
            balances[_accountKey(_user, _token)] += _amount;
            _token.safeTransferFrom(_msgSender(), address(this), _amount);//@audit H allow deposit non exist token before market exist. Also safeTransfer also check zero code size but ignore EOA
        }
