...
contract KuruForwarder is EIP712, Initializable, Ownable, UUPSUpgradeable {
    ...
    struct ForwardRequest {
        ...
        bytes4 selector;
        bytes data;
    }//@audit struct type of ForwardRequest and MarginAccountRequest is the same. Which is interesting
    ...
    function initialize(address _owner, bytes4[] memory _allowedInterfaces) public initializer {
        ...
        }//allow [ OrderBook.addBuyOrder, OrderBook.addSellOrder, OrderBook.placeAndExecuteMarketBuy, placeAndExecuteMarketSell ]
    }//allow [ MarginAccount.deposit, MarginAccount.withdraw]
        ...
        returns (bool)
    {
        ...
        ).recoverCalldata(signature);//@signature still use 64,65 check. so reuse signature is possible
        require(
    ...
    function verifyCancelPriceDependent(CancelPriceDependentRequest calldata req, bytes calldata signature)
        public//@audit price dependent use special nonce check. aka random nonce
        view
        returns (bool)
    {
        require(block.timestamp <= req.deadline, DeadlineExpired());
        ...
        require(
            !executedPriceDependentRequest[keccak256(abi.encodePacked(req.from, req.nonce))],//@audit-ok it is encodepacked. so it ignore bit does req.from still use full bytes32(address) space? it might not use safety address quote
            KuruForwarderErrors.NonceAlreadyUsed()
        );
        return signer == req.from;
    }
        ...
        returns (bool)
    {
        ...
        return req.nonce >= _nonces[req.from] && signer == req.from;//@audit check nonce using >=. this can cause race condition
    }
    ...
    {
        require(verifyMarginAccountRequest(req, signature), KuruForwarderErrors.SignatureMismatch());//@ unique signature
        require(msg.value >= req.value, KuruForwarderErrors.InsufficientValue());
        require(allowedInterface[req.selector], KuruForwarderErrors.InterfaceNotAllowed());//allow [ MarginAccount.deposit, MarginAccount.withdraw]
//[ OrderBook.addBuyOrder, OrderBook.addSellOrder, OrderBook.placeAndExecuteMarketBuy, placeAndExecuteMarketSell ]
    ...
    function execute(ForwardRequest calldata req, bytes calldata signature) public payable returns (bytes memory) {
        require(verify(req, signature), KuruForwarderErrors.SignatureMismatch());//@ unique signature
        require(msg.value >= req.value, KuruForwarderErrors.InsufficientValue());
        require(allowedInterface[req.selector], KuruForwarderErrors.InterfaceNotAllowed());//allow [ MarginAccount.deposit, MarginAccount.withdraw]
//[ OrderBook.addBuyOrder, OrderBook.addSellOrder, OrderBook.placeAndExecuteMarketBuy, placeAndExecuteMarketSell ]
        _nonces[req.from] = req.nonce + 1;
        //@audit free external call to anyone. only check allowed functions.
        (bool success, bytes memory returndata) =
            req.market.call{value: req.value}(abi.encodePacked(req.selector, req.data, req.from));

        if (!success) {//@self call to withdraw stuck msg.value? nope must be allowed function. still no whitelisted market address
            revert ExecutionFailed(returndata);
        }
        ...
        returns (bytes memory)
    {
        ...
        (bool success, bytes memory returndata) =
            req.market.call{value: req.value}(abi.encodePacked(req.selector, req.data, req.from));//@audit accept more money than actually used
    ...
    function cancelPriceDependent(CancelPriceDependentRequest calldata req, bytes calldata signature) public {
        require(verifyCancelPriceDependent(req, signature), KuruForwarderErrors.SignatureMismatch());//@use different hash so different signature
