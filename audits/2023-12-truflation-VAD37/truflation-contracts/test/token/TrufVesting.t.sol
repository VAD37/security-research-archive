...
contract TrufVestingTest is Test {
    ...
    function testPrivateClaimableAmount() external {
        console.log("Test check available claim amount");
        ...
        (,uint initialReleasePeriod,uint cliff,,) = vesting.vestingInfos(categoryId,vestingId);
        (uint256 _amount, uint256 _claimed, uint256 _locked, uint64 _startTime) =
            vesting.userVestings(categoryId, vestingId, user);

        vm.warp(block.timestamp + 1 days + 100); //@skip to TGE time
        console.log("block.timestamp", block.timestamp);
        console.log("startTime", _startTime);
        console.log("initialReleasePeriod", initialReleasePeriod);
        console.log("cliff", cliff);
        assertNotEq(initialReleasePeriod,0, "Initial release period should be non-zero");
        assertNotEq(cliff, 0, "Cliff should be non-zero");        

        vm.warp(block.timestamp +  initialReleasePeriod);        
        console.log("block.timestamp", block.timestamp);
        uint256 claimable1 = vesting.claimable(categoryId, vestingId, user);
        assertNotEq(vesting.claimable(categoryId, vestingId, user), 0, "Claimable amount should be non-zero");
        
        vm.warp(block.timestamp + cliff + 10 days );
        console.log("block.timestamp", block.timestamp);
        uint256 claimable2 = vesting.claimable(categoryId, vestingId, user);
        console.log("claimable 1 %e", claimable1);
        console.log("claimable 2 %e", claimable2);

        assertNotEq(claimable1,claimable2, "Claimable amount should change with time");
    }
}
