...
contract TrufPartner is Ownable {
    ...
    constructor(address _trufToken, address _pairToken, address _lpToken, address _lpStaking, address _uniV2Router) {
        ...
        trufToken = IERC20(_trufToken);//@TrufToken
        pairToken = IERC20(_pairToken);//@USDT
        lpToken = IERC20(_lpToken);//@uniswap LP pair
        lpStaking = IStakingRewards(_lpStaking);//@StakingRewards contract have 100e18 TRUF for reward over 7 days period
        uniV2Router = IUniswapV2Router01(_uniV2Router);//@uniswap router v2
    }

    function initiate(
        bytes32 id,//@keccak
        uint256 period,// 1 year
        uint256 startTime,//block.timestamp + 86400
        address pToken,// @protocol Token != TRUF
        uint256 pTokenAmount,//10000e18
        address pOwner,//@ protocol owner that can pay token
        uint256 trufAmount//@1000e18
    ) external {
        _checkOwner();
        ...
        //@owner initiate Partner need send in TRUF as rewards
    ...
    function pay(bytes32 id, uint256 pairTokenMaxIn, uint256 lpTokenMinOut, uint256 deadline) external {
        ...
        subscription.status = Status.Active;//@active == payed

        pairToken.safeTransferFrom(msg.sender, address(this), pairTokenMaxIn);//@get USDT from user
        pairToken.safeApprove(address(uniV2Router), pairTokenMaxIn);
        trufToken.safeApprove(address(uniV2Router), subscription.trufAmount);//@TRUF already taken during initiate subscription
        (, uint256 pairTokenIn, uint256 lpAmount) = uniV2Router.addLiquidity(//@audit frontrun uniswapV2 liquidity approve.
            address(trufToken),
            address(pairToken),
            subscription.trufAmount,
            pairTokenMaxIn,
            subscription.trufAmount,
            0,//@audit-ok M liquidity should include minimum USDT?
            address(this),
            deadline
        );
        ...
        lpStaking.stake(lpAmount);//@audit liquidity calculation is really unfair when 1 token trump price other then reversed.
    ...
    function cancel(bytes32 id, uint256 trufMinOut, uint256 pairTokenMinOut, uint256 deadline) external {
        ...
        if (subscription.status == Status.Initiated) {
            ...
            IERC20(subscription.pToken).safeTransfer(owner(),                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    );
    ...
    function _updateRewardDebt() internal {
        ...
        if (reward != 0) {
            accTrufPerLp += (reward * 1e18) / totalLpStaked;//@audit update reward lost round up? Why? Rewards share by same person. Why care accrue?
        }
    }
}
