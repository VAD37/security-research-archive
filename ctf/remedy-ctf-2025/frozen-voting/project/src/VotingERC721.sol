...
contract VotingERC721 is ERC721, Ownable {
    ...
    */
    ...
    function _delegate(address delegator, address delegatee) internal {
        address currentDelegate = delegates(delegator);//player

        _delegates[delegator] = delegatee;//new delegate admin

        uint256 amount = votesToDelegate(delegator); //check how much vote player have. could 10001, or 10000
        _moveDelegates(currentDelegate, delegatee, amount);//@normally this amount is balanceOf and not fixed amount from mint
    }

    function _moveDelegates(address srcRep, address dstRep, uint256 amount) internal {
        if (srcRep != dstRep && amount > 0) {//from player 
            if (srcRep != address(0)) {
            ...
            if (dstRep != address(0)) {//to Challenge contract, 10_000 votes back
