...
*/
    function isSolved() external returns (bool) {        
        ...
        if (votingToken.ownerOf(SUPER_ID) == ADMIN) return false;//@this always true
        console.log("pass1");
        (bool success1,) =//@delegate to self fail
            address(votingToken).call(abi.encodeWithSelector(VotingERC721.delegate.selector, address(this)));
        console.log("pass2", success1);
        (bool success2,) = address(votingToken).call(//transfer from self to admin fail
            abi.encodeWithSelector(ERC721.transferFrom.selector, address(this), ADMIN, SUPER_ID)
        );
        console.log("pass3", success1, success2);
