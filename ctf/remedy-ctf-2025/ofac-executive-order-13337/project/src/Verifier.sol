...
contract FflonkVerifier {
    ...
    uint256 constant C0x  = 8029316166247724596727224253549705646779618698978549116001000683450565528607;
    uint256 constant C0y  = 2665268914545688399957393816136896589553491394780629117007965245658955436960;
    ...
    function verifyProof(bytes32[24] calldata proof, uint256[6] calldata pubSignals) public view returns (bool) {
        assembly {
            ...
            function computeChallenges(pMem, pPublic) {
                ...
                mstore(add(pMem, 2080 ), C0x)
                mstore(add(pMem, 2112 ), C0y)

                mstore(add(pMem, 2144), calldataload(pPublic))
                
                mstore(add(pMem, 2176 ), calldataload(add(pPublic, 32)))
                
                mstore(add(pMem, 2208 ), calldataload(add(pPublic, 64)))
                
                mstore(add(pMem, 2240 ), calldataload(add(pPublic, 96)))
                
                mstore(add(pMem, 2272 ), calldataload(add(pPublic, 128)))
                
                mstore(add(pMem, 2304 ), calldataload(add(pPublic, 160)))
                
                

                mstore(add(pMem, 2336 ),  calldataload(pC1))
                mstore(add(pMem, 2368 ),  calldataload(add(pC1, 32)))

                mstore(add(pMem, pBeta),  mod(keccak256(add(pMem, lastMem), 320), q))
                mstore(add(pMem, pGamma), mod(keccak256(add(pMem, pBeta), 32), q))

                // Get xiSeed & xiSeed2
                mstore(add(pMem, lastMem), mload(add(pMem, pGamma)))
                mstore(add(pMem, 2112), calldataload(pC2))
                mstore(add(pMem, 2144), calldataload(add(pC2, 32)))
                ...
                calldatacopy(add(pMem, 2112), pEval_ql, 480)
                mstore(add(pMem, pAlpha), mod(keccak256(add(pMem, lastMem), 512), q))

                // Compute challenge.y
                mstore(add(pMem, lastMem), mload(add(pMem, pAlpha)))
                mstore(add(pMem, 2112 ),  calldataload(pW1))
                mstore(add(pMem, 2144 ),  calldataload(add(pW1, 32)))
