...
contract LuckyToken is ERC20("LuckyToken", "LTN"), AccessControl {
    ...
    function _transfer(//@ignore _mint
        address _sender,
        address _recipient,
        uint256 _amount
    ) internal virtual override {
        if (specialReceiver[_recipient]) {//@TeamVault, just transfer
            super._transfer(_sender, _recipient, _amount);
            return;
        }
        if (!specialSender[_sender]) {//uniswapRouter, and Pair,
    ...
    function openTrading() public {
        ...
        _addSpecialSender(address(UNISWAPV2_ROUTER), true);//@this is failure

        UNISWAPV2_ROUTER.addLiquidityETH{value: address(this).balance}(//10 ETH
            address(this),
