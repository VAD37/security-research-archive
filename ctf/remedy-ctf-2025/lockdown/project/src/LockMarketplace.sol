...
contract LockMarketplace is Ownable, Initializable, Pausable, ERC721Holder, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) private _stakedAmounts;
    ...
    uint256 private mintingFee; //20
    uint256 private mintingProfit;

    uint256 public initialMinNFTDeposit; //100
    uint256 public minimumStakePrice; //30
    ...
    function setMarketConditions(uint256 initDeposit, uint256 stakePrice, uint256 mintFee) public onlyOwner {
    ...
    function startMarket(
        uint256 initDeposit, //100e6
        uint256 stakePrice, //30e6
        uint256 mintFee //20e6
    ...
    function endMarket() external whenNotPaused onlyDuringMarket onlyOwner {
        _started = false;
    }

    function withdrawMintingProfit(address recipient) external onlyOwner {
        ...
        returns (uint256)
    {
        ...
        mintingProfit += mintingFee; // 1_000_000 USDC
        uint256 nextTokenId = _iLockToken.getTokenCounter() + 1;
        _iLockToken.mint(to); //@safeMint reentrancy. prevOwner set to zero, if target is market then staked = true;
        _depositIntoNFT(nextTokenId, usdcAmount); // _deposits[tokenId] += value;
        _usdc.approve(address(this), usdcAmount + mintingFee); //@interesting self approve
        _usdc.transferFrom(msg.sender, address(this), usdcAmount + mintingFee);
        return nextTokenId; //NFT now hold 1e12 USDC
    }

    function depositUSDC(uint256 tokenId, uint256 usdcAmount) external whenNotPaused onlyDuringMarket nonReentrant {
        require(usdcAmount != 0, "Cannot deposit 0 USDC");
        _iLockToken.requireApprovedOrOwner(msg.sender, tokenId);
        _depositIntoNFT(tokenId, usdcAmount); // can increase deposit of NFT if owner // _deposits[tokenId] += value;
        _usdc.approve(address(this), usdcAmount);
        _usdc.transferFrom(msg.sender, address(this), usdcAmount);//take usdc into NFT
    }

    function withdrawUSDC(uint256 tokenId, uint256 amount) external whenNotPaused onlyDuringMarket nonReentrant {
        _iLockToken.requireApprovedOrOwner(msg.sender, tokenId);
    ...
    function stake(uint256 tokenId, uint256 usdcAmount) external whenNotPaused onlyDuringMarket nonReentrant {
        _iLockToken.requireApprovedOrOwner(msg.sender, tokenId);
        require(!_stakedTokens[tokenId], "Token already staked");
        require(_deposits[tokenId] >= usdcAmount, "Not enough deposits"); //NFT must hold USDC
        require(usdcAmount >= minimumStakePrice, "Insufficient stake amount");//>= 30 USDC
        _stakedAmounts[tokenId] = usdcAmount; // move USDC from NFT into stakeNFT. not neccessary to move all USDC
        _stakedOwners[tokenId] = msg.sender;
        _stakedTokens[tokenId] = true;//false when transferOut NFT 
        _iLockToken.transferFrom(_iLockToken.ownerOf(tokenId), address(this), tokenId); //take NFT, prevOwner = owner, staked[tokenId] = true
        _swapUSDCforCUSDC(_prevOwners[tokenId], usdcAmount, tokenId);
    }

    function unStake(address to, uint256 tokenId) external whenNotPaused onlyDuringMarket nonReentrant {
        require(_stakedTokens[tokenId], "Token not staked");
        require(
            _stakedOwners[tokenId] == msg.sender || _prevOwners[tokenId] == msg.sender, "Caller is not token staker"
        );
        ...
        _iLockToken.safeTransferFrom(address(this), to, tokenId, "");//@callback safeTransfer. can edit _prevOwners
        _swapCUSDCforUSDC(_prevOwners[tokenId], tokenId);//swap owner with bigger _cUSDCInterest will be quite mishap . subtract not revert underflow zero amount
        delete _stakedAmounts[tokenId];
    }

    function _swapUSDCforCUSDC(address recipient, uint256 usdcAmount, uint256 tokenId) internal returns (uint256) {
        require(
            _deposits[tokenId] >= _stakedAmounts[tokenId] && _stakedAmounts[tokenId] >= usdcAmount,
            "Minting more CUSDC than deposited USDC"
        ); //stakedNFT  from stake()
        ...
        uint256 usdcDiff = _sub(contractUSDCBalBefore, _usdc.balanceOf(address(this))); //@lose 1M
        _withdrawOutOfNFT(tokenId, usdcDiff); //remove 1M deposit of USDC. contracts lost 1M USDC
        uint256 cusdcDiff = _sub(_cusdc.balanceOf(address(this)), contractCUSDCBalBefore);
        _cUSDCInterest[recipient] += cusdcDiff; //1M usdc to cUSDC. contract gain ~1.1M cUSDC
        _rewardsBalance[recipient] = _sub(_rewardsBalance[recipient], usdcDiff); //reward balance look wrong too
        return cusdcDiff;
    }

    function _swapCUSDCforUSDC(address recipient, uint256 tokenId) internal returns (uint256) {
        uint256 contractCUSDCBalBefore = _cusdc.balanceOf(address(this));
        uint256 contractUSDCBalBefore = _usdc.balanceOf(address(this));
    ...
    function setPrevOwner(uint256 tokenId, address prevOwner) public onlyToken {
        _prevOwners[tokenId] = prevOwner;
    }

    function setStaked(uint256 tokenId, bool staked) public onlyToken {
    ...
    function getAvailableRewards(address _address) public view returns (uint256) {
        return _rewardsBalance[_address];
    }

    function _depositIntoNFT(uint256 tokenId, uint256 value) internal {
    ...
    function _withdrawOutOfNFT(uint256 tokenId, uint256 value) internal {
        if (tokenId == 0) revert NonexistentToken();
