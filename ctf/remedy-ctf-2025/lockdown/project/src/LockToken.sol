...
contract LockToken is 
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal virtual override(ERC721, ERC721Enumerable) {
        ...
        {   
            ...
            {
                ILockMarketplace(_marketplace).setStaked(tokenId, false);//_stakedTokens[tokenId] = false;
            } else
            {
                ILockMarketplace(_marketplace).setPrevOwner(tokenId, ownerOf(tokenId));//_prevOwners[tokenId] = prevOwner;
            }
            
            if(to == _marketplace)
            {
                ILockMarketplace(_marketplace).setStaked(tokenId, true);//_stakedTokens[tokenId] = true;
            }
        }
    }
