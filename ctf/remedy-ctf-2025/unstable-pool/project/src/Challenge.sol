...
contract Challenge {
    ...
    constructor(address player) payable {
        ...
        WrappedToken(address(WRAPPEDTOKEN)).mint(address(this), initialMint); // 100_000 wrapped token
        WrappedToken(address(WRAPPEDTOKEN)).addRewards(rewards);// 10_000 TOKEN to Wrappped as rewards
        ...
        //cheats to player
        TARGET.transfer(player, initialMint);
    }
