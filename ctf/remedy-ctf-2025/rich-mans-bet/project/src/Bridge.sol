...
contract Bridge is ERC1155Receiver, Ownable {
    ...
    constructor(
        address _challengeContract,//Challenge
        uint96 _threshold,//10
        address[] memory _withdrawValidators //player
    ) payable {
        ...
        _mintAdminNFT();
        validatorWeights[msg.sender] = OWNER_WEIGHT;//10_000
    ...
    function changeBridgeSettings(
        ...
        for (uint256 i = 0; i < signatures.length; i++) {
            ...
            console.log("sign success", signer);

            require(signer != lastSigner, "Repeated signer");

            if (validatorWeights[signer] > 0) {
                accumulatedWeight += validatorWeights[signer];
                console.log("accumulatedWeight", accumulatedWeight, totalWeight);
            }
            lastSigner = signer;
        }
