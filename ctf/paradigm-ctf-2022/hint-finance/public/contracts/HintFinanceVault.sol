...
contract HintFinanceVault {
    ...
    address public immutable underlyingToken;//@ PNT SAND AMP
    ...
    function addReward(address rewardToken, uint256 rewardDuration) external {
        ...
        rewardTokens.push(rewardToken);//@ can add USDC USDT DAI to each vault
        rewardData[rewardToken].rewardsDuration = rewardDuration;//10 days
    }
    ...
    function rewardPerToken(address rewardToken) public view returns (uint256) {
        ...
        return rewardData[rewardToken].rewardPerTokenStored + newAccumulated;//old+= deltaTime * rewardRate / totalSupply
    }
    ...
    function deposit(uint256 amount) external updateReward(msg.sender) returns (uint256) {
        uint256 bal = ERC20Like(underlyingToken).balanceOf(address(this));//@audit H using balanceOf along with flashloan
        uint256 shares = totalSupply == 0 ? amount : amount * totalSupply / bal;//@ ERC4626
    ...
    function withdraw(uint256 shares) external updateReward(msg.sender) returns (uint256) {//@redeem using share?
