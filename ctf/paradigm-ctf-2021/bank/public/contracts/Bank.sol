...
//Remove Bank WETH balance to zero
contract Bank {
    ...
    struct Account {
        string accountName;
        uint uniqueTokens;//@audit uniqueTokens can be exploit to underflow.  Whats next
    ...
    //someone deposit 50 ETH to account zero token weth.
    function depositToken(uint accountId, address token, uint amount) external {//accountId = 0, token = weth, amount = 50
        require(accountId <= accounts[msg.sender].length, "depositToken/bad-account");
        
        // open a new account for the user if necessary
        if (accountId == accounts[msg.sender].length) {//open account 0
        ...
        require(ERC20Like(token).balanceOf(msg.sender) >= amount, "depositToken/low-sender-balance");//@audit external call allow overflow uniqueToken?
        
        // increment counter for unique tokens if necessary
        if (oldBalance == 0) {//@audit this can be duplicated when withdraw
            account.uniqueTokens++;//@audit does not check zero amount. uniqueToken can be inflated
        }
    ...
    function withdrawToken(uint accountId, address token, uint amount) external {
        ...
        require(ERC20Like(token).balanceOf(address(this)) >= amount, "withdrawToken/low-sender-balance");//@audit external call allow underflow accounts.length twice?
        ...
        if (account.balances[token] == 0) {
            ...
            if (account.uniqueTokens == 0 && accountId == lastAccount) {
                accounts[msg.sender].length--;//@audit underflow this then what?
            }
        }
