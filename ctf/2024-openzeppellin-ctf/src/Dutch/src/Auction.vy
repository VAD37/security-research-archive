...
DURATION: constant(uint256) = 604800//duration is 7 days

token: public(IERC20Permit) //IERC20 is WETH
nft: public(IERC721)
nftId: public(uint256)

seller: public(address)//msg.sender
startingPrice: public(uint256)//1 ether
startAt: public(uint256)// now
expiresAt: public(uint256)// now + 7 days
discountRate: public(uint256)// 1 ether / 7days
...
def buyWithPermit(
    ...
    extcall self.token.permit(buyer, self, amount, deadline, v, r, s)//WETH have no permit
