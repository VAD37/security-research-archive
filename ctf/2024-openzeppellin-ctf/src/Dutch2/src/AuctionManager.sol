...
contract AuctionManager {
        ...
        checkState(States.Reveal, auctions[id])
    {
        ...
        for (uint256 i; i < indices.length; i++) {
            ...
            BidEncrypted storage bid = auction.bids[index];//@audit indices array can be higher than 256. no check?
            ...
            if (amountBase + data.baseFilled > data.totalBase) {
                amountBase = data.totalBase - data.baseFilled;//@what does this for? This reduce amountBase if more than base auction token selling
            }

            data.baseFilled += amountBase;
            bid.baseAmountFilled = amountBase;//@this bid allow spread token between bidder? Each bidder get share of the bid
        }
